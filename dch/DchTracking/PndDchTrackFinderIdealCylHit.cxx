...
Int_t PndDchTrackFinderIdealCylHit::DoFind(TClonesArray* cylHitArray,
  ...
  if( !fMCTrackArray ) {
    ...
    if ( fVerbose)    cout <<"# MCTracks (fMCTrackArray): "<< fMCTrackArray->GetEntriesFast() << endl;
  }
  ...
  if( !fMCPointArray ) {
    ...
    if ( fVerbose)    cout <<"# MCPoints (fMCPointArray): "<< fMCPointArray->GetEntriesFast() << endl;
  }
  ...
  if( !fDigiArray ) {
    ...
    // Size of fDigiArray
    if ( fVerbose)    cout <<"# Digis (fDigiArray): "<< fDigiArray->GetEntriesFast() << endl;
  }
  ...
  if( !cylHitArray ) {
    ...
    // Size of cylHitArray
    if ( fVerbose)    cout <<"# DchCylHits (cylHitArray): "<< cylHitArray->GetEntriesFast() << endl;
  }
  ...
  for(Int_t iMCTrack = 0; iMCTrack < nMCTracks; iMCTrack++) {
    ...
    //----------------------------------------------------
    // trivial check for number of DCH hits
    if ( !cylHitMap[iMCTrack] ) continue;
    //----------------------------------------------------
    nMCacc++;
    ...
    dchTrack = (PndTrackCand*) trackArray->At(nTracks);
    ...
    dchTrack->setMcTrackId(iMCTrack);
  ...
  for(Int_t iCylHit = 0; iCylHit < nDchCylHits; iCylHit++) {
    ...
    dchTrack = (PndTrackCand*) trackArray->At(trackIndex);
  ...
  for(Int_t iTrack = 0; iTrack < nTracks; iTrack++) {
    dchTrack = (PndTrackCand*) trackArray->At(iTrack);
    dchTrack->Sort();
    
    PndTrackCandHit tch = dchTrack->GetSortedHit(0);

    dchCylHit = (PndDchCylinderHit*) cylHitArray->At(tch.GetHitId());
    dchDigi = (PndDchDigi*) fDigiArray->At(dchCylHit->GetDigiIndex());
    ptIndex = dchDigi->GetRefIndex();
    ...
    dchTrack->setTrackSeed(pos,mom.Unit(),1./mom.Mag());
  }
