...
// Public method Init 
InitStatus PndDchPreFitter::Init() {
  ...
  cout << "-I- PndDchPreFitter: Intialization successfull" << endl;
...
// Public method Exec 
void PndDchPreFitter::Exec(Option_t* opt) {
  //  cout<<"&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&"<<endl;
...
Int_t PndDchPreFitter::AddHits(Int_t trackID) {
...
Bool_t PndDchPreFitter::GetInHitAtZ(Double_t zpos, TVector3 &inhit){
  inhit.SetXYZ(0., 0., 0.);
...
Bool_t PndDchPreFitter::GetMomentum(TVector3& momentum ) {
...
Bool_t PndDchPreFitter::GetCircleParameters(TClonesArray *fInHits, TVector3& result) {
...
Bool_t PndDchPreFitter::GetLineParameters(TClonesArray *fInHits, TVector2& result) const {
