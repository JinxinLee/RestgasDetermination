...
// -----   Default constructor   -------------------------------------------
PndHypFullIdealAna::PndHypFullIdealAna() :
...
// -----   Public method Init   --------------------------------------------
InitStatus PndHypFullIdealAna::Init() {
 ...
 fGe = (TClonesArray*) ioman->GetObject("HypGePoint");
  ...
  fMicroArray = (TClonesArray*) ioman->GetObject("PndMicroIdealCandidates");
...
void PndHypFullIdealAna::SetParContainers() {
	...
	for (int k=0;k<npi;k++)
	  {
	    ...
	    PndHypPoint*  pop=(PndHypPoint*)fMc->At((pion.GetMicroCandidate().GetMvdHits())-1);
		...
		for (jj=ii+1;jj<dsi;jj++)
		  {
		    ...
		    //VAbsMicroCandidate cm;
		    //cm = pi.GetMicroCandidate();
		    po=(PndHypPoint*)fMc->At((pi.GetMicroCandidate().GetMvdHits())-1);
...
void PndHypFullIdealAna::SetEnergySpectra(int event,int cluster)
{
  Float_t E;
  E=0;
...
void PndHypFullIdealAna::SetTotESpectra(int clus)
{
  Float_t E;
  E=0;
  if(fGe->GetEntriesFast()!=0)
...
Int_t PndHypFullIdealAna::GetIonCharge(Int_t ion,Int_t &mass,Int_t &str)
{
  Int_t A,Z,L;
...
void PndHypFullIdealAna::Finish(TString cat)
{ 
