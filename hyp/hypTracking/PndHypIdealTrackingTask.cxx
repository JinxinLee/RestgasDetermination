...
InitStatus PndHypIdealTrackingTask::Init()
{
  ...
  if ( ! fHitArray ) {
    std::cout << "-W- PndHypIdealTrackingTask::Init: "<< "No fHitarray!" << std::endl;
    return kERROR;  }

  fClusterStripArray = (TClonesArray*) ioman->GetObject("HYPStripClusterCand");
  if ( ! fClusterStripArray ) {
    std::cout << "-W- PndMvdIdealTrackingTask::Init: "<< "No fClusterStripArray!" << std::endl;
    return kERROR;  }
  
  fDigiStripArray = (TClonesArray*) ioman->GetObject("HypStripDigis");
  if ( ! fDigiStripArray ) {
    std::cout << "-W- PndMvdIdealTrackingTask::Init: "<< "No fDigiStripArray array!" << std::endl;
    return kERROR;  }
  ...
  if(fMctruthArray==0)  {
    std::cout << "-W- PndHypIdealTrackingTask::Init: "<< "No McTruth array!" << std::endl;
    return kERROR;  }
...
void PndHypIdealTrackingTask::Exec(Option_t* opt)
{
  ...
  for(Int_t iHit=0;iHit<nPndHypHits;++iHit)
  {
	...
	TVector3 pos = mcmap[MCid]->GetStartVertex();
	...
	if(fVerbose>1) std::cout<<"PndHypIdealTrackingTask::Exec(), Particle Info: "
				<<pdgpart.GetName()<<" ("<<pdgcode<<") Q="<<mcCharge<<std::endl;
  ...
  std::cout<<fTrackOutputArray->GetEntriesFast()
