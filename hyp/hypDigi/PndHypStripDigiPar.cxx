...
Bool_t PndHypStripDigiPar::getParams(FairParamList* list)
{
  ...
  if (!list->fill("top_pitch", &topPitch)) return kFALSE;
	  if (!list->fill("bot_pitch", &botPitch)) return kFALSE;
 	  if (!list->fill("orient",&orient)) return kFALSE;
 	  ...
 	  if (!list->fill("nr_fe_channels", &feChannels)) return kFALSE;
 	  if (!list->fill("nr_fe_top", &topNrFE)) return kFALSE;
 	  if (!list->fill("nr_fe_bottom", &botNrFE)) return kFALSE;
 	 
 	  if (!list->fill("charge_threshold",&threshold)) return kFALSE;
 	  if (!list->fill("charge_noise",&noise)) return kFALSE;
	  Text_t stName[80];
