...
InitStatus PndHypPidIdealTask::Init()
{
  ...
  if ( ! fPointArray ) {
    std::cout << "-W- PndHypPidIdealTask::Init: "<< "No "<<fBranchName<<" array!" << std::endl;
    return kERROR;  }

  // Get MCTruth collection
  fMcArray=(TClonesArray*) ioman->GetObject("MCTrack");
  if(fMcArray==0)  {
    std::cout << "-W- PndHypPidIdealTask::Init: "<< "No McTruth array!" << std::endl;
    return kERROR;  }
  ...
  if (fAlgoName!="ideal"&&fAlgoName!="simple"&&fAlgoName!="advanced") {
    std::cout << "-W- PndHypPidIdealTask::Init: "<< "No pid algorithm named '"<< fAlgoName <<"'! Names are 'ideal', 'simple', 'advanced'" << std::endl;
    return kERROR;
  } 
  return kSUCCESS;
}
...
void PndHypPidIdealTask::Exec(Option_t* opt)
{
  ...
  for(int i=0;i<fPointArray->GetEntriesFast();i++) {
    ...
    if(dx>0)pidcand[track]->AddHypHit(dE, dx, momentum.Mag());
  }
