...
Bool_t PndVolGenerator::Init()
{
  ...
  if (fPhiMax-fPhiMin>360)
    Fatal("Init()","PndVolGenerator: phi range is too wide: %f<phi<%f",
	  fPhiMin,fPhiMax);
 ...
 std::cout<<" gGeoManager 1 "<<gGeoManager<<" "<<std::endl;
  ...
  if (! particle) Fatal("PndVolGenerator","PDG code %d not defined.",fPDGType);
...
Bool_t PndVolGenerator::ReadEvent(FairPrimaryGenerator* primGen)
{
  ...
  if(fExt) {gGeoManager->CdDown(fAbs); //stglAb0 ,0 -> stglSi0_0 ,CdUp(), CdDown(1)
  }else {fAbs = TMath::Nint(fFunc->GetRandom());
 ...
 posSens[0]=0.;  posSens[1]=0.;  posSens[2]=0.;
