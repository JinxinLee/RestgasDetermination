...
InitStatus PndHypMSAnaTask::Init()
{
  ...
  if ( !ioman){
	...
	// Get input arrays
  	...
  	if (!fMCTracks){
  		std::cout << "-W- PndHypMSAnaTask::Init: "<< "No MCTrack" << " array!" << std::endl;
  		return kERROR;
  	}
  
	fMCHits = (TClonesArray*) ioman->GetObject("HypPoint");
	if ( !fMCHits)	{
		std::cout << "-W- PndHypMSAnaTask::Init: "<< "No MVDPoint"<<" array!" << std::endl;
		return kERROR;
	}
	    ...
	    */
		...
		Motherpdg = mother->GetPdgCode();
	      ...
	      if((myTrack->GetPdgCode()==-211)&&( Motherpdg>1010000000&& 
						  Motherpdg<1020050130) )
		{  //startPoint->MomentumIn(vpi2);//vpi2 = mctruth->GetMomentum();
