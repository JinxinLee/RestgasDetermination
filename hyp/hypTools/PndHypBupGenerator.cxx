...
// -----   Default constructor   ------------------------------------------
PndHypBupGenerator::PndHypBupGenerator():fPMin(0),fPMax(0),
fThetaMin(0),fThetaMax(0),fPhiMin(0),fPhiMax(0),
fThetaRangeIsSet(0),fPRangeIsSet(0),fGamOn(0) {
 iEvent     = 0;
 fInputFile = NULL;
 fInputTree = NULL;
}
...
PndHypBupGenerator::PndHypBupGenerator(const char* fileName):fPMin(0),fPMax(0),
fThetaMin(0),fThetaMax(0),fPhiMin(0),fPhiMax(0),
fThetaRangeIsSet(0),fPRangeIsSet(0),fGamOn(0){
 ...
 // Check for number of events in input file
  ...
  for (Int_t iPart=0; iPart < nParts; iPart++) {
    ...
    if(fGamOn){
      ...
      cout<<" Secondary vertex for stopping Xi - gammas "<<vx<<" "
	  <<vy<<" "<<vz<<endl;
