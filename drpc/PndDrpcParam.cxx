...
TString PndDrpcParam::GetString(const char* key) const
{
...
Int_t PndDrpcParam::GetInteger(const char* key) const
{
  TObjString* value=(TObjString*)fVariables.GetValue(key);
  if (value==NULL)
    Fatal("GetInteger","Can't find variable named %s.", key);
...
Double_t PndDrpcParam::GetDouble(const char* key) const
{
  TObjString* value=(TObjString*)fVariables.GetValue(key);
  if (value==NULL)
...
TString PndDrpcParam::GetStringSlack(const char* key) const 
{
  TObjString* value=(TObjString*)fVariables.GetValue(key);
  if (value==NULL)
  {
...
Int_t PndDrpcParam::GetIntegerSlack(const char* key) const
{
  TObjString* value=(TObjString*)fVariables.GetValue(key);
  if (value==NULL)
  {
...
Double_t PndDrpcParam::GetDoubleSlack(const char* key) const
{
  TObjString* value=(TObjString*)fVariables.GetValue(key);
  if (value==NULL)
  {
...
void PndDrpcParam::AddVariable(const char* key, const char* value)
{
...
//=============================================================================
PndDrpcParam::PndDrpcParam(const char* name, const char* filename)
  : TNamed(name, filename), fVariables(200)
{
  ...
  fSuccess=1;
  if (!file) {
    Error("PndDrpcParam", "Can't open file %s. ", filename);
    fSuccess=0;
    return;
  }
  ...
  while(getline(file,buffer))
  {
    ...
    if (variable==message)
    {
      Error("PndDrpcParam","Syntax error: File %s. Line %d. ", fname.Data(), linenum);
      fSuccess=0;
      file.close();
      return;
    }
    ...
    if (message.find("=")==string::npos)
      Error("PndDrpcParam","Can't find value for varible %s at line %d.", variable.c_str(), linenum);
    value=message.substr(message.find("=")+1);
    if (value.find_first_not_of(" 	")==string::npos)
    {
      Info("PndDrpcParam","Value of varible %s at empty.", variable.c_str());
      value="";
    }
    else
...
void PndDrpcParam::DumpContainer(ostream& stream) const
{
  TObjString* key;
...
void PndDrpcParam::DumpContainer() const
{
  DumpContainer(cout);
}
...
Bool_t PndDrpcParam:: Differs(const PndDrpcParam* info) const
{
  TObjString* key;
  TIterator* iter=fVariables.MakeIterator();
  while((key=(TObjString*)iter->Next())!=NULL)
