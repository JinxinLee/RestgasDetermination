...
GeaneTrackRepNew::GeaneTrackRepNew()
  : AbsTrackRep(5), _pdg(211),_backw(0)
{

}

GeaneTrackRepNew::GeaneTrackRepNew(FairGeaneProNew* geane, 
...
//  GeaneTrackRepNew::GeaneTrackRepNew(const GeaneTrackRepNew& rep) 
//   : AbsTrackRep(rep)
// {
//   _geane=rep._geane;
// }
...
double
GeaneTrackRepNew::extrapolate(const DetPlane& pl, 
			   ...
			   TMatrixT<double>& covPred)
{
  ...
  if(fabs(state[0][0])>10){
    FitterException exc("GeaneTrackRepNew: PROTECT AGAINST LOW MOMENTA",__LINE__,__FILE__);
    exc.setFatal();
    throw exc;
  }
...
void
GeaneTrackRepNew::extrapolateToPoca(const TVector3& pos,
				 TVector3& poca,
				 TVector3& dirInPoca){
  if(fabs(getMom(_refPlane).Theta()/TMath::Pi()*180.) < THETACUT){
    FitterException exc("GEANE propagation not possible for p.theta<THETACUT",__LINE__,__FILE__);
  ...
  //std::cout<<"GeaneTrackRepNew::extrapolateToPoca"<<std::endl;
  //_refPlane.Print();
...
void 
GeaneTrackRepNew::extrapolateToLine(const TVector3& point1,
...
TVector3 
GeaneTrackRepNew::getPocaOnLine(const TVector3& p1, const TVector3& p2, bool back){
  
  //std::cout<<"GeaneTrackRepNew::getPocaToWire"<<std::endl;
...
TVector3 
GeaneTrackRepNew::getPos(const DetPlane& pl)
{
...
TVector3 
GeaneTrackRepNew::getMom(const DetPlane& pl)
{
  TMatrixT<double> statePred(state);
  if(pl!=_refPlane)extrapolate(pl,statePred);
  double fSPU  = _spu;
...
void
GeaneTrackRepNew::getPosMom(const DetPlane& pl,TVector3& pos, TVector3& mom)
{
  TMatrixT<double> statePred(state);
  if(pl!=_refPlane)extrapolate(pl,statePred);
...
void
GeaneTrackRepNew::getPosMomCov(const DetPlane& pl,TVector3& pos,TVector3& mom,TMatrixT<double>& cov){
  cov.ResizeTo(6,6);
