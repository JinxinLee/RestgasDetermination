...
/*
This file has been copied from COMPASS' analysis software Phast 7.064
 */
...
bool RKtrackRep::RKutta (double* SU,double* VO, double& Path) {
  ...
  if(fabs(Step)>Wmax) {
    cout<<"PaAlgo::RKutta ==> Too long extrapolation requested : "<<Step<<" cm !"<<endl;
    cout<<"X = "<<R[0]<<" Y = "<<R[1]<<" Z = "<<R[2]
	<<"  COSx = "<<A[0]<<"  COSy = "<<A[1]<<"  COSz = "<<A[2]<<endl;
    cout<<"Destination  X = "<<SU[0]*SU[3]<<endl;
    return(false);
  }
  ...
  while(fabs(Step)>Sstop) {
    ...
    TVector3 pos(r[0],r[1],r[2]);
    field.get(pos,H);
    H0[0]=PS2*H.X(); H0[1]=PS2*H.Y(); H0[2]=PS2*B.Z(); 
    ...
    H1[0]=H1vect.X(); H1[1]=H1vect.Y();H1[2]=H1vect.Z();
    ...
    H2[0]=H2vect.X(); H2[1]=H2vect.Y();H2[2]=H2vect.Z();
    ...
    if((Way+=fabs(S))>Wmax){ 
      cout<<"PaAlgo::RKutta ==> Trajectory is longer then length limit : "<<Way<<" cm !"
	  <<" P = "<<1./VO[6]<< " GeV"<<endl;
      return(false);
    }
    //
  ...
  if(Error == 1){
    cout<<"PaAlgo::RKutta ==> Do not getting closer. Path = "
	<<Way<<" cm"<<"  P = "<<1./VO[6]<<" GeV"<<endl;
    return(false);
  }
