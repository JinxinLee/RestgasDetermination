...
PndTrackingQualityBarrelTaskNewLinks::PndTrackingQualityBarrelTaskNewLinks(TString trackBranchName, TString idealBranchName, Bool_t pndTrackData) :
  FairTask("Creates PndMC Barrel test"),
  fBranchNames(),fMCInfoBranchName("MCTrackInfo"), fRecoInfoBranchName("RecoTrackInfo"),
  fMapLinkData(), fMapEfficiencies(), fMCInfoIdIdealId(),
  fNGhosts(0),fTrack(NULL), fMCTrack(NULL), fTrackCand(NULL), fSttHitArray(NULL), fMCTrackInfo(NULL), fRecoTrackInfo(NULL),
  fIdealTrack(NULL), fSttTubeArray(NULL),
  ioman(NULL),
  fMapTrackQualifikation(),
  fTrackBranchName(trackBranchName), fIdealTrackBranchName(idealBranchName), fPndTrackOrTrackCand(pndTrackData),
  fSttParameters(NULL), fTuple(NULL),
  fPHisto(NULL),fPRelHisto(NULL),fPtHisto(NULL),fPtRelHisto(NULL),fQualyHisto(NULL),fQualyStack(NULL),
  fQualyHisto_mc(NULL),fQualyHisto_neg(NULL),fQualyHisto_pos(NULL),fQualyHisto_all(NULL),
  fEventNr(0) {
}
// -------------------------------------------------------------------------
