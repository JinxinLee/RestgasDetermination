// $Id: PndCAGBHit.cxx,v 1.2 2010/09/01 10:38:27 ikulakov Exp $
//***************************************************************************
...
void PndCAGBHit::GetLocalX0X1X2( float& x0, float& x1, float &x2 ) const {
  double x0_, x1_, x2_;
  PndCAParameters::GlobalToCALocal( fX, fY, fZ, fPhi, x0_, x1_, x2_ );
  x0 = x0_;
  x1 = x1_;
  x2 = x2_;
}
...
std::istream& operator>>(std::istream& in, PndCAGBHit &a)
{
  double tmp;
  UNUSED_PARAM1(tmp);
  in >> a.fX >> a.fY >> a.fZ >> a.fR
     >> a.fC[0][0] >> a.fC[0][1] >> a.fC[0][2] // in global c.s.!
     >> a.fC[1][0] >> a.fC[1][1] >> a.fC[1][2]
     >> a.fC[2][0] >> a.fC[2][1] >> a.fC[2][2]
     >> a.fErr2R  
     >> a.fTubeR >> a.fHalfLength
     >> tmp >> tmp >> tmp
    //>> a.fEX >> a.fEY >> a.fEZ 
     >> a.fIRow >> a.fID >> a.fPhi;
  ...
  // double x,y,z;
  // PndCAParameters::GlobalToCALocal( a.fEX, a.fEY, a.fEZ, a.fPhi, x, y, z );
  // // cout << " A " << x << " " << y << " " << z << endl;
  // double T2[3][3];
