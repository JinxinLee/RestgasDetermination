...
class PndCATimerInfo {
 public:
  ...
  void Clear() { fReal = 0; fCpu = 0; }
  
  void operator =( TStopwatch& sw ) { fReal  = sw.RealTime(); fCpu  = sw.CpuTime(); };
  void operator+=( TStopwatch& sw ) { fReal += sw.RealTime(); fCpu += sw.CpuTime(); };
  void operator+=( const PndCATimerInfo& t ){ fReal += t.fReal; fCpu += t.fCpu; }
  PndCATimerInfo operator/( const float f ) const { PndCATimerInfo r; r.fName = fName; r.fReal = fReal/f; r.fCpu = fCpu/f; return r; }
...
class PndCATFIterTimerInfo {
 public:
  ...
  void Add( string name ) { fNameToI[name] = fTIs.size(); fTIs.push_back(PndCATimerInfo(name)); };
  PndCATimerInfo& operator[]( string name ) { return fTIs[fNameToI[name]]; };
  PndCATimerInfo& operator[]( int i ) { return fTIs[i]; };
  void operator+=( PndCATFIterTimerInfo& t ){ for( unsigned int i = 0; i < fTIs.size(); ++i ) fTIs[i] += t[i]; }
...
class PndCATFTimerInfo {
 public:
  ...
  void operator+=( PndCATFTimerInfo& t ){ for( unsigned int i = 0; i < fTIIs.size(); ++i ) fTIIs[i] += t[i]; fTIAll += t.GetAllInfo(); }
