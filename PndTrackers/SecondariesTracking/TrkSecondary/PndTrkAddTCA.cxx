...
InitStatus PndTrkAddTCA::Init() {
  ...
  if ( ! fPrimaryTrackArray ) {
    cout << "-W- PndTrkAddTCA::Init: No primary track array, return!" << endl;
    return kERROR;
  }
  //
  // II
  ...
  if ( ! fSecondaryTrackArray ) {
    cout << "-W- PndTrkAddTCA::Init: No secondary track array, return!" << endl;
    return kERROR;
  }
...
void PndTrkAddTCA::Exec(Option_t* opt)  {
  ...
  for(int itrk = 0; itrk < fPrimaryTrackArray->GetEntriesFast(); itrk++) {
    ...
    if(fabs(outputtrack->GetParamFirst().GetMomentum().Z()) < 1e-10) outputtrack->SetFlag(-333);
