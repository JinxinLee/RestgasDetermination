                                       ...
                                       int nTracksV )
{
  ...
  for(int iV=0; iV < nTracksV; iV++) tmpFloat[iV] = t0[iV].Z();
  ...
  for(int iV=0; iV < nTracksV; iV++) tmpFloat[iV] = t0[iV].Angle();
  tmpVec.load( tmpFloat );
  SetAngle(tmpVec);
}
...
void  PndFTSCATrackParamVector::GetDCAPoint( const float_v &x, const float_v &y, const float_v &z,
...
float_m PndFTSCATrackParamVector::TransportToX0( const float_v &x, const float_v &Bz,
...
float_m PndFTSCATrackParamVector::TransportToX0WithMaterial( const float_v &x,  PndFTSCATrackFitParam &par, const float_v &XOverX0,
const float_v &XThimesRho, const float_v &Bz, const float maxSinPhi )
{
  //* Transport the track parameters to X=x  taking into account material budget
...
float_m PndFTSCATrackParamVector::TransportToX0WithMaterial( const float_v &x, const float_v &XOverX0,
const float_v &XThimesRho, const float_v &Bz, const float maxSinPhi )
{
  //* Transport the track parameters to X=x taking into account material budget
...
float_v PndFTSCATrackParamVector::BetheBlochGeant( const float_v &bg2,
    const float_v &kp0,
    const float_v &kp1,
    const float_v &kp2,
    const float_v &kp3,
...
float_m PndFTSCATrackParamVector::Rotate( const float_v &alpha, PndFTSCATrackLinearisationVector &t0,
                                              const float maxSinPhi, const float_m &mask )
{
...
float_m PndFTSCATrackParamVector::FilterWithMaterial( const float_v &y, const float_v &z, float_v err2Y, float_v errYZ, float_v err2Z, float maxSinPhi, const float_m &mask, const int_v& hitNDF, const float_v& chi2Cut )
{
...
float_m PndFTSCATrackParamVector::FilterWithMaterial( const float_v &y, const float_v &z, const FTSCAStripInfo &info, float_v err2, float maxSinPhi, const float_m &mask, const float_v& chi2Cut )
...
float_m PndFTSCATrackParamVector::FilterWithMaterial( const float_v &y0, const float_v &z0, const float_v &r, const FTSCAStripInfo &info, float_v err2, float maxSinPhi, const float_m &mask, const float_v& chi2Cut )
...
float_m PndFTSCATrackParamVector::Transport( const int_v& ista, const PndFTSCAParam& param, const float_m &mask )
{
  ...
  active &= TransportToX0WithMaterial( param.GetX0( ista, active ), tE, fitPar, param.GetXOverX0(int_v(ista),active), param.GetXTimesRho(int_v(ista),active), param.cBz(), 0.999f, active );
  return active;
}
