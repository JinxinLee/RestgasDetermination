...
PndFTSCATrackPerformanceBase::PndFTSCATrackPerformanceBase()
{
  //*  representation of Histos
  ...
  {
#ifdef STAR_HFT
    THistoInfo( "resX1",       "track y residual [cm]",   30, -0.012, 0.012, 0,0,0, "Residual (y^{reco} - y^{mc}) [cm]","Entries" ),
    THistoInfo( "resX2",       "track z residual [cm]",   30, -0.03,   0.03, 0,0,0, "Residual (z^{reco} - z^{mc}) [cm]","Entries" ),
    THistoInfo( "resSinPhi",  "track sin#phi residual ", 30, -0.005, 0.005, 0,0,0, "Residual (sin#phi^{reco} - sin#phi^{mc}) [rad]","Entries" ),
    THistoInfo( "resDzDs",    "track dz/ds residual ",   30, -0.005, 0.005, 0,0,0, "Residual (dz/ds^{reco} - dz/ds^{mc})","Entries" ),
    THistoInfo( "resPt",      "track p_{t} resolution",  30, -0.3,   0.3, 0,0,0, "Resolution (p_{t}^{reco} - p_{t}^{mc})/p_{t}^{mc}","Entries"  ),
#elif ALICE_ITS
    THistoInfo( "resX1",       "track y residual [cm]",   30, -0.01, 0.01, 0,0,0, "Residual (y^{reco} - y^{mc}) [cm]","Entries" ),
    THistoInfo( "resX2",       "track z residual [cm]",   30, -0.03, 0.03, 0,0,0, "Residual (z^{reco} - z^{mc}) [cm]","Entries" ),
    THistoInfo( "resSinPhi",  "track sin#phi residual ", 30, -0.03, 0.03, 0,0,0, "Residual (sin#phi^{reco} - sin#phi^{mc}) [rad]","Entries" ),
    THistoInfo( "resDzDs",    "track dz/ds residual ",   30, -0.03, 0.03, 0,0,0, "Residual (dz/ds^{reco} - dz/ds^{mc})","Entries" ),
    THistoInfo( "resPt",      "track p_{t} resolution",  30, -1.0,   1.0, 0,0,0, "Resolution (p_{t}^{reco} - p_{t}^{mc})/p_{t}^{mc}","Entries"  ),
#elif defined(PANDA_STT)
    THistoInfo( "resX1",       "track y residual [cm]",   30, -.05,   .05, 0,0,0, "Residual (y^{reco} - y^{mc}) [cm]","Entries" ),
    THistoInfo( "resX2",       "track z residual [cm]",   30, -2.5,   2.5, 0,0,0, "Residual (z^{reco} - z^{mc}) [cm]","Entries" ),
    ...
    THistoInfo( "resX1",      "track x residual [cm]",   100,  -0.1,   0.1, 0,0,0, "Residual (x^{reco} - x^{mc}) [cm]","Entries" ),
    THistoInfo( "resX2",      "track y residual [cm]",   100,  -0.7,   0.7, 0,0,0, "Residual (y^{reco} - y^{mc}) [cm]","Entries" ),
    THistoInfo( "resTx1",     "track Tx residual ",      100, -0.01,  0.01, 0,0,0, "Residual (T_{x}^{reco} - T_{x}^{mc}) [rad]","Entries" ),
    THistoInfo( "resTx2",     "track Ty residual ",      100, -0.05,  0.05, 0,0,0, "Residual (T_{y}^{reco} - T_{y}^{mc})","Entries" ),
    THistoInfo( "resP",       "track p resolution",      100,  -0.2,   0.2, 0,0,0, "Resolution (p^{reco} - p^{mc})/p^{mc}","Entries"  ),
#endif

#ifdef PANDA_FTS
    THistoInfo( "pullX1",      "track x pull",            30, -PullWidth,   PullWidth,   0,0,0, "Pull x" ),
    THistoInfo( "pullX2",      "track y pull",            30, -PullWidth,   PullWidth,   0,0,0, "Pull y" ),
    THistoInfo( "pullTx1",     "track Tx pull",           30, -PullWidth,   PullWidth,   0,0,0, "Pull Tx" ),
    THistoInfo( "pullTx2",     "track Ty pull",           30, -PullWidth,   PullWidth,   0,0,0, "Pull Ty" ),
    THistoInfo( "pullQP",     "track q/p pull",           30, -PullWidth,   PullWidth,   0,0,0, "Pull q/p" ),
#else
    THistoInfo( "pullX1",      "track y pull",            30, -PullWidth,   PullWidth,   0,0,0, "Pull y" ),
    THistoInfo( "pullX2",      "track z pull",            30, -PullWidth,   PullWidth,   0,0,0, "Pull z" ),
    ...
    THistoInfo( "resRHit",       "hits R resolution [cm]",  100, -0.1, 0.1, 0,0,0, "Residual (r^{reco} - r^{mc}) [cm]","Entries" ),
    THistoInfo( "pullRHit",      "hits R pull",              30,   -PullWidth,   PullWidth, 0,0,0, "Pull r" ),
#else
    THistoInfo( "resXHit",       "hits X resolution [cm]",  1000, -1.25, 1.25, 0,0,0, "Residual (x^{reco} - x^{mc}) [cm]","Entries" ),
    THistoInfo( "resYHit",       "hits Y resolution [cm]",  100, -0.02, 0.02, 0,0,0, "Residual (y^{reco} - y^{mc}) [cm]","Entries" ),
    THistoInfo( "resZHit",       "hits Z resolution [cm]",  100,   -0.1,   0.1, 0,0,0, "Residual (z^{reco} - z^{mc}) [cm]","Entries" ),
#endif
    ...
    THistoInfo( "xMCPoint",       "MCPoint X position [cm]",  120, 0, 60, 0,0,0, "x^{mc} [cm]","Entries" ),
    THistoInfo( "rMCPoint",       "MCPoint R position [cm]",  120, 0, 60, 0,0,0, "r^{mc} [cm]","Entries" ),

    THistoInfo( "resPtVsP",    "track p_{t} resolution vs p",  20, 0, 10, 30, -0.5, 0.5, "p [GeV/c]", "Resolution (p_{t}^{reco} - p_{t}^{mc})/p_{t}^{mc}" ),
    THistoInfo( "resPVsP",    "track p resolution vs p",  20, 0, 10, 30, -0.5, 0.5, "p [GeV/c]", "Resolution (p^{reco} - p^{mc})/p^{mc}" ),
...
void PndFTSCATrackPerformanceBase::CreateHistos(string histoDir, TFile* outFile)
{
...
void PndFTSCATrackPerformanceBase::FillHistos()
{
  for ( int iMCTr = 0; iMCTr < nMCTracks; iMCTr++ ) {
    PndFTSCAPerformanceMCTrackData &mcD = mcData[iMCTr];
    PndFTSCAMCTrack &mcTr = (*fMCTracks)[iMCTr];
