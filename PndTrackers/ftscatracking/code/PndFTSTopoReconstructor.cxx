...
{
  if (fKFPTopoReconstructor) delete fKFPTopoReconstructor;
}
...
// 
//     fChiToPrimVtx[iOTr] = (trParam.Cov()[2]* trParam.Y()*trParam.Y() + trParam.Z() *(-2 *trParam.Cov()[1]*trParam.Y() + trParam.Cov()[0] *trParam.Z()))/(-trParam.Cov()[1]*trParam.Cov()[1] + trParam.Cov()[0]* trParam.Cov()[2]);
                                                    ...
                                                    vector<float>& chi)
{
  ...
  for(unsigned short iTr=0; iTr < NTr; iTr++)
  {
    ...
    float pvC[3] = { cA*cA*cov[2][0] + 2* cA *cov[1][0] *sA + cov[0][0]* sA*sA,
                     cA* cov[4][0] + cov[3][0]* sA,
                     cov[5][0]};
