 ...
 *
 * This file contains the classes PndFTSResizableArray and PndFTSFixedArray with PndFTSArray as base
...
template<typename T, int Dim> class PndFTSArray;
...
namespace PndFTSInternal
{
  ...
  template<typename T> struct TypeForAlignmentHelper<T, PndFTSFullyCacheLineAligned> { typedef CacheLineSizeHelper<T> Type; };
...
#define BOUNDS_CHECK(x, y) if (PndFTSInternal::ArrayBoundsCheck::IsInBounds(x)) {} else return y
#endif
  ...
  template<typename T> class Allocator<T, PndFTSFullyCacheLineAligned>
  {
    public:
...
//       static inline void Free( const T *const p, int ) { delete[] p; }
  ...
  template<typename T, unsigned int Size, int _alignment> class AlignedData
  {
    ...
    private:
      enum {
        Alignment = _alignment == PndFTSFullyCacheLineAligned ? 128 : _alignment,
...
class PndFTSArray : public PndFTSInternal::ArrayBase<T, Dim>
{
  public:
    ...
    template<typename Other> inline PndFTSArray<Other, Dim> ReinterpretCast() const {
...
template<unsigned int x, unsigned int y = 0, unsigned int z = 0> class PndFTSArraySize
{
  public:
    enum {
