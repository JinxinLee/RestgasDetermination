...
class L1FieldSlice{
  ...
  float_v cx[21], cy[21], cz[21]; // polinom coeff.
    ...
    L1FieldSlice()
    {
      for( int i=0; i<21; i++ ) cx[i] = cy[i] = cz[i] = Vc::Zero;
    }
    ...
    void GetFieldValue( const float_v &x, const float_v &y, CAFieldValue &B, const float_m &mask = float_m(true) ) const
    {
      ...
      B.x(mask) = cx[0] +cx[1]*x +cx[2]*y +cx[3]*x2 +cx[4]*xy +cx[5]*y2 +cx[6]*x3 +cx[7]*x2y +cx[8]*xy2 +cx[9]*y3
          +cx[10]*x4 +cx[11]*x3y +cx[12]*x2y2 +cx[13]*xy3 +cx[14]*y4
          +cx[15]*x5 +cx[16]*x4y +cx[17]*x3y2 +cx[18]*x2y3 +cx[19]*xy4 +cx[20]*y5;

      B.y(mask) = cy[0] +cy[1]*x +cy[2]*y +cy[3]*x2 +cy[4]*xy +cy[5]*y2 +cy[6]*x3 +cy[7]*x2y +cy[8]*xy2 +cy[9]*y3
          +cy[10]*x4 +cy[11]*x3y +cy[12]*x2y2 +cy[13]*xy3 +cy[14]*y4
          +cy[15]*x5 +cy[16]*x4y +cy[17]*x3y2 +cy[18]*x2y3 +cy[19]*xy4 +cy[20]*y5;

      B.z(mask) = cz[0] +cz[1]*x +cz[2]*y +cz[3]*x2 +cz[4]*xy +cz[5]*y2 +cz[6]*x3 +cz[7]*x2y +cz[8]*xy2 +cz[9]*y3
...
class L1FieldRegion{
  ...
  public:
    L1FieldRegion():
      cx0(Vc::Zero), cx1(Vc::Zero), cx2(Vc::Zero),
      cy0(Vc::Zero), cy1(Vc::Zero), cy2(Vc::Zero),
      cz0(Vc::Zero), cz1(Vc::Zero), cz2(Vc::Zero),
      z0(Vc::Zero)
    {}
    ...
    void Set( const CAFieldValue &B0, const float_v B0z,
              const CAFieldValue &B1, const float_v B1z,
              const CAFieldValue &B2, const float_v B2z )
    {
      z0 = B0z;
  ...
  friend ostream& operator<<(ostream& out, L1FieldRegion &B){
    return out << " FieldRegion " << endl <<
      B.cx0 << endl << B.cx1 << endl << B.cx2 << endl <<
      B.cy0 << endl << B.cy1 << endl << B.cy2 << endl <<
      B.cz0 << endl << B.cz1 << endl << B.cz2 << endl <<
      B.z0 << endl;
  };
};
