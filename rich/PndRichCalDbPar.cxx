...
Bool_t PndRichCalDbPar::getParams(FairParamList* list) {
  ...
  if ( ! list->fill("Cal db Type", &fType) ) return kFALSE;

  if ( fType == 0 ) {                    // constant field
    if ( ! list->fill("Cal db min p", &fPmin) ) return kFALSE;
    if ( ! list->fill("Cal db max p", &fPmax) ) return kFALSE;
    if ( ! list->fill("Cal db min x", &fXmin) ) return kFALSE;
    if ( ! list->fill("Cal db max x", &fXmax) ) return kFALSE;
    if ( ! list->fill("Cal db min y", &fYmin) ) return kFALSE;
    if ( ! list->fill("Cal db max y", &fYmax) ) return kFALSE;
    if ( ! list->fill("Cal db min t", &fTmin) ) return kFALSE;
    if ( ! list->fill("Cal db max t", &fTmax) ) return kFALSE;
    if ( ! list->fill("Cal db min f", &fFmin) ) return kFALSE;
    if ( ! list->fill("Cal db max f", &fFmax) ) return kFALSE;
    if ( ! list->fill("Cal db BetaMean", &fBetaMean) ) return kFALSE;
    if ( ! list->fill("Cal db BetaSig", &fBetaSig) ) return kFALSE;
    if ( ! list->fill("Cal db BetaEff", &fBetaEff) ) return kFALSE;
  }
