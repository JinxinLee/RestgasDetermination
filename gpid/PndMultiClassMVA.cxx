...
/*
  ...
  (fInFileNameArray.find(className) == fInFileNameArray.end()) {
  cout<<" -E- This class named "<<className<<" is not a part of your
...
// this is the main call which does the training for all the class
// types added
void PndMultiClassMVA::TrainTest()
{
  ...
  if (fNCLASS < 2 ) {
    ...
    std::cout<< " you need atleast two variables"
	     <<" for the Multivariate Analysis "<<endl;
    return; //exit(0);
  }
  
  else if (!gSystem->AccessPathName( fINFILENAME )) {
    std::cout << "--- BDTAnalysis  : accessing " 
