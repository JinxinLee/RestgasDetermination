// -------------------------------------------------------------------------
// -----                PndMvdGemTrackFinderQA source file             -----
// -----                Created 02.06.2009 by R. Karabowicz            -----
// -------------------------------------------------------------------------

#include "PndMvdGemTrackFinderQA.h"
...
// -----   Default constructor   ------------------------------------------
PndMvdGemTrackFinderQA::PndMvdGemTrackFinderQA() : FairTask("MVDGEM Track Finder QA", 1) {
  fDigiPar       = NULL;
  fMCTrackArray  = NULL;
  fMCPointArray  = NULL;
  fGemHitArray   = NULL;
  fMvdGemTrackArray = NULL;
  fNofEvents    = 0;
...
PndMvdGemTrackFinderQA::PndMvdGemTrackFinderQA(Int_t iVerbose) 
  : FairTask("MVDGEM Track Finder QA", iVerbose) {
  fDigiPar       = NULL;
  fMCTrackArray  = NULL;
  fMCPointArray  = NULL;
  fGemHitArray   = NULL;
...
// -----   Destructor   ----------------------------------------------------
PndMvdGemTrackFinderQA::~PndMvdGemTrackFinderQA() { }

// -----   Init  -----------------------------------------------------------
InitStatus PndMvdGemTrackFinderQA::Init() {
  ...
  // Get Gem track Array
  fMvdGemTrackArray  = (TClonesArray*) ioman->GetObject("MVDGEMTrack");
  if ( !fMvdGemTrackArray ) {
...
InitStatus PndMvdGemTrackFinderQA::ReInit() {
...
void PndMvdGemTrackFinderQA::SetParContainers() {
...
// -----   Public method Exec   --------------------------------------------
void PndMvdGemTrackFinderQA::Exec(Option_t* opt) {
  ...
  Int_t nofRecoTracks = fMvdGemTrackArray->GetEntriesFast();
  ...
  for ( Int_t imct = 0 ; imct < nofMCTracks ; imct++ ) {
    ...
    if ( mcTrack->GetNPoints(kGEM)*mcTrack->GetNPoints(kMVD) == 0 ) continue;

    if ( fVerbose ) {
      cout << "P    = " << mcTrack->GetMomentum().Mag() 
	   << " the = " << mcTrack->GetMomentum().Theta()*TMath::RadToDeg()
	   << " phi = " << mcTrack->GetMomentum().Phi()*TMath::RadToDeg()
	   << ", pt = " << mcTrack->GetPt() 
	   << ", pz = " << mcTrack->GetMomentum().Z() << endl;
    }
    ...
    for ( Int_t irtr = 0 ; irtr < nofRecoTracks ; irtr++ ) {
      ...
      gemTrack = (PndTrack*) fMvdGemTrackArray->At(irtr);
    ...
    if (  mcTrackReconstructed == kFALSE ) {
  ...
  if ( fVerbose ) {
    ...
    cout << "---- PndMvdGemTrackFinderQA : Event " << fNofEvents << " summary -----" << endl;
...
void PndMvdGemTrackFinderQA::PrepareMCTracks() {
...
// -----   Private method PrepareMCTrack   --------------------------------------------
void PndMvdGemTrackFinderQA::MatchRecoTracks() {
...
void PndMvdGemTrackFinderQA::CreateHistos() {
...
void PndMvdGemTrackFinderQA::DivideHistos(TH1* hist1, TH1* hist2, TH1* hist3) {
  hist1->Sumw2();
  hist2->Sumw2();
...
void PndMvdGemTrackFinderQA::Finish() {
  Double_t effAcc  = 0.;
  Double_t effPrim = 0.;
  Double_t effSec  = 0.;
  Double_t effRef  = 0.;
  ...
  cout << "-------------------- PndMvdGemTrackFinderQA : Summary ------------------" << endl;
  ...
  gDirectory->mkdir("MvdGemTrackFinderQA");
  gDirectory->cd("MvdGemTrackFinderQA");
...
ClassImp(PndMvdGemTrackFinderQA)
