// -------------------------------------------------------------------------
// -----          PndGlobalKalmanTask  source file                     -----
...
PndGlobalKalmanTask::PndGlobalKalmanTask()
  : FairTask("Kalman Filter"), fPersistence(kFALSE)
{
  fTrackBranchName = "GlobalTrack";
}
...
PndGlobalKalmanTask::Init()
{
  ...
  if(ioman==0)
    {
      Error("PndGlobalKalmanTask::Init","RootManager not instantiated!");
      return kERROR;
    }

  // Get input collection
  fTrackArray=(TClonesArray*) ioman->GetObject(fTrackBranchName.Data());

  if(fTrackArray==0)
    {
      Error("PndGlobalKalmanTask::Init","track-array not found!");
      return kERROR;
    }

  std::cout << "-I- RECO HIT FACTORY: " << std::endl;
  // Build hit factory -----------------------------
  fTheRecoHitFactory = new GFRecoHitFactory();
  // ----  MVD strip reco hit factory  ---------------------------------------------
  std::cout << "-I-     creating MVD STRIP factory... " << std::flush;
  ...
  std::cout << "-I- (" << kMVDHitsStrip << ") ...DONE! " << std::endl;
  // ----  MVD pixel reco hit factory  ---------------------------------------------
  std::cout << "-I-     creating MVD PIXEL factory... " << std::flush;
  ...
  std::cout << "-I- (" << kMVDHitsPixel << ") ...DONE! " << std::endl;
  // ----  STT reco hit factory  ---------------------------------------------------
  std::cout << "-I-     creating STT factory... " << std::flush;
  TClonesArray* sttHitArray=(TClonesArray*) ioman->GetObject("STTHit");
  if(sttHitArray==0){ //TODO Convention on detector number needed
    Error("PndGlobalKalmanTask::Init","STTHit array not found");
  } else {
    std::cout << "(stt hit array exists)" << std::flush;
    fTheRecoHitFactory->addProducer
  ...
  if(tpcHitArray==0){ //TODO Convention on detector number needed
    Error("PndGlobalKalmanTask::Init","PndTpcCluster array not found");
  } else {
    fTheRecoHitFactory->addProducer
      (kTpcCluster,new GFRecoHitProducer<PndTpcCluster,PndTpcSPHit>(tpcHitArray));
  }
  std::cout << "-I- (" << kTpcCluster << ") ...DONE! " << std::endl;
  // ----  GEM reco hit factory  ---------------------------------------------------
  std::cout << "-I-     creating GEM factory... " << std::flush;
  TClonesArray* gemHitArray=(TClonesArray*) ioman->GetObject("GEMHit");
  if(gemHitArray==0){ //TODO Convention on detector number needed
    Error("PndGlobalKalmanTask::Init","GEMHit array not found");
  } else {
    fTheRecoHitFactory->addProducer
      (kGemHit,new GFRecoHitProducer<PndGemHit,PndGemRecoHit>(gemHitArray));
  }
  std::cout << "-I- (" << kGemHit << ") ...DONE! " << std::endl;
  // ----  DCH reco hit factory  ---------------------------------------------------
  std::cout << "-I-     creating DCH factory... " << std::flush;
  ...
  if(dchCylHitArray==0){ //TODO Convention on detector number needed
    Error("PndGlobalKalmanTask::Init","PndDchCylinderHit array not found");
  } else {
    fTheRecoHitFactory->addProducer
      (kDchHit,new GFRecoHitProducer<PndDchCylinderHit,PndDchRecoHit2>(dchCylHitArray));
  }
  std::cout << "-I- (" << kDchHit << ") ...DONE! " << std::endl;
  // -------------------------------------------------------------------------------
  std::cout << "-I- RECO HIT FACTORY DONE! " << std::endl;
...
void
PndGlobalKalmanTask::Exec(Option_t* opt)
{
  std::cout<<"PndGlobalKalmanTask::Exec"<<std::endl;
  // Reset output Array
  ...
  // Detailed output
  if(fVerbose>1)std::cout<<" -I- PndGlobalKalmanTask: contains "<<ntracks<<" Tracks."<<std::endl;
  if(fVerbose>2){
  ...
  for(Int_t itr=0;itr<ntracks;++itr){
    ...
    if(trac->getTrackRep(0)->getStatusFlag()==0 && 1==0){
...
//      trac->getTrackRep(0)->Print();
  ...
  std::cout << "End of PndGlobalKalmanTask::Exec()" << std::endl;
  return;
}

void
PndGlobalKalmanTask::WriteHistograms(){
  std::cout<<"  PndGlobalKalmanTask::WriteHistograms() "<<std::endl;
  TFile* file = FairRootManager::Instance()->GetOutFile();
  file->cd();
  file->mkdir("GlobalKalmanQA");
  file->cd("GlobalKalmanQA");
...
void PndGlobalKalmanTask::Finish(){
  WriteHistograms();
}


ClassImp(PndGlobalKalmanTask);
