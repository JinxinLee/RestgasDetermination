...
void PndLmdLineTask::Exec(Option_t* opt)
{
  ...
  // Fitting ----------------------------------------------------------------------------------
  if(fVerbose>1)std::cout<<" -I- PndLmdLineTask: start Fitting "<<std::endl;
  int rec_tkr=0;
  for(Int_t track=0; track<ntcand; track++)
  {
    ...
    PndLinTrack* trackfit = new PndLinTrack("Lumi", posSeed.X(), dirSeed.X(), posSeed.Y(), dirSeed.Y(), posSeed.Z(), posSeed.Z(), 
					    0, firstHit, lastHit, track);
