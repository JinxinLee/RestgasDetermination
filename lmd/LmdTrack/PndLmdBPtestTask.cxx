...
// -----   Default constructor   -------------------------------------------
PndLmdBPtestTask::PndLmdBPtestTask() : FairTask("test with Geane Task for PANDA Lmd"), fEventNr(0), fUseMVDPoint(false)
{
  //tprop = new TNtuple();
}
// -------------------------------------------------------------------------
...
PndLmdBPtestTask::PndLmdBPtestTask(Double_t pBeam,TVector3 IP): FairTask("Geane Task for PANDA Lmd"), fEventNr(0), fUseMVDPoint(false)
{
  fPDGid=-2212;
  fPbeam = pBeam;
...
InitStatus PndLmdBPtestTask::Init()
{
  ...
  if ( !ioman){
    std::cout << "-E- PndLmdBPtestTask::Init: "<< "RootManager not instantiated!" << std::endl;
    return kFATAL;
  }
  ...
  if ( !fMCHits)	{
    std::cout << "-W- PndLmdBPtestTask::Init: "<< "No LMDPoint"<<" array!" << std::endl;
    return kERROR;
  }
 ...
 //Get MC tracks
  ...
  if ( !fRecCandTracks)	{
    std::cout << "-W- PndLmdBPtestTask::Init: "<< "No LMDTrackCand [needed only for tests!!!]"<<" array!" << std::endl;
    return kERROR;
 }
  ...
  if ( !fRecHits)	{
    std::cout << "-W- PndLmdBPtestTask::Init: "<< "No LMDHitsMerged [needed only for tests!!!]"<<" array!" << std::endl;
    return kERROR;
  }
...
FairTrackParP* PndLmdBPtestTask::PropToPlane(FairTrackParP* fStartPst, double zpos,int dir, bool& isProp){
