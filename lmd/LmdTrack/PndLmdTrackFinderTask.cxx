...
bool PndLmdTrackFinderTask::SortHitsByZ(
  ...
  //   cout << "Hits: " << nStripHits << endl;
  if (fVerbose > 2) {
    cout << "Hits: " << nStripHits << " in " << detZ.size() << " plane(s)."
         << endl;
    for (size_t idet = 0; idet < detZ.size(); idet++)
      cout << "Plane: " << idet << " DiscHits: " << hitsd.at(idet).size()
           << endl;
...
void PndLmdTrackFinderTask::FindHitsIII(
    ...
    std::vector<std::vector<std::pair<Int_t, bool> > > &hitsd,
    Int_t)  // nStripHits //FIXME [R.K.03/2017] unused variable(s)
{
  ...
  for (size_t i = 0; i < hitsd.at(1).size(); i++) {
    ...
    for (size_t k = 0; k < hitsd.at(2).size(); k++) {
      ...
      PndSdsHit *hit2 =
          (PndSdsHit *)fStripHitArray->At(hitsd.at(2).at(k).first);
  ...
  for (size_t i = 0; i < trackStart.size(); i++)  // pseudo-loop
  {
    ...
    for (Int_t idet = 3; idet < 4;
      ...
      for (size_t ihit = 0; ihit < hitsd.at(idet).size(); ihit++) {
        PndSdsHit *hit =
            (PndSdsHit *)fStripHitArray->At(hitsd.at(idet).at(ihit).first);
    ...
    Int_t)  // nStripHits //FIXME [R.K.03/2017] unused variable(s)
{
  ...
  for (size_t i = 0; i < hitsd.at(0).size(); i++) {
    ...
    for (size_t k = 0; k < hitsd.at(1).size(); k++) {
      PndSdsHit *hit2 =
          (PndSdsHit *)fStripHitArray->At(hitsd.at(1).at(k).first);
      tmp.SetXYZ(hit2->GetX(), hit2->GetY(), hit2->GetZ());
      vec = tmp - start;  // calc direction vector for FINDING
      dvec.SetXYZ(hit2->GetDx(), hit2->GetDy(), hit2->GetDz());
      if (vec.Theta() > 0.03 && vec.Theta() < 0.05 && vec.Phi() > -0.25 &&
...
void PndLmdTrackFinderTask::Exec(Option_t *) {
  ...
  // bool usedFlag[nStripHits]; //for pseudo-vector building //[R.K.03/2017]
  // unused variable
  ...
  if (flagStipSens)
    ...
    if (flagPixelSens)
      ...
      std::cout << "Algorithm is needed sensor type! Please, set it via "
                   "SetSensStripFlag(bool fS) or SetSensPixelFlag(bool fS)"
                << std::endl;
      return;
    }
  }
  ...
  if (fVerbose > 2) {
    ...
    if (hitsd.size() > 0) {
      for (size_t i = 0; i < hitsd.at(0).size(); i++) {
        PndSdsHit *hit =
            (PndSdsHit *)fStripHitArray->At(hitsd.at(0).at(i).first);
    ...
    if (hitsd.size() > 1) {
      for (size_t i = 0; i < hitsd.at(1).size(); i++) {
        PndSdsHit *hit =
            (PndSdsHit *)fStripHitArray->At(hitsd.at(1).at(i).first);
    ...
    if (hitsd.size() > 2) {
      for (size_t i = 0; i < hitsd.at(2).size(); i++) {
        PndSdsHit *hit =
            (PndSdsHit *)fStripHitArray->At(hitsd.at(2).at(i).first);
    ...
    if (hitsd.size() > 3) {
      for (size_t i = 0; i < hitsd.at(3).size(); i++) {
        PndSdsHit *hit =
            (PndSdsHit *)fStripHitArray->At(hitsd.at(3).at(i).first);
