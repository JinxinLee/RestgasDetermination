...
//TODO: change local funtion for transformation from\in LUMI frame to global (geometry file)
void PndLmdKalmanTask::combitransFromLumiFrame(TVector3& hitPos){
...
void PndLmdKalmanTask::rotateFromLumiFrame(TVector3& hitPos, bool err){
  TMatrixD hitMtx(3,3);
  if(err){
...
TMatrixD PndLmdKalmanTask::rotateFromLumiFrame(TMatrixD& hitCov){
...
void PndLmdKalmanTask::rotateToLumiFrame(TVector3& hitPos){
  TMatrixD hitMtx(3,3);
  hitMtx[0][0] = hitPos[0];
  hitMtx[1][0] = hitPos[1];
  hitMtx[2][0] = hitPos[2];
...
TMatrixD PndLmdKalmanTask::rotateToLumiFrame(TMatrixD& hitCov){
  Double_t theta=2.326;
  Double_t degrad = TMath::Pi()/180.;
