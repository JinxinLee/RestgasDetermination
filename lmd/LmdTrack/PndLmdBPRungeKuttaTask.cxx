...
// -----   Default constructor   -------------------------------------------
PndLmdBPRungeKuttaTask::PndLmdBPRungeKuttaTask() : FairTask("Back Propagation Task for PANDA Lmd (with RK)"), fEventNr(0), fUseMVDPoint(false)
{
}
// -------------------------------------------------------------------------


PndLmdBPRungeKuttaTask::PndLmdBPRungeKuttaTask(Double_t pBeam,TVector3 IP): FairTask("Geane Task for PANDA Lmd"), fEventNr(0), fUseMVDPoint(false)
{
  fPbeam = pBeam;
  ...
  // // hxresstep = new TH2D("hxresstep","x_{PCA} vs. z cood of back-propag start",1e2,0,1200,1e3,-0.5,0.5); //15 GeV
  // hxresstep = new TH2D("hxresstep","x_{PCA} vs. z cood of back-propag start",5e2,0,1500,1e3,-5,5);//1.5 GeV
}
...
InitStatus PndLmdBPRungeKuttaTask::Init()
{
  ...
  if ( !ioman){
    std::cout << "-E- PndLmdBPRungeKuttaTask::Init: "<< "RootManager not instantiated!" << std::endl;
    return kFATAL;
  }

  fMCHits = (TClonesArray*) ioman->GetObject("LMDPoint");
  if ( !fMCHits)	{
    std::cout << "-W- PndLmdBPRungeKuttaTask::Init: "<< "No LMDPoint"<<" array!" << std::endl;
    return kERROR;
  }
...
void PndLmdBPRungeKuttaTask::Exec(Option_t* opt)
{
  ...
  for (Int_t i = 0; i<glI;i++){ 
	...
	if(fVerbose>2){
	cout<<"!!! Back-propagation with Runge-Kutta didn't return result !!!"<<endl;
	cout<<"StartPos:"<<endl;
	StartPos.Print();
	cout<<"StartPosErr:"<<endl;
	StartPosErr.Print();
