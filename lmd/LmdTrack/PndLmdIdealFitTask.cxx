...
void PndLmdIdealFitTask::Exec(Option_t* opt)
{
  ...
  // Find MC-Track ------------------------------------------------------------------------------
  if(fVerbose>1)std::cout<<" -I- PndLmdIdealFitTask: start Fitting "<<std::endl;
  ...
  for(Int_t track=0; track<ntcand; track++)
  {
    ...
    PndSdsClusterStrip *clust = (PndSdsClusterStrip*)fClustArray->At(firstHit->GetClusterIndex());
    PndSdsDigiStrip *astripdigi = (PndSdsDigiStrip*)fDigiArray->At(clust->GetDigiIndex(0));
    PndSdsMCPoint *firstPoint=(PndSdsMCPoint*)fMCPointArray->At(astripdigi->GetIndex());
