...
double IcpPointToPoint::fitStep (double *T,const int32_t ,Matrix &R,Matrix &t,const std::vector<int32_t> &active) { // T_numA //FIXME [R.K.03/2017] unused variable(s)
	...
	if (dim==2) {
		...
		for (size_t i=0; i<active.size(); i++) {
			...
			// set model point
			p_m.val[i][0] = M_tree->the_data[result[0].idx][0];
			mu_m.val[0][0] += p_m.val[i][0];
			p_m.val[i][1] = M_tree->the_data[result[0].idx][1];
			mu_m.val[0][1] += p_m.val[i][1];
...
double IcpPointToPoint::fitInstant(double* T, const int32_t T_numA, Matrix& R,Matrix& t, const std::vector<int32_t>& active) {
	...
	if(dim==2){
		...
		for(int i=0; i< T_numA; i++){
			...
			// set model point
			p_m.val[i][0] = M_[2*i + 0];
			mu_m.val[0][0] += p_m.val[i][0];
			p_m.val[i][1] = M_[2*i + 1];
			mu_m.val[0][1] += p_m.val[i][1];
		...
		for(int i=0; i< T_numA; i++){
			...
			// set model point
			p_m.val[i][0] = M_[3*i + 0]; mu_m.val[0][0] += p_m.val[i][0];
			p_m.val[i][1] = M_[3*i + 1]; mu_m.val[0][1] += p_m.val[i][1];
			p_m.val[i][2] = M_[3*i + 2]; mu_m.val[0][2] += p_m.val[i][2];
	...
	return max((R_-Matrix::eye(dim)).l2norm(),t_.l2norm());
}
