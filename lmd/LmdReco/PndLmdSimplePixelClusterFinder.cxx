...
PndLmdSimplePixelClusterFinder::PndLmdSimplePixelClusterFinder(Int_t verbose):PndSdsSimplePixelClusterFinder(){
	...
	if (fDigiPar->GetChargeConvMethod() == 0){
		if(fVerbose>0) std::cout<<"Info in <PndLmdSimplePixelClusterFinder>: ideal charge conversion"<<std::endl;
		fChargeConverter = new PndSdsIdealChargeConversion(fDigiPar->GetNoise());
		}
	else if (fDigiPar->GetChargeConvMethod() == 1){
		if(fVerbose>0) std::cout<<"Info in <PndLmdSimplePixelClusterFinder>: TOT charge conversion"<<std::endl;
		...
		std::cout<<"Error in <PndLmdSimplePixelClusterFinder>: charge conversion method not defined, use ideal converter..."<<std::endl;
		fChargeConverter = new PndSdsIdealChargeConversion(fDigiPar->GetNoise());
	}
