...
InitStatus PndLmdPixelClusterTask::Init() {
  ...
  std::cout << "-I- PndSdsPixelClusterTask: Initialisation successfull"
            << std::endl;
  return kSUCCESS;
}
// -------------------------------------------------------------------------
...
void PndLmdPixelClusterTask::Exec(Option_t*) {
  ...
  if (fVerbose > 0)
    std::cout << "-I- PndSdsPixelClusterTask::Exec EventTime: " << EventTime
              << std::endl;
  ...
  if (FairRunAna::Instance()->IsTimeStamp()) {
    ...
    fDigiArray = FairRootManager::Instance()->GetData(
        fInBranchName, fStartFunctor, EventTime + 10);
  } else
    fDigiArray =
  ...
  std::vector<std::vector<Int_t> > clusters =
      fClusterFinder->GetClusters(DigiPixelArray);
  if (fVerbose > 1)
    std::cout << " -I-  PndSdsPixelClusterTask::Exec(): We have "
              << clusters.size() << " pixel clusters" << std::endl;
  // store the list
  for (UInt_t i = 0; i < clusters.size(); i++) {
    PndSdsClusterPixel* tempCluster =
        new ((*fClusterArray)[i]) PndSdsClusterPixel(fInBranchId, clusters[i]);
    if (FairRunAna::Instance()->IsTimeStamp()) {
      tempCluster->ResetLinks();
      for (UInt_t j = 0; j < clusters[i].size(); j++) {
        PndSdsDigiPixel* tempDigi =
            (PndSdsDigiPixel*)fDigiArray->At(clusters[i][j]);
  ...
  for (UInt_t i = 0; i < clusters.size(); i++) {
    ...
    new (clref[size]) PndSdsHit(
        myHit);  // PndSdsHit *freshHit =  //[R.K.03/2017] unused variable
    // myHit.Delete();
    //      myHit.Clear();
  ...
  if (fVerbose > 1) {
    std::cout << "-I- PndLmdPixelClusterTask: "
              << fClusterArray->GetEntriesFast() << " Sds Clusters and "
