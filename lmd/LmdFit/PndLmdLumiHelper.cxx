...
void PndLmdLumiHelper::fillHists(double plab, TString dir_path,
  ...
  //--- Track info -----------------------------------------------------------------
  TClonesArray* reco_tracks = new TClonesArray("PndLinTrack");
  tTrack.SetBranchAddress("LMDTrack", &reco_tracks); //Reco Track to compare (at lumi monitor system, so not backtracked)
  tTrack.SetBranchStatus("LMDTrack*", 1);
  //--------------------------------------------------------------------------------
  ...
  TLorentzVector beam(0, 0, plab, sqrt(pow(plab, 2.0) + pow(C_M_P, 2.0)));
  ...
  for (Int_t j = 0; j < num_events; j++) {
     ...
     *
     */
    for (unsigned int ik = 0; ik < nMCParticles; ik++) {
    ...
    for (Int_t iN = 0; iN < nRecoParticles; iN++) {
      ...
      if (fRes->GetLambda() != 0) {
        ...
        lv_rec.SetE(sqrt(MomRec.Mag2() + pow(C_M_P, 2.0)));
  ...
  for (int k = 0; k < lmd_vec->size(); k++) {
    (*lmd_vec)[k]->makeRooFitDataHists();
  }

  if (is_acceptance) {
    for (int k = 0; k < registered_acceptances.size(); k++) {
...
std::vector<std::pair<PndLmdData*, PndLmdAcceptance*> > PndLmdLumiHelper::generatePhiSlicedDataSet(
  ...
  double slicewidth = 2 * C_PI / num_phi_slices_;
  for (unsigned int i = 0; i < num_phi_slices_; i++) {
    PndLmdData *data = new PndLmdData(f, mom, th_bins_, phi_bins_per_slice_,
        data_th_range_low_, data_th_range_high_, -C_PI + 1.0 * i * slicewidth,
        -C_PI + 1.0 * (i + 1) * slicewidth);

    PndLmdAcceptance *acc = new PndLmdAcceptance(f, mom, i * 10, 50,
        acc_th_range_low_, acc_th_range_high_, -C_PI + 1.0 * i * slicewidth,
        -C_PI + 1.0 * (i + 1) * slicewidth, 0);
