...
PndLmdData::PndLmdData(TFile *f_, int num_events_, double plab_,
		int elastic_th_bins_, int elastic_phi_bins_,
		double elastic_th_range_low_, double elastic_th_range_high_,
		double elastic_phi_range_low_, double elastic_phi_range_high_,
		double generated_luminosity_per_event_) :
		PndLmdDataBase(f_, num_events_, plab_, elastic_th_bins_,
				elastic_phi_bins_, elastic_th_range_low_,
				elastic_th_range_high_, elastic_phi_range_low_,
				elastic_phi_range_high_) {
	//fit_map =
	//		new std::map<PndLmdLumiModel*, std::vector<PndLmdLumiFitResult*> >();

	fitter = new PndLmdLumiModelFitter();
...
PndLmdLumiFitResult* PndLmdData::Fit(PndLmdAcceptance *lmd_acc,
	...
	for (int i = 0; i < model_fit_res.size(); i++) {
	...
	if (fit_options->getFitterType() == 0) { // if user wants to use ROOT
		// now all we have to do is call fitTo()
		fit_result = fitter->fitTo(this, lmd_acc, fit_options);
	}
...
RooDataHist* PndLmdData::getMeasuredDataHist(
		PndLmdLumiFitOptions* fit_options) const {
	if (fit_options->getFitDimension() == 2) {
		if (fit_options->isSmearingOn()) {
			if (fit_options->isAcceptanceCorrOn()) {
				return roo_data_hist_reco_2d;
			} else {
				std::cout
						<< "WARNING: Requesting data which has flat acceptance and is resolution smeared."
						<< "This cannot exist... the standard reconstruction histogram will be return."
						<< "Make sure that the fit range is chosen appropriately"
						<< std::endl;
				return roo_data_hist_reco_2d;
			}
		} else {
			if (fit_options->isAcceptanceCorrOn()) {
				return roo_data_hist_mc_acc_2d;
			} else {
				return roo_data_hist_mc_2d;
			}
		}
	} else {
		if (fit_options->isSmearingOn()) {
			if (fit_options->isAcceptanceCorrOn()) {
				return roo_data_hist_reco_1d;
			} else {
				std::cout
						<< "WARNING: Requesting data which has flat acceptance and is resolution smeared."
						<< "This cannot exist... the standard reconstruction histogram will be return."
			...
			if (fit_options->isAcceptanceCorrOn()) {
				return roo_data_hist_mc_acc_1d;
			} else {
				return roo_data_hist_mc_1d;
			}
		}
	}
}
	...
	for (std::map<PndLmdAcceptance*, std::vector<PndLmdLumiFitResult*> >::iterator i =
		...
		for (int j = 0; j < i->second.size(); j++) {
			if (i->second[j]->checkFitOptions(fit_options))
				new_fit_res_vec.push_back(
						std::make_pair(i->first, i->second[j]));
		}
	}
