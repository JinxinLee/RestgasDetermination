...
std::pair<double, double> ProductModel1D::getUncertaincy(
	...
	return std::make_pair(
			first->getUncertaincy(x).first * second->eval(x) + second->getUncertaincy(x).first * first->eval(x),
			first->getUncertaincy(x).second * second->eval(x) + second->getUncertaincy(x).second * first->eval(x));
}
...
void ProductModel1D::updateDomain() {
	...
	if (first->getDomainRange() == 0 || second->getDomainRange() == 0) {
		...
		setDomain(
				std::max(first->getDomain().first, second->getDomain().first),
				std::min(first->getDomain().second,
						second->getDomain().second));
	}
}
