...
TGraphErrors* PndLmdResultPlotter::makeGraph(std::vector<combined_values> data,
  ...
  for (unsigned int i = 0; i < size; i++) {
    x[i] = data[i].xval;
    y[i] = data[i].lumival;
    y_err[i] = data[i].lumierr;
  }

  TGraphErrors *graph = new TGraphErrors(size, x, y, 0, y_err);
...
std::vector<PndLmdResultPlotter::combined_values> PndLmdResultPlotter::calcDiff(
    std::vector<combined_values> &vals) {
  //data[i].lumival - data[i].lumiref;
}
...
double PndLmdResultPlotter::calculateYPos(double line, double text_toppos,
    double text_spacing, bool log) {
  if (log) {
    return pow(
        10,
        gPad->GetUymin()
            + (gPad->GetUymax() - gPad->GetUymin())
                * (text_toppos - line * text_spacing));
  } else {
...
TGraphErrors* PndLmdResultPlotter::createGraphFromFitResult(
  ...
  double x[num_evaluations];
  double y[num_evaluations];
  double y_errs[num_evaluations];
  ...
  for (unsigned int i = 0; i < num_evaluations; i++) {
    x[i] = plot_range.first + stepsize * i;
    y[i] = fit_res->getLuminosity() * model->evaluate(&x[i])
        * data->getBinningFactor(fit_res->getLumiFitOptions());
    y_errs[i] = 0;
  }
  return new TGraphErrors(num_evaluations, x, y, 0, y_errs);
}
...
void PndLmdResultPlotter::fillSinglePad(TCanvas *c,
    graph_bundle_1D graph_bundle, const TString &label, bool log_scale,
    bool labels_on) {
  if (c) {
    if (log_scale)
      gPad->SetLogy(1);
    ...
    if (graph_bundle.fit_options->isFitRaw()) {
      ...
      sprintf(ytitle, "# of events / %.3e GeV^{2}/c^{2}", hist->GetXaxis()->GetBinWidth(1));
      ...
      sprintf(ytitle, "# of events / %.3e mrad", hist->GetXaxis()->GetBinWidth(1));
    }
...
void PndLmdResultPlotter::fillAcceptanceInPad(TCanvas *c,
...
TCanvas* PndLmdResultPlotter::makeOverviewCanvas(
  ...
  for (unsigned int i = 0; i < graph_bundles.size(); i++) {
    if (graph_bundles[i].fit_options->getBinaryOptions()
      ...
      c->cd(1);
      fillSinglePad(c, graph_bundles[i], "fit_mc_truth_t", 1);
    } else if (graph_bundles[i].fit_options->getBinaryOptions()
        == fitop_thmctruth.getBinaryOptions()) {
      c->cd(2);
      fillSinglePad(c, graph_bundles[i], "fit_mc_truth", 1);
    } else if (graph_bundles[i].fit_options->getBinaryOptions()
        == fitop_mcacc.getBinaryOptions()) {
      c->cd(3);
      fillSinglePad(c, graph_bundles[i], "fit_mc_acc", 0);
    } else if (graph_bundles[i].fit_options->getBinaryOptions()
        == fitop_normal.getBinaryOptions()) {
      c->cd(4);
      fillSinglePad(c, graph_bundles[i], "fit_reco", 0);
    }
  }
  c->cd(5);
  fillAcceptanceInPad(c, acceptance_bundle);
  return c;
}
