...
PndLmdSmearingDoubleGaussianModelParametrization1D::PndLmdSmearingDoubleGaussianModelParametrization1D(
  ...
  shared_ptr<Model1D> poly_model(new PolynomialModel1D(1));
  ...
  shared_ptr<Model1D> poly_model2(new PolynomialModel1D(1));
  ...
  shared_ptr<Model1D> poly_model3(new PolynomialModel1D(0));
  ...
  shared_ptr<Model1D> poly_model4(new PolynomialModel1D(0));
  poly_model4->getModelParameterSet().setModelParameterValue(
      "poly_poly_factor_0", 0.03449);
  shared_ptr<Model1D> exp_model(new ExponentialModel1D());
  exp_model->getModelParameterSet().setModelParameterValue(
      "exp_amplitude", 0.4413);
  exp_model->getModelParameterSet().setModelParameterValue(
      "exp_exp_factor", -0.5409);
  shared_ptr<Model1D> add_model(new AdditionModel1D(exp_model, poly_model4));
