...
PndLmdDPMModelParametrization::PndLmdDPMModelParametrization(ModelParSet &model_par_set) :
...
void PndLmdDPMModelParametrization::adjustStrictParameters() {
  E_lab->setValue(sqrt(C_M_P * C_M_P + p_lab->getValue() * p_lab->getValue()));
  S->setValue(2 * C_M_P * C_M_P + 2 * C_M_P * E_lab->getValue());
  pcm2->setValue(S->getValue() / 4. - C_M_P * C_M_P);
  beta->setValue(p_lab->getValue() / E_lab->getValue()); // this is beta of beam inside lab frame (used only for cross sections)
  beta_lab_cms->setValue(p_lab->getValue() / (C_M_P + E_lab->getValue())); // this is beta to go from lab to cms system
  gamma->setValue((E_lab->getValue() + C_M_P) / sqrt(S->getValue())); // this boost gamma from lab to cms system
}
...
void PndLmdDPMModelParametrization::adjustNonStrictParameters() {
  ...
  b->setValue(
      (A1_val / T1_val + A1_val * pow(A2_val, 2.0) / T2_val
          - 2.0 * A1_val * A2_val * (1. / 2. / T1_val + 1. / 2. / T2_val) + A3_val / T2_val)
          / (A1_val * pow(1.0 - A2_val, 2.0) + A3_val));
  sigma_tot->setValue(4.0 * sqrt(3.1416 * A1_val * 0.1 / pow(5.067, 2.0)) * (1.0 - A2_val) * 10.0);
