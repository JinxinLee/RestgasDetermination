...
public:
	...
	void initAxis(const PadBoundaries &pad_boundaries) {
		double x_range_low = pad_boundaries.x_min;
		double x_range_high = pad_boundaries.x_max;

		double y_range_low = pad_boundaries.y_min;
		double y_range_high = pad_boundaries.y_max;

		double z_range_low = pad_boundaries.z_min;
		double z_range_high = pad_boundaries.z_max;

		if (x_axis_range.active) {
			x_range_low = x_axis_range.low;
			x_range_high = x_axis_range.high;
		}

		if (y_axis_range.active) {
			y_range_low = y_axis_range.low;
			y_range_high = y_axis_range.high;
		}

		if (z_axis_range.active) {
			z_range_low = z_axis_range.low;
			z_range_high = z_axis_range.high;
		}

		axis_dummy = new TH2D("axis_hist", "", 1, x_range_low, x_range_high, 1,
				y_range_low, y_range_high);
		axis_dummy->Fill(0.0, 0.0, 0.0);

		if (pad_boundaries.is_2d) {
			axis_dummy->GetZaxis()->SetLimits(z_range_low, z_range_high);
			axis_dummy->GetZaxis()->SetRangeUser(z_range_low, z_range_high);
	...
	bool drawHistsAndGraphs(const PlotStyle &plot_style) const {
		if (hasDrawables()) {
			...
			for (hist_it = histograms.begin(); hist_it != histograms.end();
				...
				if (hist) {
					if (plot_style.palette_color_style == 1 && hist->GetDimension() > 1) {
						double maxval = std::max(total_boundaries.z_min,
								total_boundaries.z_max);
						hist->SetMinimum(-maxval);
