...
PndLmdGeometryFactory::PndLmdGeometryFactory(
    ...
    : geometry_property_tree(geometry_property_tree_),
...
void PndLmdGeometryFactory::retrieveMaterial(FairGeoLoader* geoLoad) {
  ...
  if (!FairMediumAir || !FairMediumSteel || !FairMediumAl ||
      !FairMediumKapton || !FairMediumSilicon || !FairMediumVacuum ||
      !FairMediumCopper) {
    LOG(ERROR)
...
TGeoVolumeAssembly* PndLmdGeometryFactory::generateLmdGeometry() const {
  ...
  double pipe_bend_start_position_z(
      pt_general.get<double>("pipe_bend_start_position_z"));
  double pipe_bend_radius(pt_general.get<double>("pipe_bend_radius"));

  double dx_from_bending(pipe_bend_radius *
                         (1.0 - std::cos(pipe_bend_segment_phi)));
  double dx_from_displacement_afterwards(
      (box_center_position_z - pipe_bend_start_position_z -
       pipe_bend_radius * std::sin(pipe_bend_segment_phi)) *
      std::tan(pipe_bend_segment_phi));
  double shift_x_from_dipole(dx_from_bending + dx_from_displacement_afterwards);
  TGeoCombiTrans* lmd_frame_transformation =
      new TGeoCombiTrans("lmd_translation", shift_x_from_dipole, 0.,
                         box_center_position_z, lmd_rot);
...
TGeoVolume* PndLmdGeometryFactory::generateLmdBox() const {
  ...
  // the lmd box
  new TGeoBBox("lmd_box_outer", box_dim_x / 2.0, box_dim_y / 2.0,
               box_dim_z / 2.0);
  new TGeoBBox("lmd_box_inner", box_dim_x / 2.0 - box_thickness,
               box_dim_y / 2.0 - box_thickness,
               box_dim_z / 2.0 - box_thickness);

  // the two beampipe holes
  new TGeoTube("box_hole_upstream", 0.0,
               geometry_property_tree.get_child("beam_pipe.entrance_pipe")
                   .get<double>("inner_radius"),
               box_thickness / 2.0 + 0.1);  // 0.1 safety margin
  // move the cut pipe upstream
  TGeoTranslation* comb_trans_cut_pipe_upstream =
      new TGeoTranslation("comb_trans_cut_pipe_upstream", 0., 0.,
                          -box_dim_z / 2.0 + box_thickness / 2.0);
  comb_trans_cut_pipe_upstream->RegisterYourself();

  new TGeoTube("box_hole_downstream", 0.0,
               geometry_property_tree.get_child("beam_pipe.exit_pipe")
                   .get<double>("inner_radius"),
               box_thickness / 2.0 + 0.1);
  // move the cut pipe downstream
  TGeoTranslation* comb_trans_cut_pipe_downstream =
      new TGeoTranslation("comb_trans_cut_pipe_downstream", 0., 0.,
                          +box_dim_z / 2.0 - box_thickness / 2.0);
  comb_trans_cut_pipe_downstream->RegisterYourself();

  // the z-plane reinforcement rib
  double rib_thickness =
      pt_vac_box.get_child("zplane_reinforcement_rib").get<double>("thickness");
  new TGeoBBox("lmd_box_rib", box_dim_x / 2.0 - box_thickness,
               box_dim_y / 2.0 - box_thickness, rib_thickness / 2.);
  ...
  double rib_inner_distance_to_front_plate =
      pt_vac_box.get_child("zplane_reinforcement_rib")
          .get<double>("inner_distance_to_box_front_plate");
  TGeoTranslation* comb_trans_rib = new TGeoTranslation(
      "comb_trans_rib", 0., 0., -box_dim_z / 2.0 + box_thickness +
                                    rib_inner_distance_to_front_plate +
                                    rib_thickness / 2.0);
  ...
  double origin_left[3] = {
      -box_dim_x / 2.0 + clash_plate_dim_x / 2.0 + box_thickness, 0.,
      -box_dim_z / 2.0 + box_thickness + rib_inner_distance_to_front_plate +
          rib_thickness + clash_plate_dim_z / 2.0};
  new TGeoBBox("lmd_box_clash_plate_left", clash_plate_dim_x / 2.0,
               clash_plate_dim_y / 2.0, clash_plate_dim_z / 2.0, origin_left);
  double origin_right[3] = {
      +box_dim_x / 2.0 - clash_plate_dim_x / 2.0 - box_thickness, 0.,
      -box_dim_z / 2.0 + box_thickness + rib_inner_distance_to_front_plate +
          rib_thickness + clash_plate_dim_z / 2.0};
  new TGeoBBox("lmd_box_clash_plate_right", clash_plate_dim_x / 2.0,
               clash_plate_dim_y / 2.0, clash_plate_dim_z / 2.0, origin_right);

  // compose all the parts into one luminosity vacuum box
  TGeoCompositeShape* shape_lmd_box = new TGeoCompositeShape(
      "shape_lmd_box",
  ...
  TGeoVolume* lmd_vol_box =
      new TGeoVolume("lmd_vol_box", shape_lmd_box, gGeoMan->GetMedium("steel"));
...
TGeoVolume* PndLmdGeometryFactory::generateBeamPipe() const {
  ...
  TGeoTube* pipe_upstream =
      new TGeoTube("pipe_upstream", entrance_inner_radius,
  ...
  double steel_part_thickness =
      pt_cone_clamp.get<double>("steel_part_thickness");
  double alu_part_thickness =
      pt_cone_clamp.get<double>("aluminum_part_thickness");
  double cone_angle = pt_beam_pipe.get_child("transition_cone")
                          .get<double>("angle_in_degrees") *
                      TMath::Pi() / 180;
  ...
  steel_cone_clamp->DefineSection(
      1, steel_part_thickness, entrance_inner_radius, cone_clamp_outer_radius);
  steel_cone_clamp->DefineSection(
      2, steel_part_thickness, entrance_inner_radius,
  ...
  TGeoVolume* steel_cone_clamp_vol = new TGeoVolume(
      "steel_cone_clamp_vol", steel_cone_clamp, gGeoMan->GetMedium("steel"));
  steel_cone_clamp_vol->SetLineColor(11);
  double cone_clamp_start_pos_z =
      -box_dim_z / 2.0 + pt_vac_box.get<double>("wall_thickness");
  ...
  alu_cone_clamp->DefineSection(
      0, steel_part_thickness,
      entrance_inner_radius + alu_part_thickness * std::tan(cone_angle),
      cone_clamp_outer_radius);
  alu_cone_clamp->DefineSection(1, steel_part_thickness + alu_part_thickness,
                                entrance_inner_radius, cone_clamp_outer_radius);

  TGeoVolume* alu_cone_clamp_vol = new TGeoVolume(
      "alu_cone_clamp_vol", alu_cone_clamp, gGeoMan->GetMedium("Aluminum"));
  ...
  double inner_beampipe_inner_radius(inner_beampipe_outer_radius -
                                     inner_beampipe_thickness);
  double cone_length = (entrance_inner_radius - inner_beampipe_inner_radius) /
                       std::tan(cone_angle);

  TGeoCone* lmd_al_cone = new TGeoCone(
  ...
  TGeoVolume* vol_al_cone =
      new TGeoVolume("al_cone", lmd_al_cone, gGeoMan->GetMedium("Aluminum"));
  vol_al_cone->SetLineColor(kGray);  // 39);
  TGeoTranslation* lmd_trans_cone = new TGeoTranslation(
      "lmd_trans_cone", 0., 0., cone_clamp_start_pos_z + steel_part_thickness +
                                    alu_part_thickness + cone_length / 2.0);
  lmd_trans_cone->RegisterYourself();
  lmd_beam_pipe->AddNode(vol_al_cone, 0, lmd_trans_cone);

  TGeoCone* lmd_capton_cone = new TGeoCone(
  ...
  lmd_inner_pipe->DefineSection(
      0, 0.0, outer_radius_cone_foil_glue_segment - inner_beampipe_thickness,
      outer_radius_cone_foil_glue_segment);
  lmd_inner_pipe->DefineSection(
      1, inner_beampipe_cone_foil_attach_segement_length,
  ...
  double tempzpos(inner_pipe_start_position_z +
  ...
  tempzpos +=
      pt_bellow.get<double>("flange_to_inner_beampipe.outer_radius_increase") /
      std::tan(pt_bellow.get<double>("inner_angle_in_degrees") / 180 *
               TMath::Pi());
  beampipe_bellow->DefineSection(
      2, tempzpos, inner_exit_radius,
  ...
  beampipe_bellow->DefineSection(
      3, tempzpos, inner_exit_radius,
      inner_exit_radius + pt_bellow.get<double>("thickness"));
  beampipe_bellow->DefineSection(
      4, tempzpos, inner_exit_radius,
      inner_exit_radius + pt_bellow.get<double>("thickness") +
          pt_bellow.get<double>("flange_to_box.outer_radius_increase"));
  tempzpos += pt_bellow.get<double>("flange_to_box.thickness");
  beampipe_bellow->DefineSection(
      5, tempzpos, inner_exit_radius,
      inner_exit_radius + pt_bellow.get<double>("thickness") +
          pt_bellow.get<double>("flange_to_box.outer_radius_increase"));

  TGeoVolume* vol_beampipe_bellow = new TGeoVolume(
      "vol_beampipe_bellow", beampipe_bellow, gGeoMan->GetMedium("steel"));
  vol_beampipe_bellow->SetLineColor(11);  // 39);
  lmd_beam_pipe->AddNode(vol_beampipe_bellow, 0);

  // downstream pipe part - outside of lmd
  double outer_exit_radius =
      inner_exit_radius + pt_exit_pipe.get<double>("thickness");
  ...
  shape_pipe_box_do->DefineSection(0, box_dim_z / 2.0, inner_exit_radius,
                                   outer_exit_radius);
  shape_pipe_box_do->DefineSection(
      1, box_dim_z / 2.0 + tube_length - flange_thickness, inner_exit_radius,
      outer_exit_radius);
  shape_pipe_box_do->DefineSection(
      2, box_dim_z / 2.0 + tube_length - flange_thickness, inner_exit_radius,
      flange_outer_radius);
  shape_pipe_box_do->DefineSection(3, box_dim_z / 2.0 + tube_length,
                                   inner_exit_radius, flange_outer_radius);

  TGeoVolume* vlum_pipe_box_do = new TGeoVolume(
      "vlum_pipe_box_do", shape_pipe_box_do, gGeoMan->GetMedium("steel"));
...
TGeoVolume* PndLmdGeometryFactory::generateDetectorHalf(
  ...
  double box_center_position_z(
      ...
      geometry_property_tree.get<double>("beam_pipe.entrance_pipe.length"));
  double z_shift(
      geometry_property_tree.get<double>("general.first_plane_position_z") -
      box_center_position_z);
  for (unsigned int i = 0; i < plane_z_positions.size(); ++i) {
    TGeoTranslation* trans_module =
        new TGeoTranslation(0.0, 0.0, plane_z_positions[i] + z_shift);
...
TGeoVolume* PndLmdGeometryFactory::generateDetectorHalfPlane(
  ...
  double relative_module_shift_z(
      geometry_property_tree.get<double>("general.relative_module_distance_z") /
      2.0);
  unsigned int index_offset(0);
  if (is_bottom_half) {
    index_offset += num_modules_per_plane;
    relative_module_shift_z *=
        -1;  // lower half modules need relative z shift to top half neighbors
  }
  for (unsigned int i = 0; i < num_modules_per_plane; ++i) {
    ...
    TGeoCombiTrans* rottrans_module =
...
TGeoVolume* PndLmdGeometryFactory::generateAluminumCoolingStructure() const {
  ...
  double clash_plane_thickness(geometry_property_tree.get<double>(
  ...
  TGeoTranslation* trans_tube_cut = new TGeoTranslation(
      "trans_tube_cut", 0.0, -outer_radius / 2.0 - 0.1 + 0.0001, 0.);
  ...
  TGeoCompositeShape* cool_support = new TGeoCompositeShape(
      "cool_support",
      "cool_support_tube-cool_support_tube_cutoff:trans_tube_cut");

  TGeoVolume* aluminum_support_volume = new TGeoVolume(
      "vol_cool_support", cool_support, gGeoMan->GetMedium("Aluminum"));
...
TGeoVolume* PndLmdGeometryFactory::generateSensorModule() const {
  ...
  unsigned int num_modules_per_plane(
      geometry_property_tree.get<double>("general.modules_per_half_plane"));
  double cvd_disc_inner_radius(geometry_property_tree.get<double>(
  ...
  TGeoRotation* rot_all_sensors_front =
      new TGeoRotation("rot_all_sensors", 0.0, 0.0, 0.0);
  ...
  for (unsigned int i = 0; i < num_sensors_per_module_side; ++i) {
    ...
    if (current_row_x_shift + sensor_dim_x * (column_counter + 1) >
      ...
      rot_all_sensors_front->RotateZ(geometry_property_tree.get<double>(
          "general.rotation_second_row_sensors_z"));
    ...
    TGeoCombiTrans* rottrans_side_offset = new TGeoCombiTrans(
        ...
        -0.5 * sensor_thickness - 0.5 * cvd_disc_thickness,
        rot_all_sensors_front);
  ...
  TGeoRotation* rot_all_sensors_back =
      new TGeoRotation("rot_all_sensors", 0.0, 0.0, 0.0);
  ...
  for (unsigned int i = 0; i < num_sensors_per_module_side; ++i) {
    ...
    if (current_row_x_shift + sensor_dim_x * (column_counter + 1) >
      ...
      current_row_x_shift += geometry_property_tree.get<double>(
          "general.offset_second_row_sensors_x");
      current_row_y_shift -= sensor_dim_y;
      rot_all_sensors_back->RotateZ(geometry_property_tree.get<double>(
          "general.rotation_second_row_sensors_z"));
    ...
    double x_center(current_row_x_shift + cvd_disc_inner_radius +
    ...
    TGeoCombiTrans* rottrans_side_offset =
        new TGeoCombiTrans(rot.GetTranslation()[0], rot.GetTranslation()[1],
                           0.5 * sensor_thickness + 0.5 * cvd_disc_thickness,
                           rot_all_sensors_back);
    lmd_vol_module->AddNode(sensor, num_sensors_per_module_side + i,
                            rottrans_side_offset);
...
TGeoVolume* PndLmdGeometryFactory::generateCVDCoolingDisc() const {
  ...
  new TGeoTube(
      "cvd_disc_tube", pt_cvd_disc.get<double>("inner_radius"),
      alu_support_inner_radius - gap_between_disc_and_support_structure,
      pt_cvd_disc.get<double>("thickness") / 2.0);

  new TGeoBBox("cvd_disc_cut", alu_support_inner_radius + 0.1,
               alu_support_inner_radius / 2.0 + 0.1,
               pt_cvd_disc.get<double>("thickness") / 2.0 + 0.1);
  TGeoTranslation* trans_bottom =
      new TGeoTranslation("trans_cvd_cut_bottom", 0.0,
                          -(alu_support_inner_radius / 2.0 + 0.1), 0.0);
  trans_bottom->RegisterYourself();

  TGeoRotation* rot_cvd_cut_top = new TGeoRotation(
      "rot_cvd_cut_top", 0.0, 0.0, 180.0 / num_modules_per_half_plane);
  TGeoCombiTrans* trans_top =
      new TGeoCombiTrans("transrot_cvd_cut_top",
                         -std::sin(TMath::Pi() / num_modules_per_half_plane) *
                             (alu_support_inner_radius / 2.0 + 0.1),
                         std::cos(TMath::Pi() / num_modules_per_half_plane) *
                             (alu_support_inner_radius / 2.0 + 0.1),
                         0.0, rot_cvd_cut_top);
  trans_top->RegisterYourself();

  TGeoCompositeShape* cvd_disc =
      new TGeoCompositeShape("cvd_disc",
                             "cvd_disc_tube-cvd_disc_cut:trans_cvd_cut_bottom-"
                             "cvd_disc_cut:transrot_cvd_cut_top");

  TGeoVolume* cvd_disc_volume = new TGeoVolume(
      "lmd_vol_cvd_disc", cvd_disc, gGeoMan->GetMedium("HYPdiamond"));
...
TGeoVolume* PndLmdGeometryFactory::generateSensor() const {
  ...
  new TGeoBBox("sensor_full_centered", sensor_dim_x / 2.0, sensor_dim_y / 2.0,
               thickness / 2.0);

  TGeoBBox* sensor_active_centered =
      new TGeoBBox("sensor_active_centered", active_part_dim_x / 2.0,
                   active_part_dim_y / 2.0, thickness / 2.0);
  new TGeoBBox("sensor_active_centered_cutout", active_part_dim_x / 2.0,
  ...
  TGeoVolume* passive_sensor_volume = new TGeoVolume(
      "LumPassiveRect", sensor_passive_centered, gGeoMan->GetMedium("silicon"));
...
TGeoVolume* PndLmdGeometryFactory::generateBoxVacuum(
    const TGeoVolume* lmd_vol) const {
  auto pt_vac_box = geometry_property_tree.get_child("vacuum_box");
  // general remark: watch out! TGeoShape values are often given has half sizes!
  double box_dim_x(pt_vac_box.get<double>("dimension_x"));
  double box_dim_y(pt_vac_box.get<double>("dimension_y"));
  ...
  new TGeoBBox("lmd_box_vac", box_dim_x / 2.0 - box_thickness,
               box_dim_y / 2.0 - box_thickness,
               box_dim_z / 2.0 - box_thickness);
  ...
  TGeoCompositeShape* shape_box_vac =
      new TGeoCompositeShape("shape_box_vac", ss.str().c_str());

  std::cout << ss.str() << std::endl;
  TGeoVolume* lmd_vol_vac = new TGeoVolume("lmd_vol_vac", shape_box_vac,
                                           gGeoMan->GetMedium("vacuum7"));
...
void PndLmdGeometryFactory::makeNodesAlignable(
  ...
  for (auto const& path_part : navigation_paths) {
    if (std::string(node->GetName()).find(path_part.first) !=
        std::string::npos) {
      found = true;
      break;
    }
  ...
  if (found) {
    if (std::string(node->GetName())
            .find(navigation_paths[current_navigation_path_index].first) !=
        std::string::npos) {
      if (navigation_paths[current_navigation_path_index].second)
        gGeoMan->SetAlignableEntry(full_path.str().c_str(),
                                   full_path.str().c_str());
    } else {
      std::runtime_error(
