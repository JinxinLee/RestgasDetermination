...
PndLmdTrkQTask::PndLmdTrkQTask(Double_t pBeam) :
		FairTask("Track Quality Task for PANDA Lmd"), fEventNr(0) {
	fWriteAllMC = false;
	fPbeam = pBeam;
...
InitStatus PndLmdTrkQTask::Init() {
	...
	if (!ioman) {
		std::cout << "-E- PndLmdTrkQTask::Init: "
	...
	if (!fDigis) {
		std::cout << "-W- PndLmdTrkQTask::Init: " << "No LMDPixelDigis"
				<< " array!" << std::endl;
		return kERROR;
	}
	...
	if (!fClusters) {
		std::cout << "-W- PndLmdTrkQTask::Init: " << "No LMDPixelClusterCand"
				<< " array!" << std::endl;
		return kERROR;
	}
	...
	if (!fRecHits) {
		std::cout << "-W- PndLmdTrkQTask::Init: " << "No LMDHitsMerged"
				<< " array!" << std::endl;
		return kERROR;
	}
	//Get trk cand
	...
	if (!fRecCandTracks) {
		std::cout << "-W- PndLmdTrkQTask::Init: " << "No LMDTrackCand"
				<< " array!" << std::endl;
		return kERROR;
	}
	...
	if (!fRecTracks) {
		std::cout << "-W- PndLmdTrkQTask::Init: " << "No LMDPndTrack"
				<< " array!" << std::endl;
		return kERROR;
	}
...
void PndLmdTrkQTask::Exec(Option_t* opt) {
	...
	for (int irec = 0; irec < nRecHits; irec++) {
		...
		if (mcreftop > 0 && mcrefbot > 0) {
			if (MCtrkidtop == MCtrkidbot) {
				...
				if (fVerbose > 7)
					cout << "        REChit No." << irec << "contain MCid: "
							<< MCtrkidtop << ", " << MCtrkidbot << " !" << endl;
			}
		}
		if (fVerbose > 7)
	...
	for (Int_t iN = 0; iN < nGeaneTrks; iN++) { // loop over all reconstructed trks
		...
		for (Int_t iHit = 0; iHit < Ntrkcandhits; iHit++) { // loop over rec.hits
			if (fVerbose > 7)
				cout << "No." << iHit << ": ";
			PndTrackCandHit candhit = (PndTrackCandHit) (trkcand->GetSortedHit(
					iHit));
			...
			bool badpxtop = false;
			if (mcrefbot >= 0) {
				PndSdsMCPoint* MCPointBot = (PndSdsMCPoint*) (fMCHits->At(
						mcrefbot));
				int MCtrkid = MCPointBot->GetTrackID();
				if (MCtrkid < 0) {
					emergExit = true;
			...
			if (mcreftop >= 0) {
				// if(fVerbose>5)
				//   cout<<", "<<mcreftop<<")";
				PndSdsMCPoint* MCPointTop = (PndSdsMCPoint*) (fMCHits->At(
						mcreftop));
				int MCtrkid = MCPointTop->GetTrackID();
				if (MCtrkid < 0) {
					emergExit = true;
			...
			for (int kn = 0; kn < diffIDs; kn++) {
				if (fVerbose > 3)
					cout << "countMC_IDs[" << kn << "]=" << countMC_IDs[kn]
							<< " 0.65*Ntrkcandhits = " << 0.65 * Ntrkcandhits
							<< endl;
	...
	for (Int_t iN = 0; iN < nGeaneTrks; iN++) { //ghost/good trk determination
		...
		if (goodTrk[iN]) {
			if (fVerbose > 7)
				cout << "... RECtrk#" << iN << " was defined as GOOD ..."
						<< endl;
			goodRecII++;
			trkType = 0;
		}
		if (ghostTrk[iN]) {
			if (fVerbose > 7)
				cout << "... RECtrk#" << iN << " was defined as GHOST ..."
						<< endl;
			if (trkType < 0) {
				trkType = +1;
				ghostRecII++;
		...
		for (int imc = 0; imc < nParticles; imc++) { //MC trks
			...
			if (missTrk) {
				if (MCtksREChits[imc] > minHits) { //missed during trk-search
					if (fVerbose > 7) {
						cout
								...
								<< MCtksREChits[imc] << " with limit>"
								<< minHits << ")" << endl;
						if (MCDoubleHits[imc] > 0)
							cout << "         NB:this MCtrk contains "
									<< MCDoubleHits[imc] << " double hits!"
									<< endl;
					}
					trkQ = -1;
					nMCmissedTrkSearch++;
				...
				if (!fWriteAllMC && trkRECStatus == -3) {
					...
					TClonesArray& clref = *fTrackQ;
					Int_t size = clref.GetEntriesFast();
					PndLmdTrackQ *trkqlmd = new (clref[size]) PndLmdTrackQ(
							glMommc);
					trkqlmd->SetTrkRecStatus(trkRECStatus);
					trkqlmd->SetSecondary(trkMCStatus);
					trkqlmd->SetPDGcode(glPDG);
