...
PndLmdDigiQTask::PndLmdDigiQTask(TString digiBranch) : FairTask("Digis Quality Task for PANDA Lmd"), fEventNr(0)
{
  fWriteAllMC = false;
...
InitStatus PndLmdDigiQTask::Init()
{
  ...
  if ( !ioman){
    std::cout << "-E- PndLmdDigiQTask::Init: "<< "RootManager not instantiated!" << std::endl;
    return kFATAL;
  }
  ...
  if ( !fMCHits)	{
    std::cout << "-W- PndLmdDigiQTask::Init: "<< "No LMDPoint"<<" array!" << std::endl;
    return kERROR;
  }
  ...
  if ( !fMCTracks)	{
    std::cout << "-W- PndLmdDigiQTask::Init: "<< "No MCTrack"<<" array!" << std::endl;
    return kERROR;
  }
  ...
  if ( !fDigis)	{
    std::cout << "-W- PndLmdDigiQTask::Init: "<< "No LMDPixelDigis"<<" array!" << std::endl;
    return kERROR;
  }
...
void PndLmdDigiQTask::Exec(Option_t* opt)
 {
  ...
  if(fVerbose>2) cout<<"PndLmdDigiQTask::Exec END!"<<endl;
  fEventNr++;
 }
