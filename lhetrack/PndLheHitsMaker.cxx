...
InitStatus PndLheHitsMaker::Init() {
 ...
 switch (fSttMode) 
    ...
    case 5:
      fSttPRHelixInput   = (TClonesArray *)fManager->GetObject("STTPRHelixHit");
      if ( ! fSttPRHelixInput ) 
	{
	  cout << "-W- PndLheHitsMaker::Init: No STTPRHelixHit array! Switching STT OFF" << endl;
	  fSttMode = 0;
	}
      else
	{
 ...
 if (fSttMode>0 && fTpcMode > 0)
  ...
  switch (fMvdMode) 
	...
	{
	  cout << "-W- PndMvdIdealTrackFinderTask::Init: " << "No fMvdPixelHitArray!" << endl;
	}
      
      if (( ! fMvdStripHitArray ) &&  ( ! fMvdPixelHitArray ))
	{
	  cout << "-W- PndLheHitsMaker::Init: No MVD hits array! Switching MVD OFF" << endl;
	  fMvdMode = 0;
	}
      else
	{
...
//_________________________________________________________________
void PndLheHitsMaker::GetSttPRHelixHit() {
   // Taking points from PndSttHelixHit
  ...
  for (int j=0; j < fSttPRHelixInput->GetEntriesFast(); j++ ) {
    ...
    if (isnan(sttHit->GetX()))
      ...
      {
        sttHit->Print();
        cout << "-W- PndLheHitsMaker::GetSttPRHelixHit: SttHelixHit with GetZ() == 0 - skipped (requires STT bug fix!!)" << endl;
        continue;
      }
...
void PndLheHitsMaker::Exec(Option_t * option) {
  ...
  if ((fMvdMode==1) && (fMvdMCArray->GetEntriesFast()>0))  GetMvdPoints();
  if ((fMvdMode==2) && ((fMvdStripHitArray->GetEntriesFast()+fMvdPixelHitArray->GetEntriesFast())>0))  GetMvdHits();
  ...
  if ((fSttMode==4) && (fSttHelixInput->GetEntriesFast()>0)&& (fSttHitInput->GetEntriesFast()>0) ) 
    GetSttHelixHitMC();
  if ((fSttMode==5) && (fSttPRHelixInput->GetEntriesFast()>0)) GetSttPRHelixHit();
