* $Id: millepede.f,v 1.2 2006/06/16 15:21:49 conrad Exp $
...
*     Basic dimension parameters
      PARAMETER (MGLOBL=3000,MLOCAL=10,NSTORE=100000,MCS=10) ! dimensions
...
c      write(*,*) WGHT
      DO I=IAGB,IBGB           ! global parameters
       ...
       END IF
      END DO
c      write(*,*) 'filling arest'
c      write(*,*) (arest(j),j=1,25)
      END
...
*           end-subtract
            ...
            DO J=1,JB-JA-1           ! number of derivatives
             ...
             DO K=1,J
              IK=INDST(JA+K)
              JK=(IJ*IJ-IJ)/2+IK
c	      write(*,*) jb,ja,j,k,WGHT,AREST(JA+J),AREST(JA+K)
...
*     end of data - determine local fit parameters
 21   continue
c            WRITE(*,*) 'arest=',(AREST(I),I=1,NST)
c      write (*,*) (CLMAT(J),j=1,5)
c      write (*,*) (blvec(J),j=1,3)
...
c         WRITE(*,103) (J,BLVEC(J),SQRT(CLMAT((J*J+J)/2)),J=1,JB-JA-1)
         WRITE(*,103) (J,BLVEC(J),CLMAT((J*J+J)/2),J=1,JB-JA-1)
...
*         global correlation
       ...
       END IF
       IF(I.LE.50.OR.NAGB-I.LE.50) THEN
          PATEXT='  '
