...
//    float c_Max = 1.f;
  ...
  while ((c = getopt(argc, argv, "t:d:T:l:s:c:L")) != -1)
    switch (c) {
    case 't':
    ...
    default :
      std::cout<<"\n\nFast Hough Transformation on the GPU -------\n\n"
	       ...
	       <<" cutoff\n"
	       <<"     -L:  Level at which to start cutoff\n"
	       <<std::endl;
      return 0;
    }
  ...
  //bwStyle->SetPaperSize(40,52);
  bwStyle->SetPadTopMargin(0.07);
  bwStyle->SetPadRightMargin(0.12);
  bwStyle->SetPadBottomMargin(0.14);
  bwStyle->SetPadLeftMargin(0.135);
  ...
  Float_t bwSize = 30; // size in pixels
  bwStyle->SetTextFont(bwFont);
  bwStyle->SetTextSize(bwSize);
  bwStyle->SetLabelFont(bwFont,"xyz");
  bwStyle->SetLabelSize(bwSize,"xyz");
  bwStyle->SetTitleFont(bwFont,"xyz");
  bwStyle->SetTitleSize(bwSize,"xyz");
  bwStyle->SetTitleOffset(1.1,"x");
  bwStyle->SetTitleOffset(1.1,"y");
  bwStyle->SetLabelOffset(0.015,"x");
  bwStyle->SetLabelOffset(0.015,"y");
  ...
  houghRZ->GetXaxis()->SetTitle("Slope m");
  houghRZ->GetYaxis()->SetTitle("Line Offset t (cm)");
  ...
  phic->GetXaxis()->SetTitle("#Phi_{n} (degrees)");
  phic->GetYaxis()->SetTitle("Plane offset c(#Phi_{n}, #Theta_{n}) (cm)");
  ...
  sebastian_stinkt->GetXaxis()->SetTitle("#Phi_{n} (degrees)");
  sebastian_stinkt->GetYaxis()->SetTitle("Slope m");
