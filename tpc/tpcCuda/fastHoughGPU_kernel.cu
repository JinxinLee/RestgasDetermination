...
__global__ void testIntersect(int nNodes, int level, int nClusters,
  ...
  int tID = blockIdx.x * blockDim.x + threadIdx.x;
   ...
   if(tID<nClusters) {
     ...
     float x_R = clusterDataConst_d[tID*5];
     float y_R = clusterDataConst_d[tID*5+1];
     float z_R = clusterDataConst_d[tID*5+2];
     float R = clusterDataConst_d[tID*5+3];
     float Z = clusterDataConst_d[tID*5+4];
  ...
  int tID = blockIdx.x * blockDim.x + threadIdx.x;
   ...
   if(tID<nNodes) {
     ...
     for(int n=0; n<nClusters; ++n) {
       ...
       //get this hyperplanes' data:
     float x_R = clusterDataConst_d[n*5];
     float y_R = clusterDataConst_d[n*5+1];
     float z_R = clusterDataConst_d[n*5+2];
     float R = clusterDataConst_d[n*5+3];
     float Z = clusterDataConst_d[n*5+4];
