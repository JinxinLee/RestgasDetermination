...
PndEmcExpClusterSplitter::PndEmcExpClusterSplitter(PndEmcExpClusterSplitterData  expClusterSplitterData, Int_t verbose)
{
	fVerbose=verbose;
  ...
  // Energy fall off with distance from centre of cluster is exp(-a*dist/Mr)
  // Mr is the moliere radius.  dist is distance from centre in cm,
  // a is the above parameter.
  // The optimized value for logarithimc cluster positioning is 2.5 
  // For linear cluster positioning a should be set to 1.5 
	...
	std::vector<PndEmcBump*> &theBumpList) const
{
	...
	if (theBumpList.size()!=0) {
		std::cout << "Error.  In PndEmcExpClusterSplitter:  this cluster has already been split."  << std::endl;
		return;
	}
	
	if (fVerbose>=3){
		std::cout<<"Cluster position: theta = "<<theCluster->where().Theta()<<", phi = "<<theCluster->where().Phi()<<std::endl;
	}
    ...
    // Make 1 bump with weights all equal to 1
		...
		EmcDigiPtrDict::const_iterator theDigiIterator = 
		...
		std::set<PndEmcTwoCoordIndex*>::iterator 
		...
		EmcDigiPtrDict *theDigiDict = 
			(EmcDigiPtrDict *)(theCluster->MemberDigiMap());
...
//    int numberOfDigis = theDigiDict->size();
	...
	// We will index each bump by its maximum digi's PndEmcTwoCoordIndex.  We will set up a 
	// list of bump centroids which to start with will be synonymous with the location
	// of the maxima.  We then apportion a weight to each digi, according to its distance
	// from the centroids.  We then construct the bumps according to these weights, which
	// will presumably give a different set of centroids.  This is repeated until the 
	// centroids are static within tolerance, or we reach the maximum number of iterations.
	...
	do {
      ...
      std::map<PndEmcTwoCoordIndex*, PndEmcBump*>::iterator 
		theBumpKiller = theIndexedBumps.begin();
      while(theBumpKiller != theIndexedBumps.end()){
			PndEmcBump *lambToSlaughter = theBumpKiller->second;
			EmcDigiPtrDict *digiList = 
			(EmcDigiPtrDict *)(lambToSlaughter->MemberDigiMap());
      ...
      theIndexedBumps.clear();
		...
		for (theMaximaDigisIterator = theMaximaDigis.begin();theMaximaDigisIterator != theMaximaDigis.end();++theMaximaDigisIterator) {
			...
			std::map<PndEmcTwoCoordIndex*,TVector3*>::iterator 
				theMaxPointsIterator = theCentroidPoints.begin();
			...
			// This next bit just checks to see if the maxima point in hand is the same as the
			// crystal from which we are trying to find distance - just an FP trap really.
		...
		// Put the new centroids in the list of centroid points, remembering to delete the old ones.
		std::map<PndEmcTwoCoordIndex*, TVector3*>::iterator 
		...
		theCentroidPointsIterator = theCentroidPoints.begin();
		std::map<PndEmcTwoCoordIndex*, PndEmcBump*>::iterator 
		theIndexedBumpsIterator = theIndexedBumps.begin();
	...
	std::map<PndEmcTwoCoordIndex*, PndEmcBump*>::iterator 
      theBumpsIterator = theIndexedBumps.begin();
	...
	std::map<PndEmcTwoCoordIndex*,TVector3*>::iterator 
		theGrimReaper = theMaximaPoints.begin();
