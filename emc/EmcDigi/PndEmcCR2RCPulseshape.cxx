...
PndEmcCR2RCPulseshape::PndEmcCR2RCPulseshape(double Tint, double Tdif, double Tsig) 
...
double 
PndEmcCR2RCPulseshape::operator() (const double t, 
...
double 
PndEmcCR2RCPulseshape::general_solution(const double t, 
				    const double amp, 
				    const double toffset) const 
{
	...
	double l_int=1.0/_Tint;
	double l_dif=1.0/_Tdif;
	double l_sig=1.0/_Tsig;
...
double 
PndEmcCR2RCPulseshape::degenerate_solution(const double t,  // for Tdif=Tint
				       const double amp, 
				       const double toffset) const 
{
	double dt=t-toffset;
	...
	double l_shaper=1.0/_Tdif;
	double l_sig=1.0/_Tsig;

	double dl=l_sig-l_shaper;

   double term1 = (exp(-l_shaper*dt)*(dl*dl*dt*dt/2.0 - dl*dt + 1.0 ) - 
         exp(-l_sig*dt));
