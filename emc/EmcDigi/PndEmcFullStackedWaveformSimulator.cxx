...
PndEmcFullStackedWaveformSimulator::PndEmcFullStackedWaveformSimulator() : PndEmcAbsWaveformSimulator(0.), fSamplingBeforeFirstPulse(0), fSamplingAfterLastPulse(0), fCutoff(0), fPulseshape(NULL), fScale(0.), f1GeVWaveform(NULL), fPulseRaiseTime(0), fActiveTimeIncrement(0) {
}

PndEmcFullStackedWaveformSimulator::PndEmcFullStackedWaveformSimulator(Double_t sampleRate, PndEmcAbsPulseshape* pulseshape) : PndEmcAbsWaveformSimulator(sampleRate), fSamplingBeforeFirstPulse(0), fSamplingAfterLastPulse(0), fCutoff(0), fPulseshape(pulseshape), fScale(0.), f1GeVWaveform(NULL), fPulseRaiseTime(0), fActiveTimeIncrement(0) {
}
...
void PndEmcFullStackedWaveformSimulator::Init(Double_t samplingBeforeFirstPulse, Double_t samplingAfterLastPulse, Double_t cutoff, Double_t activeTimeIncrement) {
	...
	do {
		...
		if(!maxFound) {	//search for maximum
			...
			fScale = (*max_constIt);
			fPulseRaiseTime = maxSample * tmpSamlingRate;
		}
...
void PndEmcFullStackedWaveformSimulator::GetAbsoluteTimeInterval(PndEmcWaveformData* wfData, Double_t& startTime, Double_t& activeTime) {
	...
	activeTime = hitMap.rbegin()->first + fPulseRaiseTime;
...
PndEmcWaveform* PndEmcFullStackedWaveformSimulator::MakeSingleWaveform(Double_t hitEnergy, Double_t hitTime, TClonesArray* arrayToStore, Int_t detId, Int_t trackId, Int_t hitIndex) {
	...
	Int_t nRaising = (Int_t) ((fSamplingBeforeFirstPulse + fPulseRaiseTime) * GetSampleRate());
	Int_t iSample =0;

	for(;iSample<nRaising; ++iSample) { 	//time before pulse + raising edge
