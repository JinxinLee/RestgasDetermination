...
// -----   Default constructor   -------------------------------------------
PndFtsHitProducerRealFast::PndFtsHitProducerRealFast(Bool_t useMcTruthPositionForHits):
		  ...
		  fPersistence(kTRUE), fUseMcTruthPositionForHits(useMcTruthPositionForHits)
{ 
}
// -------------------------------------------------------------------------
...
void PndFtsHitProducerRealFast::Exec(Option_t* opt) {
	...
	for (Int_t iPoint = 0; iPoint < nPoints; iPoint++) {
		...
		TVector3 position;
		if (kTRUE == fUseMcTruthPositionForHits){
			// use this for hits having same coordinates as MC points
			position = TVector3(point->GetX(), point->GetY(), point->GetZ());

		} else {
			// use this for realistic hit production
