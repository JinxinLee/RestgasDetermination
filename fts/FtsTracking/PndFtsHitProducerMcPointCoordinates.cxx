...
// modified to use MC point coordinates for hit positions instead of center of straws as in PndFtsHitProducerRealFast Martin Galuska
...
InitStatus PndFtsHitProducerMcPointCoordinates::Init() {
  ...
  std::cout<<"#########################################################"<<std::endl;
  std::cout<<"PndFtsHitProducerMcPointCoordinates: Init()#######"<<std::endl;
  std::cout<<"FTS hits are produced with MC point coordinates for hit positions instead of center of straws!\n";
  std::cout<<"Use this class only for debugging or parameter optimisation purposes!\n\n";
  std::cout<<"#########################################################"<<std::endl;
 // Get RootManager
  FairRootManager* ioman = FairRootManager::Instance();
  if ( ! ioman ) {
    cout << "-E- PndFtsHitProducerMcPointCoordinates-wintz::Init: "
  ...
  if ( ! fPointArray ) {
    cout << "-W- PndFtsHitProducerMcPointCoordinates::Init: "
 ...
 // Create and register output array
  ...
  cout << "-I- PndFtsHitProducerMcPointCoordinates: INITIALIZATION SUCCESSFUL" << endl;
...
PndFtsHit* PndFtsHitProducerMcPointCoordinates::AddHit(Int_t detID, Int_t tubeID, Int_t chamberID, Int_t layerID, Int_t skew, Int_t iPoint, TVector3& pos, TVector3& dpos, Double_t p, Double_t rsim, Double_t closestDistanceError, Double_t depcharge) {
...
PndFtsHitInfo* PndFtsHitProducerMcPointCoordinates::AddHitInfo(Int_t fileNumber, Int_t eventNumber, Int_t trackID, Int_t pointID, Int_t nMerged, Bool_t isFake){
...
ClassImp(PndFtsHitProducerMcPointCoordinates)
