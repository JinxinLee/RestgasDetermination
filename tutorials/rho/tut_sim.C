...
tut_sim(Int_t nEvents = 10, TString  SimEngine ="TGeant3", Float_t mom = 7.24)
{
  //-----User Settings:-----------------------------------------------
 ...
 //-------------------------  CAVE      -----------------
  ...
  if(UseEvtGenDirect){
          TString  EvtInput =gSystem->Getenv("VMCWORKDIR");
          EvtInput+="/tutorials/rho/psi2s_Jpsi2pi_Jpsi_mumu.dec";	
          PndEvtGenDirect *EvtGen = new PndEvtGenDirect("psi(2S)", EvtInput.Data(), mom);
