simInclusive(Int_t nEvents = 100, TString SimEngine = "TGeant3",
        Float_t mom = 15.0, UInt_t seed = 123) {
    ...
    //initializing ChiGen with custom seed
    ...
    Double_t BeamMomentum = mom; // beam momentum ONLY for the scaling of the dipole field.
    ...
    // Create and Set ChiGen Generator
    //-------------------------------
    ...
    //calculate e.c.m. using specified pbar beam momentum
    double ecm = sqrt(2. + 2. * sqrt(2 + BeamMomentum * BeamMomentum));
    //partonic model for polarized (|la| = 2) chi_{c2}
    chigen::models::chi_2_2 chi2(ecm);
    chigen::PndChiGen chigen(chi2);
