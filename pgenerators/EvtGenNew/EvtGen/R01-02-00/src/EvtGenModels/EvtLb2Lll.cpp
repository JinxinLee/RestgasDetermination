...
void EvtLb2Lll::init(){
  ...
  if(getNArg()>8){ // Decay parameters
    report(EVT_ERROR,"EvtGen") << " EVT_ERROR: EvtLb2Lll generator expected max. 8 arguments but found: " << getNArg() << std::endl;
  ...
  if(getNDaug()!=3){ // Check that there are 3 daughters only
    report(EVT_ERROR,"EvtGen") << " EVT_ERROR: EvtLb2Lll generator expected 3 daughters but found: " << getNDaug() << std::endl;
    ::abort();
  }
  ...
  if(EvtPDL::name(getParentId())=="Lambda_b0"){ // Check daughters of Lambda_b0
    ...
    //  report(EVT_ERROR,"EvtGen") << " EVT_ERROR: EvtLb2Lll generator expected Lambda0 daughter but found: " << EvtPDL::name(getDaug(0)) << std::endl;
    //  ::abort();
    //}
    if(EvtPDL::name(getDaug(1))=="e-" && EvtPDL::name(getDaug(2))=="e+"){
      ...
      report(EVT_ERROR,"EvtGen") << " EVT_ERROR: EvtLb2Lll generator expected lepton pair daughters but found:  " << EvtPDL::name(getDaug(1)) << " " << EvtPDL::name(getDaug(2)) << std::endl;
      ::abort();
    }
  //TODO: The model is known not to work correctly for anti-Lambda_b0 (A_FB does not change its sign)
    ...
    //  report(EVT_ERROR,"EvtGen") << " EVT_ERROR: EvtLb2Lll generator expected anti-Lambda0 daughter but found: " << EvtPDL::name(getDaug(0)) << std::endl;
    //  ::abort();
    //}
    if(EvtPDL::name(getDaug(1))=="e+" && EvtPDL::name(getDaug(2))=="e-"){
    ...
    report(EVT_ERROR,"EvtGen") << " EVT_ERROR: EvtLb2Lll generator expected (anti-)Lambda_b0 parent but found:  " << EvtPDL::name(getParentId()) << std::endl;
    ::abort();
  }
  ...
  if(getNArg()>0){
    if(getArg(0)>1. || getArg(0)<-1.){
      report(EVT_ERROR,"EvtGen") << " EVT_ERROR: EvtLb2Lll expects polarization to be in interval <-1,1>, not " << getArg(0) << std::endl;
      ::abort();
    }
  ...
  if(getNArg()>1){
    if(getArgStr(1).substr(1,getArgStr(1).size()-2)!="SM" &&
      ...
      report(EVT_ERROR,"EvtGen") << " EVT_ERROR: EvtLb2Lll doesn't know this physics model: " << getArgStr(1) << std::endl;
      ::abort();
    }
  ...
  if(getNArg()>2){
    if(getArgStr(2).substr(1,getArgStr(2).size()-2)!="HQET" &&
      ...
      report(EVT_ERROR,"EvtGen") << " EVT_ERROR: EvtLb2Lll doesn't know this Form-Factors model: " << getArgStr(2) << std::endl;
      ::abort();
    }
  ...
  if(getNArg()>3){
    if(getArgStr(3).substr(1,getArgStr(3).size()-2)!="Unpolarized"){
      report(EVT_ERROR,"EvtGen") << " EVT_ERROR: EvtLb2Lll doesn't know kind of introducing polarization: " << getArgStr(3) << std::endl;
      ::abort();
    }
  ...
  if(getNArg()>4){
    if(getArgStr(4).substr(1,getArgStr(4).size()-2)!="SD" && getArgStr(4).substr(1,getArgStr(4).size()-2)!="LD"){
      report(EVT_ERROR,"EvtGen") << " EVT_ERROR: EvtLb2Lll didn't find SD or LD parameter: " << getArgStr(4) << std::endl;
      ::abort();
    }
  ...
  if(getNArg()>5){
    if(fabs(getArg(5))>0.15){
      report(EVT_WARNING,"EvtGen") << " EVT_WARNING: EvtLb2Lll found very high contribution to b->sg decay at q2=0: " << getArg(5) << std::endl;
    }
    m_omega = getArg(5);
  }else{
    m_omega = 0;
  ...
  if(getNArg()>7){
    if(getArg(7)<0.){
      report(EVT_ERROR,"EvtGen") << " EVT_ERROR: EvtLb2Lll expects positive maximum probability not : " << getArg(7) << std::endl;
      ::abort();
    }
...
void EvtLb2Lll::calcAmp(EvtAmp *amp,EvtParticle *parent){
  ...
  if(m_FFtype=="HQET"){
    ...
    //report(EVT_WARNING,"EvtGen") << " EVT_WARNING: HQET-delta FF model should be checked for correctness" << std::endl;
    ...
    //report(EVT_WARNING,"EvtGen") << " EVT_WARNING: HQET-noF2 FF model should be checked for correctness" << std::endl;
    r = M_L*M_L/M_Lb/M_Lb;
    F0_1 = +0.462;
    a_F1 = -0.0182;
    b_F1 = -0.000176;
    ...
    report(EVT_ERROR,"EvtGen") << " EVT_ERROR: EvtLb2Lll - unknown Form-Factors model: " << m_FFtype << " - this should never happen !" << std::endl;
    ::abort();
  }
  ...
  if(m_HEPmodel=="SM"){
    ...
    //report(EVT_WARNING,"EvtGen") << " EVT_WARNING: SUSY-ChenGeng model should be checked for correctness" << std::endl;
    C_LL=C_LR=C_RL=C_RR=C_LRLR=C_RLLR=C_LRRL=C_RLRL=C_T=C_TE=EvtComplex(0,0);
    ...
    report(EVT_ERROR,"EvtGen") << " EVT_ERROR: EvtLb2Lll - unknown physics model: " << m_HEPmodel << " - this should never happen !" << std::endl;
    ::abort();
  }
  ...
  if(m_polarizationIntroduction=="SpinDensityMatrix"){
    ...
    report(EVT_ERROR,"EvtGen") << " EVT_ERROR: EvtLb2Lll - unknown polarization: " << m_polarizationIntroduction << " - this should never happen !" << std::endl;
    ::abort();
  }
