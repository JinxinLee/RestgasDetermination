...
void EvtHypNonLepton::init(){
  ...
  if(getNArg()<2 || getNArg()>3){ // alpha phi gamma delta
    report(EVT_ERROR,"EvtGen") << " EVT_ERROR: EvtHypNonLepton generator expected 2 or 3 arguments but found: " << getNArg() << std::endl;
  ...
  if(getNDaug()!=2){ // Check that there are 2 daughters only
    report(EVT_ERROR,"EvtGen") << " EVT_ERROR: EvtHypNonLepton generator expected 2 daughters but found: " << getNDaug() << std::endl;
    ::abort();
  }

  // Check particles spins
  if(EvtSpinType::getSpin2(EvtPDL::getSpinType(getParentId()))!=1){
    report(EVT_ERROR,"EvtGen") << " EVT_ERROR: EvtHypNonLepton generator expected dirac parent particle, but found " << EvtSpinType::getSpin2(EvtPDL::getSpinType(getParentId())) << " spin degrees of freedom" << std::endl;
    ::abort();
  }
  if(EvtSpinType::getSpin2(EvtPDL::getSpinType(getDaug(0)))!=1){
    report(EVT_ERROR,"EvtGen") << " EVT_ERROR: EvtHypNonLepton generator expected the first child to be dirac particle, but found " << EvtSpinType::getSpin2(EvtPDL::getSpinType(getDaug(0))) << " spin degrees of freedom" << std::endl;
    ::abort();
  }
  if(EvtSpinType::getSpin2(EvtPDL::getSpinType(getDaug(1)))!=0){
    report(EVT_ERROR,"EvtGen") << " EVT_ERROR: EvtHypNonLepton generator expected the second child to be scalar particle, but found " << EvtSpinType::getSpin2(EvtPDL::getSpinType(getDaug(1))) << " spin degrees of freedom" << std::endl;
    ::abort();
  }
  ...
  if(m1+m2>=M){
    report(EVT_ERROR,"EvtGen") << " EVT_ERROR: EvtHypNonLepton found impossible decay: " << M << " --> " << m1 << " + " << m2 << " GeV\n" << std::endl;
    ::abort();
  }
