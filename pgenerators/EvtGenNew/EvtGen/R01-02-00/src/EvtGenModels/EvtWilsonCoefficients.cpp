...
void EvtWilsonCoefficients::SetRenormalizationScheme(std::string scheme){
  if(scheme=="NDR") m_ksi=0;
    ...
    report(EVT_ERROR,"EvtGen") << "EVT_ERROR: EvtWilsonCoefficients knows only NDR and HV schemes !" << std::endl;
    ::abort();
  }
}
...
// calculate Lambda matching alphaS using simple iterative method
  ...
  for(i=0;i<maxstep && (difference=fabs(alphaS(mu,n_f,Lambda)-alpha))>=epsilon;i++){
    report(EVT_DEBUG,"EvtGen") << " Difference of alpha_S from " << alpha << " is " << difference << " at Lambda = " << Lambda << std::endl;
  ...
  if(difference>=epsilon){
    report(EVT_ERROR,"EvtGen") << " EVT_ERROR: Did not converge Lambda for alpha_s = " << alpha << " , difference " << difference << " >= " << epsilon << " after " << i << " steps !" << std::endl;
    ::abort();
    return -1;
  }else{
    report(EVT_INFO,"EvtGen") << " For alpha_s = " << alphaS(mu,n_f,Lambda) << " was found Lambda = " << Lambda << std::endl;
    return Lambda;
  }
}
