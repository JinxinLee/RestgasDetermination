...
extern "C" int init1_(double *Plab, double *seed, double *Elastic, double *tetmin, double *tetmax); // install DPM
...
PndDpmDirect::PndDpmDirect(Double_t Mom, Int_t Mode, Long_t Seed)
{
  ...
  PndDpmDirect(Mom, Mode, Seed, ThtMin, 180.);
}
...
{ 
  ...
  cout << "<I> Theta max = " << fThtMax << endl;

  init1_(&fMom, &fSeed, &fMode, &fThtMin, &fThtMax); // init the DPM generator
}
// ------------------------------------------------------------------------

// -----   Gas mode constructor   -----------------------------------------
PndDpmDirect::PndDpmDirect(Double_t Mom, Int_t Mode, Double_t Rsigma, TF1 *DensityFunction, Long_t Seed, Double_t ThtMin, Double_t ThtMax)
{
  Double_t logangle = TMath::Log(0.4) + (TMath::Log(15.) - TMath::Log(Mom)) * (TMath::Log(4) - TMath::Log(0.4)) / (TMath::Log(15) - TMath::Log(1.5));
  Double_t CalThtMin = TMath::Exp(logangle);
  if(CalThtMin > ThtMin) ThtMin = CalThtMin;
