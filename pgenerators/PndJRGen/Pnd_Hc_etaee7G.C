...
//Last modified: 10:th September 2003 Agnes Lundborg
...
Int_t ppProd[2]={998,221}; //Hybrid and eta
...
PndReaction reaction(BeamTarget,VtxRes, "Gen_Hc_etaee7G.root");
...
reaction.DefinePartialWaves(111,0,L1); //p-wave between photons from pion
reaction.DefinePartialWaves(221,0,L1); //p-wave between photons from eta
...
//----Take away a decay channel for eta------------------
TParticlePDG* eta=(TDatabasePDG::Instance())->GetParticle(221);
eta->NDecayChannels();
TDecayChannel* d0=eta->DecayChannel(0);
eta.PrintDecayChannel(d0,"data"); //gamma gamma
TObjArray *d=((TDatabasePDG::Instance())->GetParticle(221))->DecayList();
for(int i=7;i>0;i--)
{
  d0=eta->DecayChannel(i);
  eta.PrintDecayChannel(d0,"data");
  d->RemoveAt(i);
}
(TDatabasePDG::Instance())->GetParticle(221)->NDecayChannels();
