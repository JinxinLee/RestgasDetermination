...
// -----   Default constructor   ------------------------------------------
PndFtfGenerator::PndFtfGenerator() {
  iEvent     = 0;
  fInputFile = NULL;
  fInputTree = NULL;
}
...
PndFtfGenerator::PndFtfGenerator(const Char_t* fileName) {
  iEvent     = 0;
  fFileName  = fileName;
...
Bool_t PndFtfGenerator::ReadEvent(FairPrimaryGenerator* primGen) {

  // Check for input file
  if ( ! fInputFile ) {
    cout << "-E PndFtfGenerator: Input file nor open!" << endl;
    return kFALSE;
  }

  // Check for number of events in input file
  if ( iEvent > fInputTree->GetEntries() ) {
    cout << "-E PndFtfGenerator: No more events in input file!" << endl;
    CloseInput();
    return kFALSE;
  }
  TFile  *g=gFile;
  ...
  for (Int_t iPart=0; iPart < nParts; iPart++) {
    ...
    if ( ! pdgType ) {
      cout << "-W PndFtfGenerator: Unknown type " << part->GetPdgCode()
...
void PndFtfGenerator::CloseInput() {
  if ( fInputFile ) {
    cout << "-I PndFtfGenerator: Closing input file " << fFileName
	 << endl;
    fInputFile->Close();
    delete fInputFile;
  }
