// -------------------------------------------------------------------------
// -----            PndUrqmdSmmGenerator source file                 -----
...
Bool_t PndUrqmdSmmGenerator::ReadEvent(FairPrimaryGenerator* primGen) {
  ...
  if ( ! fInputFile ) {
    cout << "-E PndUrqmdSmmGenerator: Input file nor open!" << endl;
    return kFALSE;
  }

  // Check for number of events in input file
  if ( iEvent > fInputTree->GetEntries() ) {
   cout << "-E PndUrqmdSmmGenerator: No more events in input file!" << endl;
    CloseInput();
    return kFALSE;
  }
  TFile  *g=gFile;
  ...
  if ( ! pdgType ) {
   cout << "-W PndUrqmdSmmGenerator: Unknown type " << part->GetPdgCode()
...
void PndUrqmdSmmGenerator::CloseInput() {
  if ( fInputFile ) {
    cout << "-I PndUrqmdSmmGenerator: Closing input file " << fFileName
	 << endl;
    fInputFile->Close();
    delete fInputFile;
  }
