...
Bool_t PndJRGenerator::ReadEvent(FairPrimaryGenerator* primGen) {
  ...
  if ( ! fInputFile ) {
    cout << "-E PndJRGenerator: Input file nor open!" << endl;
    return kFALSE;
  }

  // Check for number of events in input file
  if ( iEvent > fInputTree->GetEntries() ) {
    cout << "-E PndJRGenerator: No more events in input file!" << endl;
    CloseInput();
    return kFALSE;
  }
  TFile  *g=gFile;
  ...
  for (Int_t iPart=0; iPart < nParts; iPart++) {
    ...
    if ( ! pdgType ) {
      cout << "-W PndJRGenerator: Unknown type " << part->GetPdgCode()
...
void PndJRGenerator::CloseInput() {
  if ( fInputFile ) {
    cout << "-I PndJRGenerator: Closing input file " << fFileName
	 << endl;
    fInputFile->Close();
    delete fInputFile;
  }
