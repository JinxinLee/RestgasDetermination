...
class EvtRootRandomEngine : public EvtRandomEngine {
 public:
  EvtRootRandomEngine(int s = 0) { 
    seed = s;
    // seed is ignored by EvtGen; purpose is to used the global TRandom for reproducibility
  }
  double random();
  int seed;
};
...
PndEvtGenDirect::PndEvtGenDirect(TString particle, TString decfile, Double_t Mom, Long_t Seed, TString defaultDECAY, TString defaultPDL, Double_t ATarg)
{
  ...
  if (Mom > 0)
    cout << "<I> Beam-Momentum: " << Mom << endl;
  if (Mom == 0)
    cout << "<I> Momentum: " << Mom << endl;
  if (Mom < 0)
    cout << "<I> CMS energy: " << Mom << endl;
  cout << "<I> Rnd Seed: " << Seed << endl;
  TString work = getenv("VMCWORKDIR");
  if (defaultDECAY == "")
    defaultDECAY = work + "/pgenerators/EvtGen/EvtGen/Private/DECAY.DEC";
  if (defaultPDL == "")
    defaultPDL = work + "/pgenerators/EvtGen/EvtGen/Private/evt.pdl";
  ...
  myGenerator = new EvtGen(defaultDECAY, defaultPDL, myRandomEngine, radCorrEngine, &extraModels);
#else
  ...
  if (val > 0) {
    fMomentum = val;
    if (particle.Contains("pbarpSystem") || particle.Contains("ppSystem"))
      fEnergy = mp + sqrt(fMomentum * fMomentum + mp * mp);
    if (particle.Contains("pbardSystem") || particle.Contains("pdSystem"))
      fEnergy = md + sqrt(fMomentum * fMomentum + mp * mp);
    if (particle.Contains("pbarASystem") || particle.Contains("pASystem"))
      fEnergy = mA + sqrt(fMomentum * fMomentum + mp * mp);
  } else { // val is -E_cm
    val = -val;
...
Bool_t PndEvtGenDirect::ReadEvent(FairPrimaryGenerator *primGen)
{
  ...
  // FIXME: This feature should go into FairPrimaryGenerator
  TVector3 ip = SampleInteractionVertex();
  ...
  for (Int_t i = 0; i < npart; i++) {
    ...
    if (fStoreTree || (nFD == -1 && nLD == -1)) {
      // Add tracks if they are stable (no daughters) or the tree should be stored
      ...
      // ------------------------------------------------------
      if (plotflag)
        printf("- I -: new particle %d at: %f, %f, %f (%f)-> %f %f %f (%f) ID %d ##Daughters %d %d Mothers %d %d\n", i, fX, fY, fZ, fT, Px, Py, Pz, fE, Id, nFD, nLD,
               evtstdhep.getFirstMother(i), evtstdhep.getLastMother(i));
      if (fStoreTree) {
