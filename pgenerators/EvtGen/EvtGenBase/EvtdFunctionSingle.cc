...
void EvtdFunctionSingle::init(int j,int m1,int m2){
  ...
  for(k=_kmin;k<=_kmax;k+=2){
    ...
    //report(INFO,"EvtGen") << "k, coef:"<<k/2<<" "<<_coef[(k-_kmin)/2]<<endl;
  }
...
double EvtdFunctionSingle::d(int j,int m1,int m2, double theta){
  ...
  //report(INFO,"EvtGen") << "j,m1,m2:"<<j<<","<<m1<<","<<m2<<" theta:"<<theta<<endl;
  ...
  int k;
  for(k=_kmin;k<=_kmax;k+=2){
    fd+=_coef[(k-_kmin)/2]*pow(c2,(2*_j-2*k+m2-m1)/2)*pow(s2,(2*k-m2+m1)/2);
  }
