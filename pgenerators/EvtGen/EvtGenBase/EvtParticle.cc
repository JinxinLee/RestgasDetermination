...
void EvtParticle::initDecay(bool useMinMass) {
  ...
  if ( _isInit ) {
    ...
    if ( p->getParent() && _validP4==false ) {
      if ( !useMinMass ) p->setMass(EvtPDL::getRandMass(p->getId(),parId,_ndaug,dauId,othDauId,parMass,dauMasses));
...
double EvtParticle::compMassProb() {
  ...
  double temp=1.0;
  temp=EvtPDL::getMassProb(p->getId(), fmass, parMass, nDaug, dMasses);
...
double EvtParticle::initializePhaseSpace(
  ...
  if ( resetDaughters ) {
    ...
    //for (i=0; i<numdaughter;i++) {
    //  report(INFO,"EvtGen") << "reset " <<i<< " "<< EvtPDL::name(this->getDaug(i)->getId()) << " " << EvtPDL::name(daughters[i]) << endl;
    //}
    bool t1=true;
  ...
  for (i=0; i<numdaughter;i++) {
    fmass[i]=this->getDaug(i)->mass();
    //    report(INFO,"EvtGen") << "mass " << i << " " << mass[i] << " " << this->getDaug(i) << endl;
  }

  if ( poleSize<-0.1) {
    EvtGenKine::PhaseSpace( numdaughter, fmass, p4, m_b );
    for(i=0;i<numdaughter;i++){
    ...
    if ( (whichTwo1 == 1 && whichTwo2 == 0 ) ||
	 (whichTwo1 == 0 && whichTwo2 == 1 ) ) {
      weight=EvtGenKine::PhaseSpacePole( m_b, fmass[0], fmass[1], fmass[2], 
					  poleSize, p4);
    ...
    if ( (whichTwo1 == 1 && whichTwo2 == 2 ) ||
	 (whichTwo1 == 2 && whichTwo2 == 1 ) ) {
      weight=EvtGenKine::PhaseSpacePole( m_b, fmass[2], fmass[1], fmass[0], 
					  poleSize, p4);
    ...
    if ( (whichTwo1 == 0 && whichTwo2 == 2 ) ||
	 (whichTwo1 == 2 && whichTwo2 == 0 ) ) {
      weight=EvtGenKine::PhaseSpacePole( m_b, fmass[1], fmass[0], fmass[2], 
					  poleSize, p4);
