...
double EvtDecayBase::getArg(int j) {
  ...
  char** tc=0; 
  return strtod(_args[j].c_str(),tc);
}
...
bool EvtDecayBase::matchingDecay(const EvtDecayBase &other) const {
  ...
  for ( unsigned int i=0; i<_ndaug; i++) useDs.push_back(0);

  for ( unsigned int i=0; i<_ndaug; i++) {
    bool foundIt=false;
    for ( unsigned int j=0; j<_ndaug; j++) {
  ...
  for ( unsigned int i=0; i<_ndaug; i++) if ( useDs[i]==0) return false;
