...
void EvtDecayTable::readDecayFile(const std::string dec_name){
  ...
  if ( decaytable.size() < EvtPDL::entries() ) decaytable.resize(EvtPDL::entries());
  ...
  do{
	...
	pname.c_str() << " Mass=" << newMass << " Width="<<newWidth<<endl;
      ...
      pname=parser.getToken(itoken++);
      EvtId thisPart = EvtPDL::getId(pname);
      EvtPDL::fixLSForSP6(thisPart);
      report(DEBUG,"EvtGen") <<"Fixed lineshape for SP6 --from M.Baak " << EvtPDL::name(thisPart).c_str() <<endl;
