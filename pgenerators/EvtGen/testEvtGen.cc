...
int main(int argc, char* argv[]){
  ...
  if (argc==1){
    ...
    report(INFO,"EvtGen") << "p:"<<p<<endl;
    report(INFO,"EvtGen") << "k:"<<k<<endl;
    report(INFO,"EvtGen") << "T=dual(directProd(p,k)):"<<T<<endl;
    report(INFO,"EvtGen") << "T03:"<<T.get(0,3)<<endl;
    return 1;
  }
  ...
  if (!strcmp(argv[1],"print")) {
    int nevent=atoi(argv[2]);
    runPrint(nevent,argv[3],myGenerator,t_init);
  }
  ...
  if (!strcmp(argv[1],"stdhep")) {
    int nevent=atoi(argv[2]);
    runStdHep(nevent,myGenerator,t_init);
  }
  ...
  if (!strcmp(argv[1],"finalstates")) {
    int nevent=atoi(argv[2]);
    runFinalStates(nevent,myGenerator,t_init,1);
  }
  if (!strcmp(argv[1],"finalstates2")) {
    int nevent=atoi(argv[2]);
    runFinalStates(nevent,myGenerator,t_init,2);
  }
  ...
  if (!strcmp(argv[1],"kll")) {
    int nevent=atoi(argv[2]);
    runKll(nevent, myGenerator, t_init);
  }
  if (!strcmp(argv[1],"hll")) {
    int nevent=atoi(argv[2]);
    runHll(nevent, myGenerator, t_init, argv[3]);
  }
  ...
  if (!strcmp(argv[1],"lambdab")) {
    int nevent=atoi(argv[2]);
    runLambdab(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"jpsixgamma")) {
    int nevent=atoi(argv[2]);
    runJpsiXgamma(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"jpsif0gamma")) {
    int nevent=atoi(argv[2]);
    runJpsif0gamma(nevent, myGenerator, t_init);
  }
 ...
 if (!strcmp(argv[1],"checkmass")) {
  ...
  {
      runBaryonic( atoi(argv[2]), myGenerator, t_init );
  }
...
void runEtaPrK(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
   ...
   report(INFO,"EvtGen") << "etapr kp="<<kp<<" etapr km="<<km<<endl;
 }
...
void runPi0Dalitz(int nevent, EvtGen &myGenerator, 
  ...
  int count;
    ...
    do{
      ...
      q2->Fill( (ep+em).mass2() );
      //      report(INFO,"EvtGen") << ep << em << gamma <<endl;
      root_part->deleteTree();  
...
void runTest1(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
  ...
  do{
    ...
    double costhetaV=EvtDecayAngle(p4b0,p4d+p4pi,p4d);
    ctv->Fill(costhetaV);
    if (costhetaV<0.0){
...
void runDDK(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
  ...
  report(INFO,"EvtGen") << nDDK << " " << (count-1) << " " << nDDK/float(2*(count-1)) << endl;
}
...
void runStdHep(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
  ...
  do{
     ...
     report(INFO,"EvtGen") <<stdhep<<endl;
...
void runAlias() {
  ...
  report(INFO,"EvtGen") << "Id    pi+:" << idpip << endl;
  report(INFO,"EvtGen") << "Id    pi-:" << idpim << endl;
  report(INFO,"EvtGen") << "Id    pi0:" << idpi0 << endl;
  report(INFO,"EvtGen") << "Id my_pi+:" << myidpip << endl;
  report(INFO,"EvtGen") << "Id my_pi-:" << myidpim << endl;
  report(INFO,"EvtGen") << "Id my_pi0:" << myidpi0 << endl;
  
  report(INFO,"EvtGen") << "Chg conj    pi+:" << 
    EvtPDL::chargeConj(idpip) << endl;
  report(INFO,"EvtGen") << "Chg conj    pi-:" << 
    EvtPDL::chargeConj(idpim) << endl;
  report(INFO,"EvtGen") << "Chg conj    pi0:" << 
    EvtPDL::chargeConj(idpi0) << endl;
  report(INFO,"EvtGen") << "Chg conj my_pi+:" << 
    EvtPDL::chargeConj(myidpip) << endl;
  report(INFO,"EvtGen") << "Chg conj my_pi-:" << 
    EvtPDL::chargeConj(myidpim) << endl;
  report(INFO,"EvtGen") << "Chg conj my_pi0:" << 
    EvtPDL::chargeConj(myidpi0) << endl;
} 
...
void runFinalStates(int nevent, EvtGen &myGenerator, 
  ...
  char udecay_name[100];
  strcpy(udecay_name,"FINALSTATES.DEC");
  //EvtGen myGenerator(decay_name,pdttable_name,myRandomEngine);
  myGenerator.readUDecay(udecay_name);
  ...
  for(ik=0;ik<parser.getNToken();ik++){
	...
	dList.push_back(tempVector);
	dListNum.push_back(0);
      }
      (dList[dList.size()-1]).push_back(parser.getToken(ik));
    }
  }
  std::map<std::string,int> decays[20];
  ...
  do{
    ...
    if (count==1000*(count/1000)) {
      //if (count==1*(count/1)) {
      report(INFO,"EvtGen") << "Event:"<< count << endl;
    ...
    std::vector<std::string> fs=findFinalState2(p);
    std::vector<bool> alreadyUsed(fs.size());
	...
	for(int jj=0; jj< fs.size(); jj++) if ( !alreadyUsed[jj]) foundIt=false;

	if ( foundIt ) {//report(INFO,"EvtGen") << "found a cand \n"; (histo1[j])->Fill(0.5);
	  //report(INFO,"EvtGen") << "found one " << j << std::endl; 
	  std::string name=root_part->treeStr();
	  std::map<std::string,int>::iterator it=(decays[j]).find(name);
	  if (it!=(decays[j]).end()) (decays[j])[name]++;
	  else (decays[j])[name]=1;
  ...
  int j;
  for (j=0; j<dList.size() ; j++) { 
    report(INFO,"EvtGen") <<parent.c_str() << " -> ";
    for( int i=0;i<dList[j].size();i++){
      report(INFO,"") << (dList[j])[i].c_str() << " ";
    }
    report(INFO,"") << " found " << dListNum[j] << " events out of " << count << " or BF=  "<< (dListNum[j]/(1.0*count)) << endl;

    std::map<std::string,int>::iterator it=(decays[j]).begin();
    while ( it != (decays[j]).end() ) {
...
std::vector<std::string> findFinalState( EvtParticle *tree){
  ...
  static EvtId nue=EvtPDL::getId(std::string("nu_e"));
  static EvtId num=EvtPDL::getId(std::string("nu_mu"));
  static EvtId nut=EvtPDL::getId(std::string("nu_tau"));
  static EvtId anue=EvtPDL::getId(std::string("anti-nu_e"));
  static EvtId anum=EvtPDL::getId(std::string("anti-nu_mu"));
  static EvtId anut=EvtPDL::getId(std::string("anti-nu_tau"));

  static EvtId kl=EvtPDL::getId(std::string("K_L0"));
  static EvtId gamma=EvtPDL::getId(std::string("gamma"));
  ...
  do{
    ...
    if (type==nue) fs.push_back(std::string("nu_e"));
    if (type==num) fs.push_back(std::string("nu_mu"));
    if (type==nut) fs.push_back(std::string("nu_tau"));
    if (type==anue) fs.push_back(std::string("anti-nu_e"));
    if (type==anum) fs.push_back(std::string("anti-nu_mu"));
    if (type==anut) fs.push_back(std::string("anti-nu_tau"));

    if (type==kl) fs.push_back(std::string("K_L0"));

    if (type==gamma) fs.push_back(std::string("gamma"));
...
std::vector<std::string> findFinalState2( EvtParticle *tree){
...
void runTrackMult(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
  ...
  do{
    ...
    if (count==1000*(count/1000)) {
      report(INFO,"EvtGen") << count << endl;
      //report(INFO,"EvtGen") << HepRandom::getTheSeed()<<endl;
    }
    EvtVector4R p_init(EvtPDL::getMass(UPS4),0.0,0.0,0.0);
  ...
  report(INFO,"EvtGen") << "Your average multiplicity="<<aveMulti<<endl;
  report(INFO,"EvtGen") << "Your average multiplicity for no B->semileptonic events="<<aveMultiNoSL<<endl;
  report(INFO,"EvtGen") << "Your average multiplicity for 1 B->semileptonic events="<<aveMulti1SL<<endl;
  report(INFO,"EvtGen") << "Your average multiplicity for 2 B->semileptonic events="<<aveMulti2SL<<endl;
  file->Write(); file->Close();
...
void runGeneric(int neventOrig, EvtGen &myGenerator, TLorentzVector t_init, 
  ...
  int nevent=abs(neventOrig);
	...
	TString directName(tkname);
	directName+="Direct";
	histo2[ik] = new TH1F(directName,directName,
			      30,0.0,3.0); 
	TString massName(tkname);
	massName+="Mass";
...
//  	  cout << "null" << endl;
	...
	//root_part->makeStdHep(stdhep);
	//report(INFO,"EvtGen") <<stdhep<<endl;
	//report(INFO,"EvtGen") <<secondary<<endl;
	std::string token;
	int itok;
	...
	report(INFO,"EvtGen") << "Found "<<temp[itok]<<" "<<token.c_str() 
	     ...
	     << token.c_str()<<" per B meson="<<br<<endl;
...
void runBsmix(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
  ...
  static EvtId B0=EvtPDL::getId(std::string("B0"));
  static EvtId BB=EvtPDL::getId(std::string("anti-B0"));
  ...
  EvtId partId=BS0;
  if ( nevent<0 ) partId=B0;
  do{

    
    EvtVector4R p_init(EvtPDL::getMass(partId),0.0,0.0,0.0);
    
    EvtParticle *root_part=EvtParticleFactory::particleFactory(partId,
							       p_init);
    root_part->setDiagonalSpinDensity();      
    ...
    if (root_part->getNDaug()==1){
      if (root_part->getDaug(0)->getId()==BSB || root_part->getDaug(0)->getId()==BB ){
	mix=1;
      }
    }
...
void runKstarll(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
  ...
  do{
    ...
    report(INFO,"EvtGen") << "count:"<<count<<" "<<(l1+l2).mass2()<<endl;
    
  }while (count++<nevent);
  
  for (int j=0;j<n;j++){
    cout << "["<<q2low[j]<<".."<<q2high[j]<<"]="<<counts[j]<<endl;
  }
...
void runSSDCP(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
  ...
  TH1F* tB0tagDstp = new TH1F("h4","dt Dstp B0 tag (ps)",100,-15.0,15.0);
  TH1F* tB0BtagDstp = new TH1F("h5","dt Dstp B0B tag (ps)",100,-15.0,15.0);

  TH1F* tB0tagDstm = new TH1F("h6","dt Dstm B0 tag (ps)",100,-15.0,15.0);
  TH1F* tB0BtagDstm = new TH1F("h7","dt Dstm B0B tag (ps)",100,-15.0,15.0);
  ...
  do{
    ...
    EvtVector4R p4_tag,p4_cp,p4_jpsi;
    ...
    if (root_part->getDaug(0)->getNDaug()==3){
      ...
      p_tag=root_part->getDaug(1);
      p_cp=root_part->getDaug(0);
    }
    ...
    if (p_tag->getId()==B0){
      tB0tag->Fill(dt);
      if (p_cp->getDaug(0)->getId()==Dstp||
	  p_cp->getDaug(0)->getId()==Dp
	  ) tB0tagDstp->Fill(dt);
      if (p_cp->getDaug(0)->getId()==Dstm||
	  p_cp->getDaug(0)->getId()==Dm
	  ) tB0tagDstm->Fill(dt);
    }
    if (p_tag->getId()==B0B){
      tB0Btag->Fill(dt);
...
void runKstarstargamma(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
  ...
  do{
    ...
    //report(INFO,"EvtGen") << "ctheta:"<<EvtDecayAngle(pinit,p4_kaon+p4_pion,p4_kaon)<<endl;
...
void runVVPiPi(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
  ...
  do{
    ...
    if (fabs(EvtDecayAngle(p4_b,p4_psip,p4_psi))>0.95){
      cose1->Fill(EvtDecayAngle(p4_psip,p4_psi,p4_ep));
    }
    if (fabs(EvtDecayAngle(p4_b,p4_psip,p4_psi))<0.05){
...
void runPartWave2(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
  ...
  do{
    ...
    //report(INFO,"EvtGen") << "costhetan:"<<costhetan<<endl;
    ...
    //report(INFO,"EvtGen") << "costhetarho:"<<costhrho<<endl;
...
void runTwoBody(int nevent, EvtGen &myGenerator, TLorentzVector t_init, std::string decFile) {
  ...
  do{
    ...
    do {
      ...
      if (!(nDaug==0||nDaug==2)) {
	report(INFO,"EvtGen") << "nDaug="<<nDaug<<" but can only handle 0 or 2!"<<endl;
	abort();
      }
...
void runPiPi(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
  ...
  do{
    ...
    outmix << bcp->getLifetime() << " " <<
      btag->getLifetime() << " " << tag.getId() << endl;
...
void runBtoXsgamma(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
 ...
 // Plot kinematics for b->s,gamma
  ...
  do{
      ...
      EvtParticle *root_part = (EvtParticle *)vector_part;
      root_part->setVectorSpinDensity();      
      ...
      if (Bmulti==1) report(INFO,"EvtGen") << "B1" << " bId1a=" << bId1a << " bId1b=" << bId1b << " ndaug=" << B1->getNDaug() << " Bid=" << EvtPDL::getStdHep(B1->getId()) << endl;
     ...
     if (Bmulti==1) report(INFO,"EvtGen") << "B2" << " bId2a=" << bId2a << " bId2b=" << bId2b << " ndaug=" << B2->getNDaug() << " Bid=" << EvtPDL::getStdHep(B2->getId()) << endl;
      ...
      report(INFO,"EvtGen") << "bId1a "<<bId1a<<" bId1b "<<bId1b<<" bId2a "<<bId2a<<" bId2b "<<bId2b<<" for event "<<count<<endl;
      ...
      if (pengcount == 0) {
        Bpeng=B1;
        report(INFO,"EvtGen") << "No penguin decay for event "<<count<<endl;
        bnum=0;
      } else if (pengcount == 2) {
        Bpeng=B1;
        report(INFO,"EvtGen") << "Two penguin decays in event "<<count<<endl;
        bnum=0;        
      }
...
// debug stuff:      report(INFO,"EvtGen") << "bnum=" << bnum << " pengcount=" << pengcount << " bId=" << bId << " Bmulti=" << Bmulti << " XsId=" << XId << " gId=" << gId << endl;
  ...
  report(INFO,"EvtGen")<<"End EvtGen. Ran on "<<nevent<<" events."<<endl;
...
// debug stuff:      report(INFO,"EvtGen") << "bnum=" << bnum << " pengcount=" << pengcount << " bId=" << bId << " Bmulti=" << Bmulti << " KsId=" << XId << " gId=" << gId << endl;
...
void runCheckRotBoost(){
  ...
  report(INFO,"EvtGen") << "Comparing after doing a random boost"<<endl;
  report(INFO,"EvtGen") << "Scalar "<<s<<" "<<sBoost<<s-sBoost<<endl;
  report(INFO,"EvtGen") << "PseudoScalar "<<p<<" "<<pBoost<<p-pBoost<<endl;
  report(INFO,"EvtGen") << "AxialVector "<<aDirBoost<<" "<<aBoost<<aDirBoost-aBoost<<endl;
  report(INFO,"EvtGen") << "Vector "<<vDirBoost<<" "<<vBoost<<vDirBoost-vBoost<<endl;
  report(INFO,"EvtGen") << "V-A "<<vaDirBoost<<" "<<vaBoost<<vaDirBoost-vaBoost<<endl;
  report(INFO,"EvtGen") << "Tensor "<<tDirBoost<<" "<<tBoost<<tDirBoost-tBoost<<endl;
  report(INFO,"EvtGen") << "Done comparing after doing a random boost"<<endl;
  ...
  report(INFO,"EvtGen") << "Comparing after doing a random rotation"<<endl;
  report(INFO,"EvtGen") << "Scalar "<<s<<" "<<sRot<<s-sRot<<endl;
  report(INFO,"EvtGen") << "PseudoScalar "<<p<<" "<<pRot<<p-pRot<<endl;
  report(INFO,"EvtGen") << "AxialVector "<<aDirRot<<" "<<aRot<<aDirRot-aRot<<endl;
  report(INFO,"EvtGen") << "Vector "<<vDirRot<<" "<<vRot<<vDirRot-vRot<<endl;
  report(INFO,"EvtGen") << "V-A "<<vaDirRot<<" "<<vaRot<<vaDirRot-vaRot<<endl;
  report(INFO,"EvtGen") << "Tensor "<<tDirRot<<" "<<tRot<<tDirRot-tRot<<endl;
  report(INFO,"EvtGen") << "Done comparing after doing a random rotation"<<endl;
...
int countInclusive(std::string name, EvtParticle *root_part,TH1F* mom, TH1F* mass){
  ...
  do{
    ...
    if (type==searchFor) { 
      ...
      //}	  
      //report(INFO,"EvtGen") << "LANGE " << p->getP4Lab().d3mag() << " " << p->getP4Lab().get(3)/p->getP4Lab().d3mag() << endl;
    }
...
void runBMix(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
    ...
    do{
      ...
      if (id1==b0&&id2==b0) { b0_b0->Fill(dt); outmix << dt << " 1"<<endl;}
      if (id1==b0b&&id2==b0b) {b0b_b0b->Fill(dt); outmix << dt << " 2"<<endl;}
      if (id1==b0b&&id2==b0) {b0b_b0->Fill(dt); outmix << dt << " 0"<<endl;}
      if (id1==b0&&id2==b0b) {b0_b0b->Fill(dt); outmix << dt << " 0"<<endl;}
...
void runDDalitz(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
    ...
    do{
      ...
      myGenerator.generateEvent(root_part,t_init);
      cout << root_part->getSpinDensityForward().GetDim() << " " 
	   << root_part->getSpinDensityForward().Get(0,0) << " ";
      cout << root_part->getSpinDensityBackward().GetDim() << " " 
	   << root_part->getSpinDensityBackward().Get(0,0) << endl;
      ...
      outmix << (p1+p2).mass2() << " "<<(p2+p3).mass2()<<" "<<(p1+p3).mass2()<<endl;
      root_part->deleteTree();  
...
void runBHadronic(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
    ...
    do{
      ...
      do{
        ...
        outmix << p->getId().getId()<<" "<< p->getP4Lab().d3mag() << endl;
	p=p->nextIter();
...
void runPiPiPiPi(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
	...
	(p4pi1+p4pi2).mass() <<  " " <<
        ...
        EvtDecayAngle(p4a2,p4pi1+p4pi2,p4pi1) << endl;

      root_part->deleteTree();  

      report(INFO,"EvtGen") << "count:"<<count<<endl;
...
void runLambdab(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
    ...
    TH1F* costheta = new TH1F("h1","costheta",100,-1.0,1.0); 
    TH1F* costheta1 = new TH1F("h2","costheta1",100,-1.0,1.0); 
    TH1F* costheta2 = new TH1F("h3","costheta2",100,-1.0,1.0); 

    int count;
  
    myGenerator.readUDecay("LAMBDAB.DEC");

    count=1;

    static EvtId LAMBDAB=EvtPDL::getId(std::string("Lambda_b0"));
    ...
    do{
      ...
      EvtVector4R p_init(EvtPDL::getMass(LAMBDAB),0.0,0.0,0.0);
     
      EvtParticle *root_part=EvtParticleFactory::particleFactory(LAMBDAB,
								 p_init);
      root_part->setSpinDensityForwardHelicityBasis(rho);
      ...
      EvtVector4R p4lambdab=root_part->getP4Lab();
      EvtVector4R p4lambda=root_part->getDaug(0)->getP4Lab();
      EvtVector4R p4Jpsi=root_part->getDaug(1)->getP4Lab();

      EvtVector4R p4p=root_part->getDaug(0)->getDaug(0)->getP4Lab();
      //EvtVector4R p4pi=root_part->getDaug(0)->getDaug(1)->getP4Lab();
      EvtVector4R p4mup=root_part->getDaug(1)->getDaug(0)->getP4Lab();
      //EvtVector4R p4mum=root_part->getDaug(1)->getDaug(1)->getP4Lab();


      costheta->Fill(p4lambda.get(3)/p4lambda.d3mag());
      costheta1->Fill(EvtDecayAngle(p4lambdab,p4lambda,p4p));
      costheta2->Fill(EvtDecayAngle(p4lambdab,p4Jpsi,p4mup));
...
void runJpsiXgamma(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
    ...
    TH1F* costhetagamma = new TH1F("h1","costhetagamma",100,-1.0,1.0); 
    TH1F* costhetapi1 = new TH1F("h2","costhetapi1",100,-1.0,1.0); 

    int count;

  
    myGenerator.readUDecay("JPSIXGAMMA.DEC");
    ...
    do{
      ...
      EvtVector4R p_init(EvtPDL::getMass(JPSI),0.0,0.0,0.0);
     
      EvtParticle *root_part=EvtParticleFactory::particleFactory(JPSI,
								 p_init);
      root_part->setVectorSpinDensity();
      ...
      EvtVector4R p4Jpsi=root_part->getP4Lab();
      EvtVector4R p4X=root_part->getDaug(0)->getP4Lab();
      EvtVector4R p4gamma=root_part->getDaug(1)->getP4Lab();

      EvtVector4R p4pi1=root_part->getDaug(0)->getDaug(0)->getP4Lab();
      //EvtVector4R p4pi2=root_part->getDaug(0)->getDaug(1)->getP4Lab();

      costhetagamma->Fill(p4gamma.get(3)/p4gamma.d3mag());
      costhetapi1->Fill(EvtDecayAngle(p4Jpsi,p4X,p4pi1));
...
void runJpsif0gamma(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
    ...
    TH1F* costhetaf0 = new TH1F("h1","costhetaf0",100,-1.0,1.0); 
    TH1F* costhetarho = new TH1F("h2","costhetarho",100,-1.0,1.0); 
    TH1F* costhetapip = new TH1F("h3","costhetapip",100,-1.0,1.0); 

    int count;

  
    myGenerator.readUDecay("JPSIF0GAMMA.DEC");
    ...
    do{
      ...
      EvtVector4R p4Jpsi=root_part->getP4Lab();
      EvtVector4R p4f0=root_part->getDaug(0)->getP4Lab();
      EvtVector4R p4rho=root_part->getDaug(0)->getDaug(0)->getP4Lab();

      EvtVector4R p4pip=root_part->getDaug(0)->getDaug(0)->getDaug(0)->getP4Lab();

      costhetaf0->Fill(p4f0.get(3)/p4f0.d3mag());
      costhetarho->Fill(EvtDecayAngle(p4Jpsi,p4f0,p4rho));
      costhetapip->Fill(EvtDecayAngle(p4f0,p4rho,p4pip));
...
void runRandom(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
    ...
    for(itry=0;itry<2;itry++){
      ...
      report(INFO,"EvtGen") << "In Random:"<<EvtRandom::Flat(0.0,1.0)<<endl;
	...
	root_part->deleteTree();  
      ...
      report(INFO,"EvtGen") << "Out Random:"<<EvtRandom::Flat(0.0,1.0)<<endl;
    }
...
void runUCLAII(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
    ...
    do{
      ...
      if (count==100*(count/100)) {
       report(DEBUG,"EvtGen") << count << endl;
      ...
      do{
        ...
        outmix << EvtPDL::getStdHep(p->getId())<<" "<< p->getP4Lab().d3mag() << endl;
...
void runD1(int nevent, EvtGen &myGenerator, TLorentzVector t_init) {
    ...
    do{
      ...
      report(DEBUG,"EvtGen") << "count:"<<count<<endl;
...
void runBaryonic(int nEvent, EvtGen& myGenerator, TLorentzVector t_init)
{
    ...
    myGenerator.readUDecay("BARYONIC.DEC");
