...
PndMvdCalcPixel::PndMvdCalcPixel()
{
  fCperL = -1;
  fQuad = kQuadUNDEF;
...
PndMvdCalcPixel::PndMvdCalcPixel(Double_t w, Double_t l, Double_t threshold, Double_t noise)
{
  fPixelWidth = w;
  fPixelLength= l;
...
std::vector<PndMvdPixel> PndMvdCalcPixel::GetPixels(Double_t inx, Double_t iny, Double_t inz,
...
//  ApplyConMatrix(); //fCon anwenden
  ...
  while (fStop != true){
    ...
    switch (fNextPixel) {
      ...
      case kPixelUNDEF : std::cout << "PndMvdCalcPixel::GetPixels no next pixel."<<std::endl; break;
    }
...
void PndMvdCalcPixel::CalcConMatrix()
{
  if (fIn.getX() < 0)
    fCon.setX(-1);
  else fCon.setX(1);
...
void PndMvdCalcPixel::ApplyConMatrix()
{
...
void PndMvdCalcPixel::CalcQuadrant()
{
  if (fDir.getX() > 0)
    if (fDir.getY() > 0)
      fQuad = kUR;
...
void PndMvdCalcPixel::CalcCperL(Double_t Energy)
{
  Double_t Charge = Energy / (3.61e-9);
  fCperL = Charge / fDir.length();  
}

void PndMvdCalcPixel::CalcStartPixel ()
{
...
void PndMvdCalcPixel::CalcPixel()
{
  Double_t borderX = 0;
  Double_t borderY = 0;
...
switch (fQuad){
    ...
    case kDR : borderX = fPixelLength * (fActivePixel.GetCol()+1);
        ...
        break;
    case kQuadUNDEF : std::cout<<"PndMvdCalcPixel::CalcPixel : Quadrant not defined!"<<std::endl;
...
void PndMvdCalcPixel::ConvertPixels()
{
  Int_t col, row;
...
std::ostream& PndMvdCalcPixel::operator<<(std::ostream& out)
{
