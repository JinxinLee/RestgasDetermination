...
PndMvdStripClusterFinder::PndMvdStripClusterFinder()
{
  fNmCh=2;
  fNmTS=0;
}

PndMvdStripClusterFinder::PndMvdStripClusterFinder(Int_t NrofmissedChannels, Int_t NrofmissedTimestamp)
{
...
std::vector< PndMvdClusterStrip >  PndMvdStripClusterFinder::SearchClusters()
{
  ///  -----  search for clusters  -----
...
std::cout<<"beim Cluster"<<std::endl;
  ...
  {																				// iterate over sensor
	...
	{																			// iterate over sensor side
		          //CH             TS      ID,Used
		std::map<Int_t, std::map<Int_t,std::vector<Int_t> > > hitmap;
		for (Indextriple::iterator itTime = (itSide->second).begin(); itTime != (itSide->second).end();
																								++itTime)
		{																		// iterate over timestamp
			for (Indexpair::iterator itStrip = (itTime->second).begin(); itStrip!= (itTime->second).end();
																								++itStrip)
		...
		for (std::map<Int_t, std::map<Int_t,std::vector<Int_t> > >::const_iterator ch = hitmap.begin();
																ch!=hitmap.end(); ++ch)
		{
...
//				std::vector< Int_t > onecluster;
				...
				if (onecluster.size()>0)
      			{
//          			std::cout<<" -W- PndMvdStripClusterFinder::SearchClusters(): cluster hangover? "<<onecluster.size()<<std::endl;
...
void PndMvdStripClusterFinder::ClusterIterator(Int_t newCh, Int_t newFrameID, std::map<Int_t,
...
ClassImp(PndMvdStripClusterFinder);
