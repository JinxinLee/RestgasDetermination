#include "PndGemRiemannTrackFinderTask.h"
...
PndGemRiemannTrackFinderTask::PndGemRiemannTrackFinderTask() :
	FairTask("Gem Riemann Track Finder"), fMaxSZChi2(1), fMaxSZDist(10), fMinPointDist(6), fMaxDist(1), fEventNr(0), fB(2.0), fInitDone(kFALSE), fCutChi2H(0), fCutDistH(0), fPersistence(kTRUE)
{
...
void PndGemRiemannTrackFinderTask::SetParContainers()
{
}

InitStatus PndGemRiemannTrackFinderTask::ReInit()
{
...
// -----   Public method Init   --------------------------------------------
InitStatus PndGemRiemannTrackFinderTask::Init()
{
  FairRootManager* ioman = FairRootManager::Instance();

  if ( ! ioman )
    {
      std::cout << "-E- PndGemRiemannTrackFinderTask::Init: "
...
//		  std::cout << "-W- PndGemRiemannTrackFinderTask::Init: " << "No Branch Names given with AddHitBranch(TString branchName)! Standard BranchNames taken!" << std::endl;
...
//	  fHitBranch.push_back("MVDHitsStrip");
  ...
  ioman->Register("GEMRiemannTrackCand", "MVD", fTrackCandArray, fPersistence);

  fTrackArray = new TClonesArray("PndTrack");
  ioman->Register("GEMTrack", "GEM", fTrackArray, fPersistence);
...
//  ioman->Register("MVDRiemannTrack", "MVD", fRiemannTrackArray, kTRUE);
  ...
  std::cout << "-I- PndGemRiemannTrackFinderTask: Initialisation successfull" << std::endl;
  fInitDone = kTRUE;
  return kSUCCESS;
}

void PndGemRiemannTrackFinderTask::AddHitBranch(TString branchName)
{
	if (fInitDone == kFALSE)
...
//void PndGemRiemannTrackFinderTask::AddHitBranch(FairTSBufferParameters param)
//{
//	if (fInitDone == kFALSE)
...
// -----   Public method Exec   --------------------------------------------
void PndGemRiemannTrackFinderTask::Exec(Option_t* opt)
{
...
//  std::cout << "------------- event " << fEventNr << "----------------" << std::endl;
  ...
  if (fVerbose > 0) std::cout << " -I- PndGemRiemannTrackFinderTask::Exec : Found Tracks: " << trackFinder.NTracks() << " in event no. " << fEventNr++ << std::endl;
  if (fVerbose > 0) std::cout << " -I- PndGemRiemannTrackFinderTask::Exec : ----------------" << std::endl;
  ...
  for (int i = 0; i < trackFinder.NTracks(); i++){
	  ...
	  if (fVerbose > 1)trackFinder.GetTrack(i).Print();
...
void PndGemRiemannTrackFinderTask::FinishEvent()
{
	fTrackCandArray->Delete();
	fTrackArray->Delete();
//	fRiemannTrackArray->Delete();
...
void PndGemRiemannTrackFinderTask::InitHitArray(TString branchName)
{
	  TClonesArray* tempArray = (TClonesArray*) FairRootManager::Instance()->GetObject(branchName);
	  if (tempArray == 0){
		  std::cout << "-W- PndGemRiemannTrackFinderTask::Init: " << "No hitArray for BranchName " << branchName.Data() << std::endl;
	  }
	  fHitArray.push_back(tempArray);
}

void PndGemRiemannTrackFinderTask::FillHitArray()
{
	...
	if (FairRunAna::Instance()->IsTimeStamp()){
		...
		fHitArray[0] = FairRootManager::Instance()->GetData("GEMHit", fTimeGapFunctor, 10); //FairRootManager::Instance()->GetEventTime() +
		std::cout << "GemHits: " << fHitArray[0]->GetEntriesFast() << std::endl;
...
ClassImp(PndGemRiemannTrackFinderTask);
