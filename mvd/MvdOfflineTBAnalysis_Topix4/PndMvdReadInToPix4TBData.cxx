	...
	if(fileHandle->good())
	  {
	    if (fVerbose > 2) 
	      {
	      std::cout << std::endl;
	      std::cout << "PndMvdReadInToPix4TBData::ReadInRawData reading file " << std::endl;
	    }
		try
		  {
		  ...
		  if (fVerbose > 1)
		    {
		      std::cout << "PndMvdReadInToPix4TBData::ReadInRawData: Error found in reading file " << " : " << fileHandle->good() << " " << fileHandle->eof() << " Exception: " << exception.code << std::endl;
		    }
		  if (fVerbose > 1)
		    {
		...
		if (fVerbose > 2) std::cout << fFE << " PndMvdReadInToPix4TBData::ReadInRawData: NWords: " << tempdata->getNumWords() << std::endl;
...
void PndMvdReadInToPix4TBData::AnalyzeToPixFrame(Double_t clockFrequency)
{
  if (fVerbose > 2)  
    {
      std::cout << fFE << " PndMvdReadInToPix4TBData::AnalyzeToPixFrame: fToPixFrame size: " << std::dec << fToPixFrame.size() << " header " << std::hex << fToPixFrame[0] <<  std::endl;
    }	


  ULong_t hammingcheck = fHamming.CheckHammingCode(fTopix.ConvertToPix4HammingToStandardHamming(fToPixFrame[0]),40);
  if(hammingcheck!=0)
    {
      if (fVerbose > 1)  
	{
  ...
  hammingcheck = fHamming.CheckHammingCode(fTopix.ConvertToPix4HammingToStandardHamming(fToPixFrame[fToPixFrame.size()-1]),40);
  if(hammingcheck!=0)
    {
      if (fVerbose > 1)  
	{
...
{
	if (fVerbose > 2) std::cout << "PndMvdReadInToPix4TBData::AnalyzeData rawData.size(): " << rawData.size() << std::endl;
...
PndSdsDigiTopix4 PndMvdReadInToPix4TBData::ProcessData(ULong64_t& data, frameHeader& header, Double_t& clockFrequency)
{
