...
PndLumiStripClusterFinder::PndLumiStripClusterFinder()
{
	fNmCh=2;
	fNmTS=0;
}
...
PndLumiStripClusterFinder::PndLumiStripClusterFinder(Int_t NrofmissedChannels, Int_t NrofmissedTimestamp)
{
...
std::vector< PndLumiCluster >  PndLumiStripClusterFinder::SearchClusters()
{
	///  -----  search for clusters  -----
	...
	itSensors != fSortedDigis.end(); ++itSensors){	   		// iterate over sensor
		...
		ch!=hitmap.end(); ++ch){			// loop over all frames in hitmap in this channel

			for (std::map<Int_t,std::vector<Int_t> >::const_iterator ts = ch->second.begin();
			ts!=ch->second.end(); ++ts){
...
//          			std::cout<<" -W- PndLumiStripClusterFinder::SearchClusters(): cluster hangover? "<<onecluster.size()<<std::endl;
...
void PndLumiStripClusterFinder::ClusterIterator(Int_t newCh, Int_t newFrameID, std::map<Int_t,
...
ClassImp(PndLumiStripClusterFinder);
