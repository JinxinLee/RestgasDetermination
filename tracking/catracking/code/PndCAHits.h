...
class PndCAHit {
 public:
  PndCAHit(){}
  PndCAHit(  PndCAGBHit& h, int id ):fIStation(h.IRow()), fId(id), fErr2X1(h.Err2X1()), fErrX12(h.ErrX12()), fErr2X2(h.Err2X2()),
					fR(h.R()), fErr2R(h.Err2R()), fErr2A(h.Err2A()), fIsLeft(h.IsLeft()),
...
template <typename T> class PndCAElementsOnStation;
...
class PndCAHits { // same as in PndCAStationArray
 public:
  ...
  PndCAElementsOnStation<T>&       OnStation(char i)       { assert((unsigned char)i<fElement.size() ); return fElement[i]; }
  const PndCAElementsOnStation<T>& OnStation(char i) const { assert((unsigned char)i<fElement.size() ); return fElement[i]; }
  const PndCAElementsOnStation<T>& OnStationConst(char i) const { assert((unsigned char)i<fElement.size() ); return fElement[i]; }
  PndCAElementsOnStation<T>&       operator[](char i)       { assert((unsigned char)i < fElement.size() ); return fElement[i]; }
  const PndCAElementsOnStation<T>& operator[](char i) const { assert((unsigned char)i<fElement.size() ); return fElement[i]; }
