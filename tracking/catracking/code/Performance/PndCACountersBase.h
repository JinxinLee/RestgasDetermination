...
#ifndef PNDCACOUNTERSBASE_H
#define PNDCACOUNTERSBASE_H
...
struct TTracksCatCounters // counters for different tracks categories
{
  ...
  TTracksCatCounters(int nCounters):NCounters(nCounters),counters(){ counters.resize( NCounters, T(0)); };
  ...
  friend std::fstream & operator<<(std::fstream &strm, const TTracksCatCounters<T> &a ){
  ...
  friend std::ostream & operator<<(std::ostream &strm, const TTracksCatCounters<T> &a ){
    strm << a.NCounters << " " << a.counters.size() << " ";
    for(unsigned int iV=0; iV<a.counters.size(); iV++)
      strm << a.counters[iV] << " ";
    strm << std::endl;
...
{
  ratio_reco = reco/mc;
  if (mc.counters[indices["total"]] > 0){
    ratio_clones = clones/double(mc.counters[indices["total"]]);
  }
  else{
    ratio_clones = -1;
  }
  if (reco.counters[indices["total"]]+ghosts > 0){
    ratio_ghosts = ghosts/double(reco.counters[indices["total"]]+ghosts);
  }
  else{
    ratio_ghosts = -1;
  }
