...
//---------stampe.
if(istampa>0){
	cout<<"da PndTracking, dopo SciTil section.\n";
...
//---------stampe.
if(istampa>0){
	cout<<"da PndTracking, dopo || hits loop.\n";
	stampetta(nSttTrackCand, keepit);
}
//-------fine stampe.
	...
	)
{
	...
	*o_x = -0.5*d/a;
	*o_y = -0.5*e/a;
	*r_r = sqrt( (x1-*Ox)*(x1-*Ox) + (y1-*Oy)*(y1-*Oy) );
...
//-------------------------  begin function  PndTracking::ComparisonwithMC
void PndTracking::ComparisonwithMC(
	Short_t * Charge,
	Short_t * daTrackFoundaTrackMC,
	Double_t *FI0,
	Double_t info[][7],
...
//-------------------------  end function  PndTracking::ComparisonwithMC
...
//	const Double_t  MvdCut=0.1,
	...
	for(i=0;i<*nPixelHitsinTrack;i++){
		if( fabs(Dist_SZ(Rr,KAPPA,FI0,ZED[i],S[i],&nrounds0)) < MvdCut
						&&
...
//--------------fine stampe.
			dista =
		fabs(Dist_SZ(Rr,KAPPA,FI0,ZED[i]+DriftRadius[i],S[i],&nrounds0));
			ddd = fabs(Dist_SZ(Rr,KAPPA,FI0,ZED[i]-DriftRadius[i],S[i],&nr2));
...
//  fitting with superfast MILP code
 ...
 Double_t
	...
	cout<<"evt41, nTracksFoundSoFar "<< nTracksFoundSoFar <<",  nHitsinTrack "
	<<nHitsinTrack[nTracksFoundSoFar]<<", ErrorDriftRadiusconformal "<<endl;
...
cout<<"\tindice loop fino a "<<nummm<<endl;
...
// and the other case.
 ...
 if( R[nTracksFoundSoFar] < RSTRAWDETECTORMAX/2){
	...
	if(istampa>=3) { cout<<"prima di TrkAssociatedParallelHitsToHelixQuater, evt. "
	<<IVOLTE<<", track "<<nTracksFoundSoFar<<endl;
	}
...
//  use the trick of increasing the rotation angle by 10 degrees in order to obtain always a positive m
	...
	nSttHits = nMvdHits = 0;
      for(i=0;i<NpointsInFit; i++){
       Oxx[i] = Xconformal[ i ] *cose +
               Yconformal[ i ]*sine;
       Oyy[i] = -Xconformal[ i ] *sine +
               Yconformal[ i ]*cose;
...
//-------------- stampaggi
if(istampa>=3){
	cout<<"from FitHelixCylinder, dopo rotazione, Evento "<<IVOLTE
...
// nameRows[0]=&auxnameRows[0][0];
 ...
 for(i=0 , ii=0 ; i< NpointsInFit ; i++) {
	...
	if( mvdhit[i]){
	  ...
	  sprintf(&(auxnameRows[9+ii][0]),"LM%d",i);  nameRows[9+ii]=&auxnameRows[9+ii][0];
	  ii+=9;
	}
 }
...
//----------------------  begin function   PndTracking::LoadPndTrack_TrackCand

void PndTracking::LoadPndTrack_TrackCand(
	bool *keepit,
	bool *SttSZfit,
	UShort_t nTotalCandidates,
	Short_t *Charge,
...
//----------end function PndTracking::LoadPndTrack_TrackCand
...
//--------------------------------  begin function   PndTracking::MakeInclusionListStt
...
// it needs to be initialized for each event !
 ...
 size_t	len = sizeof(InclusionListStt);
	memset (InclusionListStt,true,len);
 len = sizeof(SingleHitListStt);
	memset (SingleHitListStt,true,len);
	...
	)
{
	...
	for(i=0; i<nSttTrackCand; i++){
if(istampa>=3 ) cout<<"MatchMvdHitsToSttTracks, i= "<<i<<", FI0[i] = "<<
...
//   handle those cases when FI0 and Fifirst are just around 0.
	if( fabs( FI0[i] - Fifirst[i]) > PI ) {     //  this is the special case
		...
		if( FI0[i] < Fifirst[i] ){
			...
			Fifirst[i] += 2.*PI;
if(istampa>=3) cout<<"MatchMvdHitsToSttTracks, track candidate n. = "<<i
...
if(istampa>=3) cout<<"MatchMvdHitsToSttTracks, trackcandidate n. = "<<i
...
//----------begin of function PndTracking::SciTil

void PndTracking::StartFromSciTil(
	Short_t * Charge,
	UShort_t * FiConformalIndex,
	Double_t *Fi_final_helix_referenceframe,
	Double_t *Fi_initial_helix_referenceframe,
	...
	UShort_t * RConformalIndex,
	Double_t *trajectory_vertex,
	Double_t *UU, // it corresponds to  U[MAXTRACKSPEREVENT][nSttParHit];
	Double_t *VV // it corresponds to  V[MAXTRACKSPEREVENT][nSttParHit].
	)
{
 bool outcome;
 ...
 Double_t
	...
	outcome = FindTrackInXYProjection(
			...
			Fi_final_helix_referenceframe,
			Charge,
			(UU+nSttTrackCand*nSttParHit),
			(VV+nSttTrackCand*nSttParHit)
				);
	if(!outcome){
		continue;
	}
...
//------------------ end function  PndTracking::SciTil
...
//----------begin of function PndTracking::stampafinale
void PndTracking::stampafinale(
	...
	)
{
 ...
 cout<<"Evt. n. "<<IVOLTE<<",fine del trackfinding; n. Track Cand totali = "
	<<nTotalCandidates<<endl;

 for(int it=0; it<nTotalCandidates;it++){
	cout<<"\n\tTrack Cand n. "<<it<<", keepit = "<<keepit[it];
	if(!keepit[it]){
		cout<<" e percio' non c'e' stampa\n";
		continue;
	}
   ...
   for(l =0;l<nSttSkewHitsinTrack[it]+nSttParHitsinTrack[it]+nMvdStripHitsinTrack[it]
	...
	cout<<"\thit n. "<<ListTrackCandHit[it][l]<<
	", hit type = "<< ListTrackCandHitType[it][l]<<endl;
   };
   cout<<"-------------------\n";

 } //  end for(int it=0; it<nTotalCandidates;it++)


}

//----------end function PndTracking::stampafinale
...
//----------begin of function PndTracking::stampadopoTrackCand
void PndTracking::stampadopoTrackCand(
	Double_t info[][7],
	UShort_t nTotalCandidates
		)
{
...
for(l =0;l<nMvdPixelHitsinTrack[it]+nMvdStripHitsinTrack[it]
	...
	if( ListTrackCandHitType[it][l]==0){
		...
		cout<<"\tHit di tipo "<<ListTrackCandHitType[it][l]<<endl;
	}
     }
  }
...
//----------end of function PndTracking::stampadopoTrackCand


//----------begin of function PndTracking::
	...
	)
{
	...
	cout<<"da PndTracking, inizia stampetta ---------------------------\n";
...
//---------- begin of function PndTracking::SttInfoXYZParal


    void PndTracking::SttInfoXYZParal(
                             Double_t info[][7],
                             UShort_t infopar,
                             Double_t Oxx,
                             Double_t Oyy,
...
//---------- end function PndTracking::SttInfoXYZParal
...
void PndTracking::SttMatchedSpurious(
	...
	UShort_t ntotalHits,
	Double_t info[][7],
	UShort_t nTracksFoundSoFar, // those found by PR
...
//----------begin function PndTracking::WriteAllMacros
void  PndTracking::WriteAllMacros(
	Short_t *Charge,
	Double_t *FI0,
	Double_t info[][7],
	Double_t *KAPPA,
...
//----------end function PndTracking::WriteAllMacros
