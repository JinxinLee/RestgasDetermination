...
class KFParticle :public KFParticleBase
{
 ...
 //* Initialisation from ALICE track, PID hypothesis shoould be provided 
  ...
  Double_t GetRapidity() const { return 0.5*log((fP[6] + fP[5])/(fP[6] - fP[5])); }
  Double_t GetTheta()    const { return atan2(GetPt(),fP[5]); }
...
#ifdef HomogeneousField
inline Double_t KFParticle::GetDistanceFromVertex( const KFPVertex &Vtx ) const
{
  return GetDistanceFromVertex( KFParticle(Vtx) );
}

inline Double_t KFParticle::GetDeviationFromVertex( const KFPVertex &Vtx ) const
{
