...
void KFParticleFinder::FindParticles(vector<KFPTrack> &vRTracks, vector<float>& ChiToPrimVtx,
                                           ...
                                           KFParticleSIMD& PrimVtx, const vector<int>& vTrackPDG, const float cuts[2][3])
{
  ...
  {
    KFPTrack &kfTrack = vRTracks[iTr];
    bool ok = 1;
    for(unsigned short iT=0; iT<6; iT++)
      ok = ok && finite(kfTrack.GetTrack()[iT]);
    for(unsigned short iC=0; iC<21; iC++)
      ok = ok && finite(kfTrack.GetCovMatrix()[iC]);
      ok = ok && (kfTrack.GetCovMatrix()[0] < 100. && kfTrack.GetCovMatrix()[0] > 0.)
              && (kfTrack.GetCovMatrix()[2] < 100. && kfTrack.GetCovMatrix()[2] > 0.)
              && (kfTrack.GetCovMatrix()[5] < 100. && kfTrack.GetCovMatrix()[5] > 0.)
              && (kfTrack.GetCovMatrix()[9] < 1. && kfTrack.GetCovMatrix()[9] > 0.)
              && (kfTrack.GetCovMatrix()[14] < 1. && kfTrack.GetCovMatrix()[14] > 0.)
              && (kfTrack.GetCovMatrix()[20] < 1. && kfTrack.GetCovMatrix()[20] > 0.);
      ok = ok && kfTrack.GetChi2() < 10*kfTrack.GetNDF();
//     if(!ok) continue;
...
//std::cout << "NPart estim " << nPart << std::endl;
  ...
  const float massXiPDG = 1.32171;

#if defined(PANDA_STT) || defined(PANDA_FTS)
  const float massLambdaSigma=3.7e-3;
  const float massK0sSigma=2.2e-3; //TODO tune
  const float massXiSigma=2.e-3; //TODO tune
#endif
#ifdef ALICE_ITS
  const float massLambdaSigma=5.9e-3;
  const float massK0sSigma=17.7e-3;
  const float massXiSigma=7.3e-3;
#endif
#ifdef STAR_HFT
  const float massLambdaSigma=5.9e-3;
  const float massK0sSigma=17.7e-3;
  ...
  for(unsigned short iTr=0; iTr < vRTracks.size(); iTr++) {
    KFPTrack& kfTrack = vRTracks[iTr] ;
//    kfTrack.SetId(iTr);
    KFParticle tmp(kfTrack, vTrackPDG[iTr]);
  ...
  const float SecCuts[3] = {3.f,5.f,5.f};
//  const float SecCuts[3] = {3.f,5.f,4.f};


  //K0s -> pi+ pi-
  Find2DaughterDecay(vRTracks, 
#ifdef NonhomogeniousField
                     vField,
#endif
                     Particles, pdgNeg[5], pdgPos[5], 310,
                     idNegSec[5], idPosSec[5], PrimVtx, cuts[0], 0, &vK0sTopoChi2Ndf,
                     SecCuts, massK0sPDG, massK0sSigma, &vK0sPrim, 0);
  //Lambda -> p pi-
  Find2DaughterDecay(vRTracks,
#ifdef NonhomogeniousField
                     vField, 
#endif
                     Particles, pdgNeg[5], pdgPos[7], 3122,
                     idNegSec[5], idPosSec[7], PrimVtx, cuts[1], 0,&vLambdaTopoChi2Ndf,
                     SecCuts, massLambdaPDG, massLambdaSigma, &vLambdaPrim, &vLambdaSec);
  //Lambda_bar -> pi+ p-
  Find2DaughterDecay(vRTracks,
#ifdef NonhomogeniousField
                     vField, 
#endif
                     Particles, pdgPos[5], pdgNeg[4], -3122,
                     idPosSec[5], idNegSec[4], PrimVtx, cuts[1], 0, &vLambdaBarTopoChi2Ndf,
                     SecCuts, massLambdaPDG, massLambdaSigma, &vLambdaBarPrim, &vLambdaBarSec);
  //K*0 -> K+ pi-
  Find2DaughterDecay(vRTracks,
#ifdef NonhomogeniousField
                     vField, 
#endif
                     Particles, pdgPos[3], pdgNeg[2], 313,
  ...
  Find2DaughterDecay(vRTracks,
                     ...
                     vField, 
#endif
                      Particles, pdgNeg[3], pdgPos[2], -313,
  ...
  Find2DaughterDecay(vRTracks,
                     ...
                     vField, 
#endif
                     Particles, pdgNeg[3], pdgPos[4], 3124,
  ...
  Find2DaughterDecay(vRTracks,
                     ...
                     vField, 
#endif
                     Particles, pdgNeg[4], pdgPos[3], -3124,
  ...
  Find2DaughterDecay(vRTracks,
                     ...
                     vField, 
#endif
                     Particles, pdgNeg[3], pdgPos[3], 333,
...
//                      PrimVtx, cuts[1]);
  ...
  Find2DaughterDecay(vRTracks,
                     ...
                     vField, 
#endif
                     Particles, pdgNeg[0], pdgPos[0], 22,
  ...
  Find2DaughterDecay(vRTracks,
                     ...
                     vField, 
#endif
                     Particles, pdgNeg[0], pdgPos[0], 443,
  ...
  Find2DaughterDecay(vRTracks,
                     ...
                     vField, 
#endif
                     Particles, pdgNeg[1], pdgPos[1], 100443,
  ...
  Find2DaughterDecay(vRTracks,
                     ...
                     vField, 
#endif
                     Particles, pdgNeg[0], pdgPos[0], 100113,
  ...
  Find2DaughterDecay(vRTracks,
                     ...
                     vField, 
#endif
                     Particles, pdgNeg[1], pdgPos[1], 200113,
...
//  float cutXi[3] = {-300.,10.,10.};
   FindTrackV0Decay(3312, Particles, vLambdaSec, vRTracks,
#ifdef NonhomogeniousField
                     vField, 
#endif
                     pdgNeg[5], idNegSec[5],
                   PrimVtx, cutXi, 0, 0, &vXiPrim, massXiPDG, massXiSigma );


  float cutLL[3] = {10.,10000000.,3.};
  float cutLL2[3] = {10.,3.,3.};
  vector<KFParticle> vLL;
  FindTrackV0Decay(3002, vLL, vLambdaSec, vRTracks,
#ifdef NonhomogeniousField
                     vField, 
#endif
                     pdgNeg[5], idNegSec[5],
  ...
  // Find H0->Lambda p pi-
  //Find Omega*-
  FindTrackV0Decay(3001, Particles, vLL, vRTracks,
#ifdef NonhomogeniousField
                     vField, 
#endif
                     pdgPos[4], idPosSec[4],
  ...
  FindTrackV0Decay(-3312, Particles, vLambdaBarSec, vRTracks,
#ifdef NonhomogeniousField
                     vField, 
#endif
                     pdgPos[5], idPosSec[5],
                   PrimVtx, cutXiPlus, 0, 0, &vXiBarPrim, massXiPDG, massXiSigma);
  //Find Omega-
  float cutOmega[3] = {10.,3.,3.};
  FindTrackV0Decay(3334, Particles, vLambdaSec, vRTracks,
#ifdef NonhomogeniousField
                     vField, 
#endif
                     pdgNeg[6], idNegSec[6],
  ...
  FindTrackV0Decay(-3334, Particles, vLambdaBarSec, vRTracks,
#ifdef NonhomogeniousField
                     vField, 
#endif
                     pdgPos[6], idPosSec[6],
  ...
  FindTrackV0Decay(1003314, Particles, vLambdaPrim, vRTracks,
#ifdef NonhomogeniousField
                     vField, 
#endif
                     pdgNeg[3], idNegPrim[3],
                   PrimVtx, cutXiStarMinus, 1);
  //Find Xi*+
  float cutXiStarPlus[3] = {-100.,10000.,3.};
  FindTrackV0Decay(-1003314, Particles, vLambdaBarPrim, vRTracks,
#ifdef NonhomogeniousField
                     vField, 
#endif
                     pdgPos[3], idPosPrim[3],
  ...
  FindTrackV0Decay(3324, Particles, vXiPrim, vRTracks,
#ifdef NonhomogeniousField
                     vField, 
#endif
                     pdgPos[5], idPosPrim[5],
                   PrimVtx, cutXiStar0, 1, 0, &vXiStarPrim);
  //Find Xi*0 bar
  float cutXiBarStar0[3] = {-100.,10000.,3.};
  FindTrackV0Decay(-3324, Particles, vXiBarPrim, vRTracks,
#ifdef NonhomogeniousField
                     vField, 
#endif
                     pdgNeg[5], idNegPrim[5],
  ...
  for(unsigned int iPart=0; iPart<vXiStarPrim.size(); iPart++)
    CombineTrackPart(vRTracks,
#ifdef NonhomogeniousField
                     vField, 
#endif
                     Particles, vXiStarPrim[iPart], pdgNeg[3], 
  ...
  for(unsigned int iPart=0; iPart<vXiStarBarPrim.size(); iPart++)
    CombineTrackPart(vRTracks,
                     ...
                     vField, 
#endif
                     Particles, vXiStarBarPrim[iPart], pdgPos[3],
                     -1003334, idPosPrim[3], cutOmegaStar, 0);
  // Find K*+
  float cutKStarPlus[3] = {-100.,10000.,3.};
  FindTrackV0Decay(323, Particles, vK0sPrim, vRTracks,
#ifdef NonhomogeniousField
                     vField, 
#endif
                     pdgPos[5], idPosPrim[5],
  ...
  FindTrackV0Decay(3224, Particles, vLambdaPrim, vRTracks,
#ifdef NonhomogeniousField
                     vField, 
#endif
                     pdgPos[5], idPosPrim[5],
  ...
  FindTrackV0Decay(-3114, Particles, vLambdaBarPrim, vRTracks,
#ifdef NonhomogeniousField
                     vField, 
#endif
                     pdgPos[5], idPosPrim[5],
  ...
  FindTrackV0Decay(3114, Particles, vLambdaPrim, vRTracks,
#ifdef NonhomogeniousField
                     vField, 
#endif
                     pdgNeg[5], idNegPrim[5],
  ...
  FindTrackV0Decay(-3224, Particles, vLambdaBarPrim, vRTracks,
#ifdef NonhomogeniousField
                     vField, 
#endif
                     pdgNeg[5], idNegPrim[5],
  ...
  vector<short>* DMesLambdcIdTrack[5] = {&idPosSec[5],
                                         ...
                                         &idPosSec[3],
                                         &idPosSec[4]};
  FindDMesLambdac(vRTracks,
#ifdef NonhomogeniousField
                     vField, 
#endif
                     Particles, DMesLambdcDaughterPDG, DMesLambdcMotherPDG,
  ...
  FindDMesLambdac(vRTracks,
                     ...
                     vField, 
#endif
                     Particles, DMesLambdcBarDaughterPDG, DMesLambdcBarMotherPDG,
...
void KFParticleFinder::Find2DaughterDecay(vector<KFPTrack>& vTracks,
#ifdef NonhomogeniousField
                                                ...
                                                vector<KFParticle>* vMotherSec )
{
  ...
  {
    KFPTrack &kfTrackNeg = vTracks[idNeg[iTrN]];
                                                ...
                                                const float* PCut)
{
  ...
  {
    KFPTrack& kfTrack = vTracks[idPos[iEl]];
    float pt = kfTrack.GetPt();
    float p = kfTrack.GetP();
    if(PCut)
      if(p < *PCut) continue;
  ...
  {
    KFPTrack& kfTrack = vTracks[idNeg[iEl]];
    float pt = kfTrack.GetPt();
    float p = kfTrack.GetP();
    if(PCut)
      if(p < *PCut) continue;
...
void KFParticleFinder::FindDMesLambdac(vector<KFPTrack>& vTracks,
#ifdef NonhomogeniousField
                                             const vector<L1FieldRegion>& vField,
#endif
                                             vector<KFParticle>& Particles,
                                             ...
                                             vector<float> ChiToPrimVtx)
{
  ...
  for(int iId=0; iId<5; iId++)
  {
    ...
    {
      KFPTrack& kfTrack = vTracks[idTrack[iId]->at(iTr)];
      float pt = kfTrack.GetPt();
  ...
  Find2DaughterDecay(vTracks,
                     ...
                     vField,
#endif
                     kpi, DaughterPDG[0], DaughterPDG[1], MotherPDG[0],
  ...
  for(unsigned int iKPi=0; iKPi<kpi.size(); iKPi++)
  {
    ...
    CombineTrackPart(vTracks,
#ifdef NonhomogeniousField
                     vField,
#endif
                     kpipi,kpi[iKPi],DaughterPDG[0], MotherPDG[1], id[0], cuts[1], startPiIndex, 1);
    CombineTrackPart(vTracks,
#ifdef NonhomogeniousField
                     vField,
#endif
                     kpik ,kpi[iKPi],DaughterPDG[3], MotherPDG[3], id[3], cuts[3], 0, 1);
    CombineTrackPart(vTracks,
#ifdef NonhomogeniousField
                     vField,
#endif
                     kpip ,kpi[iKPi],DaughterPDG[4], MotherPDG[4], id[4], cuts[4], 0, 1);
  }
  for(unsigned int iKPiPi=0; iKPiPi<kpipi.size(); iKPiPi++)
    CombineTrackPart(vTracks,
                     ...
                     vField,
#endif
                     kpipipi,kpipi[iKPiPi],DaughterPDG[2], MotherPDG[2], id[2], cuts[2], 0, 1);
  ...
  for(unsigned int iKPiPi=0; iKPiPi<kpipi.size(); iKPiPi++)
    CombineTrackPart(vTracks,
                     ...
                     vField,
#endif
                     d0pi,kpipi[iKPiPi],DaughterPDG[2], MotherPDG[5], id[2], cuts[5], 0, 0);
  ...
  for(unsigned int iKPi=0; iKPi<kpi.size(); iKPi++)
    CombineTrackPart(vTracks,
                     ...
                     vField,
#endif
                     d2pi,kpi[iKPi],DaughterPDG[0], MotherPDG[6], id[0], cuts[6], 0, 0);
  ...
  for(unsigned int iKPiPiPi=0; iKPiPiPi<kpipipi.size(); iKPiPiPi++)
    CombineTrackPart(vTracks,
                     ...
                     vField,
#endif
                     d4pi,kpipipi[iKPiPiPi],DaughterPDG[0], MotherPDG[7], id[0], cuts[7], 0, 0);
...
void KFParticleFinder::CombineTrackPart(vector<KFPTrack>& vTracks,
#ifdef NonhomogeniousField
                                              const vector<L1FieldRegion>& vField,
#endif
                                              vector<KFParticle>& Particles,
