...
class FTSCAElementsOnStation: public vector<T> {
 ...
 public:
  FTSCAElementsOnStation():fHitsRef(0),fISta(-1),fFirstElementIByHit0(){}
  FTSCAElementsOnStation( const FTSCAHits* hits ):fHitsRef(hits),fFirstElementIByHit0(){}

  void SetStation( char s ) { fISta = s; fFirstElementIByHit0.resize((*fHitsRef)[s].size() + 1, -1); }
  ...
  FTSCAElementsOnStation<T>& operator=( const FTSCAElementsOnStation<T>& a ) { // do not copy fHitsRef
    vector<T>::operator=( a );
