...
class FTSCAHit {
 public:
  FTSCAHit(){}
  FTSCAHit( const PndFTSCAGBHit& h, int id ):fIStation(h.IRow()), fId(id), fErr2X1(h.Err2X1()), fErrX12(h.ErrX12()), fErr2X2(h.Err2X2()),
#ifdef DRIFT_TUBES
...
class FTSCAHits { // same as in FTSCAStationArray
 public:
  ...
  const FTSCAElementsOnStation<T>& OnStationConst(char i) const { assert((unsigned char)i<fElement.size() ); return fElement[i]; }
