...
// -----   Public method Init   --------------------------------------------
InitStatus PndTrkLegendreSecTask::Init() {
  ...
  if ( ! ioman ) {
    cout << "-E- PndTrkLegendreSecTask::Init: "
  ...
  if ( ! fSttHitArray ) {
    cout << "-W- PndTrkLegendreSecTask::Init: "
  ...
  if ( !fMvdPixelHitArray){
    std::cout << "-W- PndTrkLegendreSecTask::Init: " << "No MVD Pixel hitArray, return!" << std::endl;
    return kERROR;
  }
  //
  // MVD STRIP
  ...
  if ( !fMvdStripHitArray){
    std::cout << "-W- PndTrkLegendreSecTask::Init: " << "No MVD Strip hitArray, return!" << std::endl;
    return kERROR;
  }
...
void PndTrkLegendreSecTask::Exec(Option_t*) {
  ...
  if(!fSecondary)  {
    ...
    Int_t nchits = FillConformalHitList(isec);
...
PndTrkCluster PndTrkLegendreSecTask::CreateClusterByConfDistance(double fitm, double fitq) {
...
PndTrkCluster PndTrkLegendreSecTask::CreateClusterByRealDistance(double xc0, double yc0, double R0) {
...
PndTrkCluster PndTrkLegendreSecTask::CreateClusterByMixedDistance(double fitm, double fitq) {
...
Bool_t PndTrkLegendreSecTask::DoesRealHitBelong(PndTrkHit *hit, double x0, double y0, double R) {
...
Bool_t PndTrkLegendreSecTask::DoesConfHitBelong(PndTrkConformalHit *chit, double fitm, double fitp) {
...
PndTrkCluster PndTrkLegendreSecTask::CreateClusterByDistance(Int_t mode, double fitm, double fitp) {
  PndTrkCluster cluster;
...
void PndTrkLegendreSecTask::FromConformalToRealTrack(double fitm, double fitp, double &x0, double &y0, double &R) {
 // CHECK if this needs to be kept --> change xc0 to xc etc
  // center and radius
...
void PndTrkLegendreSecTask::DrawZGeometry(int whichone, double phimin, double phimax, double zmin, double zmax)
{
...
void PndTrkLegendreSecTask::DrawGeometryConf(double x1, double y1, double x2, double y2) {
  // CHECK
  char goOnChar;
...
void PndTrkLegendreSecTask::RefreshConf() { // CHECK delete
  // CHECK
  char goOnChar;
...
void PndTrkLegendreSecTask::RefreshZ() { // CHECK delete
  // CHECK
  char goOnChar;
  // cout << "RefreshZ?" << endl;
  //  cin >> goOnChar;
...
void PndTrkLegendreSecTask::DrawConfHit(double u, double v, double r, int marker) {
  display->cd(2);
  if(r >= 0) {
...
Bool_t PndTrkLegendreSecTask::IsSttAssociate(PndTrkHit *hit1, PndTrkHit *hit2) {
...
// ----------------------- FOR SECONDARIES
PndTrkHit *PndTrkLegendreSecTask::FindSttReferenceHit(int isec) {
  int ntot = 0; 
...
void PndTrkLegendreSecTask::ComputeTraAndRot(PndTrkHit *hit, Double_t &delta, Double_t trasl[2]) {
...
Int_t PndTrkLegendreSecTask::ApplyLegendre(double &theta_max, double &r_max) {
  PrepareLegendre();
  return ExtractLegendre(0, theta_max, r_max);
}

Int_t PndTrkLegendreSecTask::ApplyLegendre(PndTrkCluster *cluster, double &theta_max, double &r_max) {
...
Int_t  PndTrkLegendreSecTask::ExtractLegendre(Int_t mode, double &theta_max, double &r_max) {
  if(fDisplayOn) {
    char goOnChar;
    //      cin >> goOnChar;
    DrawLegendreHisto();
...
PndTrkCluster* PndTrkLegendreSecTask::CleanupZPhiFit(PndTrkCluster *cluster, double fitm, double fitp) {
...
Bool_t PndTrkLegendreSecTask::ZPhiFit(int iter, PndTrkCluster *cluster, double &fitm, double &fitp)
{  
...
double PndTrkLegendreSecTask::ComputeZRediduals(PndTrkCluster *cluster, double fitm, double fitp) {
...
double PndTrkLegendreSecTask::CorrectZ(PndTrkCluster *cluster, double deltaz, double fitm, double fitp) {
  ...
  ClassImp(PndTrkLegendreSecTask)
