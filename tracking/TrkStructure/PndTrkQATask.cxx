...
// -----   Default constructor   -------------------------------------------
PndTrkQATask::PndTrkQATask() : FairTask("QA plots", 0), fPersistence(kTRUE), fUseMVDPixHits(kTRUE), fUseMVDStrHits(kTRUE), fUseSTTHits(kTRUE), fUseSTTSkewHits(kTRUE), fUseFTSHits(kTRUE), fUseFTSSkewHits(kTRUE) {
...
// ---------------------------------------------------------
PndTrkQATask::PndTrkQATask(TString detectorsToStudy, Int_t verbose=0) : FairTask("QA plots", verbose), fPersistence(kTRUE), fUseMVDPixHits(kTRUE), fUseMVDStrHits(kTRUE), fUseSTTHits(kTRUE), fUseSTTSkewHits(kTRUE), fUseFTSHits(kTRUE), fUseFTSSkewHits(kTRUE) {
...
void PndTrkQATask::Exec(Option_t* opt) {
	...
	if(fVerbose > 9) cout << "Found " << fSttPointArray->GetEntriesFast() << " STT points\n";
	if(fVerbose > 9) cout << "Found " << fSttHitArray->GetEntriesFast() << " STT hits\n";
	if(fVerbose > 9) cout << "Found " << fFtsPointArray->GetEntriesFast() << " FTS points\n";
	if(fVerbose > 9) cout << "Found " << fFtsHitArray->GetEntriesFast() << " FTS hits\n";
	if(fVerbose > 9) cout << "Found " << fMvdPointArray->GetEntriesFast() << " MVD points\n";
	if(fVerbose > 9) cout << "Found " << fMvdPixelHitArray->GetEntriesFast() << " MVD Pixel hits\n";
	if(fVerbose > 9) cout << "Found " << fMvdStripHitArray->GetEntriesFast() << " MVD Strip hits\n";
...
Bool_t PndTrkQATask::IdealTrackFinding() {
	...
	Int_t hitMap[nMCTracks][NOFDETECTORS];
	...
	if(fVerbose > 11) cout << "nMvdPoints = " << nMvdPoints << endl;
	...
	if(fVerbose > 11) cout << "nSttPoints = " << nSttPoints << endl;
	...
	if(fVerbose > 11) cout << "nFtsPoints = " << nFtsPoints << endl;
	for (Int_t ipnt = 0; ipnt < nFtsPoints; ipnt++){
	...
	for (Int_t iHit = 0; iHit < nFtsHits; iHit++)
	{
