...
PndSttParAsciiFileIo::PndSttParAsciiFileIo(fstream* f) : CbmDetParAsciiFileIo(f) 
{
  // constructor calls the base class constructor
  fName="PndSttParIo";
}

Bool_t PndSttParAsciiFileIo::init(CbmParSet* pPar) 
{
  ...
  if (pFile) 
  {
    if (!strcmp(name,"PndSttDigiPar")) return read((PndSttDigiPar*)pPar,0,kTRUE);
...
Int_t PndSttParAsciiFileIo::write(CbmParSet* pPar) 
{
    // calls the appropriate write function for the container
    if (pFile) 
    {
	const  Text_t* name=pPar->GetName();
	if (!strcmp(name,"PndSttDigiPar")) return writeFile2((PndSttDigiPar*)pPar);
...
template<class T> Bool_t PndSttParAsciiFileIo::read(T* pPar, Int_t* set,
                                                    Bool_t needsClear) 
{
...
template<class T> Int_t PndSttParAsciiFileIo::writeFile2(T* pPar) {
