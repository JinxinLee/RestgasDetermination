...
#include "PndSttHitProducerReal.h"
...
InitStatus PndSttHitProducerReal::Init() {
  ...
  if ( ! fPointArray ) {
    cout << "-W- PndSttHitProducerReal::Init: "
...
//  fVolumeArray = geoMan->GetListOfVolumes();
  ...
  cout << "-I- PndSttHitProducerReal: Intialization successfull" << endl;
...
// -----   Public method Exec   --------------------------------------------
void PndSttHitProducerReal::Exec(Option_t* opt) {
  ...
  // Event summary
  cout << "-I- PndSttHitProducerReal: " << nPoints << " SttPoints, "
       << nPoints << " Hits created." << endl;
  
}
// -------------------------------------------------------------------------
void PndSttHitProducerReal::FoldZPosWithResolution(Double_t &zpos, Double_t &zposError, 
...
PndSttHit* PndSttHitProducerReal::AddHit(Int_t detID, TVector3& pos, TVector3& dpos, Int_t iPoint, Int_t trackID, Double_t p, Double_t rsim, Double_t rtrue, Double_t closestDistanceError, TVector3 wireDirection){
  // see PndSttHit for hit description
  TClonesArray& clref = *fHitArray;
  Int_t size = clref.GetEntriesFast();
  //cout << "-I- PndSttHitProducerReal: Adding Hit: track"<<trackID<<" event: "<<eventID<<" pulse = " << p << ", rsim = " << rsim 
...
PndSttHitInfo* PndSttHitProducerReal::AddHitInfo(Int_t fileNumber, Int_t eventNumber, Int_t trackID, Int_t pointID, Int_t nMerged, Bool_t isFake){
...
ClassImp(PndSttHitProducerReal)
