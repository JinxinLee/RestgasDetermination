...
{
  fEventCounter = 0;
  //  fHotArray = new TClonesArray("PndSttHOT");
}
...
void PndSttHelixTrackFitter::Init()
{
  ...
  //  // Create fHotArray
  //   fHotArray = new TClonesArray("PndSttHOT",1000);


  if(rootoutput) {
    h1 = new TH2F("h1","event display - transverse plane",100,-50,50,100,-50,50);
    h2 = new TH2F("h2","z fit",100, -40, 40, 100, -75, 75); // -4,100,-50,50);  
    ...
    eventCanvas4 = new TCanvas("eventcanvas4", "eventcanvas4", 650, 0 ,600, 600); 
    eventCanvas5 = new TCanvas("eventcanvas5", "eventcanvas5", 650, 0 ,600, 600); 
    hougCanvas = new TCanvas("hougCanvas", "hougCanvas", 650, 0 ,600, 600); 
 }
...
Int_t PndSttHelixTrackFitter::DoFit(PndSttTrack* pTrack, Int_t pidHypo)
{
  ...
  if(fit == 0 || pTrack->GetParamLast()->GetTx() == 0 || !(pTrack->GetParamLast()->GetTx()) || pTrack->GetParamLast()->GetTx() > 3000) {
    ...
    // if refit is OK
    if(Rint == kTRUE) {
      fit = Fit4b(pTrack, 2);   // CHECK flag to be corrected!!!!!
      Rint = IntersectionFinder(pTrack, pTrack->GetParamLast());
      if(Rint == kTRUE)  fit = MinuitFit(pTrack, 2);  
      Rint = IntersectionFinder(pTrack, pTrack->GetParamLast());
      if(Rint == kTRUE)  fit = MinuitFit(pTrack, 2);  

    }
    else return 0;
    if(fit == 1 && pTrack->GetParamLast()->GetTx()>0&& pTrack->GetParamLast()->GetTx() < 3000) { 
     
      pTrack->SetFlag(2); // refit done 
      //      Bool_t zint = ZFinder(pTrack, 1); // old
      Bool_t zint = ZFinderbb3(pTrack, 1); // last working version
      //      if(zint == kFALSE)  zint = ZFinder6b(pTrack, 1); 

      if(zint == kTRUE) {
	// Int_t zfit = Zfit(pTrack, 1); // old
	Int_t zfit = Zfitbb2(pTrack, 1); // last working version
	if(zfit == 1) {
...
// Int_t PndSttHelixTrackFitter::AddHitOnTrack(PndSttTrack *pTrack) { 
//   if(!pTrack) return 0;
...
Int_t PndSttHelixTrackFitter::Fit4(PndSttTrack* pTrack, Int_t pidHypo) {
...
Int_t PndSttHelixTrackFitter::Fit4b(PndSttTrack* pTrack, Int_t pidHypo) {
...
// -------- ZFinder --------------------------------------------
Bool_t PndSttHelixTrackFitter::ZFinder(PndSttTrack* pTrack, Int_t pidHypo) {
...
// -------- ZFinder2 --------------------------------------------
Bool_t PndSttHelixTrackFitter::ZFinder2(PndSttTrack* pTrack, Int_t pidHypo) {
  // CHANGED Scosl
...
// -------- ZFinder --------------------------------------------
Bool_t PndSttHelixTrackFitter::ZFinderbb3(PndSttTrack* pTrack, Int_t pidHypo) {
  ...
  if(pMhit == NULL) continue;
    ...
    // in xy plabe: angle of the PCA to the origin
  ...
  pTrack->GetParamFirst()->SetX(outz.Z());
  ...
  Int_t okcounter = 0;
  for(Int_t i = 0; i < (2*hitcounter); i+=2) {
    //  for(Int_t i = 0; i < hitcounter; i+=2) {   // CHECK Controlla
    ...
    if(vi == NULL) continue;
    Double_t scos = CalculateScosl(h, pTrack->GetParamLast()->GetX(), pTrack->GetParamLast()->GetY(), R, vi->X(), vi->Y());
    ...
    vi = (TVector3*) ZPointsArray->At(okcounter+1); 
    if(vi == NULL) continue;
    scos = CalculateScosl(h, pTrack->GetParamLast()->GetX(), pTrack->GetParamLast()->GetY(), R, vi->X(), vi->Y());
...
// -------- ZFinder5 --------------------------------------------
Bool_t PndSttHelixTrackFitter::ZFinder6b(PndSttTrack* pTrack, Int_t pidHypo) {
  cout << "ZFINDER6b" << endl;
  ...
  for (Int_t i = 0; i < hitcounter; i++) {
    ...
    if(rootoutput && wiredirection != TVector3(0.,0.,1.)) {
      ...
      pt2z->SetMarkerColor(6);
      pt2z->Draw("SAME");
      TLine *ztubeline = new TLine(x_1, z_1, x_2, z_2);
      ztubeline->SetLineColor(wireOk);
      ztubeline->Draw("SAME");
      ...
      pt2z2->SetMarkerColor(6);
      pt2z2->Draw("SAME");
      TLine *ztubeline2 = new TLine(y_1, z_1, y_2, z_2);
      ztubeline2->SetLineColor(wireOk);
      ztubeline2->Draw("SAME");

      //     // mio // calcolato con checkWire.C da cancellare ....
      //       Double_t mio1[3] = {33.6418, 4.31172, 15.9381};
      //       Double_t mio2[3] = {32.8066, 5.75846, -15.9381};
      //       eventCanvas5->cd();
      //       TLine *miotubeline = new TLine(mio1[1], mio1[2], mio2[1], mio2[2]); // wire position
      //       miotubeline->SetLineColor(3);
      //       miotubeline->Draw("SAME");
      //       eventCanvas4->cd();
      //       TLine *miotubeline2 = new TLine(mio1[0], mio1[2], mio2[0], mio2[2]); // wire position
      //       miotubeline2->SetLineColor(3);
      //       miotubeline2->Draw("SAME");
      //       //....................................................
	...
	// intersection point between the reconstructed 
	// circumference and the line of the projected tube
	if(fabs(x_2-x_1) != 0) {              // OK
	  a =(y_2-y_1)/(x_2-x_1);
	  b =(y_1-a*x_1);
	  Double_t A = a*a+1;
	  Double_t B = x_0+a*y_0-a*b;
	...
	// 	cout << "ex: " << x_1 << " " << y_1 << " " << x_2 << " " << y_2 << endl;
	// 	cout << "ce: " << cenposition.X() << " " << cenposition.Y() << endl;
	...
	// zed association
	//	if(fabs(x_2-x_1)==0.) return kFALSE;
	// 	Double_t t_      =(xcen0-x_1)/(x_2-x_1); // x= a_x*t + x_1 [t=1 x=x_2]
	// 	Double_t z_      =(z_2-z_1)*t_ +z_1;     // z= a_z*t + z_1 [t=1 z=z_2]
	...
	//----------------------------------------
	Double_t t1, t2, p1, p2;
	Double_t zcen0, zcen1;
	if((x_2-x_1) != 0.) {
	  t1 = (max.Z() - min.Z())/(max.X() - min.X());
	  p1 = min.Z() - t1 * min.X();
	  zcen0 = t1 * xcen0 + p1;
	  zcen1 = t1 * xcen1 + p1;
	}
	else if((y_2-y_1) != 0.) {
	  t2 = (max.Z() - min.Z())/(max.Y() - min.Y());
	  p2 = min.Z() - t2 * min.Y();
	  zcen0 = t2 * ycen0 + p2;
	  zcen1 = t2 * ycen1 + p2;
	}
	...
	if(rootoutput){
	  eventCanvas4->cd();
	  TMarker *mrkt2 = new TMarker(x_, z_, 6);
	  mrkt2->SetMarkerColor(wireOk-1);
	  mrkt2->Draw("SAME");
	  TMarker *mrkt2bis = new TMarker(x_, z_bis, 6);
	  mrkt2bis->SetMarkerColor(wireOk-1);
	  mrkt2bis->Draw("SAME");
	  ...
	  eventCanvas5->cd();
	  TMarker *mrkt2b = new TMarker(y_, z_, 6);
	  mrkt2b->SetMarkerColor(wireOk-1);
	  mrkt2b->Draw("SAME");
	  TMarker *mrkt2bbis = new TMarker(y_, z_bis, 6);
	  mrkt2bbis->SetMarkerColor(wireOk-1);
	  mrkt2bbis->Draw("SAME");
	  ...
	  mcmrkt2bbis->Draw("SAME");
 	  ...
 	  cout << "WIRE: " << cenposition.X() << " " << cenposition.Y() << " " << cenposition.Z() << endl;
	  cout << "WIREDIR: " << wiredirection.X() << " " << wiredirection.Y() << " " << wiredirection.Z() << endl;
	  // 	  cout << "WIRE1: " << x_1 << " " << y_1 << " " << z_1 << endl;
	  // 	  cout << "WIRE2: " << x_2 << " " << y_2 << " " << z_2 << endl;
	  // 	  cout << "POINT: " << iPoint->GetXtot() << " " << iPoint->GetYtot() << " " << iPoint->GetZtot() << endl;
	  // 	  cout << "TOFIT1: " << x_ << " " << y_ << " " << z_ << endl;
	  // 	  cout << "TOFIT2: " << x_ << " " << y_ << " " << z_bis << endl;
	...
	//	cout << "1: " << scos << " " << tofit->Z() << endl;
	if(pMhit->GetIsochrone() != 0.) sigz = pMhit->GetIsochrone()*10;   // CHECK DA CAMBIARE
	else continue;
	//	else sigz = 0.1;
	//====================  
	// zfit
	...
	if(rootoutput) {     
	  eventCanvas2->cd();
	  TMarker *mcmrk2 = new TMarker(scos, iPoint->GetZtot(), 6);
	  mcmrk2->SetMarkerColor(4);
	  mcmrk2->Draw("SAME");
	}

	//	cout << "M: " << scos << " " << iPoint->GetZtot() << endl;
  ...
  if(rootoutput) {  
    eventCanvas2->cd();
    TLine *line = new TLine(-40, -40*fitm + fitp, 40, (40*fitm + fitp));
    line->Draw("SAME");
  }
...
Int_t PndSttHelixTrackFitter::Zfit(PndSttTrack* pTrack, Int_t pidHypo) {
  ...
  // SCOSL ======
  // get 1st hit
  PndSttHit *fMhit = (PndSttHit*) fHitArray->At(pTrack->GetHitIndex(0));
  // phi0
  TVector3 * v0;
  Double_t Phi0;
  // ============
...
// ----- Zfit2  ----------------------------------------
Int_t PndSttHelixTrackFitter::Zfit2(PndSttTrack* pTrack, Int_t pidHypo) {
...
// ----- Zfit  ----------------------------------------
Int_t PndSttHelixTrackFitter::Zfitbb2(PndSttTrack* pTrack, Int_t pidHypo) {
  ...
  // fill in parameters
  pTrack->GetParamLast()->SetTy(fitm); // tan(lambda)
  pTrack->GetParamLast()->SetZ(fitp);  // z0
...
// PndSttHOT* PndSttHelixTrackFitter::AddHOT(Double_t x, Double_t y, Double_t z, Int_t hitindex, Int_t pointindex, Int_t trackindex)
// {
//   TClonesArray& 
//       clref = *fHotArray;
...
//   return new(clref[size]) PndSttHOT(x, y, z, hitindex, pointindex, trackindex);
...
Int_t PndSttHelixTrackFitter::GetCharge(Double_t dCenter, Double_t phiCenter, Double_t radius)
{
  ...
  PndSttGeomPoint
    vertex(0., 0., 0.),
    firstPoint(startHit->GetX(), startHit->GetY(), startHit->GetZ()), 
    lastPoint(endHit->GetX(), endHit->GetY(), endHit->GetZ());
...
TVector3 *PndSttHelixTrackFitter::PCAToPoint(PndSttTrack *pTrack, TVector3 *point){
  ...
  // transverse  
  Double_t R = pTrack->GetParamLast()->GetTx();
  Double_t xc = (pTrack->GetParamLast()->GetX() + pTrack->GetParamLast()->GetTx()) * TMath::Cos(pTrack->GetParamLast()->GetY());
  Double_t yc = (pTrack->GetParamLast()->GetX() + pTrack->GetParamLast()->GetTx()) * TMath::Sin(pTrack->GetParamLast()->GetY());
...
TVector3 *PndSttHelixTrackFitter::MomentumAtPoint(PndSttTrack *pTrack, TVector3 *point){
  ...
  if((pTrack->GetParamLast()->GetQp() < 0 &&  (yc - point->Y()) < 0) || (pTrack->GetParamLast()->GetQp() > 0 &&  (yc - point->Y()) > 0))
    {
      px = -px;
      py = -py;
    }
