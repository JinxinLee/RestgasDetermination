...
// 	FillMassHisto(hpsimass, psi);
	...
	for (j=0;j<jpsi.GetLength();++j) 
	{
		...
		vtxfitter.Fit();
		const TCandidate *jfit = vtxfitter.FittedCand(jpsi[j]);  // access the fitted cand
	...
	for (j=0;j<psi2s.GetLength();++j) 
	{
		...
		if (mcm->MctMatch(psi2s[j], mctrk)) 
		{
			...
			const TCandidate *truePsi = mctrk.GetConst(psi2s[j].GetMcIdx());
			const TCandidate *trueJ   = mctrk.GetConst(psi2s[j].Daughter(0)->GetMcIdx());
	...
	for (j=0;j<psi2s.GetLength();++j) 
	{
		...
		const TCandidate *jfit = fitter.FittedCand(*(psi2s[j].Daughter(0)));
		
		const TCandidate *epfit = fitter.FittedCand(*(psi2s[j].Daughter(0)->Daughter(0)));
		const TCandidate *emfit = fitter.FittedCand(*(psi2s[j].Daughter(0)->Daughter(1)));
...
void PndMyAnalysisTask::FillMassHisto(TH1F* h, TCandList &l)
{
	Int_t i=0;
...
int PndMyAnalysisTask::SelectPdgCode(TCandList &mct, TCandList &l)
{
	int removed = 0;
	int pdgcode=0;
