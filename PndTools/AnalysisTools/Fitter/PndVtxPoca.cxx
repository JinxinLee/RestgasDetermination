...
PndVtxPoca::PndVtxPoca( const TCandidate& b) : 
VAbsFitter( b )
{
}
...
void PndVtxPoca::GetPocaVtx(TVector3 &vertex)
{
  //if ( fHeadOfTree->NDaughters() != 2 ) {
  //vertex.SetXYZ(0.,0.,0.); 
  //return 0.;
  //}
  ...
  TVector3 dB(0,0,1.0);
  for(Int_t daug1 =0;daug1<fHeadOfTree->NDaughters();daug1++)
  {
    TCandidate* a=fHeadOfTree->Daughter(daug1);
    ...
    for(Int_t daug2=daug1+1;daug2<fHeadOfTree->NDaughters();daug2++)
    {
      TCandidate* b=fHeadOfTree->Daughter(daug2);
      // Position vectors
      ...
      TVector3 theVertex=0.5*(newapos[best]+newbpos[best]);
