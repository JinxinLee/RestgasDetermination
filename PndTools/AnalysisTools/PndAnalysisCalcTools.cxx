...
Bool_t PndAnalysisCalcTools::FillHelixParams(TCandidate* cand, Bool_t skipcov)
{
  if (0==cand) {
    Error("FillHelixParams():", "TCandidate pointer is zero.");
    return kFALSE;
  }
...
{ 
  ...
  // These parameters should NOT be written into the TCandidate!
  ...
  if(!skipcov)
  {
    ...
    if (fVerbose>0) {
      std::cout<<"covrho (epsilon,Z0,Theta,Phi0,rho): "; covrho.Print();
...
Bool_t PndAnalysisCalcTools::SDtoHelix(FairTrackParH* par, TCandidate* cand, Bool_t skipcov)
{
...
Double_t PndAnalysisCalcTools::GetBz(const TVector3 &pos)
{
  ...
  FairRunAna::Instance()->GetField()->GetFieldValue(pnt, Bf); //[kGs]
  //return sqrt(Bf[0]*Bf[0]+Bf[1]*Bf[1]+Bf[2]*Bf[2]); 
  return Bf[2];
}
