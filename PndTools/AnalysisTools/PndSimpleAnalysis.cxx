...
void PndSimpleAnalysis::InitGenericLists()
{
  ...
  //very Loose Lists
  fGenericListNames.push_back("ElectronVeryLooseP");fGenericListNames.push_back("ElectronVeryLooseM");
  fGenericListNames.push_back("MuonVeryLooseP");fGenericListNames.push_back("MuonVeryLooseM");
  fGenericListNames.push_back("PionVeryLooseP");fGenericListNames.push_back("PionVeryLooseM");
  fGenericListNames.push_back("KaonVeryLooseP");fGenericListNames.push_back("KaonVeryLooseM");
  fGenericListNames.push_back("ProtonVeryLooseP");fGenericListNames.push_back("ProtonVeryLooseM");
  
   //Loose Lists
  fGenericListNames.push_back("ElectronLooseP");fGenericListNames.push_back("ElectronLooseM");
  fGenericListNames.push_back("MuonLooseP");fGenericListNames.push_back("MuonLooseM");
  fGenericListNames.push_back("PionLooseP");fGenericListNames.push_back("PionLooseM");
  fGenericListNames.push_back("KaonLooseP");fGenericListNames.push_back("KaonLooseM");
  fGenericListNames.push_back("ProtonLooseP");fGenericListNames.push_back("ProtonLooseM");
  
  //tight Lists
  fGenericListNames.push_back("ElectronTightP");fGenericListNames.push_back("ElectronTightM");
  fGenericListNames.push_back("MuonTightP");fGenericListNames.push_back("MuonTightM");
  fGenericListNames.push_back("PionTightP");fGenericListNames.push_back("PionTightM");
  fGenericListNames.push_back("KaonTightP");fGenericListNames.push_back("KaonTightM");
  fGenericListNames.push_back("ProtonTightP");fGenericListNames.push_back("ProtonTightM");
  
  //very tight Lists
  fGenericListNames.push_back("ElectronVeryTightP");fGenericListNames.push_back("ElectronVeryTightM");
  fGenericListNames.push_back("MuonVeryTightP");fGenericListNames.push_back("MuonVeryTightM");
  fGenericListNames.push_back("PionVeryTightP");fGenericListNames.push_back("PionVeryTightM");
  fGenericListNames.push_back("KaonVeryTightP");fGenericListNames.push_back("KaonVeryTightM");
  fGenericListNames.push_back("ProtonVeryTightP");fGenericListNames.push_back("ProtonVeryTightM");
		...
		if (cur->fDumpList)
		{
			...
			for (j=0;j<(int)curdump->fNtpFNames.size();j++)
			{
				...
				for (k=0;k<cur->GetLength();k++)
				{
					...
					VAbsMicroCandidate *mic=&(cur->fList[k].GetMicroCandidate());
					...
					switch (key) 
					{
						case 100: theArF[off+k]=cur->fList[k].Px(); break;
						case 101: theArF[off+k]=cur->fList[k].Py(); break;
						case 102: theArF[off+k]=cur->fList[k].Pz(); break;
						case 103: theArF[off+k]=cur->fList[k].E(); break;
						
						case 104: theArF[off+k]=cur->fList[k].M(); break;
						case 105: theArF[off+k]=cur->fList[k].Charge(); break;
						
						case 106: theArF[off+k]=cur->fList[k].Pos().X(); break;
						case 107: theArF[off+k]=cur->fList[k].Pos().Y(); break;
						case 108: theArF[off+k]=cur->fList[k].Pos().Z(); break;
					
						case 109: theArF[off+k]=cur->fList[k].GetVect().Phi(); break;
						case 110: theArF[off+k]=cur->fList[k].GetVect().Theta(); break;
						case 111: theArF[off+k]=cur->fList[k].GetVect().CosTheta(); break;
						case 112: theArF[off+k]=cur->fList[k].GetVect().Mag(); break;
						case 113: theArF[off+k]=( fpInit - (cur->fList[k].P4()) ).M(); break;
						
						case 120:  //  Px in CMS
							if (!cachefilled) { TLorentzVector lv=cur->fList[k].P4(); 
								lv.Boost(pInitBoost);lvcache.push_back(lv); }
						...
						case 150: theArF[off+k]=cur->fList[k].Daughter(0)->M(); break;
						case 151: theArF[off+k]=cur->fList[k].Daughter(1)->M(); break;
						case 152: theArF[off+k]=cur->fList[k].Daughter(2)->M(); break;
						case 153: theArF[off+k]=cur->fList[k].Daughter(3)->M(); break;
						...
						case 172: if (mic) theArF[off+k]=mic->GetPionPidLH(); break; 		// Pion LH 
						case 173: if (mic) theArF[off+k]=mic->GetKaonPidLH(); break; 		// Kaon LH
						case 174: if (mic) theArF[off+k]=mic->GetProtonPidLH(); break; 		// Proton LH
						
						// PID Info based in TCandidate entries... obsolete
						/*
				...
				for (k=0;k<cur->GetLength();k++)
				{
					...
					switch (key) 
					{
						...
						case 505:
						{
							...
							TCandidate *dauC=cur->fList[k].Daughter(dnum);
							if (0==dauC) break;
						...
						case 510: theArI[k+off]=cur->fList[k].GetMcIdx(); break;
						...
						case 515: // determine the best pid level
							...
							{
								int pidl=0;
								VAbsPidSelector *theSel=0;
								
								switch (abs(cur->fPdgCode))
								{
								case 11:   theSel=eSel;break;
								case 13:   theSel=muSel;break;
								case 211:  theSel=piSel;break;
								case 321:  theSel=kSel;break;
								case 2212: theSel=pSel;break;
								}
								if (theSel)
								{
						...
						case 520: theArI[k+off]=cur->fList[k].PdgCode(); break;
...
void PndSimpleAnalysis::PrintTree(TCandidate *tc, int level)
{
  int i=0;
...
void PndSimpleAnalysis::SetConfigFile(std::string filename)
{
...
void PndSimpleAnalysis::FillGenericLists()
{
  ...
  if (fMicroArray)
  {
		...
		{
			PndMicroCandidate *mic = (PndMicroCandidate *)fMicroArray->At(i);
			TCandidate tc(*mic,i);
	...
	while (!cfgFile.eof()) 
	{
		...
		{ 
			...
			if (IsGenericListName(dName)) 
			{
				...
				{
				  case   11: dName="ElectronVeryLoose";break;
				  case   13: dName="MuonVeryLoose";break;
				  case  211: dName="PionVeryLoose";break;
				  case  321: dName="KaonVeryLoose";break;
				  case 2212: dName="ProtonVeryLoose";break;
				}
...
/*		if (tokenVec[0]=="MaxEntries")
		...
		if (tokenVec[0]=="select")
		{
			...
			if (tokenVec[1]=="Mass")
			{
				...
				TPidMassSelector *sel=new TPidMassSelector((currentList->fName+"sel").c_str(),mean,width);
...
bool PndSimpleAnalysis::ErrorMessage(int mid, int line, std::string arg)
{
    cout << "-E- PndSimpleAnalysis::SetupAnalysis: In '"<< fCfgFileName << "', line "<<line<<": ";
    
	switch (mid) 
	{
	case 100: cout << "Missing 'End' statement!";
		break;
		
	case 101: cout << "List with name '"<< arg<< "' already defined";
		break;
		
	case 200: cout << "Missing 'DefineList' statement!";
		break;
		
	case 201: cout << "DecayMode already defined!";
		break;
			
	case 203: cout << "More than 5 daughters not allowed!";
		break;
			
	case 301: cout << "Missing 'decayMode' statement!";
		break;
		
	case 302: cout << "Too many daughterLists defined!";
		break;
		
	case 303: cout << "Unknown List '"<<arg<<"'!";
		break;
				
	case 304: cout << "Particle type mismatch between daughterList and decayMode!";
		break;
				
	case 305: cout << "McTruth cannot be used as daughterList!";
		break;
				
	case 400: cout << "Wrong number of daughters or daughterLists!";
		break;
				
	case 500: cout << "Mass of particle '"<<arg <<"' unknown; must specify selector mean value!";
		break;
				
	case 501: cout << "Must specify name of tuple column!";
		break;
				
	case 502: cout << "Must specify name of list and tuple column!";
		break;
				
	case 503: cout << "Warning: Initial 4 vector already set before!";
		break;
				
	case 504: cout << "Branch '"<<arg<<"' already exists in NTuple!";
		break;
		
	case 510: cout << "Invalid number for parameter 'MaxEntries'.";
		break;
