...
PndListProvider::PndListProvider(std::string name,std::string pdgType) 
{
  ...
  fMinusSel = new TPidMinusSelector;
  fPlusSel  = new TPidPlusSelector;
  fMassSel  = 0;
...
void PndListProvider::SetCandList(TCandList &cl)
{
  Reset();
...
void
PndListProvider::GetCandList(TCandList &tcl)
{
  tcl.Cleanup();
...
std::string 
PndListProvider::GetDecayProdName(int i)
{
  if (i<fNDaughters)
...
void PndListProvider::SetMassSelector(double mean, double width)
{
  if (!fMassSel)
   fMassSel=new TPidMassSelector((fName+"sel").c_str(),mean,width);
}


/*
void PndListProvider::SetHisto(double min, double max, int bins)
{
  fFillHisto=true;
  double tmp=0;
  if (min>max) 
