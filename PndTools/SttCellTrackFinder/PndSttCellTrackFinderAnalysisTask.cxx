...
InitStatus PndSttCellTrackFinderAnalysisTask::Init() {
	...
	fHistoNumberOfLinks1 =
			new TH1I("fHistoNumberOfLinks1",
					"Number Of Links Per Track Candidate ; # Links  ;  # Track Candidates ",
					5, -0.5, 4.5);

	fHistoNumberOfTracklets1 = new TH1I("fHistoNumberOfTracklets1",
			"Number Of Tracklets Per MCTrack; #Tracklets ;  # MCTracks ", 20,
			-0.5, 19.5);

	fHistoNumberOfAssignedHits1 = new TH1I("fHistoNumberOfAssignedHits1",
			"Ratio Of Assigned And Unassigned STTHits ;  ;  # Hits ", 2, -0.5,
			1.5);

	fHistoNumberOfHits1 = new TH1I("fHistoNumberOfHits1",
			"Number Of Hits Per Tracklet ; # Hits ;  # Tracklets ", 40, -0.5,
			39.5);

	fHistoNumberOfErrors1 = new TH1I("fHistoNumberOfErrors1",
			"Number Of Errors Per Event ; # Errors ;  # Events ", 11, -1.5,
			9.5);

	fHistoNumberOfLinks2 =
			new TH1I("fHistoNumberOfLinks2",
					"Number Of Links Per Track Candidate ; # Links  ;  # Track Candidates ",
					5, -0.5, 4.5);

	fHistoNumberOfTracklets2 = new TH1I("fHistoNumberOfTracklets2",
			"Number Of Tracklets Per MCTrack; # Tracklets ;  # MCTracks ", 25,
			-0.5, 24.5);

	fHistoNumberOfHits2 = new TH1I("fHistoNumberOfHits2",
			"Number Of Hits Per Tracklet ; # Hits ;  # Tracklets ", 40, -0.5,
			39.5);

	fHistoNumberOfErrors2 = new TH1I("fHistoNumberOfErrors2",
			"Number Of Errors Per Event ; # Errors ;  # Events ", 11, -1.5,
			9.5);

	fHistoQualityCombi = new TH1I("fHistoQualityCombi",
			"Quality Of Trackfinding; ; # Tracks", 8, -0.5, 7.5);

	fHistoQualityFirstStep = new TH1I("fHistoQualityFirstStep",
			"Quality Of Trackfinding; ; # Tracks", 8, -0.5, 7.5);

	fHistoMaxHitsOfMCTrack1 = new TH1I("fHistoMaxHitsOfMCTrack1",
			"Max Number Of Hits At A Stretch Per MCTrack; # Hits; # MCTracks",
			40, -0.5, 39.5);

	fHistoMaxHitsOfMCTrack2 = new TH1I("fHistoMaxHitsOfMCTrack2",
			"Max Number Of Hits At A Stretch Per MCTrack; # Hits; # MCTracks",
			40, -0.5, 39.5);

	fHistoNumberOfMissingHits1 =
			new TH1I("fHistoNumberOfMissingHits1",
					"Amount Of Missing Hits Of Spurious Tracklets; Missing Hits in %; # Tracklets",
					4, -0.5, 3.5);

	fHistoNumberOfMissingHits2 =
			new TH1I("fHistoNumberOfMissingHits2",
					"Amount Of Missing Hits Of Spurious Tracklets; Missing Hits in %; # Tracklets",
					4, -0.5, 3.5);
	...
	fHistoNumberOfHitsMCTrack = new TH1I("fHistoNumberOfHitsMCTrack",
			"Number Of Hits Per MCTrack; # Hits; # MCTracks", 70, -0.5, 69.5);
...
void PndSttCellTrackFinderAnalysisTask::Exec(Option_t *opt) {
	...
	if (fVerbose > 0) {
		cout << "============= Begin PndSttCellTrackFinderAnalysisTask::Exec"
				<< endl;
		cout << endl;
	}
...
void PndSttCellTrackFinderAnalysisTask::CheckFirstTracklets() {
	...
	PndMCResult myResultCandToMCTrack = fMCMatch->GetMCInfo("FirstTrackCand",
			"MCTrack");
	PndMCResult myResultHitsToMCTrack = fMCMatch->GetMCInfo("STTHit",
			"MCTrack");
	...
	for (int i = 0; i < fFirstTrackCand->GetEntriesFast(); ++i) {
		...
		for (int j = 0; j < myResultCandToMCTrack.GetEntry(i).GetNLinks();
				++j) {
			trackletsCounter[myResultCandToMCTrack.GetEntry(i).GetLink(j).GetIndex()] +=
					1;
		}
		...
		if (myResultCandToMCTrack.GetEntry(i).GetNLinks() > 1) {
			...
			numberOfLinks = GetNumLinksOfHits(hitLinks, myResultHitsToMCTrack,
					myResultHitToPoint);
			...
			if (numberOfLinks > 1) {
				...
				if (fVerbose > 2) {
					...
					cout << "Nlinks:"
							<< myResultCandToMCTrack.GetEntry(i).GetNLinks()
							<< endl;
				}
			}
...
void PndSttCellTrackFinderAnalysisTask::CheckTrackletCombinations() {
	...
	// get FairLinks of stored data
	PndMCResult myResultCandToMCTrack = fMCMatch->GetMCInfo("CombiTrackCand",
			"MCTrack");
	PndMCResult myResultHitsToMCTrack = fMCMatch->GetMCInfo("STTHit",
			"MCTrack");
	PndMCResult myResultHitToPoint = fMCMatch->GetMCInfo("STTHit", "STTPoint");
...
int PndSttCellTrackFinderAnalysisTask::GetNumLinksOfHits(
		FairMultiLinkedData& hitLinks, PndMCResult& sttHitsToMCTrack,
	...
	priorTrackIndex =
			sttHitsToMCTrack.GetEntry(priorHitIndex).GetLink(0).GetIndex();
	...
	if (fVerbose > 3)
		cout << "TubeID: " << fMapHitIndexToTubeID[priorHitIndex]
	...
	for (int j = 1; j < hitLinks.GetNLinks(); ++j) {
		...
		curHitIndex =
				sttHitToPoint.GetEntry(curLink.GetIndex()).GetLink(0).GetIndex();
		curTrackIndex =
				sttHitsToMCTrack.GetEntry(curHitIndex).GetLink(0).GetIndex();

		if (fVerbose > 3)
			cout << "TubeID: " << fMapHitIndexToTubeID[curHitIndex]
		...
		if (priorTrackIndex != curTrackIndex) {
			...
			if (fMapTubeIDToHits[fMapHitIndexToTubeID[curHitIndex]].size()
					> 1) {
				...
				trackIndices.insert(
						sttHitsToMCTrack.GetEntry(tmpHitIndex).GetLink(0).GetIndex());
				...
				if (fVerbose > 3)
					cout << "---------> TubeID of faulty hit: "
							<< fMapHitIndexToTubeID[curHitIndex] << endl;
			}
...
void PndSttCellTrackFinderAnalysisTask::DrawFirstRiemannPlots(int eventNumber) {
	...
	// save plot
	fCanvas->Print(
			"Plots/FirstTracklets/Event" + TString::Itoa(eventNumber, 10)
					+ ".pdf", "pdf");
}
...
void PndSttCellTrackFinderAnalysisTask::DrawCombiRiemannPlots(int eventNumber) {
	...
	// save plot
	fCanvas->Print(
			"Plots/Combinations/EventCombi" + TString::Itoa(eventNumber, 10)
					+ ".pdf", "pdf");
}
...
void PndSttCellTrackFinderAnalysisTask::CheckConformalMap() {
	...
	double strawHits[6][2] = { { -1, 2 }, { 0, 3 }, { 1, 2 }, { 2, 3 },
			{ 3, 2 }, { 4, 3 } };
	...
	for (int i = 0; i < 6; ++i) {
		...
		div = (strawHits[i][0] - refHitX) * (strawHits[i][0] - refHitX)
				+ (strawHits[i][1] - refHitY) * (strawHits[i][1] - refHitY);
	...
	for (double y = -1; y <= 3; y += 0.5) {
		...
		oldPoints[pointCounter][0] = TMath::Sqrt(
				radius * radius - ((y - centerY) * (y - centerY))) + centerX;
		oldPoints[pointCounter][1] = y;
		pointsOnCircle->SetPoint(pointCounter, oldPoints[pointCounter][0],
				oldPoints[pointCounter][1]);
		++pointCounter;
	}
	...
	for (int i = 0; i < 9; ++i) {
		div = (oldPoints[pointCounter][0] - refX)
				* (oldPoints[pointCounter][0] - refX)
				+ (oldPoints[pointCounter][1] - refY)
						* (oldPoints[pointCounter][1] - refY);
		mappedPoints[pointCounter][0] = (oldPoints[pointCounter][0] - refX)
				/ div;
		mappedPoints[pointCounter][1] = -(oldPoints[pointCounter][1] - refY)
				/ div;
		pointsOnLine->SetPoint(pointCounter, mappedPoints[pointCounter][0],
				mappedPoints[pointCounter][1]);
		++pointCounter;
	}

	double mappedErrPoint[2];
	mappedErrPoint[0] = (errPoint[0] - refX)
			/ ((errPoint[0] - refX) * (errPoint[0] - refX)
					+ (errPoint[1] - refY) * (errPoint[1] - refY));
	mappedErrPoint[1] = -(errPoint[1] - refY)
			/ ((errPoint[0] - refX) * (errPoint[0] - refX)
					+ (errPoint[1] - refY) * (errPoint[1] - refY));
...
void PndSttCellTrackFinderAnalysisTask::TestRecoQualityFirstStep() {
	...
	PndMCResult trackCandToMCTrack = fMCMatch->GetMCInfo("FirstTrackCand",
			"MCTrack");
	...
	for (int i = 0;
			i < fMCTrack->GetEntries() && i < MCTrackToSttHits.GetNEntries();
			i++) {

		FairMultiLinkedData STTHitsOfMCTrack =
				MCTrackToSttHits.GetEntry(i).GetLinksWithType(
						ioman->GetBranchId("STTHit"));
		...
		if (STTHitsOfMCTrack.GetNLinks() > 1) {
			...
			for (int j = 0; j < trackCandToMCTrack.GetNEntries(); j++) {
				...
				PndTrackCand* myTrackCand = (PndTrackCand*) fFirstTrackCand->At(
						j);
				FairMultiLinkedData STTHitsOfTrackCand =
						myTrackCand->GetLinksWithType(
								ioman->GetBranchId("STTHit"));

				if (trackCandToMCTrack.GetEntry(j).GetNLinks() == 1
						|| GetNumLinksOfHits(STTHitsOfTrackCand,
								myResultHitsToMCTrack, myResultHitToPoint)
								== 1) {
					// hits of trackCand points to one MCTrack

					if (trackCandToMCTrack.GetEntry(j).GetLink(0).GetIndex()
							== i) {
			...
			for (int j = 0; j < trackCandToMCTrack.GetNEntries(); j++) {
				...
				if (STTHitsOfTrackCand.GetNLinks() > 1) {
					...
					// check all links
					for (int k = 0;
							k < trackCandToMCTrack.GetEntry(j).GetNLinks();
							++k) {
						if (trackCandToMCTrack.GetEntry(j).GetLink(k).GetIndex()
								== i) {
							// found current MCTrackIndex
			...
			if (fVerbose > 0) {
				...
				cout << "!!! MCTrack not found, tube: "
						<< fMapHitIndexToTubeID[index] << endl;
	...
	for (int i = 0; i < trackCandToMCTrack.GetNEntries(); i++) {
		...
		if (trackCandToMCTrack.GetEntry(i).GetNLinks() == 1
				|| GetNumLinksOfHits(hitLinks, myResultHitsToMCTrack,
						myResultHitToPoint) == 1) {
			// clean trackCand

			// get MCTrackIndex
			candTrackIndex =
					trackCandToMCTrack.GetEntry(i).GetLink(0).GetIndex();

			// get number of hits of MCTrack -> compare

			FairMultiLinkedData STTHitsOfMCTrack = MCTrackToSttHits.GetEntry(
					candTrackIndex).GetLinksWithType(
					ioman->GetBranchId("STTHit"));
			...
			if ((numMCHits - numSkewedTubesMC)
					== (numTrackHits - numSkewedTubesCand)) {

				fHistoQualityFirstStep->Fill(6);
				if (fVerbose > 0) {
					index = hitLinks.GetLink(0).GetIndex();
					cout << "FULLY RECO: " << fMapHitIndexToTubeID[index]
							<< endl;
				}
				...
				amount = 1
						- (double) (numTrackHits - numSkewedTubesCand)
								/ (numMCHits - numSkewedTubesMC);
...
void PndSttCellTrackFinderAnalysisTask::TestRecoQualityCombi() {
	...
	PndMCResult trackCandToMCTrack = fMCMatch->GetMCInfo("CombiTrackCand",
			"MCTrack");
	PndMCResult MCTrackToSttHits = fMCMatch->GetMCInfo("MCTrack", "STTHit");
	...
	for (int i = 0;
		...
		if (STTHitsOfMCTrack.GetNLinks() > 1) {
			...
			for (int j = 0; j < trackCandToMCTrack.GetNEntries(); j++) {
				...
				PndTrackCand* myTrackCand = (PndTrackCand*) fCombiTrackCand->At(
						j);
			...
			if (fVerbose > 0) {
				...
				if (fFoundMC[i] == true)
					cout << "FOUND LESS: tube " << fMapHitIndexToTubeID[index]
							<< endl;
			}
			...
			for (int j = 0; j < trackCandToMCTrack.GetNEntries(); j++) {
				...
				if (trackCandToMCTrack.GetEntry(j).GetNLinks() == 1
					...
					if (trackCandToMCTrack.GetEntry(j).GetLink(0).GetIndex()
						...
						maxHits = TMath::Max(maxHits,
								STTHitsOfTrackCand.GetNLinks());
					}
...
void PndSttCellTrackFinderAnalysisTask::Finish() {
	...
	if (fVerbose > 0) {
		...
		std::cout << "fHistoQualityFirstStep: #Possible Tracks "
				<< fHistoQualityFirstStep->GetBinContent(1)
				<< " #Found in clean: "
				<< fHistoQualityFirstStep->GetBinContent(2)
				<< " #Found in faulty: "
				<< fHistoQualityFirstStep->GetBinContent(3) << " #Not Found: "
				<< fHistoQualityFirstStep->GetBinContent(4) << " #TrackCands: "
				<< fHistoQualityFirstStep->GetBinContent(5) << " #Spurious: "
				<< fHistoQualityFirstStep->GetBinContent(6) << " #FullyReco: "
				<< fHistoQualityFirstStep->GetBinContent(7) << " #Ghosts: "
				<< fHistoQualityFirstStep->GetBinContent(8) << std::endl;

		std::cout << "fHistoQualityCombi: #Possible Tracks "
				<< fHistoQualityCombi->GetBinContent(1) << " #Found in clean: "
				<< fHistoQualityCombi->GetBinContent(2) << " #Found in faulty: "
				<< fHistoQualityCombi->GetBinContent(3) << " #Not Found: "
				<< fHistoQualityCombi->GetBinContent(4) << " #TrackCands: "
				<< fHistoQualityCombi->GetBinContent(5) << " #Spurious: "
				<< fHistoQualityCombi->GetBinContent(6) << " #FullyReco: "
				<< fHistoQualityCombi->GetBinContent(7) << " #Ghosts: "
				<< fHistoQualityCombi->GetBinContent(8) << std::endl;
	}
