#ifndef PNDDRPCPOINT_H
#define PNDDRPCPOINT_H
...
class PndDrpcPoint : public FairMCPoint 
{
 ...
 public:
  ...
  PndDrpcPoint(Int_t trackID, Int_t evtID,
  ...
  /** Copy constructor **/
  PndDrpcPoint(const PndDrpcPoint& point) { *this = point; };
  

  /** Destructor **/
  virtual ~PndDrpcPoint();
 ...
 protected:
  ...
  ClassDef(PndDrpcPoint,1)

};

inline void PndDrpcPoint::SetPositionIn(const TVector3& pos) {
  fXin = pos.X();
  fYin = pos.Y();
  fZin = pos.Z();
}

inline void PndDrpcPoint::SetMomentumIn(const TVector3& mom) {
  fPxin = mom.X();
  fPyin = mom.Py();
  fPzin = mom.Pz();
}

inline void PndDrpcPoint::SetPositionOut(const TVector3& pos) {
  fXout = pos.X();
  fYout = pos.Y();
  fZout = pos.Z();
}

inline void PndDrpcPoint::SetMomentumOut(const TVector3& mom) {
  fPxout = mom.Px();
  fPyout = mom.Py();
  fPzout = mom.Pz();
}
