...
public: 
    PndDiscDigitizedHit();
    PndDiscDigitizedHit(FairLink const & fairln_mc_point,
                          ...
                          const Double_t & tdc_time,   const Double_t & panda_timestamp,
    ...
    virtual bool SortOrder(const FairTimeStamp* s2) const {
        ...
        if      (detector_id      < digit.GetDetectorID())      return true;
        else if (detector_id      > digit.GetDetectorID())      return false;
        if      (readout_id       < digit.GetReadoutID())       return true;
        else if (readout_id       > digit.GetReadoutID())       return false;
        if      (dead_time_entity < digit.GetDeadTimeEntity())  return true;
        else if (dead_time_entity > digit.GetDeadTimeEntity())  return false;
    ...
    virtual bool operator == (const PndDiscDigitizedHit & digit)  const {
        return (   ( detector_id       == digit.GetDetectorID()  )
                && ( readout_id        == digit.GetReadoutID()   )
                && ( dead_time_entity  == digit.GetDeadTimeEntity() )
               );
    }
