...
void PndSttGeomPoint::Draw(Double_t size, Int_t color) const
{
    ...
    TPolyLine3D
	*xstart = new TPolyLine3D(2, xstartx, xstarty, xstartz),
	*ystart = new TPolyLine3D(2, ystartx, ystarty, ystartz),
...
Bool_t PndSttGeomPoint::IsBetween(PndSttGeomPoint firstPoint, PndSttGeomPoint secondPoint) const
{
    ...
    if (((firstPoint.GetX() < fX) && (fX < secondPoint.GetX())) || 
	((firstPoint.GetX() > fX) && (fX > secondPoint.GetX())))
    {
	betweenX = kTRUE;
    }

    if (((firstPoint.GetY() < fY) && (fY < secondPoint.GetY())) || 
	((firstPoint.GetY() > fY) && (fY > secondPoint.GetY())))
    {
	betweenY = kTRUE;
    }

    if (((firstPoint.GetZ() < fZ) && (fZ < secondPoint.GetZ())) || 
	((firstPoint.GetZ() > fZ) && (fZ > secondPoint.GetZ())))
    {
	betweenZ = kTRUE;
    }
...
{
    return sqrt(((myPoint.GetX() - fX) * (myPoint.GetX() - fX)) +
		((myPoint.GetY() - fY) * (myPoint.GetY() - fY)) +
		((myPoint.GetZ() - fZ) * (myPoint.GetZ() - fZ)));
}
