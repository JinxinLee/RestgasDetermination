...
class PndPidCandidate : public FairTimeStamp
{
  ...
  public:
    ...
    TVector3  GetFirstHit() const {
      return TVector3(fFirstHitX,fFirstHitX,fFirstHitX);
    }
    TVector3  GetLastHit() const {
      return TVector3(fLastHitX,fLastHitY,fLastHitZ);
    }
    ...
    void  SetHelixParams(Double_t *par) {
      if (par!=0) for (int i=0; i<5; i++) fParams[i] = (Float_t)par[i];
    }
    void  SetHelixCov(Double_t* cov) {
      if (cov!=0) for (int i=0; i<15; i++) fCov[i] = (Float_t)cov[i];
    }
