#ifndef PNDFTSPOINT_H
#define PNDFTSPOINT_H
...
class PndFtsPoint : public FairMCPoint 
{
 ...
 public:
   ...
   **/
  PndFtsPoint(Int_t trackID, Int_t detID, Int_t tubeID, Int_t chamberID, Int_t layerID,
...
// tube ID // CHECK added
 ...
 //////
  ...
  ClassDef(PndFtsPoint,1)

    };



inline void PndFtsPoint::SetPositionOutLocal(TVector3 pos) {
  fX_out_local = pos.X();
  fY_out_local = pos.Y();
  fZ_out_local = pos.Z();
}

inline void PndFtsPoint::SetPositionInLocal(TVector3 pos) {
  fX_in_local = pos.X();
  fY_in_local = pos.Y();
  fZ_in_local = pos.Z();
}
...
inline void PndFtsPoint::SetMomentumOut(TVector3 mom) {
  fPx_out = mom.Px();
  fPy_out = mom.Py();
  fPz_out = mom.Pz();
}
