...
public:
  ...
  //if correspondences are clear (or given by default), the iterative process can worsen result. disable iterative portion of icp.
  void forceInstantResult(bool val){ instantForce = val; }

  //return matrices even if no convergence achieved
  void giveOutputIfNotConverged(bool val){ giveOutputIfNotConvergedB = val; }
  ...
  void setEventTimeCheck(bool val){ checkEventTime = val; }
  ...
  double fit(double *T,const int32_t T_num,Matrix &R,Matrix &t,const double indist); //returns delta between the last two transfomartion matrices before termination

  //Euclidean fitness score. The lower the better, but if it returns 0, something wrong has happened.
...
protected:
  ...
  double  euclidean_fitness; //fitness score for error estimation
