...
PndLmdDigiPara :: PndLmdDigiPara(const char* name,const char* title,const char* context)
...
Bool_t PndLmdDigiPara::getParams(FairParamList* list)
{
	...
	if(!list->fill("circle_pitch", &fCirclePitch))        return kFALSE;
    if(!list->fill("right_pitch", &fRightPitch))          return kFALSE;
    if(!list->fill("left_pitch", &fLeftPitch))            return kFALSE;
    if(!list->fill("left_orient", &fLeftOrient))          return kFALSE;
    if(!list->fill("right_orient", &fRightOrient))        return kFALSE;
    Double_t x, y;
    if(!list->fill("circle_anchor_x", &x))                return kFALSE;
    if(!list->fill("circle_anchor_y",&y))                 return kFALSE;
    fCircleAnchor.Set(x,y);
    if(!list->fill("right_anchor_x", &x))                 return kFALSE;
    if(!list->fill("right_anchor_y", &y))                 return kFALSE;
    fRightAnchor.Set(x,y);
    if(!list->fill("left_anchor_x", &x))                  return kFALSE;
    if(!list->fill("left_anchor_y", &y))                  return kFALSE;
    fLeftAnchor.Set(x,y);

    if(!list->fill("nr_fe_channels", &fNrFeChannels))     return kFALSE;
    if(!list->fill("nr_fe_circle", &fNrCircleFe))         return kFALSE;
    if(!list->fill("nr_fe_right", &fNrRightFe))           return kFALSE;
    if(!list->fill("nr_fe_left", &fNrLeftFe))             return kFALSE;
