...
PndLmdDigiProducer::PndLmdDigiProducer(Int_t k,SensorSegment segment)
                    :FairTask("Pnd LMD Digi Producer")
{
	fBranchName = "PndLmdMCPoint";
...
PndLmdDigiProducer::PndLmdDigiProducer(Double_t k,SensorSide side)
                   :FairTask("Pnd LMD Digi Producer")
 {
	fBranchName = "PndLmdMCPoint";
  //  fDigiParTrap = digipar;
...
InitStatus PndLmdDigiProducer::Init()
{
    ...
    if(! fDigiParTrap)
    {
       std::cout<<"-E- PndLmdDigiProducer::DigiParTrap Container doesn't exist!"<<std::endl;
       return kERROR;
    }
...
void PndLmdDigiProducer::Exec(Option_t*)
{
	...
	for(Int_t iPoint = 0;iPoint<nPoints;iPoint++)
	{
		...
		std::vector<PndLmdStrips> leftStrips =
			fCurrentStripCalLeft->DigiStripFromPoint(posInL.X(),posInL.Y(),posInL.Z(),
              ...
              std::vector<PndLmdStrips> rightStrips =
            	  fCurrentStripCalRight->DigiStripFromPoint(posInL.X(),posInL.Y(),posInL.Z(),
					                                posOutL.X(),posOutL.Y(),posOutL.Z(),
					                                eLoss);
        //      cout<<"Call the calStrip class to get digi successfully"<<endl;

              if(rightStrips.size()!=0){
            	  for(std::vector<PndLmdStrips>::const_iterator kit=rightStrips.begin();
            	       kit!=rightStrips.end();++kit){
            		    //register the strips fired
            		    AddDigi(iStrip,iPoint,kLMDHitsStrip,point->GetDetName(),
...
*/
              ...
              }else std::cout<<"the right segment is empty"<<std::endl;
 ...
 //   if(fVerbose>0)
    	std::cout<<"-I- PndLmdDigiProducer: "<<nPoints<<" PndLmdMCPoint, "
                    <<iStrip<<" PndLmdDigis"<<std::endl<<std::endl;

}//public method for Exec

void PndLmdDigiProducer::AddDigi(Int_t &iStrip,Int_t iPoint,Int_t detID,TString detName,
		                         Int_t fe, Int_t channel,Double_t charge)
{
    Bool_t found = kFALSE;
    PndLmdDigiStrips* aDigi= 0;
