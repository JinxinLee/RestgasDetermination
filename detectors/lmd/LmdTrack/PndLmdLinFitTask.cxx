...
PndLmdLinFitTask::PndLmdLinFitTask(TString tTCandBranchName, TString tRecoBranchName,
    TString tOutputBranchName, TString tOutputFolder) :
		FairTask("3D-Straight-Line-Fit"), fTCandArray(0), fRecoArray(0), fTrackArray(0), fGeoH(0) {
...
double PndLmdLinFitTask::distance_MS(double x, double y, double z, double errx, double erry,
    const double* p, double* zpr) {
...
double PndLmdLinFitTask::line3DfitMS(TGraph2DErrors* gr, const TVector3& posSeed,
    const TVector3& dirSeed, std::vector<double>& fitpar, TMatrixDSym* covmatrix) {
