	  ...
	  while (1)
	    {
	      ...
	      if (find_iter==mapBox.end()) break; // if no close box
	  ...
	  while (1)
	    {
	      ...
	      if (find_iter==mapStrip.end()) break; // if no close strip
...
Bool_t PndMdtClusterTask::MdtMapping() 
{
  ...
  if (nBox==0 || nStrip==0) return kFALSE;
  
  Reset();
  PndMdtDigi *mdtDigi = NULL;
