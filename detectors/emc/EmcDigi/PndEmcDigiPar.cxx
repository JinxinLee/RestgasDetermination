...
Bool_t PndEmcDigiPar::getParams(FairParamList* list)
{
	...
	if (!list->fill("FWD_Shaping_int_time",&fFWD_Shaping_int_time)) return kFALSE;
	if (!list->fill("FWD_time_constant",&fFWD_time_constant)) return kFALSE;
	...
	if (!list->fill("SampleRate_FWD",&fSampleRate_FWD)) return kFALSE;
	...
	if (!list->fill("Number_of_samples_in_waveform_fwd",&fNumber_of_samples_in_waveform_fwd)) return kFALSE;
