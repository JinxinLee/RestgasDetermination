...
InitStatus PndEmcMakeCluster::Init()
{
	...
	cout << "-I- PndEmcMakeCluster: Intialization successful" << endl;
	return kSUCCESS;
}
...
void PndEmcMakeCluster::Exec(Option_t *)
{
	...
	{
		if (isAdded[i] < fClusterArray->GetEntriesFast())
		{																			 // if isAdded[i] is smaller than the current #clusters, the cluster to which this digi belongs already exists, and we should add it to this cluster
		...
		{ // if not, make a new cluster for this digi
