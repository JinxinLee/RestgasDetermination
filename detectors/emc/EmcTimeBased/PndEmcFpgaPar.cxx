...
Bool_t PndEmcFpgaPar::getParams(FairParamList* list)
{
	...
	if (!list->fill("BarrelMAFilterLength",&fMAFilterLengthBarrel)) return kFALSE;
	if (!list->fill("BarrelCFFilterLength",&fCFFilterLengthBarrel)) return kFALSE;
	if (!list->fill("BarrelCFFilterRatio",&fCFFilterRatioBarrel)) return kFALSE;
	if (!list->fill("BarrelCFFitterLength",&fCFFitterLengthBarrel)) return kFALSE;
	if (!list->fill("BarrelCFDelayLength",&fCFDelayLengthBarrel)) return kFALSE;
	if (!list->fill("BarrelMWDFilterUsed",&fMWDFilterUsedBarrel)) return kFALSE;
	if (!list->fill("BarrelMWDFilterLength",&fMWDFilterLengthBarrel)) return kFALSE;
	if (!list->fill("BarrelMWDFilterLifeT",&fMWDDecayConstantBarrel)) return kFALSE;
	if (!list->fill("BarrelPulseThreshold",&fPulseThresholdBarrel)) return kFALSE;

	if (!list->fill("ForwardMAFilterLength",&fMAFilterLengthForward)) return kFALSE;
	if (!list->fill("ForwardCFFilterLength",&fCFFilterLengthForward)) return kFALSE;
	if (!list->fill("ForwardCFFilterRatio",&fCFFilterRatioForward)) return kFALSE;
	if (!list->fill("ForwardCFFitterLength",&fCFFitterLengthForward)) return kFALSE;
	if (!list->fill("ForwardCFDelayLength",&fCFDelayLengthForward)) return kFALSE;
	if (!list->fill("ForwardMWDFilterUsed",&fMWDFilterUsedForward)) return kFALSE;
	if (!list->fill("ForwardMWDFilterLength",&fMWDFilterLengthForward)) return kFALSE;
	if (!list->fill("ForwardMWDFilterLifeT",&fMWDDecayConstantForward)) return kFALSE;
	if (!list->fill("ForwardPulseThreshold",&fPulseThresholdForward)) return kFALSE;

	if (!list->fill("ShashylikMAFilterLength",&fMAFilterLengthShashylik)) return kFALSE;
	if (!list->fill("ShashylikCFFilterLength",&fCFFilterLengthShashylik)) return kFALSE;
	if (!list->fill("ShashylikCFFilterRatio",&fCFFilterRatioShashylik)) return kFALSE;
	if (!list->fill("ShashylikCFFitterLength",&fCFFitterLengthShashylik)) return kFALSE;
	if (!list->fill("ShashylikCFDelayLength",&fCFDelayLengthShashylik)) return kFALSE;
	if (!list->fill("ShashylikMWDFilterUsed",&fMWDFilterUsedShashylik)) return kFALSE;
	if (!list->fill("ShashylikMWDFilterLength",&fMWDFilterLengthShashylik)) return kFALSE;
	if (!list->fill("ShashylikMWDFilterLifeT",&fMWDDecayConstantShashylik)) return kFALSE;
	if (!list->fill("ShashylikPulseThreshold",&fPulseThresholdShashylik)) return kFALSE;
