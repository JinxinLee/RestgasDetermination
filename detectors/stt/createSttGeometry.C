...
// GasManifold Pipe
	...
	for (Int_t i = 0; i < 6; i++) {
		for (Int_t j = 0; j < 5; j++) {
			sPoint[i * 5 + j][0] = 42.3
					* cos((75 - 1.5 * j + Angleda12[i]) * PI / 180);
			sPoint[i * 5 + j][1] = 42.3
					* sin((75 - 1.5 * j + Angleda12[i]) * PI / 180);
			if (j != 3) {
	...
	for (Int_t i = 0; i < 6; i++) {
		for (Int_t j = 0; j < 5; j++) {
			if (gmPipelavel[i * 5 + j] != 0) {
				...
				for (Int_t k = 0; k < dLevel; k++) {
					...
					if (k == 0) {
						...
						if (specArf[i * 5 + j][k][2] != 0) {
							x =
									x
											+ (specArf[i * 5 + j][sIdPoint][0]
													- specArf[i * 5 + j][k][2])
													* sin(
															specArf[i * 5 + j][sIdPoint][1]
																	* PI / 180);
							y =
									y
											- (specArf[i * 5 + j][sIdPoint][0]
					...
					std::string tubefname = "tubef" + std::to_string(i)
							+ std::to_string(j) + std::to_string(k);
					std::string gasfname = "gasf" + std::to_string(i)
							+ std::to_string(j) + std::to_string(k);
					std::string trfname = "trf" + std::to_string(i)
							+ std::to_string(j) + std::to_string(k);
					tubef.push_back(
							new TGeoTube(tubefname.c_str(), 0.15, .2,
									(specArf[i * 5 + j][k][0]) / 2.0));
					gasf.push_back(
							new TGeoTube(gasfname.c_str(), 0, .1499,
									(specArf[i * 5 + j][k][0]) / 2.0));

					trf.push_back(
							new TGeoCombiTrans(trfname.c_str(), x, y, z,
									new TGeoRotation("r",
											specArf[i * 5 + j][k][1], angle,
											0)));
				...
				gasdel += ")";
				gastmpin += gasdel;
				tubeshapecomp.push_back(
						new TGeoCompositeShape(name.c_str(),
								tubetmpin.c_str()));
				gasshapecomp.push_back(
						new TGeoCompositeShape(name.c_str(), gastmpin.c_str()));

				gmPipeArr.push_back(
						new TGeoVolume("gmPipe", tubeshapecomp[index],
								gGeoManager->GetMedium("Aluminum")));
				TGeoVolume *gas;
				gas = new TGeoVolume("ArCO2", gasshapecomp[index],
						gGeoManager->GetMedium("ArCO2"));
				gas->SetLineColor(20);
...
TGeoVolume *constructMechanicalFrame() {
	...
	tubeFrame = new TGeoVolume("tubeFrame", tubeFrameShape,
			gGeoManager->GetMedium("ABS"));
	...
	smallRingShape = new TGeoTube("smallRingShape", 15.0, 16.1,
			frameThickness / 2);

	TGeoBBox *BoxShape;
	BoxShape = new TGeoBBox("BoxShape", 3.5 / 2, 13.0, frameThickness / 2);
	TGeoCombiTrans *tbs1 = new TGeoCombiTrans("tbs1", 3.5 / 2 + 1.0, 19.0 + 9.0,
			0, new TGeoRotation("dd", 0, 0, 0));
	tbs1->RegisterYourself();
	TGeoCombiTrans *tbs2 = new TGeoCombiTrans("tbs2", 29.0 * sin(60 * PI / 180),
			29.0 * cos(60 * PI / 180), 0, new TGeoRotation("dd", 0, 0, -60));
	tbs2->RegisterYourself();
	TGeoCombiTrans *tbs3 = new TGeoCombiTrans("tbs3", 29.0 * sin(60 * PI / 180),
			-29.0 * cos(60 * PI / 180), 0, new TGeoRotation("dd", 0, 0, 60));
	tbs3->RegisterYourself();
	TGeoCombiTrans *tbs4 = new TGeoCombiTrans("tbs4", 3.5 / 2 + 1.0,
			-19.0 - 9.0, 0, new TGeoRotation("dd", 0, 0, 0));
	...
	delBigRingShape = new TGeoTube("delBigRingShape", 41.6, 45.0,
			frameThickness / 2);
	...
	for (Int_t i = 0; i < 4; i++) {
		fts8.push_back(
				TGeoTranslation(43.0 * cos(Angles8[i] * PI / 180),
						43.0 * sin(Angles8[i] * PI / 180), 0));
	}
	...
	std::string tempStr =
			"(smallRingShape + bigRingShape + BoxShape:tbs1+ BoxShape:tbs2+ BoxShape:tbs3+BoxShape:tbs4)-delBigRingShape:tdbrs-delBoxShape:tdbs";
	for (Int_t i = 0; i < 4; i++) {
		tempStr = tempStr + "-tubeHoleShape:tbh" + std::to_string(i);
	}
	for (Int_t i = 0; i < 6; i++) {
		for (Int_t j = 0; j < 5; j++) {
			tempStr = tempStr + " - tubeHoleShape2:tThs"
					+ std::to_string(i * 5 + j);
		}
	}

	TGeoCompositeShape *supportedFrameShape = new TGeoCompositeShape(
			"supportedFrameShape", tempStr.c_str());

	supportedFrame = new TGeoVolume("supportedFrame", supportedFrameShape,
			gGeoManager->GetMedium("Aluminum"));
	supportedFrame->SetLineColor(29);

	TGeoVolume *gasPibeCoverM = gGeoManager->MakeTubs("gasPibeCoverM",
			gGeoManager->GetMedium("ABS"), 42.0, 43.0, tubeLength / 2, 67.6,
			76.4);
	TGeoVolume *gasPibeCoverD = gGeoManager->MakeTubs("gasPibeCoverD",
			gGeoManager->GetMedium("ABS"), 41.8, 42.8, tubeLength / 2 + 10.0,
			67.87, 76.13);
	TGeoShape *gPCM = gasPibeCoverM->GetShape();
	gPCM->SetName("gPCM");
	TGeoShape *gPCD = gasPibeCoverD->GetShape();
	gPCD->SetName("gPCD");
	TGeoCompositeShape *gasPibeCoverP1Shape = new TGeoCompositeShape(
			"gasPibeCoverP1Shape", "gPCM-gPCD");
	TGeoVolume *gasPibeCoverP1 = new TGeoVolume("gasPibeCoverP1",
			gasPibeCoverP1Shape, gGeoManager->GetMedium("ABS"));
	gasPibeCoverP1->SetLineColor(45);

	TGeoVolume *gasPibeHolderM = gGeoManager->MakeTubs("gasPibeHolderM",
			gGeoManager->GetMedium("ABS"), 41.8, 42.8, 1.0, 68.15, 75.85);
	TGeoShape *gPHM = gasPibeHolderM->GetShape();
	gPHM->SetName("gPHM");

	TGeoCompositeShape *gasPibeHolderShape =
			new TGeoCompositeShape("gasPibeHolderShape",
					"gPHM - tubeHoleShape2:tThs0- tubeHoleShape2:tThs1- tubeHoleShape2:tThs2- tubeHoleShape2:tThs3- tubeHoleShape2:tThs4");
	TGeoVolume *gasPibeHolder = new TGeoVolume("gasPibeHolder",
			gasPibeHolderShape, gGeoManager->GetMedium("ABS"));
	gasPibeHolder->SetLineColor(29);

	TGeoVolume *gasPibeCoverMb = gGeoManager->MakeTubs("gasPibeCoverMb",
			gGeoManager->GetMedium("ABS"), 41.6, 42.8, tubeLength / 2,
			19.87 + 48, 40.13 + 48);
	TGeoVolume *gasPibeCoverDb = gGeoManager->MakeTubs("gasPibeCoverDb",
			gGeoManager->GetMedium("ABS"), 41.8, 42.8 + 1, tubeLength / 2 + 100,
			32.13 + 48, 39.87 + 48);
	TGeoVolume *gasPibeCoverDb2 = gGeoManager->MakeTubs("gasPibeCoverDb2",
			gGeoManager->GetMedium("ABS"), 41.8, 42.8, tubeLength / 2 + 100,
			28.13 + 48, 31.87 + 48);
	...
	TGeoVolume *gasPibeCoverMb2 = gGeoManager->MakeTubs("gasPibeCoverMb2",
			gGeoManager->GetMedium("ABS"), 41.6, 42.8, tubeLength / 2,
			19.87 + 48, 32 + 48);
	TGeoShape *gPCMb2 = gasPibeCoverMb2->GetShape();
	gPCMb2->SetName("gPCMb2");
	TGeoVolume *gasPibeCoverDb3 = gGeoManager->MakeTubs("gasPibeCoverDb3",
			gGeoManager->GetMedium("ABS"), 41.8, 42.8 + .1,
			tubeLength / 2 + 10.0, 28.13 + 48, 35 + 48);
	TGeoShape *gPCDb3 = gasPibeCoverDb3->GetShape();
	gPCDb3->SetName("gPCDb3");

	TGeoCompositeShape *gasPibeCoverP1Shapeb2 = new TGeoCompositeShape(
			"gasPibeCoverP1Shapeb", "gPCMb2-gPCDb:rd -gPCDb3");
	TGeoVolume *gasPibeCoverP1b2 = new TGeoVolume("gasPibeCoverP1b2",
			gasPibeCoverP1Shapeb2, gGeoManager->GetMedium("ABS"));
	gasPibeCoverP1b2->SetLineColor(49);
	TGeoCompositeShape *gasPibeCoverP1Shapeb = new TGeoCompositeShape(
			"gasPibeCoverP1Shapeb", "gPCMb-gPCDb:rd-gPCDb -gPCDb2");
	TGeoVolume *gasPibeCoverP1b = new TGeoVolume("gasPibeCoverP1b",
			gasPibeCoverP1Shapeb, gGeoManager->GetMedium("ABS"));
	gasPibeCoverP1b->SetLineColor(49);

//  Translation for carbonfiber tube
	TGeoTranslation *tx1 = new TGeoTranslation("tx1", 43.0 * cos(78 * PI / 180),
			43.0 * sin(78 * PI / 180), (tubeLength) / 2 + 1.2);
	tx1->RegisterYourself();
	TGeoTranslation *tx2 = new TGeoTranslation("tx2", 43.0 * cos(30 * PI / 180),
			43.0 * sin(30 * PI / 180), (tubeLength) / 2 + 1.2);
	tx2->RegisterYourself();

	TGeoTranslation *tx3 = new TGeoTranslation("tx3",
			43.0 * cos(-30 * PI / 180), 43.0 * sin(-30 * PI / 180),
			(tubeLength) / 2 + 1.2);
	tx3->RegisterYourself();
	TGeoTranslation *tx4 = new TGeoTranslation("tx4",
			43.0 * cos(-78 * PI / 180), 43.0 * sin(-78 * PI / 180),
			(tubeLength) / 2 + 1.2);
	tx4->RegisterYourself();

	mechanicalFrame->AddNode(gasPibeCoverP1b2, 0,
			new TGeoTranslation(0, 0, (tubeLength) / 2 + 1.2));
	mechanicalFrame->AddNode(gasPibeCoverP1b, 0,
			new TGeoCombiTrans("tx4f", 0, 0, (tubeLength) / 2 + 1.2,
					new TGeoRotation("r", -48, 0, 0)));
	mechanicalFrame->AddNode(gasPibeCoverP1b, 1,
			new TGeoCombiTrans("tx4f", 0, 0, (tubeLength) / 2 + 1.2,
					new TGeoRotation("r", -108, 0, 0)));
	mechanicalFrame->AddNode(gasPibeCoverP1b2, 1,
			new TGeoCombiTrans("tx4f", 0, 0, (tubeLength) / 2 + 1.2,
					new TGeoRotation("r", 0, 180, 0)));

	Double_t ArLgasPibeHolder[] = { 4.51353, 15.7344, 17.5, 17.5, 17.5, 17.5,
			17.5, 17.5, 17.5 };
	Double_t GasPibeHolderPosZ = 0;
	for (Int_t i = 0; i < 9; i++) {
		GasPibeHolderPosZ = GasPibeHolderPosZ + ArLgasPibeHolder[i];
		frameStrip->AddNode(gasPibeHolder, 65,
				new TGeoTranslation(0, 0, GasPibeHolderPosZ + 1.2));
	}
	frameStrip->AddNode(gasPibeCoverP1, 61,
			new TGeoTranslation(0, 0, (tubeLength) / 2 + 1.2));

	mechanicalFrame->AddNode(frameStrip, 0);
	mechanicalFrame->AddNode(frameStrip, 1,
			new TGeoCombiTrans("tx4f", 0, 0, 0,
					new TGeoRotation("r", -36, 0, 0)));
	mechanicalFrame->AddNode(frameStrip, 2,
			new TGeoCombiTrans("txg", 0, 0, 0,
					new TGeoRotation("r", -48, 0, 0)));
	mechanicalFrame->AddNode(frameStrip, 3,
			new TGeoCombiTrans("tx4f", 0, 0, 0,
					new TGeoRotation("r", -96, 0, 0)));
	mechanicalFrame->AddNode(frameStrip, 4,
			new TGeoCombiTrans("txg", 0, 0, 0,
					new TGeoRotation("r", -108, 0, 0)));
	mechanicalFrame->AddNode(frameStrip, 5,
			new TGeoCombiTrans("txg", 0, 0, 0,
					new TGeoRotation("r", -144, 0, 0)));
	...
	sttCoverShape = new TGeoTube("sttCoverShape",
			42.395 + tubeRadius + .08 + 1.3, 42.395 + tubeRadius + .13 + 1.3,
			tubeLength / 2 + 6.0);
	TGeoTube *sttCoverShape2;
	sttCoverShape2 = new TGeoTube("sttCoverShape2", 15.0, 15.05,
			tubeLength / 2);

	TGeoCompositeShape *sttCoverShapeCom = new TGeoCompositeShape("sttShapeCom",
			"(sttCoverShape+sttCoverShape2)-delBoxShape2:tdbs");
	sttCover = new TGeoVolume("sttCover", sttCoverShapeCom,
			gGeoManager->GetMedium("ABS"));
	sttCover->SetLineColor(23);

	mechanicalFrame->AddNode(supportedFrame, 17,
			new TGeoTranslation(0, 0, frameThickness / 2));
	mechanicalFrame->AddNode(supportedFrame, 18,
			new TGeoCombiTrans(0, 0, (tubeLength) + 1.4,
					new TGeoRotation("r", 180, 180, 180)));
...
int createstt() {
	...
	Double_t tubeLengthArr[] = { 140.0, 140.0 / cos(2.9 * PI / 180.0), 118.5,
			98.5, 78.5, 58.5, 38.5, 18.5 };
	...
	for (Int_t i = 0; i < 8; i++) {
		...
		TGeoVolume *tubeTemp;
		TGeoVolume *cap, *EndCap, *CrimpPin, *Wire, *MylarLayer, *AlLayer1,
				*AlLayer2, *RetainingRing, *SpringSmallRing, *SpringBigRing,
				*NobleGas;
		...
		WireCoveredL2->DefineSection(0, 0, (.1 + (.040 + .025) / 2) / 2 + 0.03,
				.125);
		WireCoveredL2->DefineSection(1, 0.025,
				(.1 + (.040 + .025) / 2) / 2 + 0.05, .15);
		WireCoveredL2->DefineSection(2, 0.03, (.1 + (.040 + .025) / 2) / 2,
				.15);
		WireCoveredL2->DefineSection(3, 0.045, (.1 + (.040 + .025) / 2) / 2,
				.15);
		WireCoveredL2->DefineSection(4, 0.045, (.1 + (.040 + .025) / 2) / 2,
				.115);
		WireCoveredL2->DefineSection(5, 0.15, (.1 + (.040 + .025) / 2) / 2,
				.115);
		WireCoveredL2->DefineSection(6, 0.15, (.1 + (.040 + .025) / 2) / 2,
				.15);
		WireCoveredL2->DefineSection(7, 0.5, (.1 + (.040 + .025) / 2) / 2, .15);
		WireCoveredL2->DefineSection(8, 0.5, (.1 + (.040 + .025) / 2) / 2, .45);
		WireCoveredL2->DefineSection(9, 0.55, (.1 + (.040 + .025) / 2) / 2,
				.45);
		WireCoveredL2->DefineSection(10, 0.55, (.1 + (.040 + .025) / 2) / 2,
				.42);
		WireCoveredL2->DefineSection(11, 0.58, (.1 + (.040 + .025) / 2) / 2,
				.42);
		WireCoveredL2->DefineSection(12, 0.58, (.1 + (.040 + .025) / 2) / 2,
				.1);
		...
		TGeoCompositeShape *EndCapShape = new TGeoCompositeShape("EndCapShape",
				"WireCoveredLayer3:t1 + WireCoveredLayer2 - gts:t2");
		EndCap = new TGeoVolume("EndCapShape", EndCapShape,
				gGeoManager->GetMedium("ABS"));
		...
		CrimpPinShape = new TGeoTube("CrimpPinShape", 0.002,
				(0.1 + (.040 + .025) / 2) / 2, crimpPinLength / 2);
		CrimpPin = new TGeoVolume("w1", CrimpPinShape,
				gGeoManager->GetMedium("copper"));
		CrimpPin->SetLineColor(25);
		TGeoTranslation *t3 = new TGeoTranslation("t3", 0, 0,
				0.7 - crimpPinLength / 2);
		t3->RegisterYourself();

		// Creat simple ring (Instead of Spring)
		TGeoTube *SmallRingShape;
		SmallRingShape = new TGeoTube("SmallRingShape", 0.2, 0.23, .01748);
		SpringSmallRing = new TGeoVolume("SRS", SmallRingShape,
				gGeoManager->GetMedium("CuBe2"));
		SpringSmallRing->SetLineColor(5);

		TGeoTube *BigRingShape;
		BigRingShape = new TGeoTube("BigRingShape", 0.995 / 2 - .03, 0.995 / 2,
				.01748);
		SpringBigRing = new TGeoVolume("BRS", BigRingShape,
				gGeoManager->GetMedium("CuBe2"));
		SpringBigRing->SetLineColor(5);

		// Create RetainingRing
		RetainingRing = gGeoManager->MakeTubs("RRS",
				gGeoManager->GetMedium("PVC"), 0.115, 0.25, 0.0525, 125, 55);
		...
		TGeoTranslation *t5 = new TGeoTranslation(tempt5.c_str(), 0, 0,
				tubeLength / 2 + 0.35);
		t5->RegisterYourself();

		std::string tempt6 = tubeType + "t6";
		TGeoTranslation *t6 = new TGeoTranslation(tempt6.c_str(), 0, 0,
				-tubeLength / 2 - 0.35);
		t6->RegisterYourself();
		...
		AlLayerShape1 = new TGeoTube(tempal1.c_str(), 0.4975000, 0.4975030,
				tubeLength / 2);
		AlLayer1 = new TGeoVolume("al1", AlLayerShape1,
				gGeoManager->GetMedium("Aluminum"));
		AlLayer1->SetLineColor(10);

		TGeoTube *MylarShape;
		std::string tempml1 = tubeType + "MylarLayerShape1";
		MylarShape = new TGeoTube(tempml1.c_str(), 0.4975030, 0.5002030,
				tubeLength / 2);
		MylarLayer = new TGeoVolume("ml1", MylarShape,
		...
		AlLayerShape2 = new TGeoTube(tempal2.c_str(), 0.5002030, 0.5002060,
				tubeLength / 2);
		AlLayer2 = new TGeoVolume("al2", AlLayerShape2,
				gGeoManager->GetMedium("Aluminum"));
...
//		t4->RegisterYourself();//0.7 - crimpPinLength / 2
		std::string tempt4 = tubeType + "t4";
		TGeoTranslation *t4 = new TGeoTranslation(tempt4.c_str(), 0, 0,
				0.7 - crimpPinLength / 2 - tubeLength/2);
		t4->RegisterYourself();		//



		std::string temptr1 = tubeType + "tr1";
		TGeoCombiTrans *tr1 = new TGeoCombiTrans(temptr1.c_str(), 0, 0,
				tubeLength/2 + .35, new TGeoRotation("d", 0, 180, 180));
		tr1->RegisterYourself();

		std::string tempt7 = tubeType + "t7";
		TGeoTranslation *t7 = new TGeoTranslation(tempt7.c_str(), 0, 0,
				-.7 +tubeLength/2 + crimpPinLength / 2);
		t7->RegisterYourself();

		TGeoTube *NobleGasTubeShape;
		std::string tempgs = tubeType + "GasShape";
		NobleGasTubeShape = new TGeoTube(tempgs.c_str(), 0, 0.4970 - fac,
				(tubeLength - 0.4) / 2.0 - fac);
		//std::clockt cstart4 = std::clock();
		std::string tempStr = tempgs
				+ " -( EndCapShape:"+ tempt6 + "+ EndCapShape:" + temptr1
				+ "+ CrimpPinShape:" + tempt4 + " + CrimpPinShape:" + tempt7
				+ "+ WireShape)";
		TGeoCompositeShape *NobleGasShape = new TGeoCompositeShape("r",
				tempStr.c_str());
		NobleGas = new TGeoVolume("ArCO2Sensitive", NobleGasShape,
				gGeoManager->GetMedium("ArCO2"));
		...
		testVol = new TGeoVolume("e1", testShape,
				gGeoManager->GetMedium("ABS"));
...
//        cap->AddNode(testVol, 1);
		...
		cap->AddNode(SpringBigRing, 5,
				new TGeoTranslation("t", 0, 0, 0.5 - .02));
		cap->AddNode(RetainingRing, 6,
				new TGeoTranslation("t", 0, 0, 0.045 + 0.0525));
	...
	for (Int_t sector = 0; sector < 3; sector++) {
		...
		for (Int_t mPart = 0; mPart < 8; mPart++) {
			...
			for (Int_t i = parity; i < nRow + parity; i++) {
				...
				std::string tubeLayerName = "strawLayer-"
						+ std::to_string(layer);
				...
				if (i % 2 == 0) {
					...
					if (sIndex[i][0] == 0 || sIndex[i][1] == 0) {
						...
						tubeLayer->AddNode(tubeArr[0],colindex ,
								new TGeoCombiTrans("tr",
										startingPointRow + i * 0.878, 0, 0,
										new TGeoRotation("r", 0, deg * tDeg,
												0)));
						Node++;
					}
				}
				...
				for (Int_t l = 0; l <= 1; l++) {
					for (Int_t j = sIndex[i][l]; j < eIndex[i][l]; j++) {
						...
						tubeLayer->AddNode(tubeArr[0], colindex,
								new TGeoCombiTrans("tr",
										startingPointRow + i * 0.878,
										(startingPointCol + j * tubeRadius * 2)
												* sign, 0,
										new TGeoRotation("d", 0, deg * tDeg,
												0)));
						Node++;
					}
					sign = -1;
...
//						}
						TGeoCombiTrans *trad1 = new TGeoCombiTrans("tr",
								startingPointRow + i * 0.878,
								-tDeg
										* ((startingPointCol
												+ (k + eIndex[i][(1 + tDeg) / 2]
														- 2) * tubeRadius * 2)- (tubeLengthArr[1]
																								- tubeLengthArr[k])/2
																								* sin(
																										2.9 * PI
																												/ 180.0)),
														-(tubeLengthArr[1] - tubeLengthArr[k] )
																										* cos(2.9 * PI / 180.0)/2, new TGeoRotation("r", 0, deg * tDeg, 0));
						trad1->RegisterYourself();

						TGeoCombiTrans *trad2 =
								new TGeoCombiTrans("tr",
										startingPointRow + i * 0.878,
										tDeg
												* ((startingPointCol
	...
	halfSide->AddNode(sectorArr[0], 0,
			new TGeoCombiTrans(0, 0, 4 + tubeLengthArr[0] / 2,
					new TGeoRotation("d", 0, 0, -60)));
	halfSide->AddNode(sectorArr[1], 0,
			new TGeoTranslation(0, 0, 4.0 + tubeLengthArr[0] / 2));
	halfSide->AddNode(sectorArr[2], 0,
			new TGeoCombiTrans(0, 0, 4 + tubeLengthArr[0] / 2,
					new TGeoRotation("d", 0, 0, 60)));

	halfSide->AddNode(constructMechanicalFrame(), 1,
			new TGeoTranslation(0, 0, 0));
	halfSide->AddNode(constructGasManifold(), 2,
			new TGeoTranslation(0, 0, -.5));

	sttVol->AddNode(halfSide, 0, new TGeoTranslation("t1", 0, 0, -34.7));
	sttVol->AddNode(halfSide, 1,
			new TGeoCombiTrans( 0, 0, -34.7,
					new TGeoRotation("d", 0, 0, 180)));
	...
	std::cout << gGeoManager->GetVisOption() << std::endl;
// top->CheckOverlaps(0.00001);
