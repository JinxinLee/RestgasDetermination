...
//    std::cout << "GetSector StringVector.size() " << stringVector.size() << std::endl;
    auto i = std::find_if(stringVector.begin(), stringVector.end(),[&](const std::string& val){auto found = val.find("sector"); return found != std::string::npos;});
...
int PndSttTubeIdMapCreatorRoot::GetRow(TString& name){
    ...
    PndStringSeparator sep(nameString, "/-_");
	std::vector<std::string> stringVector = sep.GetStringVector();
    auto i = std::find_if(stringVector.begin(), stringVector.end(),[&](const std::string& val){auto found = val.find("strawLayer"); return found != std::string::npos;});
	int pos = std::distance (stringVector.begin(), i);
