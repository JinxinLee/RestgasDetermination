...
PndRichCalDbData::PndRichCalDbData(const char* mapName)
  :TNamed(mapName, "PND Rich Cal db"),
  fPmin(0), 
  fPmax(0),
  fXmin(0), 
  fXmax(0),
...
PndRichCalDbData::PndRichCalDbData(const char* name,
  ...
  fNt( cal.GetNt()),
  fNf( cal.GetNf()),
  fBetaMean( new TArrayF(*(cal.GetBetaMean()))),
  fBetaSig( new TArrayF(*(cal.GetBetaSig()))),
  fBetaEff( new TArrayF(*(cal.GetBetaEff())))
{
}
// ------------------------------------------------------------------------
...
PndRichCalDbData::PndRichCalDbData(const  PndRichCalDbData& L) 
  ...
  fNt( L.GetNt()),
  fNf( L.GetNf()),
  fBetaMean( new TArrayF(*(L.GetBetaMean()))),
  fBetaSig( new TArrayF(*(L.GetBetaSig()))),
  fBetaEff( new TArrayF(*(L.GetBetaEff())))
