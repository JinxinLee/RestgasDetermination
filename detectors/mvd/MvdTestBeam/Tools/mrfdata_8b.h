...
#ifndef __MRFDATA_8b_H__
#define __MRFDATA_8b_H__
...
class TMrfData_8b
{
	public:
		...
		//TMrfData_8b& operator=(const TMrfData_8b& rhs);
		...
		*/
		...
		<b>Error codes:</b>
			- \b 0: Everything OK.
			- \b mrfdata_8b_error::out_of_bounds: The data word had to be truncated to fit into the data stream.
		*/
		...
		*/
		...
		//void copyFragment(const UInt_t sourcestart, const UInt_t destposition, const UInt_t size, const mrf::registertype value);
		...
		*/
		...
		//void setItemValue(const std::string& item, const std::map<const std::string, TConfItem>& bitmap, const mrf::registertype& value, const UInt_t& offset = 0);

		//! Retrieves a bitfield from the data storage based on the configuration stored in the map parameter bitmap.
		//UInt_t getItemValue(const std::string& item, const std::map<const std::string, TConfItem>& bitmap, const UInt_t& offset = 0) const;
		...
		*/
		...
		\param offset Optional offset (unit is bits) to change the import start position in the binary data stream.
		*/
		//void importHexString(const std::string data, const UInt_t offset = 0);
...
//	protected:
		...
		*/
		...
		//void copyBlock(const UInt_t sourcestart, const UInt_t length, const UInt_t deststart, const UInt_t value);
};



#endif // __MRFDATA_8b_H__
