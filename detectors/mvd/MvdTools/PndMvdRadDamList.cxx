...
Double_t PndMvdRadDamList::Interpolate(Double_t energy, Int_t index)
{
	...
	if (index == -1){		//energy is lower than the smallest energy entry in the list
		ew1 = fList[0];
		ew2 = fList[1];
		std::cout << "-W- PndMvdRadDamList::Interpolate: Energy is lower than list values" << std::endl;
	}
	else if (index < -1){
		ew1 = fList[fList.size()-2];
		ew2 = fList[fList.size()-1];
		std::cout << "-W- PndMvdRadDamList::Interpolate: Energy is bigger than list values" << std::endl;
	}
	else{
		ew1 = fList[index];
		ew2 = fList[index+1];
