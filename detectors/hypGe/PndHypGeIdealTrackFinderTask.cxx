...
PndHypGeIdealTrackFinderTask::PndHypGeIdealTrackFinderTask() :
  FairTask("HYP Ideal Track Finding Task"), fEventNr(0)
{
...
// Get input array
  ...
  if ( !fMcArray){
    std::cout << "-W- PndHypGeIdealTrackFinderTask::Init: " << "No mcArray!" << std::endl;
    return kERROR;
  }

  fTrackArray = (TClonesArray*) ioman->GetObject(fTrackBranch);
  if ( !fTrackArray){
    std::cout << "-W- PndHypGeIdealTrackFinderTask::Init: " << "No trackArray!" << std::endl;
    return kERROR;
  }
  ...
  std::cout << "-I- PndHypGeIdealTrackFinderTask: Initialisation successfull" << std::endl;
  return kSUCCESS;
}
// -------------------------------------------------------------------------
...
void PndHypGeIdealTrackFinderTask::Exec(Option_t*) 
{
  ...
  for (std::map<int, std::vector<int> >::const_iterator it = mcHitMap.begin(); 
       it!= mcHitMap.end(); it++) {  //go through all tracks
    
    PndMCTrack* myTrack = (PndMCTrack*)(fTrackArray->At(it->first));
    if(myTrack==0)continue;
