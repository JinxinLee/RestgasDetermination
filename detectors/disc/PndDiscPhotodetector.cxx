...
void DiscDIRC_Photodetector::Init(DesignID design_id) {
    ...
    if(design_id == DESIGN_SIPM) {
        ...
        if(new_sensor_grid == 0) FairLogger::GetLogger()->Fatal(MESSAGE_ORIGIN, "Allocation of sensor failed.");

        BasicGrid * grid = NULL;

        grid = new BasicGrid( -1.5900/2.0, 1.5900/2.0, 1.0,    1  ,
                              -0.672,      0.0064,     0.0064, 105 );
        if(grid == 0) FairLogger::GetLogger()->Fatal(MESSAGE_ORIGIN, "Allocation of sensor failed.");

        new_sensor_grid->AddGrid(grid);


        grid = new BasicGrid(  0.0,        1.5900/2.0, 1.0,    1  ,
                              -0.672,      0.0064,     0.0064, 105 );
        if(grid == 0) FairLogger::GetLogger()->Fatal(MESSAGE_ORIGIN, "Allocation of sensor failed.");
