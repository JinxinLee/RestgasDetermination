...
PndFtsSignalOverlap::PndFtsSignalOverlap(Int_t , TClonesArray * OriginalHitArray) { // verbose //[R.K.03/2017] unused variable(s)
   fVerbose = 0;
   fOriginalHitArray = OriginalHitArray;
}
// -------------------------------------------------------------------------
...
Bool_t PndFtsSignalOverlap::OverlapSimultaneousSignals(TClonesArray * OverlapHitArray) {
  ...
  if(fOriginalHitArray == NULL) return kFALSE;
  if(OverlapHitArray == NULL) return kFALSE;
  ...
  for(size_t itube = 0; itube < firingtubes.size(); itube++) {
    ...
    if(wrote == kFALSE) cout << "error in writing hit " << hitid << endl;
  }
