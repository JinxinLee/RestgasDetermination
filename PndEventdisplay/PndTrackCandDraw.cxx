...
InitStatus PndTrackCandDraw::Init()
{
   ...
   if(fTrackCandList==0){
      cout << "PndTrackCandDraw::Init()  branch " << GetName() << " Not found! Task will be deactivated "<< endl;
      SetActive(kFALSE);
    }
    if(fVerbose>2){
     cout<<  "PndTrackCandDraw::Init() get track list" <<  fTrackCandList<< endl;
   ...
   if(fVerbose>2)
     cout<<  "PndTrackCandDraw::Init() get instance of FairEventManager " << endl;
   fq=0;
}
// -------------------------------------------------------------------------
void PndTrackCandDraw::Exec(Option_t* option)
{
   TrackCand *tc;
   ...
   for (Int_t i=0;  i<fTrackCandList->GetEntriesFast(); ++i) {
      ...
      for (Int_t j = 0; j < tc->getNHits(); j++){
        ...
        if (p != 0){
          ...
          fq->QuadValue(i);
          if(fVerbose>2)
            cout<<  "PndTrackCandDraw::Exec() Add point " << hitId << ": " <<p->GetX()<< " "<<p->GetY()<< " "<< p->GetZ()<< " for Track " << i << endl;
        }
      }
    }
...
// -------------------------------------------------------------------------
void PndTrackCandDraw::SetParContainers()
{

}
// -------------------------------------------------------------------------
  /** Action after each event**/
void PndTrackCandDraw::Finish()
{
}
// -------------------------------------------------------------------------
void PndTrackCandDraw::Reset()
{
    if(fq!=0){
//	   fq->Reset();
