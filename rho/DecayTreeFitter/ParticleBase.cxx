...
{
  if(vtxverbose>=3){std::cout<<"ParticleBase::updateIndex() start"<<std::endl;}
  // first the daughters
  for(const_iterator it = begin(); it!= end(); ++it) (*it)->updateIndex(offset) ;
  // now the real work
  m_index = offset ;
  offset += dim() ;
  if(vtxverbose>=3){std::cout<<"ParticleBase::updateIndex() end"<<std::endl;}
}
                            ...
                            const Configuration& config)
{
  ...
  if(vtxverbose>=2)
    std::cout << "DecayTreeFitter::ParticleBase::createParticle: " << config.forceFitAll() << std::endl ;
  ParticleBase* rc=0 ;
  ...
  bool validfit = !config.forceFitAll() && particle->IsLocked();//TODO needed? && particle.endVertex() != 0 ;
  ...
  if(!mother) { // 'head of tree' particles
    ...
    if( validfit /*|| isconversion*/ ) {  // fitted composites
      ...
      if( isresonance ) { 
        if(vtxverbose>=2) std::cout<<" R ";
        rc = new Resonance(particle,mother,config) ;
      } else { 
        if(vtxverbose>=2) std::cout<<" I ";
                                          ...
                                          Projection& p) const
{
  ...
  //       double px0 = fitparams->par()(momindex+0) ;
  //       double py0 = fitparams->par()(momindex+1) ;
  ...
  //  p.r(0) += -tau*px0 + (py-py0)/lambda ;
  //  p.r(1) += -tau*py0 - (px-px0)/lambda ;

  //  p.H(0,lenindex+0) += -px0 + px ;
  //  p.H(0,momindex+0) += -tau + sinlt/lambda ;
  //  p.H(0,momindex+1) +=        (coslt-1)/lambda ;
  //  p.H(1,lenindex+0) += -py0 + py ;
