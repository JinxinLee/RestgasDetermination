...
//
// Description:
//      Implementation of class PndFwdKalmanTask
//      see PndFwdKalmanTask.hh for details
//
// Environment:
...
PndFwdKalmanTask::PndFwdKalmanTask()
...
PndFwdKalmanTask::Init()
{
  ...
  if(ioman==0)
    {
      Error("PndFwdKalmanTask::Init","RootManager not instantiated!");
      return kERROR;
    }
  ...
  if(fTrackArray==0)
    {
      Error("PndFwdKalmanTask::Init","track-array not found!");
      return kERROR;
    }
...
void
PndFwdKalmanTask::Exec(Option_t* opt)
{
  std::cout<<"PndFwdKalmanTask::Exec"<<std::endl;
  // Reset output Array
  ...
  // Detailed output
  if(fVerbose>1)std::cout<<" -I- PndFwdKalmanTask: contains "<<ntracks<<" Tracks."<<std::endl;
  if(fVerbose>2){
...
void PndFwdKalmanTask::Finish(){
  WriteHistograms();
}


ClassImp(PndFwdKalmanTask);
