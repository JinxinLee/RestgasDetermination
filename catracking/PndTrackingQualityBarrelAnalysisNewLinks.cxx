...
PndTrackingQualityBarrelAnalysisNewLinks::PndTrackingQualityBarrelAnalysisNewLinks (TString trackBranchName, TString idealTrackName, Bool_t pndTrackData):
  fVerbose(1), ioman(NULL), fNGhosts(0),
  fTrackBranchName(trackBranchName), fIdealTrackName(idealTrackName), fPndTrackOrTrackCand(pndTrackData), fPossibleTrack(0),
  fUseCorrectedSkewedHits(kFALSE),
  fBranchNames(),fTrackIdMCId(),fMCIdIdealTrackId(),fMCTrackFound(),fMapTrackMCStatus(),fMapTrackQualification(),
  fMapEfficiencies(),fMapPResolution(),fMapP(),fMapPtResolution(),fMapPt(),fMapPResolutionRel(),fMapPtResolutionRel(),
  fTrack(NULL),fMCTrack(NULL),fIdealTrack(NULL),fIdealTrackCand(NULL)
{
	if(fPossibleTrack == 0){
		std::cout << "-I- PndTrackingQualityBarrelAnalysisNewLinks::PndTrackingQualityBarrelAnalysisNewLinks no PossibleTrackFunctor given. Taking Standard!" << std::endl;
...
PndTrackingQualityBarrelAnalysisNewLinks::PndTrackingQualityBarrelAnalysisNewLinks (TString trackBranchName, TString idealTrackName, PossibleTrackFunctor* posTrack, Bool_t pndTrackData):	
  fVerbose(1), ioman(NULL), fNGhosts(0),
  fTrackBranchName(trackBranchName), fIdealTrackName(idealTrackName), fPndTrackOrTrackCand(pndTrackData), fPossibleTrack(posTrack),
  fUseCorrectedSkewedHits(kFALSE),
  fBranchNames(),fTrackIdMCId(),fMCIdIdealTrackId(),fMCTrackFound(),fMapTrackMCStatus(),fMapTrackQualification(),
  fMapEfficiencies(),fMapPResolution(),fMapP(),fMapPtResolution(),fMapPt(),fMapPResolutionRel(),fMapPtResolutionRel(),
  fTrack(NULL),fMCTrack(NULL),fIdealTrack(NULL),fIdealTrackCand(NULL)
...
//			}
		...
		//if (nHits > 2) atLeastThreeHits = kTRUE;
		 if (nHits >= 5 && (nMVDPixel + nMVDStrip >=0 )) atLeastThreeHits = kTRUE; //SG!!!
...
Int_t PndTrackingQualityBarrelAnalysisNewLinks::NBarrelMVD( FairMultiLinkedData &links )
{
  ...
  for( Int_t ihit=0; ihit<links.GetNLinks(); ihit++ ){
    if( IsBarrelMVD( links, ihit ) ) nBarrel++;
  }
  return nBarrel;
}
