...
class PndCATrack {
 public:
  ...
  const PndCATrackParam Fit(const PndCAHits& hits, const PndCATarget& target, const PndCAParam& caParam, bool dir = true ); // dir = true - get outer parameters, dir = false - get inner ones
...
inline void PndCATracks::SelectAndSaveTracks(PndCATracks& tracks)
{
  ...
  for (vector<PndCATrack*>::iterator   trIt = vptrackcandidate.begin();
