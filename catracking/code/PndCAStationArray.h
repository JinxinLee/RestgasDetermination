...
class PndCAElementsOnStation: public vector<T> {
 public:
  PndCAElementsOnStation():fHitsRef(0),fISta(-1){}
  ...
  const PndCAHit& GetHit( int iV, int IH, int i ) const {
    return (*fHitsRef)[ (*this)[i].IHit(IH)[iV] ];
  }

  const PndCAHit& GetHit( int IH, int i ) const {
    return (*fHitsRef)[ (*this)[i].IHit(IH) ];
  }

  const PndCAHits* HitsRef() const { return fHitsRef; }
  
  PndCAElementsOnStation<T>& operator=( const PndCAElementsOnStation<T>& a ) { // do not copy fHitsRef
    vector<T>::operator=( a );
...
class PndCAStationArray {
 public:
  ...
  const PndCAHits* HitsRef() const {
    if ( fElement.size() ) return fElement[0].fHitsRef;
    else return 0;
  }
