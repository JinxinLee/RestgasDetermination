 ...
 *
 * PndCATrackParam class describes the track parametrisation
...
//     friend std::ostream &operator<<( std::ostream &, const PndCATrackParam & );
    ...
    void InitDirection( float r0, float r1, float r2 ) // initialize direction parameters according to a given tangent vector
  {
    ...
    bool  TransportToX( float x, PndCATrackLinearisation &t0,
    ...
    bool  TransportToXWithMaterial( float x,  PndCATrackLinearisation &t0,
        PndCATrackFitParam &par, float Bz, float maxSinPhi = .999 );
  ...
  void Reset() { fX = 0; fSignCosPhi = 0; for(int i=0; i<5; i++) fP[i] = 0; for(int i=0; i<15; i++) fC[i] = 0; fChi2 = 0; fNDF = 0; fISec=0;}
