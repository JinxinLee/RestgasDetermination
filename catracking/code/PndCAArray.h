 ...
 *
 * This file contains the classes PndCAResizableArray and PndCAFixedArray with PndCAArray as base
...
enum {
  PndCAFullyCacheLineAligned = -1
};

namespace PndCAArrayInternal
{
...
template<typename T, int Dim> class PndCAArray;

namespace PndCAInternal
{
  ...
  template<typename T> struct TypeForAlignmentHelper<T, PndCAFullyCacheLineAligned> { typedef CacheLineSizeHelper<T> Type; };
  ...
  // dtor since polymorphism is not used (PndCAResizableArray and PndCAFixedArray are allocated on
...
#define BOUNDS_CHECK(x, y) if (PndCAInternal::ArrayBoundsCheck::IsInBounds(x)) {} else return y
#endif
  ...
  template<typename T> class Allocator<T, PndCAFullyCacheLineAligned>
  {
    public:
...
//       static inline void Free( const T *const p, int ) { delete[] p; }
  ...
  template<typename T, unsigned int Size, int _alignment> class AlignedData
  {
    ...
    private:
      enum {
        Alignment = _alignment == PndCAFullyCacheLineAligned ? 128 : _alignment,
...
class PndCAArray : public PndCAInternal::ArrayBase<T, Dim>
{
  public:
     ...
     */
    inline PndCAArray operator+( int x ) const;
    /**
     * moves the array base pointer so that the data that was once at index 0 will then be at index x
     */
    inline PndCAArray operator-( int x ) const;

    template<typename Other> inline PndCAArray<Other, Dim> ReinterpretCast() const {
...
class PndCAResizableArray : public PndCAArray<typename PndCAInternal::TypeForAlignmentHelper<T, alignment>::Type, Dim>
{
  public:
    typedef typename PndCAInternal::TypeForAlignmentHelper<T, alignment>::Type T2;
    typedef PndCAInternal::ArrayBase<T2, Dim> Parent;
    /**
     * does not allocate any memory
     */
    inline PndCAResizableArray();
    /**
     * use for 1-dim arrays: allocates x * sizeof(T) bytes for the array
     */
    inline PndCAResizableArray( int x );
    /**
     * use for 2-dim arrays: allocates x * y * sizeof(T) bytes for the array
     */
    inline PndCAResizableArray( int x, int y );
    /**
     * use for 3-dim arrays: allocates x * y * z * sizeof(T) bytes for the array
     */
    inline PndCAResizableArray( int x, int y, int z );

    /**
     * frees the data
     */
    inline ~PndCAResizableArray() { PndCAInternal::Allocator<T, alignment>::Free( Parent::fData, Parent::fSize ); }
...
template<unsigned int x, unsigned int y = 0, unsigned int z = 0> class PndCAArraySize
{
  public:
    enum {
...
class PndCAFixedArray : public PndCAArray<typename PndCAInternal::TypeForAlignmentHelper<T, alignment>::Type, Size::Dim>
{
  public:
    typedef typename PndCAInternal::TypeForAlignmentHelper<T, alignment>::Type T2;
    typedef PndCAInternal::ArrayBase<T2, Size::Dim> Parent;
    inline PndCAFixedArray() {
...
namespace PndCAInternal
{
  ...
  template<typename T>
  inline PndCAArray<T, 1> ArrayBase<T, 2>::operator[]( int x )
  {
    x *= fStride;
    typedef PndCAArray<T, 1> AT1;
    BOUNDS_CHECK( x, AT1() );
    PndCAArray<T, 1> a;
    a.fData = &fData[x];
  ...
  template<typename T>
  inline const PndCAArray<T, 1> ArrayBase<T, 2>::operator[]( int x ) const
  {
    x *= fStride;
    typedef PndCAArray<T, 1> AT1;
    BOUNDS_CHECK( x, AT1() );
  ...
  template<typename T>
  inline typename PndCAInternal::ArrayBase<T, 3>::R &ArrayBase<T, 3>::operator()( int x, int y, int z )
  {
    BOUNDS_CHECK( x * fStrideX + y + fStrideY + z, fData[0] );
    return fData[x * fStrideX + y + fStrideY + z];
  }
  template<typename T>
  inline const typename PndCAInternal::ArrayBase<T, 3>::R &ArrayBase<T, 3>::operator()( int x, int y, int z ) const
  {
    BOUNDS_CHECK( x * fStrideX + y + fStrideY + z, fData[0] );
    return fData[x * fStrideX + y + fStrideY + z];
  }
  template<typename T>
  inline PndCAArray<T, 2> ArrayBase<T, 3>::operator[]( int x )
  {
    x *= fStrideX;
    typedef PndCAArray<T, 2> AT2;
    BOUNDS_CHECK( x, AT2() );
    PndCAArray<T, 2> a;
    a.fData = &fData[x];
    a.fStride = fStrideY;
  ...
  template<typename T>
  inline const PndCAArray<T, 2> ArrayBase<T, 3>::operator[]( int x ) const
  {
    x *= fStrideX;
    typedef PndCAArray<T, 2> AT2;
    BOUNDS_CHECK( x, AT2() );
...
inline PndCAArray<T, Dim> PndCAArray<T, Dim>::operator+( int x ) const
{
  PndCAArray<T, Dim> r( *this );
  r.fData += x;
  r.MoveBounds( -x );
  return r;
}
...
inline PndCAResizableArray<T, Dim, alignment>::PndCAResizableArray()
{
  Parent::fData = 0;
...
template<typename T, int Dim, int alignment>
inline PndCAResizableArray<T, Dim, alignment>::PndCAResizableArray( int x )
{
...
template<typename T, int Dim, int alignment>
inline PndCAResizableArray<T, Dim, alignment>::PndCAResizableArray( int x, int y )
{
...
template<typename T, int Dim, int alignment>
inline PndCAResizableArray<T, Dim, alignment>::PndCAResizableArray( int x, int y, int z )
{
...
// #include <iostream>
template<typename T, int Dim, int alignment>
inline void PndCAResizableArray<T, Dim, alignment>::Resize( int x )
{
...
template<typename T, int Dim, int alignment>
inline void PndCAResizableArray<T, Dim, alignment>::Resize( int x, int y )
{
...
template<typename T, int Dim, int alignment>
inline void PndCAResizableArray<T, Dim, alignment>::Resize( int x, int y, int z )
{
