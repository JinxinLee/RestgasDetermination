  ...
  for ( std::vector<RKStep>::const_iterator it = steps.begin() + materialsFXStart; it !=  steps.begin() + materialsFXStop; ++it) { // loop over steps
    ...
    if (matZ_ > 1.E-3) { // don't calculate energy loss for vacuum
      ...
      if (doNoise){
        ...
        double momA(0), gammaSquare(0), gamma(0), betaSquare(0);
        this->getMomGammaBeta(E_, momA, gammaSquare, gamma, betaSquare);
        double momSquare = momA*momA;

        if (energyLossBetheBloch_ && noiseBetheBloch_)
          this->noiseBetheBloch(*noise, momA, betaSquare, gamma, gammaSquare);
...
double MaterialEffects::dEdx(double Energy) const {
  ...
  if (energyLossBetheBloch_)
    dEdxResult += dEdxBetheBloch(betaSquare, gamma, gammaSquare);

  if (energyLossBrems_)
    dEdxResult += dEdxBrems(mom);
...
double MaterialEffects::dEdxBetheBloch(double betaSquare, double gamma, double gammaSquare) const
{
  ...
  // calc dEdx_, also needed in noiseBetheBloch!
  double dEdxResult( 0.307075 * matZ_ / matA_ * matDensity_ / betaSquare * charge_ * charge_ );
  ...
  dEdxResult *= log(argument) - betaSquare; // Bethe-Bloch [MeV/cm]
  dEdxResult *= 1.E-3;  // in GeV/cm, hence 1.e-3
  if (dEdxResult < 0.) {
    dEdxResult = 0;
  }
