...
PndGemDigiAna::PndGemDigiAna(Int_t iVerbose) 
  : FairTask("GEM Digis Ana", iVerbose) ,
    fDigiPar(NULL),
    fGemDigiArray(NULL),
    fNofEvents(0),
    fHistoList(NULL),
...
InitStatus PndGemDigiAna::Init() {
  ...
  if ( !fGemDigiArray ) {
    cout << "-E- " << GetName() << "::Init: No PndGemDigi array!" << endl;
    return kERROR;
  }
...
void PndGemDigiAna::Exec(Option_t* opt) {
  ...
  PndGemDigi* gemDigi = NULL;

  for ( Int_t idigi = 0 ; idigi < nofGemDigis ; idigi++ ) {
    gemDigi = (PndGemDigi*)fGemDigiArray->At(idigi);

    Int_t digiCode = 
      4*(gemDigi->GetStationNr()-1)+
      2*(gemDigi->GetSensorNr()-1)+
      gemDigi->GetSide();

    if ( digiCode < 0 || digiCode > 11 ) 
      cout << "oops, wrong digiCode " << digiCode
	   << " from station " << gemDigi->GetStationNr()
...
void PndGemDigiAna::CreateHistos() {
  ...
  for ( Int_t istat = 0 ; istat < nofStations ; istat++ ) {
	...
	new ((*fHistoArray)[nofHists++]) TH1F(Form("fhLDTS_s%d_s%d_s%d",istat,isens,iside),
					      Form("Last digi time stamp, station %d, sensor %d, %s side",istat,isens,(iside?"back":"front")),
  ...
  for ( Int_t istat = 0 ; istat < nofStations ; istat++ ) {
	...
	new ((*fHistoArray)[nofHists++]) TH1F(Form("fhTimeBetweenDigis_s%d_s%d_s%d",istat,isens,iside),
					      Form("Time between digis, station %d, sensor %d, %s side",istat,isens,(iside?"back":"front")),
					      100000,0,100000);
      }
    }
  }
