...
InitStatus PndTimeStructureAnaTask::Init()
{
    ...
    if (!ioman)
    {
        std::cout << "-E- PndTimeStructureAnaTaskT::Init: " << "RootManager not instantiated!" << std::endl;
        return kFATAL;
    }
    ...
    if (fData.size() == 0)
    {
        ...
        fData.push_back(DataObject("MVDSortedPixelDigis", 20));
        fData.push_back(DataObject("MVDSortedStripDigis", 20));
        fData.push_back(DataObject("STTSortedHits", 50));
        fData.push_back(DataObject("GEMSortedDigi", 20));
        fData.push_back(DataObject("SciTSortedHit", 5));
        fData.push_back(DataObject("EmcDigiSorted", 10));
        fData.push_back(DataObject("FTSSortedHit", 50));
        fData.push_back(DataObject("FtofHit", 10));
        fDataPrim.push_back(DataObject("MVDSortedPixelDigisPrim", 20));
        fDataPrim.push_back(DataObject("MVDSortedStripDigisPrim", 20));
        fDataPrim.push_back(DataObject("STTSortedHitsPrim", 50));
        fDataPrim.push_back(DataObject("GEMDigiSortedPrim", 20));
        fDataPrim.push_back(DataObject("SciTSortedHitPrim", 5));
        fDataPrim.push_back(DataObject("EmcDigiSortedPrim", 10));
        fDataPrim.push_back(DataObject("FTSSortedHitPrim", 50));
        fDataPrim.push_back(DataObject("FtofHitPrim", 10));
    }
    ...
    fHistoTimeGapsEvents = new TH1D("hTimeGapsEvent", "Time gap between two consecutive events", 1000, 0, 5000);
...
{
    std::cout << " ------------- Event " << FairRootManager::Instance()->GetEntryNr() << " at "
            << fEventHeader->GetEventTime() << " ns ----------------" << std::endl;
    FillTimeGapsEvent();
    ...
    for (size_t i = 0; i < fData.size(); i++)
    {
        if (fData[i].fBranch == nullptr)
        {
            continue;
        }
//        std::cout << "Branch entries: " << fData[i].fBranch->GetEntries() << std::endl;
...
//            std::cout << fData[i].fBranchName.Data() << " " << j << " : " << *data << std::endl;
            ...
            if (links.GetNLinks() > 0)
            {
                ...
                if (mcTrack != 0) if (mcTrack->GetMotherID() < 0)
                {
...
//                    std::cout << "Primary Particle found!" << std::endl;
            ...
            if (links.GetNLinks() > 1) std::cout << fData[i].fBranchName.Data() << " : " << links << std::endl;
//            std::cout << "CurrentEventNr: " << currentEventNr << " : OldEventNr " << fData[i].fOldEventNr << std::endl;
            ...
            {
                fData[i].fEventStartStopMap[currentEventNr] = std::pair<Double_t, Double_t>(data->GetTimeStamp(),
                        data->GetTimeStamp());
            } else
            {
...
//                    std::cout << "Event Mixture!" << std::endl;
            ...
            {
//                std::cout << "HistoFilled: " << data->GetTimeStamp() << " - " << fData[i].fOldTimeStamp
//                        << data->GetTimeStamp() - fData[i].fOldTimeStamp << std::endl;
            ...
            if (primaryParticle)
            {
                ...
                {
                    fDataPrim[i].fEventStartStopMap[currentEventNr] = std::pair<Double_t, Double_t>(
...
//                        std::cout << "Event Mixture!" << std::endl;
                ...
                {
//                    std::cout << "PrimHistoFilled: " << data->GetTimeStamp() << " - " << fDataPrim[i].fOldTimeStamp
//                            << data->GetTimeStamp() - fDataPrim[i].fOldTimeStamp << std::endl;
...
void PndTimeStructureAnaTask::FillTimeGapsEvent()
{
    fHistoTimeGapsEvents->Fill(fEventHeader->GetEventTime() - fTimeOfPreviousEvent);
    fTimeOfPreviousEvent = fEventHeader->GetEventTime();
}
...
void PndTimeStructureAnaTask::FinishTask()
{
    for (size_t i = 0; i < fData.size(); i++)
    {
        ...
        for (std::map<Int_t, std::pair<Double_t, Double_t> >::iterator iter = fData[i].fEventStartStopMap.begin();
                iter != fData[i].fEventStartStopMap.end(); iter++)
        {
        ...
        for (std::map<Int_t, std::pair<Double_t, Double_t> >::iterator iter = fDataPrim[i].fEventStartStopMap.begin();
                iter != fDataPrim[i].fEventStartStopMap.end(); iter++)
        {
