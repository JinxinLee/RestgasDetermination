...
class PndSdsCalcFePixel
{
  public :
      ...
      PndSdsCalcFePixel(Int_t ncols, Int_t nrows, Int_t maxFe){
        ...
        virtual void SetNCols(Int_t ncols){fNcols = ncols;};
        virtual void SetNRows(Int_t nrows){fNrows = nrows;};
        virtual void SetMaxFE(Int_t maxFE){fMaxFEperCol = maxFE;};
        virtual Int_t GetNCols() const { return fNcols;};
        virtual Int_t GetNRows() const { return fNrows;};
        virtual Int_t GetMaxFE() const { return fMaxFEperCol;};

        virtual void SetSensorHits(std::vector<PndSdsPixel> hits){
          fSensorHits = hits;
        }

        ///Main function to calculate front-end hits
        virtual std::vector<PndSdsPixel> CalcFEHits();

        ///Main function to calculate front-end hits
        virtual std::vector<PndSdsPixel> CalcFEHits(std::vector<PndSdsPixel> SensorHits){
          SetSensorHits(SensorHits);
          return CalcFEHits();
        };

        ///Main function to calculate sensor hits
        virtual std::vector<PndSdsPixel> CalcSensorHits(const std::vector<PndSdsPixel> FePixel);
        virtual PndSdsPixel CalcSensorHit(const PndSdsPixel FePixel) const;
        virtual void CalcSensorColRow(Int_t& col, Int_t& row, const Int_t fe) const;
