...
PndSdsStripHitProducerDif::PndSdsStripHitProducerDif(Double_t topPitch, Double_t botPitch,
...
//  ioman->Register("SSDDigiParam", "SSD", fDigiParRect, fPersistance);
  ...
  if (!fDigiParRect){
     std::cout<<"-E- PndSdsStripHitProducer: DigiPar Rect Container does not exist!"<<std::endl;
     return kERROR;
  }
  if (!fDigiParTrap){
     std::cout<<"-E- PndSdsStripHitProducer: DigiPar Trap Container does not exist!"<<std::endl;
     return kERROR;
  }
  ...
  fStripCalcTopTrap = new PndSdsCalcStripDif(fDigiParTrap, kTOP);
  fStripCalcBotTrap = new PndSdsCalcStripDif(fDigiParTrap, kBOTTOM);
