    ...
    : FairWriteoutBuffer() 
{
    ...
    if(!buffer) throw std::runtime_error(std::string("WriteoutBuffer::AddNewDataToTClonesArray: cannot allocate buffer."));
#endif
}
...
#ifndef RRTTI
    if (fVerbose > 1) std::cout << "Data Inserted: "  <<  *(PndDiscDigitizedHit*)(data) << std::endl;
...
#else
    DataMap_t::iterator it = fData_map.find(data);
    if(it != fData_map.end()) fData_map.erase(it);
#endif
}
#endif
