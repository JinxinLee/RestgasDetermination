...
//   cout << endl;
    ...
    double lensBase = 7.5; // roughly measured lens thickness: 7.5 ; old: 5 ; need better measurement
...
//         XYZPoint  pos(20,-7.5,-440); 
        ...
        beta = sqrt( 1 - pow( mass_p / (kinE + mass_p) ,2 ) ); // T = 
        cout << "beta = " << beta << endl;
        
        // hit pos. on bar (slab_width / 2, Y, -500) is now independent of the incidence angle
        double startX = 20;
        double startY = 0;
        double startZ = -500 - (20 - slab_width / 2) * tan(inci/180.*pi);
