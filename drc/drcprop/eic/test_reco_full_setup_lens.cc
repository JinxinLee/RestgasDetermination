...
void setup_geometry_lens(PndDrcOptDevManager* manager)
{
  //  double bar_height = 17;
  //double bar_width  = 34;
  // double bar_length = 800;
  double bar_height = 17.0;
  double bar_width  = 35.0;
  double bar_length = 500;
  //double bar_length = 1250;
  
  double radius_lens1  = 30;
  double thick_lens1   =   1;
  double radius_lens2 = -radius_lens1;
  double thick_lens2   = 9; 

  double dist_plane=300;
  double dist_plane1=300;
  double conical = 0; 
  ...
  PndDrcOptLens lens1(bar_width/2.,bar_height/2.,thick_lens1/2,radius_lens1,9999,
		      conical,conical);
  lens1.SetName("lens1");
  lens1.AddTransform(Transform3D(XYZVector(0,0,0
					   -thick_lens1/2)));
  ...
  PndDrcOptLens lens2(bar_width/2.,bar_height/2.,
  ...
  lens2.SetName("lens2");
  lens2.AddTransform(Transform3D(XYZVector(0,0,0
					   -thick_lens1
					   -thick_lens2/2)));
  //lens2.SetOptMaterial(PndDrcOptMatF2G12());
  //lens2.SetOptMaterial(PndDrcOptMatBK7G18());
  lens2.SetOptMaterial(PndDrcOptMatLithotecQ0());
  lens2.Surface("side1")->SetName("out");




  /*PndDrcOptBrick box(dist_plane/2,dist_plane/2,dist_plane1/2); // expansion box  300x300x300mm
  box.SetOptMaterial(PndDrcOptMatLithotecQ0());
  box.SetName("box");
  //box.AddTransform(Transform3D(XYZVector(300/2-17.5/2,300/2-17.5/2,-thick_lens1 - thick_lens2 - dist_plane/2)));
  box.AddTransform(Transform3D(XYZVector(0,dist_plane/2 - bar_height/2,-thick_lens1 - thick_lens2 - dist_plane/2)));
  //box.AddTransform(Transform3D(XYZVector(0,0,-thick_lens1 - thick_lens2 - dist_plane1/2)));
  box.Surface("side1")->SetPixel();
  box.Surface("side1")->SetName("pixel");

  box.Surface("side2")->SetName("side2");
  box.Surface("side3")->SetName("side3");
  box.Surface("side4")->SetName("side4");
  box.Surface("side5")->SetName("side5");*/



  double  bx1 = 300.0;
 
  XYZPoint b1a(-bx1/2., -bx1/2.,  bx1/2.);
  XYZPoint b1b(+bx1/2., -bx1/2.,  bx1/2.);
  XYZPoint b2a(-bx1/2., +bx1/2.,  bx1/2.);
  XYZPoint b2b(+bx1/2., +bx1/2.,  bx1/2.);
  XYZPoint b3a(-bx1/2., +bx1/2., -bx1/2.);
  XYZPoint b3b(+bx1/2., +bx1/2., -bx1/2.);
  XYZPoint b4a(-bx1/2., -bx1/2., -bx1/2.);
  XYZPoint b4b(+bx1/2., -bx1/2., -bx1/2.);
  ...
  b1.AddPoint(b3b);
  b1.AddPoint(b3a);
  b1.SetPrintColor(2);  // paint it in red
  b1.SetPixel(); 
  b1.SetName("pixel");
  ...
  b5.AddPoint(b3b);
  b5.AddPoint(b2b);
  //cout<<"coordinates of surface 4" <<endl;
  // cout<<b2a.X()<<" "<<b2a.Y()<<" " <<b2a.Z()<<endl;
  //cout<<b3a.X()<<" "<<b3a.Y()<<" " <<b3a.Z()<<endl;
  //cout<<b3b.X()<<" "<<b3b.Y()<<" " <<b3b.Z()<<endl;
  //cout<<b2b.X()<<" "<<b2b.Y()<<" " <<b2b.Z()<<endl;
  ...
  PndDrcOptVol exp_box;
  exp_box.SetOptMaterial(PndDrcOptMatLithotecQ0());
  exp_box.AddSurface(b1);
  exp_box.AddSurface(b2);
  exp_box.AddSurface(b3);
  exp_box.AddSurface(b4);
  exp_box.AddSurface(b5);
  exp_box.AddSurface(b6);
  exp_box.SetName("exp_box");
  exp_box.AddTransform(Transform3D(XYZVector(0,0,-thick_lens1 - thick_lens2  - bx1/2.)));
