...
TFile*  hfile;
fstream geo;
void job_killed_interupt(int signum){
  cout<< "JOB terminated Correctly"<<endl;
  hfile->Write();
  hfile->Close();
...
{
  signal(SIGINT,job_killed_interupt); 
  ...
  //  PndDrcOptDevManager* 
  manager = new PndDrcOptDevManager();


  //setup_geometry_lens_modf_50dgupwal_40dgscr(manager); // geometrical setup in test_reco_full_setup.cc
  setup_geometry_lens(manager);


  string sfile = "Geo_n.C";
  fstream geo;
  ...
  //  TFile*  
  //hfile = (TFile*)gROOT->FindObject("310mm_9mm_0.001_modf_3mmpix_300w_50dgupwal_40dgscr_quarzlens.root");
  hfile = (TFile*)gROOT->FindObject("300mm_box_9mm_0.001_3mmpix_quarzlens.root");
  //hfile = (TFile*)gROOT->FindObject("check0.root");
  if (hfile) hfile->Close();
  // hfile = new TFile("310mm_9mm_0.001_modf_3mmpix_300w_50dgupwal_40dgscr_quartzlens.root","RECREATE","LUT files");
  //hfile = new TFile("check0.root","RECREATE","LUT files");
  hfile =  new TFile("300mm_box_9mm_0.001_3mmpix_quarzlens.root","RECREATE","LUT files");


  TH1D*   htheta = new TH1D("htheta"   ," ",100,0,90);
  ...
  for (int ievent=0; ievent<nevents; ievent++)
    {
      if(int(ievent/10000)*10000==ievent)cout << ievent  << endl;
      list_photon.clear();
      // create a list of photons in bar
      //      XYZPoint  pos(0,0,+0.01); // 1mm below bar at the
      XYZPoint  pos(0,0,-9.99);
      
      //XYZPoint  pos(0,0,-9.99);
      // bars half length.
      costh = ran.Uniform(0.0,1.0);
      sinth = sqrt(1.0-costh*costh);

      double theta0=acos(costh)*180/pi;
      int iter=1;
      //      if (theta0>55||theta0<8){
      //	iter=0;
      //	if(theta0>65)iter=0;
      // }
	...
	phi = ran.Uniform(0,2*pi);
      ...
      double kx = sinth*cos(phi);
      double ky = sinth*sin(phi);
      double kz = -costh;
      ...
      //cout<<"initial  "<< kx<<" "<<ky<<" "<<kz<<endl;

      PndDrcPhoton ph;
      ph.SetPosition(pos);
      ph.SetDirection(dir);
      ph.SetWavelength(440);
      //      ph.SetPrintFlag(false);
      //if(ievent==3880||ievent==3473){
      //	ph.SetPrintFlag(true);}
 
      list_photon.push_back(ph);
      manager->SetPhotonList(list_photon,"lens2");
      ...
      for(iph=list_photon.begin(); iph != list_photon.end(); ++iph) 
	{
	  ...
	  int irefl = (*iph).Reflections(); 

	  //cout<<irefl<<endl;

	  if      ((*iph).Fate()==Drc::kPhotMeasured && irefl<=5)
	    {
	      icnt_measured++;
	      // recontruction
	      ...
	      //cout<<"irefl measured"<<" "<<irefl<<endl;
	      //cout<<"--------"<<endl;
	      ...
	      if (list_surf.begin() != list_surf.end())
		{
	      ...
	      htheta->Fill(theta0);
	      //      if ((vec_surf[0]=="out"||vec_surf[1]=="out"||vec_surf[2]=="out"||vec_surf[3]=="out")&&theta0>th0){th0=theta0;cout<<th0<<" "<<vec_surf[0]<<" "<<vec_surf[0]<<" "<<vec_surf[1]<<" "<<vec_surf[2]<<" "<<vec_surf[3]<<" "<<iii<<endl;}
	      // if ((vec_surf[0]=="out"||vec_surf[1]=="out"||vec_surf[2]=="out"||vec_surf[3]=="out")&&theta0<thm){thm=theta0;cout<<"m "<<thm<<" "<<vec_surf[0]<<" "<<vec_surf[0]<<" "<<vec_surf[1]<<" "<<vec_surf[2]<<" "<<vec_surf[3]<<" "<<iii<<endl;}
	      
	      
	      // cout<<vec_surf[2]<<" "<<vec_surf[3]<<" "<<vec_surf[4]<<endl;
	      double kx0,ky0,kz0;
		...
		{	
		  ...
		  kx0=  kbarx_side2 -> GetBinContent(kbarx_side2->FindBin(xx,yy));
		  ky0=  kbary_side2 -> GetBinContent(kbary_side2->FindBin(xx,yy));
		  kz0=  kbarz_side2 -> GetBinContent(kbarz_side2->FindBin(xx,yy));
		  
		  
		  
		  // if(fabs(ky0/ky-1)>0.05&&kx0!=0)//cout <<kx0<<"="<<kx<<" "<<ky0<<"="<<ky<<" "<<kz0<<"="<<kz<<endl;
		  //  kbarx_side2 -> SetBinContent(kbarx_side2->FindBin(xx,yy),kx);
		  //kbary_side2 -> SetBinContent(kbary_side2->FindBin(xx,yy),ky);
		  //kbarz_side2 -> SetBinContent(kbarz_side2->FindBin(xx,yy),kz); 
		  

		  //cout<<"This is surface 2 "<<endl;
		  //cout<<" kx, ky, kz "<<kx<<" "<<ky<<" "<<kz<<endl;
		  //cout<<vec_surf[0]<<" "<<vec_surf[1]<<" "<<vec_surf[2]<<" "<<vec_surf[3]<<" "<<vec_surf[4]<<endl; 
		  
		  //cout<<" kkx, kky,kkz "<<kkx<<" "<<kky<<" "<<kkz<<endl;
		  ...
		  //  <<(*iph).Direction().Z()<<" "
		  //<<endl;
		  // cout<<" ph id "<<cnt_ph<<" " <<ievent<<endl;
		  //cout<<"gen angles"<<costh<<" " <<phi<<endl; 
