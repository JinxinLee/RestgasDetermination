...
PndDrcOptCylLens::PndDrcOptCylLens(double dx, double dy, double dz,
			    double r1, double r6)
{
...
//---------------------------------------------------------------------
PndDrcSurfAbs* const PndDrcOptCylLens::Surface(string name)
{
  ...
  for(kSurf=SurfaceList().begin(); kSurf != SurfaceList().end(); ++kSurf)
      ...
      if ( (*kSurf)->Name() == name )
	{
	  if (!result)
	    ...
	    {
	      cerr<<" *** PndDrcOptCylLens::surface: surface name "<<name<<" exists more than one time"<<endl;
	      exit(EXIT_FAILURE);
	    }
	}
    }
...
//----------------------------------------------------------------------
void PndDrcOptCylLens::Make_surf6(PndDrcSurfPolyFlat& surf6,double r6)
{
  ...
  //std::cout<<" *** PndDrcOptCylLens::make_surf6: dist_min="<<dist_min<<endl; //###
...
//----------------------------------------------------------------------
void PndDrcOptCylLens::Make_surf1(PndDrcSurfPolyFlat& surf1,double r1)
{
  static const double kPi=3.1415926535;
  ...
  PndDrcSurfPolyFlat side31;
  side31.AddPoint(fP3);
  side31.AddPoint(fP2);
  side31.AddPoint(surf1.LimitingPoint(1));
  side31.AddPoint(surf1.LimitingPoint(2));
  side31.SetName("side31");
  AddSurface(side31);
  ...
  PndDrcSurfPolyFlat side51;
  side51.AddPoint(fP1);
  side51.AddPoint(fP4);
  side51.AddPoint(surf1.LimitingPoint(3));
  side51.AddPoint(surf1.LimitingPoint(0));
  side51.SetName("side51");
  AddSurface(side51);
