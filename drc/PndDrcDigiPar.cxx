...
PndDrcDigiPar::PndDrcDigiPar(const char* name, const char* title, const char* context)
...
Bool_t PndDrcDigiPar::getParams(FairParamList* list)
{
  ...
  if(!list->fill("parameters",&fDrcParameters)) 
    {std::cout<<"par: fPndParameters not found"<<std::endl; return kFALSE;}
  if(!list->fill("NumberOfHitPixels",&fNOfHitPixels))
    {std::cout<<"par: NumberOfHitPixels not found"<<std::endl; return kFALSE;}
  if(!list->fill("NumberOfAmbiguities",&fNOfAmbiguities))
    {std::cout<<"par: NumberOfAmbiguities not found"<<std::endl; return kFALSE;}
  if(!list->fill("NumberOfParametersPerPixel",&fNOfParametersPerPixel))
    {std::cout<<"par: NumberOfParametersPerPixel not found"<<std::endl; return kFALSE;}
