...
using std::cout;
using std::endl;
bool PndMdtHitProducer::PndMdtDigiLess::operator() 
  (const PndMdtDigi* lv, const PndMdtDigi* rv) const
...
void PndMdtHitProducer::Exec(Option_t* opt) 
{
  ...
  if(fTimeOrderedDigi){
    ...
    if(FairRunAna::Instance()->IsTimeStamp()){
      fDigiArray = FairRootManager::Instance()->GetData("TSMdtDigi"
	  , fFunctor
	  , fTimeWindow);
    }
    if(fVerbose>1){
      cout<<"------------------------PndMdtHitProducer-------------------------------------------"<<endl;
      cout<<"|Event No. #"<<fEventCounter<<", time-based simulation,"
	<<fDigiArray->GetEntriesFast()<<" digis in time window ["
	<<fEvtTime <<", "<<fTimeWindow<<"] (ns)  |"<<endl;
      if(fVerbose>=3){
  ...
  for (Int_t iBox=0; iBox<fBoxClusterArray->GetEntriesFast(); iBox++) 
  {
    ...
    for (Int_t iStrip=0; iStrip<fStripClusterArray->GetEntriesFast(); iStrip++) 
    {
      ...
      if(PndMdtID::LayerID(boxDigi->GetDetectorID()) != 
	  PndMdtID::LayerID(stripDigi->GetDetectorID()))
	continue;
      //++ nMatchStripDigi1;
      if(TMath::Abs(boxDigi->GetTimeStamp() - stripDigi->GetTimeStamp()) > 3.*14.85)  continue;
      //++ nMatchStripDigi2;
