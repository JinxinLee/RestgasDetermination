...
{
  if(!fInstance) fInstance = new PndMdtIGeometry();
  return fInstance;
}
// -----   Default constructor   -------------------------------------------
PndMdtIGeometry::PndMdtIGeometry() 
  : TNamed("PndMdtIGeometry", "parameterized simulation of MDT")
{
 fNumofMdt = 0;
 fNumofStrip = 0;
 fVerbose = 0;
...
namespace shadow {
  struct FullPathConverter
  {
    ...
    void operator()(TString aStr) { fullPath.Append(aStr); fullPath.Append("/"); }
    private:
    TString& fullPath; 
  };
}
...
void PndMdtIGeometry::InsertLayerInfo(Int_t layerID, const TVector3& pos, const TVector3& lwh)
{
  ...
  if( it == fLayerInfoMap.end())
  {
    ...
    if(pos1.X() < aLayerBound.minX) aLayerBound.minX = pos1.X();
    if(pos1.Y() < aLayerBound.minY) aLayerBound.minY = pos1.Y();
    if(pos1.Z() < aLayerBound.minZ) aLayerBound.minZ = pos1.Z();
    if(pos2.X() > aLayerBound.maxX) aLayerBound.maxX = pos2.X(); 
    if(pos2.Y() > aLayerBound.maxY) aLayerBound.maxY = pos2.Y(); 
    if(pos2.Z() > aLayerBound.maxZ) aLayerBound.maxZ = pos2.Z(); 
  }
}
Bool_t PndMdtIGeometry::GetStripInfo() 
{
  ...
  for(; it != end; ++it)
  {
    ...
    if(fVerbose>2)
      cout<<"layer ID #"<<layerID<<", mod#"<<mod<<", sec#"<<sec<<", layer#"<<layer
	<<", X#"<<theBoundary.minX<<", "<<theBoundary.maxX
	<<", Y#"<<theBoundary.minY<<", "<<theBoundary.maxY
	<<", Z#"<<theBoundary.minZ<<", "<<theBoundary.maxZ<<", "<<stripNO<<" strips."<<endl;
    fNumofStrip += stripNO;
  }
  return kTRUE;
}
Bool_t PndMdtIGeometry::GetGeometryInfoV2(const TString& fFullPath, const TString& Name)
{
  ...
  if(isSucc){
	...
	fBarrelVersion = 1;
	sscanf(fFullPath.Data(),"cave_1/MdtBarrel_0/MdtBarrelOct%i_%i/MdtBarrelOct%iLayer%i_%i/BP%is%il%ib%iw%i_%i/BA%i_%i/MDT%is%il%ib%iw%i_%i"
	    , &iSec1, &iSec, &iholder
	    , &iLayer, &iLayer1
	    , &iMod, &iholder, &iholder, &iBox, &iholder, &iholder
	    , &iholder, &iholder
	    , &iholder, &iholder, &iholder, &iholder, &iholder, &iWire);
      }else if(fFullPath.Contains("MdtEndcap")){
	fEndcapVersion = 1;
	sscanf(fFullPath.Data(),"cave_1/MdtEndcap_0/MdtEndcapLayer%i_%i/BP%is%il%ib%iw%i_%i/BA%i_%i/MDT%is%il%ib%iw%i_%i"
	    , &iLayer1, &iLayer //MdtEndcapLayer ...
	...
	sscanf(fFullPath.Data(),"cave_1/MdtMF_0/MdtMFLayer%i_%i/BP%is%il%ib%iw%i_%i/BA%i_%i/MDT%is%il%ib%iw%i_%i"
	    , &iLayer1, &iLayer
	    , &iMod, &iSec, &iholder, &iBox, &iholder, &iholder
	    , &iholder, &iholder
	    , &iholder, &iholder, &iholder, &iholder, &iholder, &iWire);
      }else if(fFullPath.Contains("MdtForward")){
	fForwardVersion = 1;
	sscanf(fFullPath.Data(),"cave_1/MdtForward_0/MdtForwardLayer%i_%i/BP%is%il%ib%iw%i_%i/BA%i_%i/MDT%is%il%ib%iw%i_%i"
	    , &iLayer1, &iLayer
	    , &iMod, &iSec, &iholder, &iBox, &iholder, &iholder
	    , &iholder, &iholder
	    , &iholder, &iholder, &iholder, &iholder, &iholder, &iWire);
    ...
    Double_t wirelen ;
    if( iMod == 1 ) wirelen = 2.*itsLWH.Z() ;
    else if(iMod == 2 && iLayer < 2) wirelen = 2.*itsLWH.X();
    else wirelen = 2.*itsLWH.Y();
...
Bool_t PndMdtIGeometry::AddTubeInfo(Int_t detID, const TVector3& center, Double_t wirelen, TGeoMatrix* matrix)
{
  if(fGeoMap.find(detID) != fGeoMap.end()){
    ...
    if(TMath::Abs(_info.Position.X()) < 1.e-8) _info.Position.SetX(0.);
    if(TMath::Abs(_info.Position.Y()) < 1.e-8) _info.Position.SetY(0.);
    if(TMath::Abs(_info.Position.Z()) < 1.e-8) _info.Position.SetZ(0.);
...
void PndMdtIGeometry::Print() const 
{
  ...
  ofs1<<"============================================================"<<endl;
  ofs1<<"Mod\t"<<"Sec\t"<<"Layer\t"<<"Box\t"<<"NO.\t"<<"X\t"<<"Y\t"<<"Z\t"<<endl;
  for(; cit != cend; ++cit){
    if(PndMdtID::isWire(cit->first))
    {
      ofs1<<PndMdtID::Module(cit->first)<<"\t"
	<<PndMdtID::Sector(cit->first)<<"\t"
	<<PndMdtID::Layer(cit->first)<<"\t"
	<<PndMdtID::Box(cit->first)<<"\t"
	<<PndMdtID::Wire(cit->first)<<"\t"
	<<cit->second.Position.X()<<"\t"
	<<cit->second.Position.Y()<<"\t"
	<<cit->second.Position.Z()<<"\t"<<endl;
    }
  }
  ofs1.close();
  ...
  ofs2<<"============================================================"<<endl;
  ofs2<<"Mod\t"<<"Sec\t"<<"Layer\t"<<"NO.\t"<<"X\t"<<"Y\t"<<"Z\t"<<endl;
  cit =  fGeoMap.begin();;
  for(; cit != cend; ++cit){
    if(PndMdtID::isStrip(cit->first))
    {
      ofs2<<PndMdtID::Module(cit->first)<<"\t"
	<<PndMdtID::Sector(cit->first)<<"\t"
	<<PndMdtID::Layer(cit->first)<<"\t"
	<<PndMdtID::Strip(cit->first)<<"\t"
	<<cit->second.Position.X()<<"\t"
	<<cit->second.Position.Y()<<"\t"
	<<cit->second.Position.Z()<<"\t"<<endl;
    }
