...
void runMC_dpm(double mom, int mode=0,int nevts=10,TString fname="")
{
  TStopwatch timer;
  timer.Start();
  gDebug=0;
  ...
  char tmp[100];
  sprintf(tmp,"dpm%d_%6.4f_%dk",mode,M,nevts/1000);
  if (fname=="") fname=TString(tmp);
  
  cout <<"\n####### Basename: "<<fname <<"\n"<<endl;
  cout <<"####### Processing "<<nevts <<" events...\n"<<endl;
...
//  primGen->AddGenerator(urqmdGen);
 ...
 // Box Generator
  ...
  PndDpmDirect *dpmGen=new PndDpmDirect(p,0);
