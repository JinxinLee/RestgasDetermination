...
void createRootGeometry_DIRC_fsEVdroplens_MCPs(Int_t fFocusingSystem = 0, Bool_t fprizm = kFALSE){ 
  ...
  Double_t barWin_hthick =  0.1;		  // [cm] thickness of the 'glas' window at the readout end of the barbox;
  ...
  Double_t gluehthick 	 =  0.01;//[cm]		  // glue layer, connects two halfs into one long radiator bar
  ...
  //parameters for MCPs:
  Double_t MCPsize = 5.9; //[cm] width=height of an MCP
  ...
  cout<<"pixel gap  = "<< PixelGap<<endl;
  Double_t PDwindowThick = 0.1; //[cm]
  Double_t PhCathodeThick = 0.1; //[cm]
  Double_t PDsensitiveThick = 0.1; //[cm]
  Double_t PDgreaseLayer = 0.1; //[cm]
  
  // parameters for the EV
  Double_t EVdrop	 =  fGeo->EVdrop();	  //1. [cm] drop of the EV - inner radius  ##############
  Double_t EVoffset	 =  fGeo->EVoffset();	  //0. [cm] offset of the EV - outer radius
  Double_t EVgreaseLayer =  0.1;		  //[cm] grease layer thickness btw the bar window and the EV 
  ...
  TString fGeoFile= Form("../../geometry/dirc_l%d_p%d_mirrorGap_MCPs1_updated.root",fFocusingSystem, fprizm);
...
// Medium: DIRCairNoSens
   ...
   // Mixture: Epotek301_2 (optical glue)
   nel     = 2;
   density = 2.200000;
   TGeoMixture* Epotek301_2 = new TGeoMixture("Epotek301_2", nel,density);
      aa = 28.090000;   z = 14.000000;   w = 0.467475;  // SI
   Epotek301_2->DefineElement(0,aa,z,w);
      aa = 15.999400;   z = 8.000000;   w = 0.532525;  // O
   Epotek301_2->DefineElement(1,aa,z,w);
   //Epotek301_2->SetIndex(0);
// Medium: Epotek
   ...
   par[6]  = 0.001000; // epsil
   par[7]  = -1.000000; // stmin
   TGeoMedium *Epotek301_2_m = new TGeoMedium("Epotek301_2", numed, Epotek301_2, par);  
...
// 1 lens
  if (fFocusingSystem == 1){  // L E N S E S  (no airgaps, thin nlak33)
    ...
    Double_t alpha = TMath::ASin(bbX/12./r);  // lside -> bbX, assuming that there are 6 bars in the bar box??
    ...
    Double_t alpha3 = TMath::ASin(bbX/12./r3);  // lside -> bbX, assuming that there are 6 bars in the bar box??
...
*/			    
 ...
 //--------------------------------------------------------------------------------------------- 
  ...
  logicbbS = new TGeoBBox("logicbbS", bbX/2., hthick+boxgap, bbox_hlen-barWin_hthick);
  ...
  abox->SetLineColor(19); // grau
  
  //create windows at the readout end of the bar boxes:
  TGeoBBox* logicBarWin = new  TGeoBBox("logicBarWin", bbX/2., hthick+boxgap, barWin_hthick);
  TGeoVolume* barwin = new TGeoVolume("DrcBarboxWindow", logicBarWin, FusedSil_m);
  ...
  TGeoBBox* logicEVgrease = new  TGeoBBox("logicEVgrease", bbX/2., hthick+boxgap, EVgreaseLayer/2.);
  TGeoVolume* evgrease = new TGeoVolume("DrcEVgrease", logicEVgrease, OpticalGrease_m);  
  ...
  // create logic bar:  
  TGeoBBox* logicBar = new TGeoBBox("logicBar", ((bbX/barnum)/2.)-bargap, hthick, bbox_hlen-fabs(len)/2.-mirr_hthick-barWin_hthick); 
  TGeoVolume* bar;
  ...
  TGeoBBox* logicBarGlue  = new TGeoBBox("logicBarGlue", bbX/barnum/2.-bargap, hthick, gluehthick); //$$$$$$$
  TGeoVolume *barglue  = new TGeoVolume("DrcBarGlue", logicBarGlue,  Epotek301_2_m);
  ...
  TGeoVolume* baseEV = new TGeoVolume("DrcEVSensor", logicEV, FusedSil_m);//Marcol82_7_m);  //####################################
...
/*  if(sob_angleB != 90.){     
  ...
  TGeoVolume *pdbase = new TGeoVolume("DrcPDbase", logicPDbase, DIRCcarbonFiber_m);
...
*/  
  ...
  TGeoVolume* pix = new TGeoVolume("DrcPDSensor",logicPixel, FusedSil_m);
  ...
  TGeoVolume *pixelholder  = new TGeoVolume("DrcPixelHolder", logicPixelHolder,  DIRCcarbonFiber_m);
 ...
 for(Int_t itr=0; itr<Npix; itr++){
  ...
  TGeoVolume *phcathode  = new TGeoVolume("DrcPhCathodeSensor", logicPhCathode,  FusedSil_m);
  ...
  TGeoVolume *window  = new TGeoVolume("DrcPDwindowSensor", logicWindow,  FusedSil_m);
  ...
  TGeoVolume *mcpgrease  = new TGeoVolume("DrcMcpGrease", logicMCPgrease,  OpticalGrease_m);
  mcpgrease->SetLineColor(kSpring);
  
  //version 1: place MCPs in global cs
  Double_t hgap = 0.5*(MCPsize - MCPactiveArea + MCPgap);
  Double_t step = MCPactiveArea + 2.*hgap;  
  Double_t xpos = 0.;
  Double_t ypos = 0.;
  Double_t xcurr = 0.;
  Double_t ycurr = 0.;
...
/*  //version 2: place MCPs in local bar coordinate systems:
  ...
  for(Int_t m = 0; m < bbnum; m ++){       
    ...
    cout<<"rad corner out = "<<radiusCornerOut<<", step = "<<step<<endl;
    for(Int_t nrow=0; nrow < Int_t((radiusCornerOut*cos(dphi/2./180.*pi)-radiusMiddleSmall)/step); nrow++){
