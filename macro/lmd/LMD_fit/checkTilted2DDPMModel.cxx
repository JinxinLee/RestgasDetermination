...
int main(int argc, char* argv[]) {
	const double plab(1.5);

	// create 2d model and initialize tilt parameters
	PndLmdModelFactory model_factory;

	LumiFit::PndLmdFitModelOptions fit_op_full;
	fit_op_full.use_theta_xy_coordinate_system = false;

	shared_ptr<Model2D> model_old = model_factory.generate2DModel(fit_op_full,
			plab);
	if (model_old->init()) {
	...
	fit_op_full.use_theta_xy_coordinate_system = true;

	shared_ptr<Model2D> model_fast = model_factory.generate2DModel(fit_op_full,
			plab);
	if (model_fast->init()) {
		std::cout << "Error: not all parameters have been set!" << std::endl;
	}
