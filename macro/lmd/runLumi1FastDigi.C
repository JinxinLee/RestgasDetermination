...
//   Double_t  lx=0.01, ly=0.01, threshold=600, noise=200;
   //PndLmdHybridHitProducer* mvdPixProd = new PndLmdHybridHitProducer(lx,ly,threshold,noise);
  PndLmdPixelHitProducerFast* ssdHitProd = new PndLmdPixelHitProducerFast(pitch,pitch);
  ssdHitProd->SetVerbose(verboseLevel);
  fRun->AddTask(ssdHitProd);

  // PndLmdNoiseProducer* lmdNoiseMaker = new PndLmdNoiseProducer();
  // lmdNoiseMaker->SetVerbose(verboseLevel);
  // fRun->AddTask(lmdNoiseMaker);
  //PndLmdHitProducerIdeal* lmdStripProd = new PndLmdHitProducerIdeal();
