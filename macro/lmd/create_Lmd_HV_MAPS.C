...
*/
	...
	// ************************** Parameters to misalign Detector *******************
	//bool misalign = false;

	double dx_sens=0, dy_sens=0, da_sens=0;//x&y coords on support, a angle to tilt sensor
	double dx_supp=0, dy_supp=0, dz_supp=0;//x&y&z coords of support-diamond
	double da_supp=0, db_supp=0, dc_supp=0;//a,b,c angles to rotate support around all axis
        TRandom* dice = new TRandom(); //dice->Gaus(0,dMIS);
	if(misalign){
          	dx_sens=dy_sens=da_sens=0.0;//x&y coords on support, a angle to tilt sensor
                dx_supp=dy_supp=dz_supp=0.0;//x&y&z coords of support-diamond
                da_supp=db_supp=dc_supp=100.1;//a,b,c angles to rotate support around all axis
        }
	...
	const double inner_rad = 3.7; //4.5;
	const double outer_rad = 11.;
	const int n_cvd_discs = 10;
	const double cvd_disc_rad = 7.95/2.;
	...
	for (int i_plane = 0; i_plane < 4; i_plane++) {
		...
		for (int i_cvd_disc = 0; i_cvd_disc < n_cvd_discs; i_cvd_disc++) {
			...
			TGeoTranslation* trt = new TGeoTranslation(
					0, 0,
					kTransZ + kPlaneZPosition[i_plane] + dice->Gaus(0,dz_supp) + offset_z - kRotUmZ);
			...
			for (int iside = 0; iside < 2; iside++){ // modules are placed on both sides
				...
				for (int idie = 0; idie < 2; idie++){
                                        int maps_col = 0;
					if(idie) maps_col++;
					for (maps_col; maps_col < maps_n_col; maps_col++){
						for (int maps_row = 0; maps_row < maps_n_row; maps_row++){
							...
							if (iside == 0){
								...
								if (idie == 0){
									_rot_z = -90. -delta_phi/2./TMath::Pi()*180. + dice->Gaus(0,da_sens);
									...
									_trans_x = _trans_x * cos(-delta_phi/2.+angle) - _trans_y * sin(-delta_phi/2.+angle) + dice->Gaus(0,dx_sens);
									_trans_y = temp_trans_x * sin(-delta_phi/2.+angle) + _trans_y * cos(-delta_phi/2.+angle) + dice->Gaus(0,dy_sens);
								} else {
									_rot_z = 90. -delta_phi/2./TMath::Pi()*180.+ dice->Gaus(0,da_sens);
									// place it left of the middle line
									_trans_x = - maps_height*3. -die_gap - maps_row * maps_height*2.;
									// rotate now to the right edge
									double temp_trans_x = _trans_x;
									_trans_x = _trans_x * cos(-delta_phi/2.+angle) - _trans_y * sin(-delta_phi/2.+angle) + dice->Gaus(0,dx_sens);
									_trans_y = temp_trans_x * sin(-delta_phi/2.+angle) + _trans_y * cos(-delta_phi/2.+angle) + dice->Gaus(0,dy_sens);
