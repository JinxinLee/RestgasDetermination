void MCandRECTrkmatches(const int nEvents=2, const int startEvent=0, TString storePath="tmpOutput", const int verboseLevel=3, double dv=0.5, bool no4d=false)
{
  ...
  TH2 *hntrkcandvsMC = new TH2F("hntrkcandvsMC","Number of track-candidates per event vs. Number of simulated tracks;
N_{MC};N_{trk-cand}",100,0,100,100,0,100);
  ...
  TH1 *hResMom = new TH1F("hResMom","P_{MC}-P_{rec};#deltaP,GeV/c",1e2,-6e-5,6e-5);
  TH1 *hErrMom = new TH1F("hErrMom","#sigma_{P};#sigmaP,GeV/c",1e3,0,1e-1);
  TH1 *hPullMom = new TH1F("hPullMom","(P_{MC}-P_{rec})/#sigma_{P};",1e3,-1e1,1e1);
  TH1 *hResTheta = new TH1F("hResTheta","#theta_{MC}-#theta_{rec};#delta#theta,rad",1e2,-6e-4,6e-4);
  TH1 *hErrTheta = new TH1F("hErrTheta","#sigma(#theta_{rec});#sigma,rad",1e3,0,0.1);
  TH1 *hPullTheta = new TH1F("hPullTheta","(#theta_{MC}-#theta_{rec})/#sigma_{#theta};",1e2,-10,10);
  TH1 *hResPhi = new TH1F("hResPhi","#phi_{MC}-#phi_{rec};#delta#phi,rad",2e2,-0.1,0.1);
  TH1 *hErrPhi = new TH1F("hErrPhi","#sigma(#phi_{rec});#sigma,rad",1e3,0,1.);
  ...
  TH1 *hResPointZ = new TH1F("hResPointZ","Z_{MC}-Z_{rec};#deltaZ,cm",1e2,-0.1,0.1);
  ...
  TH2 *hRecGEANEX = new TH2F("hRecGEANEX",";X_{rec}, cm;X_{GEANE}, cm",1e3,0,50.,1e3,-500,500.);
  TH2 *hRecGEANEY = new TH2F("hRecGEANEY",";Y_{rec}, cm;Y_{GEANE}, cm",1e3,-15,15.,1e3,-500,500.);
  TH2 *hRecGEANER = new TH2F("hRecGEANER",";R_{rec}, cm;R_{GEANE}, cm",1e3,0,50.,1e3,0,500.);
  ...
  TH2 *hSeedGEANEX = new TH2F("hSeedGEANEX",";X_{cand}, cm;X_{GEANE}, cm",1e3,0,50.,1e3,-5.,5.);
  TH2 *hSeedGEANEY = new TH2F("hSeedGEANEY",";Y_{cand}, cm;Y_{GEANE}, cm",1e3,-15,15.,1e3,-5.,5.);
  TH2 *hSeedGEANER = new TH2F("hSeedGEANER",";R_{cand}, cm;R_{GEANE}, cm",1e3,0,50.,1e3,0,5.);
  ...
  TH2 *hSeedThetaDiffIDs = new TH2F("hSeedThetaDiffIDs",";# IDs;#theta_{cand}, rad",7,0,7,1e3,0,0.15);
  TH2 *hSeedPhiDiffIDs = new TH2F("hSeedPhiDiffIDs",";# IDs;#phi_{cand}, rad",7,0,7,1e3,-1.5,1.5);
 ...
 TH2 *hchi2nTrkCand = new TH2F("hchi2nTrkCand"," ;Number of trk-cand;#chi^2",
  ...
  for (Int_t j=0; j<nEvents; j++){
    ...
    //    if(nParticles>0 && nTrkCandidates<1) cout<<"Event #"<<j<<" doesn't have Trk-Cand!"<<endl;
    else{
      // cout<<"Event #"<<j<<" has "<<nParticles<<" MC-trk(s)";
      // cout<<" And "<<nTrkCandidates<<" trk-cand(s), "
      // 	  <<nRecTrks<<" reconst. trk(s)"
      // 	  <<" and "<<nGeaneTrks<<" trk(s) from GEANE"<<endl;
    }
    //  if(nTrkCandidates<1) continue;
        if(nTrkCandidates>numTrk) cout<<"Event #"<<j<<" has "<<nTrkCandidates<<" trk-cands!"<<endl;
