...
//void TrksAlgoResults(TString storePath="/home/karavdin/datastorage/TrackPerformancePixel_June2013/10000000/mom_1_5/")
//void TrksAlgoResults(TString storePath="/home/karavdin/datastorage/TrackPerformancePixel_June2013/10000000/mom_4_06/")
void TrksAlgoResultsJune2013(TString storePath="/home/karavdin/datastorage/TrackPerformance_June2013_theta_4_8mrad_trkCand11mrad250mradCut/5000000/mom_15/")
{
  const int nsimTrk=5e6;
  const int nsimTrkF=5e6;
  const int nsimTrkCA=5e6;
  // gROOT->Macro("/panda/pandaroot/macro/lmd/Anastasia/test_macros/Style_Imported_Style.C");
  // gROOT->SetStyle("Imported_Style"); 
  const int numTrks = 8;
  double Ntrks[numTrks]={1,2,3,4,5,10,15,20};
  double eNtrks[numTrks]={0,0,0,0,0,0,0,0};
  // const int numTrks = 7;
  // double Ntrks[numTrks]={1,2,3,4,5,10,15};
  ...
  // double timeF[numTrks]={7.5,9.11,10.84,12.16,13.94};// pixel, 15 GeV
  //Cellular Automaton
  ...
  //   double timeCA[numTrks]={7.86,9.59,11.53,13.58,15.91};//pixel, 15 GeV/c
  for(int iNtrk=0;iNtrk<numTrks;iNtrk++){
    ...
    ///PIXEL -----------------------------------------------------------------
    TString fileCAname=storePath+"/Lumi_missed_ghost_";
    fileCAname+="CA_";
    TString fileFname=storePath+"/Lumi_missed_ghost_";
    fileFname+="Follow_";
    fileCAname+=Ntrks[iNtrk];
    fileFname+=Ntrks[iNtrk];
    fileCAname+="trks.root";
    fileFname+="trks.root";
    ...
    int nrecCA = hntrkgoodCA_II->GetEntries();
    ...
    int nrecF = hntrkgoodF_II->GetEntries();
    ...
    // TH1F*  hnhitsCA = (TH1F*)fileCA->Get("hnhits");
    // int nsimTrkCA = (hnhitsCA->GetEntries()); //diff sim number because for some samples reconstruction can fail
    // TH1F*  hnhitsF = (TH1F*)fileF->Get("hnhits");
    // int nsimTrkF = (hnhitsF->GetEntries()); //diff sim number because for some samples reconstruction can fail
    // ghostCA_I[iNtrk]=100*double(nghostCA_I)/(nsimTrkCA);
    // missedCA_I[iNtrk]=100*double(nmissCA_I)/(nsimTrkCA);
    // ghostF_I[iNtrk]=100*double(nghostF_I)/(nsimTrkF);
    // missedF_I[iNtrk]=100*double(nmissF_I)/(nsimTrkF);
    // ghostCA_II[iNtrk]=100*double(nghostCA_II)/(nsimTrkCA);
    // missedCA_II[iNtrk]=100*double(nmissCA_II)/(nsimTrkCA);
    // ghostF_II[iNtrk]=100*double(nghostF_II)/(nsimTrkF);
    // missedF_II[iNtrk]=100*double(nmissF_II)/(nsimTrkF);


    cout<<"nsimTrkF = "<<nsimTrkF<<" nsimTrkCA = "<<nsimTrkCA<<" (with "<<Ntrks[iNtrk]<<" trks/ev) nrecF = "<<nrecF<<" nrecCA = "<<nrecCA<<endl;
    cout<<"(hit lost): nmissF_I  = "<<nmissF_I<<" nmissCA_I = "<<nmissCA_I<<endl;
    cout<<"(only trk lost): nmissF_II = "<<nmissF_II<<" nmissCA_II = "<<nmissCA_II<<endl;
    cout<<" (mixture) nghostF_I = "<<nghostF_I<<" nghostCA_I = "<<nghostCA_I<<endl;
    cout<<" (doubled) nghostF_II = "<<nghostF_II<<" nghostCA_II = "<<nghostCA_II<<endl;
  ...
  leg->AddEntry(grMissCA_value_I,"CA (source: hits)","lep");
  leg->AddEntry(grMissF_value_I,"Follow (source: hits)","lep");
  leg->AddEntry(grMissCA_value_II,"CA (source: trk)","lep");
  leg->AddEntry(grMissF_value_II,"Follow (source: trk)","lep");
