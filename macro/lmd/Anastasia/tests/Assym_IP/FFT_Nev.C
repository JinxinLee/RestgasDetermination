void FFT_Nev(TString path = "/panda/myResults/AssymIP/DPM/mom_1_5/", TString file_name="/HitsAssym_x_y_1_5GeV.root", int x_sh = -5, int y_sh = -5,int angx_sh = -5, int angy_sh = -5){
...
//N
 ...
 //calculate frequency
  ...
  for(int i=1; i<Nmod_2;i++){
    ...
    //  cout<<"frequencyID[i] = "<<frequencyID[i]<<endl;
  }
  // frequencyID[Nmod_2] = 0;
  // for(int i=Nmod_2+1; i<Nmod;i++){
  //   int k=i-Nmod_2;
  //   frequencyID[i] = -(Nmod-1)/(Nmod*modulesID[k]);
  // }
 // //TODO: think automaticly how extend number of point here????
 //  for(int i=1; i<Nmod;i++){
 //    frequencyID[i] = (2*Nmod-1)/(2*Nmod*modulesID_doub[i]);
 //  }
 //  frequencyID[Nmod] = 0;
 //  for(int i=Nmod+1; i<2*Nmod;i++){
 //    int k=i-Nmod;
 //    frequencyID[i] = -(2*Nmod-1)/(2*Nmod*modulesID_doub[k]);
 //  }
 //  for(int i=0;i<2*Nmod;i++)
 //    cout<<"frequencyID["<<i<<"] ="<<frequencyID[i]<<endl;
...
if(y_sh<0)  this_name2 +="mn";
 ...
 this_name2 +=fabs(angx_sh);
  ...
  // for(int i=modN_ideal->GetN();i<2*modN_ideal->GetN();i++){
  //   h_Nideal->Fill(i,0);
  //   h_Nshifted->Fill(i,0);
  // }
  ...
  // int ikind = 1;
  // TVirtualFFT* fft_my = TVirtualFFT::SineCosine(1,&Nmod,&ikind,"M");
   //  TVirtualFFT::SetTransform(fft_my);
 ...
 // //That's the way to get the current transform object:
   ...
   TGraph *gr_amp = new TGraph(Nmod_2,frequencyID,amp_full);
...
//[END]Do FFT -------------------------------------------------------------------------
  ...
  TLegend *leg = new TLegend(0.71,0.75,0.99,0.99);
  leg->SetFillColor(0);
  leg->SetTextFont(42);
  leg->SetTextSize(0.04);
  leg->SetHeader("(X,Y)_{POS} [mm]; (X,Y)_{TILT} [mrad]");
 TLegend *leg2 = new TLegend(0.6,0.8,0.98,0.99);
  leg2->SetFillColor(0);
  leg2->SetTextFont(42);
 ...
 // hp->SetBinContent(1,0);
  hp->Draw();
//  gr_ph->Draw("AP");
// gr_ph->GetXaxis()->SetTitle("frequencyID");
//  gr_ph->GetYaxis()->SetTitle("Phase");
leg2->Draw();
 c1->cd(2); 
 gr_amp->Draw("AP");
 // gr_amp->GetXaxis()->SetRangeUser(0,0.5);
 gr_amp->GetXaxis()->SetTitle("frequencyID");
 gr_amp->GetYaxis()->SetTitle("Magnitude");
 leg2->Draw();
//  // hm_ph->SetBinContent(1,0,0);
//  // hm_ph->SetMarkerColor(kBlue);
//  // hm_ph->Draw();
//  // leg2->Draw();
// // gr_im_re->Draw("AP");//TEST
//  //gr_m_ph->Draw("APC");
//  // gr_m_ph->GetXaxis()->SetTitle("phase");
//  // gr_m_ph->GetYaxis()->SetTitle("ampl");
//  c1->cd(5);
//  gr_im->Draw("AP");
//  gr_im->GetXaxis()->SetTitle("frequencyID");
//  gr_im->GetYaxis()->SetTitle("Im");
//  // him->SetBinContent(1,0);
//  // him->Draw();
//  c1->cd(6);
//  gr_re->Draw("AP");
//  gr_re->GetXaxis()->SetTitle("frequencyID");
//  gr_re->GetYaxis()->SetTitle("Re");
//  // hre->SetBinContent(1,0);
//  // hre->Draw();
