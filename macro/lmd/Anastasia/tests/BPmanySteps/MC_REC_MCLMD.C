...
//    cout<<conditionLMD.Data()<<endl;
 ...
 //  c5_mc.Close();
  ...
  //Plot just reconstructed variables ---------------------------------

  TH2D *hthzPCA = new TH2D("hthzPCA",";z_{MC}, cm;#theta_{REC}, mrad",1200.,0,1200,1e2,0,50);//4.06
  tbp->Project("hthzPCA","1e3*(thetarec):zmc",condition);
 TH2D *hphzPCA = new TH2D("hphzPCA",";z_{MC}, cm; #phi_{REC}, rad",1200.,0,1200,1e2,-3.15,3.15);
  tbp->Project("hphzPCA","(phimc-phirec):zmc",condition);
 TH2D *hxzPCA = new TH2D("hxzPCA",";z_{MC}, cm;x_{REC}, cm",1200.,0,1200,4e2,-40.,40.);//1.5
  tbp->Project("hxzPCA","xrec:zmc",condition);
 TH2D *hyzPCA = new TH2D("hyzPCA",";z_{MC}, cm;y_{REC}, cm",1200.,0,1200,2e2,-20.,20.);//1.5
  tbp->Project("hyzPCA","yrec:zmc",condition);
  TH2D *hzzPCA = new TH2D("hzzPCA",";z_{MC}, cm;z_{REC}, cm",1200.,0,1200,1200,0,1200.);//1.5
  tbp->Project("hzzPCA","zrec:zmc",condition);
 TH2D *hmomPCA = new TH2D("hmomPCA",";z_{MCLMD}, cm;mom_{REC}, MeV/c",1200.,0,1200,4e2,0,5.);//1.5
  tbp->Project("hmomPCA","prec:zmc",condition);
  TCanvas c6_PCA;
  c6_PCA.Divide(2,3);
 TVirtualPad *v1 = c6_PCA.cd(1);
  v1->SetLogz(1);
  hxzPCA->Draw("colz");
  TVirtualPad *v2 = c6_PCA.cd(2);
  v2->SetLogz(1);
  hyzPCA->Draw("colz");
  TVirtualPad *v3 = c6_PCA.cd(3);
  //  v3->SetLogz(1);
  hzzPCA->Draw("colz");
  TVirtualPad *v4 = c6_PCA.cd(4);
  v4->SetLogz(1);
  hmomPCA->Draw("colz");
 TVirtualPad *v5 = c6_PCA.cd(5);
  v5->SetLogz(1);
  hthzPCA->Draw("colz");
 TVirtualPad *v6 = c6_PCA.cd(6);
 //v6->SetLogz(1);
  hphzPCA->Draw("colz");
  c6_PCA.Print(resname_pdf_c);
  c6_PCA.Write();
  c6_PCA.Close();
  hxzPCA->Write();
  hyzPCA->Write();
  ...
  hphzPCA->Write();
  hmomPCA->Write();
  //[END] Plot just reconstructed variables ---------------------------------
