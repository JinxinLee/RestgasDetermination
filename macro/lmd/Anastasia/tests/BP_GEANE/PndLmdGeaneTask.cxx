...
PndLmdGeaneTask::PndLmdGeaneTask(Double_t pBeam,TVector3 IP): FairTask("Geane Task for PANDA Lmd"), fEventNr(0), fUseMVDPoint(false)
{
  ...
  tprop->Branch("zrec_err",&fzrec_err);
   tprop->Branch("pxrec",&fpxrec);
  tprop->Branch("pyrec",&fpyrec);
  tprop->Branch("pzrec",&fpzrec);
  tprop->Branch("pxrec_err",&fpxrec_err);
  tprop->Branch("pyrec_err",&fpyrec_err);
  tprop->Branch("pzrec_err",&fpzrec_err);
  tprop->Branch("thetarec_err",&fthetarec_err);
  tprop->Branch("phirec_err",&fphirec_err);

  tprop->Branch("xmc",&fxmc);
  tprop->Branch("ymc",&fymc);
  tprop->Branch("zmc",&fzmc);
  tprop->Branch("pxmc",&fpxmc);
  tprop->Branch("pymc",&fpymc);
  tprop->Branch("pzmc",&fpzmc);

  tprop->Branch("xmcb",&fxmcb);
  tprop->Branch("ymcb",&fymcb);
  tprop->Branch("zmcb",&fzmcb);
  tprop->Branch("pxmcb",&fpxmcb);
  tprop->Branch("pymcb",&fpymcb);
  tprop->Branch("pzmcb",&fpzmcb);
  tprop->Branch("thetamcb",&fthetamcb);
  tprop->Branch("phimcb",&fphimcb);
  tprop->Branch("pmcb",&fpmcb);
 ...
 tprop->Branch("phiMCip",&fphiMCip);
 tprop->Branch("thetaMCip",&fthetaMCip);
}
...
void PndLmdGeaneTask::Exec(Option_t* opt)
{
  ...
  for (Int_t i = 0; i<glI;i++){ 
    ...
    //  cout<<"PndLmdGeaneTask::Exec for track#"<<i<<endl;
      ...
      int ierr0;
      FairTrackParH fFittedTrkH(&fFittedTrkP,ierr0);
      //      cout<<"CONVERTION Parabola->Helix ierr = "<<ierr0<<endl;
     
      TVector3 PosRecLMD(fFittedTrkH.GetX(),fFittedTrkH.GetY(),fFittedTrkH.GetZ());
      if(fFittedTrkH.GetZ()>1130) continue;// TEST: skip trks from 2nd plane. TODO: check are they fine???
      TVector3 MomRecLMD(fFittedTrkH.GetPx(),fFittedTrkH.GetPy(),fFittedTrkH.GetPz());
      MomRecLMD *=fPbeam/MomRecLMD.Mag();//external assumption about mom magnitude
      fFittedTrkH.SetPx(MomRecLMD.X()); fFittedTrkH.SetPy(MomRecLMD.Y()); fFittedTrkH.SetPz(MomRecLMD.Z());
      double covMARS[6][6];
      fFittedTrkH.GetMARSCov(covMARS);
	...
	MCPointHit = (PndSdsMCPoint*)(fMCHits->At(mcrefbot));
      ...
      PosMClmd.SetXYZ(xMClmdNew, yMClmdNew,zMClmdNew);  //shift 100 mkm to avoid adding MC errors during back propagation;
      ...
      //      fStartMCLMD = new FairTrackParH(PosMClmd, MomMClmd, PosMClmderr, MomMClmderr, fCharge);
      }
      //-------------------------------------------------------------------------------
      ...
      // // //TEST: 8 steps from LMD to IP
      // const int nstep=8;
      // double zbend[nstep]={fFittedTrkP.GetZ(), 661, 660.5, 660., 659, 319, 316, 220};//entarance and exit mag.field

      // //TEST: many-many steps -------------------------------------------------
      // const int nstep0=10;
      ...
      //      const int nstep0=9;
      const int nstep0=10;
      //      double zbend0[nstep0]={fFittedTrkH.GetZ()-0.010, 661, 660.5, 660., 659, 319, 316, 220, 0};//entarance and exit mag.field
      //      double zbend0[nstep0]={fFittedTrkH.GetZ(), 661, 660.5, 660., 659, 319, 316, 220, 0};//entarance and exit mag.field
      double zbend0[nstep0]={fFittedTrkH.GetZ(), fFittedTrkH.GetZ()-5., 661, 660.5, 660., 659, 319, 316, 220, 0};//entarance and exit mag.field
       //      TEST for backward and forward propagation: more steps!
      //      const int nintstep = 100;
      const int nintstep = 50;
      //      const int nintstep = 1;
      const int nstep=(nstep0-1)*nintstep;
      double zbend[nstep];
       for(int is=1;is<nstep0;is++){
      	 const double z0=zbend0[is-1];
      	 const double z1=zbend0[is];
       ...
       //   zbend[nstep-1] = zbend0[nstep0-1];
       //(end) many-many steps -------------------------------------------------


 vector<double> vxmc(nstep),vymc(nstep),vzmc(nstep),vpxmc(nstep),vpymc(nstep),vpzmc(nstep),vthetamc(nstep),vphimc(nstep),vpmc(nstep),vvmc(nstep),vwmc(nstep),vtvmc(nstep),vtwmc(nstep),vvmc_err(nstep),vwmc_err(nstep),vtvmc_err(nstep),vtwmc_err(nstep);
       FairTrackParP *fStartMC = new FairTrackParP();
 // FairTrackParH *fStartMC = new FairTrackParH();
       if(fVerbose>2){
 	 cout<<"------------------------------------------"<<endl;      
 	 cout<<"StartPos:"<<endl;
 	 StartPos.Print();
 	 ...
 	 if(fVerbose>5 &&  (fTracks->GetEntries()==fMCTracks->GetEntries())){ //check results for MC track propagation to LMD plane
	   ...
	   // FairTrackParH *fStartPst = new FairTrackParH(fFittedTrkH);
	   ...
	   //   fStartMC = new FairTrackParH(PosMC, MomMC, PosMCerr, MomMCerr, fCharge);
 	   for(int sj=nstep-1;sj>-1;sj--){
	     //   cout<<"MC forward: to z["<<sj<<"]="<<zbend[sj]<<endl;
 	     bool isPropMC;
	     TVector3 fmomStepMC = fStartMC->GetMomentum();//TEST: set back mom value
	     // cout<<"current MC mom: "<<endl;
	     // fmomStepMC.Print();
	     if(fabs(fmomStepMC.Mag()-fPbeam)>fMomLim){
	       fmomStepMC*=fPbeam/fmomStepMC.Mag();
	       fStartMC->SetPx(fmomStepMC.X());
	       fStartMC->SetPy(fmomStepMC.Y());
	       fStartMC->SetPz(fmomStepMC.Z());
	     }
	     FairTrackParP *fResMC = PropToPlane(fStartMC,zbend[sj],+1,isPropMC);//forward propagation
	     //   FairTrackParH *fResMC = PropToPCA(fStartMC,zbend[sj],+1,isPropMC);//forward propagation
 	     if(isPropMC){
 	       delete fStartMC;
 	       fStartMC=fResMC;
 	       vxmc[sj]= fResMC->GetX();
 	       vymc[sj]= fResMC->GetY();
 	       vzmc[sj]= fResMC->GetZ();
	       //      cout<<"Save zmc = "<<vzmc[sj]<<endl;
	       vpxmc[sj]= fResMC->GetPx();
 	       vpymc[sj]= fResMC->GetPy();
 	       vpzmc[sj]= fResMC->GetPz();
       ...
       if(abs(fStartMC->GetZ()-fFittedTrkH.GetZ())>10 && fVerbose>5){
       	 cout<<"abs(fStartMC->GetZ()-fFittedTrkH.GetZ()) = "<<abs(fStartMC->GetZ()-fFittedTrkH.GetZ())
       	     <<" fStartMC->GetZ() = "<<fStartMC->GetZ()<<" fFittedTrkH.GetZ() =  "<<fFittedTrkH.GetZ()<<endl;
       	 cout<<"MC particle wasn't propagated to LMD plane ????"<<endl;
       }
            if(abs(fStartMC->GetZ()-fFittedTrkH.GetZ())>10 && fVerbose>5) break;//MC particle wasn't propagated to LMD plane ????
	    //MC forward propagated, to check if forward and backward give similar results
	    FairTrackParP *fStartPstMCB = new FairTrackParP(*fStartMC);//MC forward propagated to  LMD, should be treated as REC
	...
	TVector3 fmomStep = fStartPst->GetMomentum();//TEST: set back mom value
	if(fabs(fmomStep.Mag()-fPbeam)>fMomLim){
	  fmomStep*=fPbeam/fmomStep.Mag();
	  fStartPst->SetPx(fmomStep.X());
	  fStartPst->SetPy(fmomStep.Y());
	  fStartPst->SetPz(fmomStep.Z());
	}
	...
	if(!isProp){
	  cout<<"js = "<<js<<" to zbend = "<<zbend[js]<<endl;
	  cout<<"smth went wrong with back-propagation for REC trk!"<<endl;
	}
 	if(isProp){
	  //  delete fStartPst;
 	  fStartPst = fResPst;
 	  if(fVerbose>5){
	    ...
	    TVector3 fmomStepMCLMD = fStartPstMCLMD->GetMomentum();//TEST: set back mom value
	    if(fabs(fmomStepMCLMD.Mag()-fPbeam)>fMomLim){
	      fmomStepMCLMD*=fPbeam/fmomStepMCLMD.Mag();
	      fStartPstMCLMD->SetPx(fmomStepMCLMD.X());
	      fStartPstMCLMD->SetPy(fmomStepMCLMD.Y());
	      fStartPstMCLMD->SetPz(fmomStepMCLMD.Z());
	    }
	    // 	    FairTrackParH*  fResPstMCLMD  = PropToPCA(fStartPstMCLMD,zbend[js],-1,isPropMClmd);//back propagation
 	    bool isPropMClmd;
 	    FairTrackParP*  fResPstMCLMD  = PropToPlane(fStartPstMCLMD,zbend[js],-1,isPropMClmd);//back propagation
 	    bool isPropMCB;
 	    FairTrackParP*  fResPstMCB  = PropToPlane(fStartPstMCB,zbend[js],-1,isPropMCB);//back propagation of forward propagated track
	   
	    if(js==0){//"back-propagation" to exacly the same point
	      isPropMCB = true;
	      isPropMClmd = true;
	      fResPstMCB = fStartPstMCB;
	      fResPstMCLMD = fStartPstMCLMD;
	    }
	    if(!isPropMCB){
	      cout<<"js = "<<js<<endl;
	      cout<<"Trk with z="<<fStartPstMCB->GetZ()<<" to zbend = "<<zbend[js]<<endl;
	      cout<<"smth went wrong with back-propagation for forward propagated trk!"<<endl;
	    }
	    if(!isPropMClmd){
	      cout<<"js = "<<js<<endl;
	      cout<<"Trk with z="<<fStartPstMCB->GetZ()<<" to zbend = "<<zbend[js]<<endl;
	      cout<<"smth went wrong with back-propagation for MCLMD trk!"<<endl;
	    }
	    if(isPropMClmd && isPropMCB){
	    // 	    if(isPropMClmd){
 	      ...
 	      TVector3 finPosMCB(fResPstMCB->GetX(),fResPstMCB->GetY(),fResPstMCB->GetZ());//back propagation of forward propagated track
 	      TVector3 finDirMCB(fResPstMCB->GetPx(),fResPstMCB->GetPy(),fResPstMCB->GetPz());
	      ...
	      // finPos.Print();
 	      ...
 	      fyrec = finPos.Y();
	      fzrec = finPos.Z();
	      //   cout<<"fzrec = "<<fzrec<<endl;
 	      fxrec_err = fResPst->GetDX();
 	      fyrec_err = fResPst->GetDY();
 	      fzrec_err = fResPst->GetDZ();
	      fpxrec =  fResPst->GetPx();
	      fpyrec =  fResPst->GetPy();
	      fpzrec =  fResPst->GetPz();
 	      fpxrec_err = fResPst->GetDPx();
 	      fpyrec_err = fResPst->GetDPy();
 	      fpzrec_err = fResPst->GetDPz();


 	      fprec = finDir.Mag();

	      int ierr;
	      FairTrackParH *fResPstH= new FairTrackParH(fResPst,ierr);
 	      fthetarec = 0.5*(TMath::Pi()) - fResPstH->GetLambda();
 	      fphirec = fResPstH->GetPhi();
 	      fthetarec_err = fResPstH->GetDLambda();
 	      fphirec_err = fResPstH->GetDPhi();
	      delete fResPstH;
 	      fxmc = vxmc[js];
 	      fymc = vymc[js];
 	      fzmc = vzmc[js];
	      ...
	      FairTrackParH *fResPstMCLMDH= new FairTrackParH(fResPstMCLMD,ierr);
 	      fthetamclmd = 0.5*(TMath::Pi()) - fResPstMCLMDH->GetLambda();
 	      fphimclmd = fResPstMCLMDH->GetPhi();
	      delete fResPstMCLMDH;

	      fxmcb = finPosMCB.X();
 	      fymcb = finPosMCB.Y();
	      fzmcb = finPosMCB.Z();
 	      fpmcb = finDirMCB.Mag();
	      FairTrackParH *fResPstMCBH= new FairTrackParH(fResPstMCB,ierr);
 	      fthetamcb = 0.5*(TMath::Pi()) - fResPstMCBH->GetLambda();
 	      fphimcb = fResPstMCBH->GetPhi();
	      delete fResPstMCBH;
	      ...
	      fphiMCip = vphimc[nstep-1];
	      fthetaMCip = vthetamc[nstep-1];

 	      tprop->Fill();
 	      delete fStartPstMCLMD;
 	      fStartPstMCLMD = fResPstMCLMD;
 	      fStartPstMCB = fResPstMCB;
     	    }
 	    else
 	      break;
 	  }
      ...
      //    FairTrackParH *fStart = new (clref1[size1]) FairTrackParH(*fStartPst);
	...
	//	FairTrackParH *fStartMClmd = new FairTrackParH(*fStartPstMCLMD);
	fPro->SetPoint(vtx);
	fPro->PropagateToPCA(1,-1);// back-propagate to point
...
FairTrackParP* PndLmdGeaneTask::PropToPlane(FairTrackParP* fStartPst, double zpos,int dir, bool& isProp){
	...
	if(zpos>1e3){//external assumption about mom magnitude [use it while in BOX and we know mom should be diff]
	  ...
	  if(fabs(MomStartPos.Mag()-fPbeam)>fMomLim){
...
FairTrackParH* PndLmdGeaneTask::PropToPCA(FairTrackParH* fStartPst, double zpos,int dir, bool& isProp){
  //  cout<<"Z@"<<zpos<<" dir propag = "<<dir<<endl;
	TVector3 stStartPos(fStartPst->GetX(),fStartPst->GetY(),fStartPst->GetZ());
	if(zpos>1e3){//external assumption about mom magnitude [use it while in BOX and we know mom should be diff]
	  TVector3 MomStartPos(fStartPst->GetPx(),fStartPst->GetPy(),fStartPst->GetPz());
	  if(fabs(MomStartPos.Mag()-fPbeam)>fMomLim){
	...
	//propagate pca
	TVector3 oc(fStartPst->GetX(),fStartPst->GetY(),zpos);
	//	if(dir<0) fPro->setBackProp();
	FairTrackParH *fResPst = new FairTrackParH();
	fPro->PropagateToPCA(1,dir);
	fPro->SetPoint(oc);
	//	TVector3 oc1(0,0,zpos-1);
	//	fPro->SetWire(oc, oc1);
	isProp = fPro->Propagate(fStartPst, fResPst,fPDGid);
	if(isProp){
	return fResPst;
	  ...
	  cout<<"Oooops! PndLmdGeaneTask::PropToPCA is making you sad!"<<endl;
	  return NULL;
	}
}
