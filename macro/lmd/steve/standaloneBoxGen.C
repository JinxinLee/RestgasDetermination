void standaloneBoxGen(double pabs, int nevents, double fThetaMin, double fThetaMax, TString output_filename, double seed = 123456.) {
  double mp = 0.938272;
  double fPhiMin = 0.0;
  ...
  for (unsigned int i = 0; i < nevents; i++) {
    ...
    TLorentzVector mom(px,
        py, pz,
        sqrt(mp * mp + pabs * pabs));
