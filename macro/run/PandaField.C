...
//   map->WriteRootFile("TransMap.root","TransMap");
     ...
     Int_t fnz=map->GetNz()+map1->GetNz()+map2->GetNz();
     Int_t fZmin=map2->GetZmin();
     Int_t fZmax=map->GetZmax();
     Int_t fdz=map->GetZstep();
     Double_t x=0;
     Double_t y=0;
     Double_t z=0;
     Double_t po[3], BB[3];
     Double_t Btot=0;
     TH1F *Bx=new TH1F("Bx(z)","Bx(z)",fnz,fZmin,fZmax);
     TH1F *By=new TH1F("By(z)","By(z)",fnz,fZmin,fZmax);
     TH1F *Bz=new TH1F("Bz(z)","Bz(z)",fnz,fZmin,fZmax);
     TH1F *Btotal=new TH1F("B(z)","B(z)",fnz,fZmin,fZmax);
   ...
   Bz->Draw();
     ...
     Int_t fXmin=map1->GetXmin();
     Int_t fXmax=map1->GetXmax();
     Int_t fdx=map1->GetXstep();
     Int_t fnx=map1->GetNx();
     y=0;
     TH2F *B=new TH2F("B mod","B mod y=0 plane ",fnz+3,fZmin,fZmax,2*fnx+3,-fXmax,fXmax);

  	for (Int_t iz=0; iz<fnz; iz++) 
	{
	  z = fZmin + Double_t(iz) * fdz;
	      po[1]=y; po[2]=z;
              for (Int_t ix=0; ix<2*fnx; ix++) {
		  x= -fXmax-1 + Double_t(ix) * fdx;
