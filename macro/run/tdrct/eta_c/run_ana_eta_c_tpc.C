...
void run_ana_eta_c_tpc(int nevts=0)
{
  ...
  while (evr.GetEvent() && i++<nevts)
	...
	for (l=0;l<p3.GetLength();++l) {
		ii=l-n_removed;
		if (p3[ii].GetMicroCandidate().GetMcIndex()>-1){
			PndMCTrack *mcTrack = (PndMCTrack*)mc_array->At(p3[ii].GetMicroCandidate().GetMcIndex());
			if (mcTrack!=0)
			{
				if (mcTrack->GetPdgCode()!=321)
				{
	...
	for (l=0;l<p4.GetLength();++l) {
		ii=l-n_removed;
		if (p4[ii].GetMicroCandidate().GetMcIndex()>-1){
			PndMCTrack *mcTrack = (PndMCTrack*)mc_array->At(p4[ii].GetMicroCandidate().GetMcIndex());
			if (mcTrack!=0)
			{
				if (mcTrack->GetPdgCode()!=-321)
				{
