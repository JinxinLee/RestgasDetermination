...
void run_ana_eta_c_tpc(int nevts=0)
{
  ...
  if (use4cfit==1)
    ...
    OutFile="etac_histo_vtx_tpc.root";
  ...
  TH1F *h_mphi_nocuts=new TH1F("h_mphi_nocuts","#phi: m(K+ K-) (no
cuts)",100,0.95,1.1);
  ...
  TH1F *h_etac_phimass=new TH1F("h_etac_phimass","m(eta_c), (cut on #phi mass);E,
GeV",100,2.5,3.5);
          ...
          if(/*(best_chi2<270)&&*/(etac.GetLength()!=0))
            {
              ...
              if
(((m_phi1>1.02-0.03)&&(m_phi1<1.02+0.03))&&((m_phi2>1.02-0.03)&&(m_phi2<1.02+0.03)))
                      {
