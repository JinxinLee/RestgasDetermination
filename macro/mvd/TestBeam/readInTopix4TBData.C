void readInTopix4TBData(TString inFileName, Double_t clockFrequency = 50)
{
  ...
  tempOutFile.Append("_MHz_ModTime.root");
...
//  readTask->AddFile(inFileName);
  ...
  readTask->AddFile(1, tempFileName);

  tempFileName = inFileName;
  tempFileName.Append("--102-data--1.txt");
  readTask->AddFile(2, tempFileName);

  tempFileName = inFileName;
  tempFileName.Append("--103-data--1.txt");
  readTask->AddFile(3, tempFileName);

  tempFileName = inFileName;
  tempFileName.Append("--104-data--1.txt");
  readTask->AddFile(4, tempFileName);

  readTask->SetFrequency(clockFrequency);

  readTask->SetVerbose(0);
  fRun->AddTask(readTask);

  PndMapSorterTask* sorterTask = new PndMapSorterTask("ToPix4Hits", "ToPix4HitsSorted","Mvd");
  sorterTask->SetTimeOffset(1000000000);
