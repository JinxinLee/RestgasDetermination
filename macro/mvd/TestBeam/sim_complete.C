...
//  fRun->AddModule(Stt);
  ...
  Mvd->SetGeometryFileName("MvdTBGeometry.root");
  fRun->AddModule(Mvd);
  //-------------------------  GEM       -----------------
...
//  fRun->AddModule(Rich);
  ...
  if(UseBoxGenerator){	// Box Generator
    ...
    boxGen->SetThetaRange(0., 2.); // Polar angle in lab system range [degree]
  ...
  PndConstField *fField= new PndConstField("ZeroField", -1000., 1000, -1000, 1000, -1000, 1000, 0, 0, 0);
