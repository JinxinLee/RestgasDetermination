...
runMvdSimLambdaDisks(Int_t nEvents=10)
{
  ...
  //FileNames
  TString simOutput="Mvd_Sim_Lambda.root";
  ...
  FairDetector *LambdaDisks = new PndMvdDetector("MVD", kTRUE);
  LambdaDisks->SetGeometryFileName("LambdaDisksNew.root");
  LambdaDisks->SetVerboseLevel(verboseLevel);
