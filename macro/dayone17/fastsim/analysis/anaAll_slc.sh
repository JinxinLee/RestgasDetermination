...
root -l -b -q 'analyse_J_slc.C("ntp1_slc_Jee_B.root","xd0d0pide>0.75&&xd0d1pide>0.75&&chi24c<50",120,"Setup B, ppb#rightarrowJ/#psi(e^{+}e^{-})#pi^{+}#pi^{-}")'

#root -l -b -q 'analyse_J_slc.C("ntp1_slc_Jmm_A.root","abs(f4cxd0m-3.097)<0.1&&esfw1<-0.05&&chi24c<50&&xd0d0muoiron>25&&xd0d1muoiron>25",200,"Setup A, ppb#rightarrowJ/#psi(#mu^{+}#mu^{-})#pi^{+}#pi^{-}")'
...
root -l -b -q 'analyse_J_slc.C("ntp1_slc_Jmm_A.root","chi24c<50&&abs(f4cxd0m-3.097)<0.1&&xd0d0pidmu>0.8&&xd0d1pidmu>0.8&&esfw1<-0.02",200,"Setup A, ppb#rightarrowJ/#psi(#mu^{+}#mu^{-})#pi^{+}#pi^{-}")'
root -l -b -q 'analyse_J_slc.C("ntp1_slc_Jmm_B.root","chi24c<50&&abs(f4cxd0m-3.097)<0.1&&esfw1<-0.1&&xd0oang<2.5&&xd0d0pidpi<0.5&&xd0d1pidpi<0.5&&xd0d0thtcm>1",220,"Setup B, ppb#rightarrowJ/#psi(#mu^{+}#mu^{-})#pi^{+}#pi^{-}")'
