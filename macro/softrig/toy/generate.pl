...
if ( @ARGV!=2 ) 
{
    ...
    print "          mode='all' generates all (caution! takes long)\n";
    print "          mode='core' generates 10 core modes (caution! takes also long)\n";
...
my @core24 = qw(24000 24300 24400 24900);
my @core38 = qw(38000 38100 38120 38200 38201 38221 38300 38400 38900);
my @core45 = qw(45000 45100 45120 45140 45200 45201 45221 45300 45400 45900);
my @core55 = qw(55000 55100 55120 55140 55200 55201 55221 55300 55400 55420 55900);
...
foreach (@modelist)
{
	my $en   = substr($enmode,0,2);
	my $pmom = $moms{$en};
	my $mode = substr($enmode,2,3);
	my $decfile = `ls ../decfiles/M$mode*dec 2> /dev/null`;
	chomp($decfile);

	if ($nevts<=0) {print "invalid number of events: $nevts\n";exit(0);}
	if ($decfile eq "" && $mode!="900") {print "no decay file found for mode $mode\n";exit(0);}
	...
	if ($mode!="900")
	{
		my $outfile = "EVT_".$enmode."_".$nevtsk."k.root";
		system("./simpleEvtGenRO $res $decfile $nevts $pmom; mv evtOutput.root data/$outfile");
	}
	else
	{
		my $outfile = "EVT_".$enmode."_".$nevtsk."k.root";
