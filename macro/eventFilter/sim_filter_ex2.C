...
sim_filter_ex2(Int_t nEvents = 100, TString  SimEngine ="TGeant3", Float_t mom = 6.231552)
{
	//-----User Settings:-----------------------------------------------
...
//	fRun->AddModule(Rich);
	...
	// now add the event filter(s)

	// set the maximal number of attempts to find an event with matching properties
	primGen->SetFilterMaxTries(1000000); // for testing small number, for real production set usually to 9999999 or something very big
	primGen->SetVerbose(); // highest commenting level of the FairPrimaryGenerator

	// filter on events with pi+ pi- only
	FairEvtFilterOnSingleParticleCounts* pionFilter = new FairEvtFilterOnSingleParticleCounts("pionFilter");
//	pionFilter->SetVerbose();// highest commenting level of the FairEvtFilterOnCounts
	pionFilter->AndMinMaxPdgCodes(1,1,211);// min. 1 and max. 1 pi+
	pionFilter->AndMinMaxPdgCodes(1,1,-211);// min. 1 and max. 1 pi-
	pionFilter->AndMinMaxAllParticles(2,2);// min. 2 and max. 2 particles in total
