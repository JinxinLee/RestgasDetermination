void tracks_BARREL_1000(char* type="proof", Int_t nofFiles = 1, TString proofName="") {
  ...
  // Parameter file
  TString parFile  = Form("%s/params_%dPart_n1000.root",workDir.Data(),nparts);
  ...
  fRun->SetInputFile(Form("file://%s/points_%dPart_n1000_f%d.root",workDir.Data(),nparts,0));
  for ( Int_t ifile = 1 ; ifile < nofFiles ; ifile++ )
    fRun->AddFile   (Form("file://%s/points_%dPart_n1000_f%d.root",workDir.Data(),nparts,ifile));
