...
)
{
    ...
    TLegend* leg1 = new TLegend(0.55, 0.7, 0.9, 0.9);
    leg1->AddEntry(hData, Form("Data (Bias-Corr) (N=%.0f)", hData->Integral()), "lp");
    ...
    if (hDataGen) {
        ...
        TLegend* leg5 = new TLegend(0.55, 0.7, 0.9, 0.9);
        leg5->AddEntry(hCorrected, Form("Corrected (N=%.1f)", hCorrected->Integral()), "lp");
        leg5->AddEntry(hDataGen, Form("Truth (N=%.1f)", hDataGen->Integral()), "l");
        leg5->Draw();
    }
