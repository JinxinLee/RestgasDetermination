...
def main():
    ...
    for p in tasks:
        ...
        if p.back_prop_vertex == 'poca':
            ...
            poca_worker_cmd = f"--array=0-{p.njobs-1} {submit_script_path} python3 {os.path.join(script_dir, 'poca_step1_worker.py')} {common_args}"
            poca_worker_job_id = submit_sbatch(poca_worker_cmd)

            # --- Step 2: Merge POCA Files ---
            print("--- Submitting POCA Merge Job ---")
            poca_merge_cmd = f"{submit_script_path} python3 {os.path.join(script_dir, 'merge.py')} {common_args}"
            ...
            poca_ana_cmd = f"{submit_script_path} python3 {os.path.join(script_dir, 'poca_step2_analysis.py')} {common_args}"
            ...
            mc_worker_cmd = f"--array=0-{p.njobs_mc-1} {submit_script_path} python3 {os.path.join(script_dir, 'mc_step_worker.py')} {common_args}"
            mc_worker_job_id = submit_sbatch(mc_worker_cmd)

            # --- Step 2: Merge MC Files ---
            print("--- Submitting MC Merge Job ---")
            mc_merge_cmd = f"{submit_script_path} python3 {os.path.join(script_dir, 'merge.py')} {common_args} --file-types mc"
