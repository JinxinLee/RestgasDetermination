...
int printsttcoords() {
	...
	TString outFileName = "Sim_Dpm_500_printsttcoords.root";
	...
	/*
	for (Int_t entrynumber = 0; entrynumber < treedigi->GetEntriesFast(); entrynumber++) {
		treedigi->GetEntry(entrynumber);
		for (Int_t clonesnumber = 0; clonesnumber < sttsortedhits->GetEntriesFast(); clonesnumber++) {
			PndSttHit* myHit = (PndSttHit*) sttsortedhits->At(clonesnumber);
			TVector3 hitPos;
			myHit->Position(hitPos);
			Int_t tubeID = myHit->GetTubeID();
			PndSttTube* myTube = (PndSttTube*) tubearray->At(tubeID);
			TVector3 tubePos = myTube->GetPosition();
			cout << entrynumber << " " << clonesnumber << " " << myHit->GetTimeStamp() << " " << tubeID << " " << hitPos.X() << " " << hitPos.Y() << " - " << tubePos.X() << " " << tubePos.Y() << endl;
		}
	}
	*/
  return 0;
