...
resVs = dict([(i, ROOT.TH1D("StatsResV"+str(cuts[i]), 
              ...
              for i in range(len(cuts))])
resVza = dict([(j,dict([(i,ROOT.TH1D("StatsResV"+str(cuts[i])+str(ampcuts[j]), 
...
gresVz = dict([(j, ROOT.TGraph(len(cuts)-1)) 
...
pullsVs = dict([(i, ROOT.TH1D("StatsPullV"+str(cuts[i]), 
              ...
              for i in range(len(cuts))])
pullsVa = dict([(i, ROOT.TH1D("StatsPullV"+str(ampcuts[i]), 
                            "Cosmic Pulls V (X') amp="+str(ampcuts[i]), 100,-3,3)) 
              for i in range(len(ampcuts))])
...
chi2prob = ROOT.TH1D("chi2prob", "ChiSqu Probability Distribution", 10000,0,1)
chi2probID = ROOT.TH1D("chi2probID", "ChiSqu Probability Distribution", 10000,0,1)
  ...
  #          continue
        ...
        for tfs in e.TrackFitStat :
            ...
            for p in range(numHits) :
                ...
                for i in range(len(ampcuts)) :
                    if (amp < ampcuts[i]) and (cl2Dsize > 1) and ( cuts[5]< z < cuts[6]):
                        pullsVa[i-1].Fill(resV/sigV)
                        break
...
for i in range(6) :
    c18.cd(i+1)
    pullsVa[i].SetFillColor(ROOT.kAzure-8)
    pullsVa[i].GetXaxis().SetTitle("Pulls V")
    pullsVa[i].Draw()

    testfit = ROOT.TF1("testfitX"+str(1),"gaus",-3,3)
    pullsVa[i].Fit(testfit, "N+", "", -3,3)
...
for i in range(6) :
    for j in range(6) : 
        c19.cd((i)*6+j+1)
        resVza[j][i].SetFillColor(ROOT.kAzure-8)
        resVza[j][i].GetXaxis().SetTitle("Residiual V")
        resVza[j][i].Draw()
               
        testfit = ROOT.TF1("testfitX"+str(1),"gaus",-0.5,0.5)
        resVza[j][i].Fit(testfit, "N+", "", -0.5,0.5)
...
c20 = ROOT.TCanvas()    
c20.Divide(2,1)
mgresVz = ROOT.TMultiGraph()
mgresVa = ROOT.TMultiGraph()

for i in range(6) :
    mgresVz.Add(gresVz[i])
    mgresVa.Add(gresVa[i])
