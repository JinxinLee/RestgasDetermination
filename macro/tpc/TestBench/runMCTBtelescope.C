...
{
  //  MC for TB beam telescope
  // Sverre Doerheim 12.04.10
...
//  PndGemDetector *PndGem = new PndGemDetector("GEM",kTRUE);
//  PndGem->SetGeometryFileName("gem_4Stations.root");
//  PndGem->SetVerboseLevel(10);
//  fRun->AddModule(PndTpc);
//  fRun->AddModule(PndGem);
  PndTpcAlignmentManager *alMan = PndTpcAlignmentManager::getInstance("testAlignment.txt");
  alMan->write("testAlignment2.txt");
  PndTpcTBDetector *PndTBDet=new PndTpcTBDetector("TBdet",kTRUE );
  PndTBDet->SetGeometryFileName("tb-tel.root");
  fRun->AddModule(PndTBDet);
  PndTBDet->SetVerboseLevel(10);
  ...
  boxGen->SetThetaRange(0, 1); // Polar angle in lab system range [degree]
