...
TF2* f2 =new TF2("f2",func,&SplineTF2Interface::eval,-40,110,15.5,40,
		 (NlamdaX-4)*(NlamdaY-4),"Function", "eval");
TF2* f3 =new TF2("f3",func2,&SplineTF2Interface::eval,-40,110,15.5,40,
		 (NlamdaX-4)*(NlamdaY-4),"Function", "eval");
...
//read file
std::string filename = "DevMap_corrected_E_and_B_30-05-08.dat";
...
TFile* laserrecofile = new TFile("eumel.laser.reco.root");
...
TH1D* diff_hist = new TH1D("blub", "blob", 100, -0.1, 0.1);


for(int r=0; r<50; r++)
  for(int z=0; z<50; z++) {
  	if(z*zstep+lzmin>110 || r*rstep+lrmin> 42) continue;
    double d = f2->Eval(z*zstep+lzmin,r*rstep+lrmin)-f3->Eval(z*zstep+lzmin,r*rstep+lrmin);
...
printf("RealTime=%f seconds, CpuTime=%f seconds\n",rtime,ctime);
 ...
 TString outfile = "runLaserRecoFit_OUT.root";

TList* Hlist=gDirectory->GetList();
//Hlist->Remove(tree);

//filename.ReplaceAll(".eps",".root");
