...
void SetCuts()
{
  ...
  cout << "This is the USER SPECIFIED file!"<<endl;
  ...
  gMC->SetProcess("LOSS",5); /**energy loss*/
  ...
  gMC->SetCut("CUTELE",1.0E-3);   /** electrons (GeV)*/
  ...
  gMC->SetCut("DCUTE",1.0E-3);    /** delta-rays by electrons (GeV)*/
  gMC->SetCut("DCUTM",1.0E-3);    /** delta-rays by muons (GeV)*/

  gMC->SetCut("PPCUTM",cut1);   /** direct pair production by muons (GeV)*/
  gMC->SetCut("TOFMAX",tofmax); /**time of flight cut in seconds*/

  //  gMC->SetCut("STEMAX", 1E7);  /**max # of steps??*/ 
