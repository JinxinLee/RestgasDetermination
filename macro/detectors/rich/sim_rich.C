...
void sim_rich(Int_t nEvents = 100, TString  SimEngine ="TGeant3", Float_t mom = 6.231552)
{
  //-----User Settings:-----------------------------------------------
  ...
  fRun->SetUserConfig(vmcdir+"/macro/rich/g4Config_Cherenkov.C");
  ...
  //Rich->SetGeoVersion(nver);
  Rich->SetVerboseLevel(0);
  //Rich->UseProtection(kTRUE);
  fRun->AddModule(Rich);
...
//     primGen->AddGenerator(Ftf);								       	//
     TString ftfrootfile = "FTF.root"; // obtained in build/bin with ./FTFGen PbarP.mac; more details in pgenerators/FtfEvtGen/README
     PndFtfGenerator *Ftf = new PndFtfGenerator(ftfrootfile);
     primGen->AddGenerator(Ftf);
  }										        	//
  if(UseEvtGenDirect){
