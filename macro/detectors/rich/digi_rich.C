int digi_rich()
{
  ...
  if(timeBased) fRun->SetEventMeanTime(50);
  // -----  Parameter database   --------------------------------------------
  ...
  //richhit->SetGeoVersion(nver);
  richhit->SetPhDetNoiseON(kTRUE);
  if(timeBased) richhit->RunTimeBased();
  fRun->AddTask(richhit);
   
  PndRichHitSorterTask* richSorter = new PndRichHitSorterTask(1000, 0.4, "RichDigi", "RichDigiSorted", "PndRich");
  richSorter->SetVerbose(3);
  if(timeBased) fRun->AddTask(richSorter);
