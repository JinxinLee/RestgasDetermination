...
//cout<<" geom "<<geom<<endl;           
  geom->SetTopVolume(top);
	...
	// neutron detectors
		...
		KermitVolume->SetFillColor(kWhite);
		...
		NeutronBall->SetFillColor(kBlack);
	...
	//germanium
		...
		Int_t CrystalNumber = 1;
			if (!GlueStackOnly && !ElectronicsStackOnly ) SingleCluster ->PlaceCluster(ActivesTop,  0,15, 0,60,0,&CrystalNumber);
	...
	// piezos
		...
		TGeoVolume* Piezo2 = geom->MakeBox("CrystalPiezo2",acrylicGlass,0.75,0.25,0.2);
			...
			if (!GlueStackOnly && !ElectronicsStackOnly ) ActivesTop->AddNode(Piezo,0,new TGeoCombiTrans(-1,0,5.2,new TGeoRotation("",90,0,0)));
			Piezo2->SetLineColor(kBlue);
			Piezo2->SetFillColor(kBlue);
			if (!GlueStackOnly && !ElectronicsStackOnly ) ActivesTop->AddNode(Piezo2,0,new TGeoCombiTrans(1,0,5.2,new TGeoRotation("",90,0,0)));
