...
TString hypGeGeoCOSYsetup2014Actives(Double_t TargetThickness_Ext = 5, Double_t Angle = 0)
{
  ...
  gSystem->Load("libHypGe");
	...
	TString outfileWithoutPath ="hypGeGeoCOSY2014setupActives.root";//BuildOutputNameTarget(TargetThickness_Ext, Angle); //"hypGeGeoCOSYTarget_";
	////char buf[10];
	////sprintf(buf,"%.2f",TargetThickness_Ext);
	////outfileWithoutPath += buf;
	//outfileWithoutPath +=TargetThickness_Ext;
...
//cout<<" geom "<<geom<<endl;           
  geom->SetTopVolume(top);
	...
	for (int i= 0; i < 23; i++)
	{
		...
		GlueStackAssembly->AddNode(GlueSamples[i],i, new TGeoCombiTrans (0,0,-(0.4 + i*(0.203)), new TGeoRotation("",i*90,0,0)));
	}
