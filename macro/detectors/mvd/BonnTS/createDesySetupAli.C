	...
	
	{
	  ...
	  const Double_t  kPos1    = 5.; // (cm) Box 1
	  const Double_t  kPos2    = 185.5; // (cm) Box 4
	
	  // single sided sensors
	  // this is the position of the center of the system
	  // containing the two single sides detectors
	
	  const Double_t  kPos3    = 86.; // (cm) Box 2
	  const Double_t  kPos4    = 145.; // (cm) Box 3
	  ...
	  const Double_t offs1x = +0..; // box1
	  const Double_t offs1y = +0.;
	  
	  const Double_t offs2x = +0.; // box2 (sensor 2-3)
 	  const Double_t offs2y = +0.;

	  const Double_t offs3x = +0.; // box3 (sensor 4-5)
	  const Double_t offs3y = +0.;

	  const Double_t offs4x = +0.; // box4 (sensor 6)
	  ...
	  // TO COMBINE DIFFERENT TILTS

	  TGeoRotation rot2;
	  rot2.SetAngles(0.,-45.,0.);	
	  TGeoRotation rot3;
	  rot3.SetAngles(90.,0.,0.);
