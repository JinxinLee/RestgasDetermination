...
{
    //-----User Settings:-----------------------------------------------
	TString FileName="LambdaLambdaBarPol_1-642GeV_1000events";
...
TString AnaFile		= creator.GetCustomFileName("ana");
  ...
  testFile = new TFile(AnaFile.Data());
  if (!testFile->IsZombie()){
	  fRun->AddFriend(AnaFile.Data());
	  FairGeane *Geane = new FairGeane();
	  fRun->AddTask(Geane);
  }
  else {
 ...
 //----------------------Traks and points -------------------------------------
  ...
  if (enableTrackDraw) {
	  ...
	  PndTrackCandDraw* IdealTrackCand = new PndTrackCandDraw("PndIdealTrackCand");

	  fMan->AddTask(SttMvdTrack);
	  fMan->AddTask(SttMvdGemTrack);
	  fMan->AddTask(FtsIdealTrack);
	  fMan->AddTask(IdealTrackCand);
  }
