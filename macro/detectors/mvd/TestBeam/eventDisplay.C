...
eventDisplay()
{
  ...
  TString  RecoFile      ="2014-10-27-00-06-02_50_MHz_Filtered_PosCorrected_Event.root";
  TString  TrackFile	 ="2014-10-27-00-06-02_50_MHz_Filtered_PosCorrected_Tracks.root";
  ...
  fRun->SetInputFile(RecoFile.Data());
  fRun->SetOutputFile("tst.root");

  TFile* testFile;
  testFile = new TFile(TrackFile.Data());
  if (!testFile->IsZombie()){
	  fRun->AddFriend(TrackFile.Data());
  }
//  else {
...
//  }
  ...
  testFile = new TFile(InputFile.Data());
  if (!testFile->IsZombie()){
	  fRun->AddFriend(InputFile.Data());
...
//	  fMan->AddTask(MvdPoints);
  ...
  if (enableHitDraw) {
	  FairHitDraw *MvdRecoHit =   new FairHitDraw ("MVDHitsPixelPosCorrected");
	  MvdRecoHit->SetBoxDimensions(0.01,0.01,0.01);
...
//	  STTIsochrone->UseIsochroneTime();
  ...
  if (enableTrackDraw) {
      ...
      PndTrackCandDraw* MvdTrackCand = new PndTrackCandDraw("MVDTestBeamTrackCand");
    MvdTrackCand->SetBoxDimensions(0.02, 0.02, 0.02);
