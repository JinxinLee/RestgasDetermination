	...
	if (cluster_array->GetEntriesFast() > 0)
	  {
		...
		HISTtotdigis_clusterPhi->Fill(cluster_phi,totdigis_fromClusterAnalysis);
	      ...
	      {
	        for (int k=0; k<NumberOfClusters; k++)           //(k=0 corresponds to the cluster with biggest energy)
	         {
		   ...
		   ///if (k==NumberOfClusters-1)                  //eventually write the summation of the cluster enegies
		     ///cout <<"highestenergy_sumOfclusters =  "<< highestenergy_sumOfclusters <<'\n'<<endl;

		   for (int i=0; i<NumberOfClusters; i++)//finding the original cluster indices of the clusters
	      ...
	      for (int k0=1; k0<NumberOfClusters; k0++)
	        {
	      ...
	      for (int k1=2; k1<NumberOfClusters; k1++)
	        {
	      ...
	      for (int k2=3; k2<NumberOfClusters; k2++)
	        {
	      ...
	      for (int k3=4; k3<NumberOfClusters; k3++)
	        {
	      ...
	      for (int k4=5; k4<NumberOfClusters; k4++)
	        {
	      ...
	      for (int k5=6; k5<NumberOfClusters; k5++)
	        {
...
//////////////////////////////////////////////////////////////////////////writing into a root file:
   TString outfile= "./invMass_histo_allGammasAnalysis_onlyFwEndCapIncluded_GetEnergyCorrected.root";
