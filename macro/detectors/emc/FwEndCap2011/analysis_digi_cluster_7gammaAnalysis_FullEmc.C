...
////////////////////////////////////////////////////////////////////////// CLUSTER ANALYSIS
	...
	Double_t highestenergy_sumOf7clusters=0;    // (set per event)
        ...
        Double_t highestenergy[7], cluster_x_7[7], cluster_y_7[7],cluster_z_7[7],cluster_dist_7[7],cluster_module_7[7];
	for (int k=0; k<7;k++){
	  highestenergy[k]=0;                       // (set per event)
	  cluster_x_7[k]=cluster_y_7[k]=cluster_z_7[k]=cluster_dist_7[k]=cluster_module_7[k]=-1000; // (set per event)
	}
		...
		HISTtotdigis_clusterPhi->Fill(cluster_phi,totdigis_fromClusterAnalysis);
	      ...
	      if (NumberOfClusters_noModule5>=7)  //finding at least 7 clusters on modules 1,2,3,4 in total
	      {
	        for (int k=0; k<7; k++)           //finding the first 7 biggest cluster energies (k=0 corresponds to the cluster with biggest energy)
	         {
		   highestenergy[k] = clusterEnergies[(NumberOfClusters_noModule5-1)-k];
		   ...
		   for (int i=0; i<NumberOfClusters_noModule5; i++) //finding the original cluster indices of the 7 biggest energy clusters
		   ...
		   cluster_module_7[k]=cluster->GetModule();
	         }

	      for (int k0=1; k0<7; k0++)
	        {
		  //if (cluster_module_7[0] != cluster_module_7[k0])
		  {
	      ...
	      for (int k1=2; k1<7; k1++)
	        {
		  //if (cluster_module_7[1] != cluster_module_7[k1])
		  {
	      ...
	      for (int k2=3; k2<7; k2++)
	        {
		  //if (cluster_module_7[2] != cluster_module_7[k2])
		  {
	      ...
	      for (int k3=4; k3<7; k3++)
	        {
		  //if (cluster_module_7[3] != cluster_module_7[k3])
		  {
	      ...
	      for (int k4=5; k4<7; k4++)
	        {
		  //if (cluster_module_7[4] != cluster_module_7[k4])
		  {
	      ...
	      for (int k5=6; k5<7; k5++)
	        {
		  //if (cluster_module_7[5] != cluster_module_7[k5])
		  {
...
//////////////////////////////////////////////////////////////////////////writing into a root file:
   TString outfile= "./invMass_histo_7gammaAnalysis_FullEmcIncluded.root";
