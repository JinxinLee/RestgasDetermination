{
  ...
  // The geometry parameters are like for the Forward EndCap (FwEndCap).
  //
...
//TGeoVolume *top = gGeoMan->MakeTube("top",gGeoMan->GetMedium("air"),0,98,22.5);//cm
  ...
  TGeoVolume* SubunitVol9; //added 26.02.09
  TGeoVolume* BoxVol;
  TGeoVolume* BoxVol1;
  TGeoVolume* BoxVol2;
  TGeoVolume* BoxVol3;
 ...
 // SubunitVol = new TGeoVolume(name,SubunitShape,gGeoMan->GetMedium(medium));
  ...
  SubunitVol9 = new TGeoVolumeAssembly(name9);//added 26.02.09
  ...
  for(Int_t b=0; b<kNumOfBoxes; b++){
    ...
    if (b==0){
      ...
      SubunitVol9->AddNode(BoxVol4,b,trrotBox);//added 26.02.09
    }
    if (b==1 || b==2){
      name+=b;
      trrotBox->SetName(name);
  ...
  for (Int_t row=0; row<4; row++){
	...
	else if (flag==10) // middle subunit, added 26.02.09
	  {
