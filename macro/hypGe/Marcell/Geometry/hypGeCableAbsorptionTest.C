...
int hypGeCableAbsorptionTest(Double_t FoilThickness_Ext = 2)
{
  ...
  gSystem->Load("libHypGe");
	...
	cout << FoilThickness_Ext;
	TString outfileWithoutPath = "hypGeGeoCableAbsorptionTest_";
	//char buf[10];
	//sprintf(buf,"%.2f",FoilThickness_Ext);
	//outfileWithoutPath += buf;
	outfileWithoutPath +=FoilThickness_Ext;
...
//cout<<" geom "<<geom<<endl;           
  geom->SetTopVolume(top);
	...
	Foil->SetLineColor(kYellow);
	Foil->SetFillColor(kYellow);
