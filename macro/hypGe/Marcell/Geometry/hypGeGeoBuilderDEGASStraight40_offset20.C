...
void hypGeGeoBuilderDEGASStraight40_offset20(Double_t ZShift = 0)
{
  ...
  gSystem->Load("libHypGe");
	...
	TString outfile= "../../../../geometry/hypGeGeoDEGASstraight40_20.root";
...
//cout<<" geom "<<geom<<endl;           
  geom->SetTopVolume(top);
	...
	PndGeoHypGeDEGAS *DEGAS = new PndGeoHypGeDEGAS(Ge,Al,1);
	
	//Read coordinates and angles from file

	const Int_t NumberOfCluster=20;
	Double_t CoordinatesAndAngles[NumberOfCluster*6];
	string Filename = "DEGASStraightCoordinatesAndAngles40_20.txt";


	DEGAS->ReadCoordinatesAndAngles(Filename,NumberOfCluster,CoordinatesAndAngles);

	for (Int_t i = 0; i < NumberOfCluster*6; i++)
		cout << CoordinatesAndAngles[i] << endl;
	//	Double_t ClusterRadius = 30;

	for (Int_t iCluster = 0; iCluster < 20; iCluster++)
	{

			
		DEGAS ->PlaceCluster(Realtop,CoordinatesAndAngles[6*(iCluster)],CoordinatesAndAngles[6*(iCluster)+1],CoordinatesAndAngles[6*(iCluster)+2],  GlobalZOffset,CoordinatesAndAngles[6*(iCluster)+3],CoordinatesAndAngles[6*(iCluster)+4],CoordinatesAndAngles[6*(iCluster)+5],&CrystalNumber);
