...
int ana_multi(int nevts=100)
{
	...
	TString OutFile="outputmult.root";

	// *** the files coming from the simulation
	TString inPidFile  = "psi2s_jpsi2pi_jpsi_mumu_pidmult.root";    // this file contains the PndPidCandidates and McTruth
	//TString inRecFile  = "psi2s_jpsi2pi_jpsi_mumu_recomult.root";    // this file contains the PndTracks and McTruth
	//TString inSimFile  = "psi2s_jpsi2pi_jpsi_mumu_sim.root";    // this file contains the PndTracks and McTruth
	//TString inDigiFile  = "psi2s_jpsi2pi_jpsi_mumu_digi.root";    // this file contains the PndTracks and McTruth
	...
	TFile *out = TFile::Open("output_anamulti.root","RECREATE");
  ...
  //theAnalysis->SetVerbose(5);
	...
	RhoCandList muplusloose, muminusloose, piplusloose, piminusloose, jpsiloose, psi2sloose;
	RhoCandList muplustight, muminustight, piplustight, piminustight, jpsitight, psi2stight;
    ...
    //cout << jpsi.GetLength() << " jpsis "
         //<< chrg.GetLength() << " chrg "
         //<< muplus.GetLength() << " muplus "
         //<< muminus.GetLength() << " muminus "
         //<< piplus.GetLength() << " piplus "
         //<< piminus.GetLength() << " piminus "
         //<< endl;
		...
		theAnalysis->FillList(muplusloose,  "MuonLoosePlus",  "PidAlgoMvd;PidAlgoStt;PidAlgoDrc;PidAlgoDisc;PidAlgoMdtHardCuts");
		theAnalysis->FillList(muminusloose, "MuonLooseMinus", "PidAlgoMvd;PidAlgoStt;PidAlgoDrc;PidAlgoDisc;PidAlgoMdtHardCuts");
		theAnalysis->FillList(piplusloose,  "PionLoosePlus",  "PidAlgoMvd;PidAlgoStt;PidAlgoDrc;PidAlgoDisc");
		theAnalysis->FillList(piminusloose, "PionLooseMinus", "PidAlgoMvd;PidAlgoStt;PidAlgoDrc;PidAlgoDisc");

		jpsiloose.Combine(muplusloose, muminusloose);
		for (j=0;j<jpsiloose.GetLength();++j) hjpsim_lpid->Fill( jpsiloose[j]->M() );
    //cout << jpsiloose.GetLength() << " jpsisloose "
         //<< muplusloose.GetLength() << " muplusloose "
         //<< muminusloose.GetLength() << " muminusloose "
         //<< piplusloose.GetLength() << " piplusloose "
         //<< piminusloose.GetLength() << " piminusloose "
         //<< endl;
		jpsiloose.Select(jpsiMassSel);

		psi2sloose.Combine(jpsiloose, piplusloose, piminusloose);
		for (j=0;j<psi2sloose.GetLength();++j) hpsim_lpid->Fill( psi2sloose[j]->M() );
		...
		theAnalysis->FillList(muplustight,  "MuonTightPlus",  "PidAlgoMdtHardCuts");
		theAnalysis->FillList(muminustight, "MuonTightMinus", "PidAlgoMdtHardCuts");
		theAnalysis->FillList(piplustight,  "PionLoosePlus",  "PidAlgoMvd;PidAlgoStt;PidAlgoDrc");
		theAnalysis->FillList(piminustight, "PionLooseMinus", "PidAlgoMvd;PidAlgoStt;PidAlgoDrc");

		jpsitight.Combine(muplustight, muminustight);
		for (j=0;j<jpsitight.GetLength();++j) hjpsim_tpid->Fill( jpsitight[j]->M() );
    //cout << jpsitight.GetLength() << " jpsistight "
         //<< muplustight.GetLength() << " muplustight "
         //<< muminustight.GetLength() << " muminustight "
         //<< piplustight.GetLength() << " piplustight "
         //<< piminustight.GetLength() << " piminustight "
         //<< endl;
		jpsitight.Select(jpsiMassSel);

		psi2stight.Combine(jpsitight, piplustight, piminustight);
		for (j=0;j<psi2stight.GetLength();++j) hpsim_tpid->Fill( psi2stight[j]->M() );

	}

	// *** write out all the histos
  cout<<"Write histograms to file "<<out->GetName()<<endl;
	out->cd();
	...
	out->Save();
  cout<<"Write histograms to file "<<out->GetName()<<" done."<<endl;
