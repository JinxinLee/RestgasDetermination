int digi_complete_tb()
{
  ...
  // -----   MDT hit producers   ---------------------------------
  PndMdtPointsToWaveform* mdtWaveform = new PndMdtPointsToWaveform(0, kTRUE);
  fRun->AddTask(mdtWaveform);
  PndMdtDigitization* mdtDigitization = new PndMdtDigitization(0, kTRUE);
  fRun->AddTask(mdtDigitization);

  mdtWaveform->RunTimeBased();
  mdtDigitization->RunTimeBased();
  PndMdtDigiSorterTask* mdtDigiSorter = new PndMdtDigiSorterTask(20, 50, "MdtDigi", "TSMdtDigi", "Mdt");
  fRun->AddTask(mdtDigiSorter);
  mdtDigiSorter->SetVerbose(0);
