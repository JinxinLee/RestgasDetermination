...
{
  //his->SetName(Form("%s%i",his->GetName(),gCanvasCounter));
  double range=1.1*his->GetMaximum();
  his->SetAxisRange(0.,range,"Y");
  Double_t par[3]={0.,0.,0.};
  TString fitname="mygaus";fitname+=gCanvasCounter;
  ...
  DrawText( 0.15, 0.8, str.Data(),0.06,1);
  str=Form("#mu = %.3g",mean);
  DrawText( 0.15, 0.73, str.Data(),0.06,1);
  str=Form("#sigma = %.3g",sigma);
  DrawText( 0.15, 0.66, str.Data(),0.06,1);
  TH2F coloredbox("colbox",";;",2,3.5,4.5,1,0.85*range,0.9*range);
  coloredbox.Fill(3.8,0.86*range,2*mean+1.);
  coloredbox.Fill(4.2,0.86*range,sigma);
  coloredbox.SetAxisRange(0.,2.,"Z");
  coloredbox.DrawCopy("col same");
  gCanvasCounter++;
  return 0;
}
...
{
  //his->SetName(Form("%s%i",his->GetName(),gCanvasCounter));
  double range=1.2*his->GetMaximum();
  his->SetAxisRange(0.,range,"Y");
  Double_t par[2]={0.,0.};
  TString fitname="myline";fitname+=gCanvasCounter;
  TF1* fitted = new TF1(fitname.Data(),"pol1",0.05,0.95);
