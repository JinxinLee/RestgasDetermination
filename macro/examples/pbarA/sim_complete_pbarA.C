...
int sim_complete_pbarA(Int_t nEvents = 100, UInt_t seed=-1, TString  SimEngine ="TGeant3", Float_t mom = 8.0, Float_t atarg = 20.0)
{
  ...
  TLorentzVector fIni(0, 0, mom, sqrt(mom*mom+9.3827203e-01*9.3827203e-01)+atarg*0.931494);
  TDatabasePDG::Instance()->AddParticle("pbarASystem","pbarASystem",fIni.M(),kFALSE,0.1,0, "",88890);
  TDatabasePDG::Instance()->AddParticle("pbarpSystem","pbarpSystem",5.5,kFALSE,0.1,0, "",88888);
  TDatabasePDG::Instance()->AddParticle("pbarnSystem","pbarnSystem",5.5,kFALSE,0.1,-3, "",88887);
  TDatabasePDG::Instance()->AddParticle("A-1System","pbarASystem",1.8,kFALSE,0.1,0, "",88891);
  //------------------------------------------------------------------
  TStopwatch timer;
  timer.Start();
  //gRandom->SetSeed();
  ...
  if(UseEvtGenDirect){
    ...
    EvtInput+="/mysim/pbarNe20_0/pbarNe20_pbarpX.dec";
    TString  defDECAY =gSystem->Getenv("VMCWORKDIR");
    defDECAY+="/pgenerators/EvtGen/EvtGen/Private/DECAY.DEC";
    TString  defPDL =gSystem->Getenv("VMCWORKDIR");
    defPDL+="/pgenerators/EvtGen/EvtGen/Private/evt.pdl";
    //PndEvtGenDirect *EvtGen = new PndEvtGenDirect("pbarpSystem", EvtInput.Data(), mom);
    PndEvtGenDirect *EvtGen = new PndEvtGenDirect("pbarASystem",EvtInput.Data(),mom,seed,defDECAY.Data(),defPDL.Data(),atarg);
