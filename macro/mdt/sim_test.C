...
sim_test(Int_t nEvents = 100, TString  SimEngine ="TGeant3", Float_t mom = 2)
{
  //-----User Settings:-----------------------------------------------
  TString  OutputFile     ="sim_complete.root";
  TString  ParOutputfile  ="simparams.root";
  TString  MediaFile      ="media_mdt.geo";
  gDebug                  = 0;
  ...
  Muo->SetBarrel("muon_barrel_test_noGeo.root");
  //Muo->SetEndcap("fast");
  ...
  if(UseBoxGenerator){	// Box Generator
    ...
    boxGen->SetThetaRange(40., 90.); // Polar angle in lab system range [degree]
